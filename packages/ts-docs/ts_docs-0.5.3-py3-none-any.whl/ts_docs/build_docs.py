...
def main():
    ...
    for language in languages:
        for builder in builders:
            source_dir = os.path.join(docs_path, args.source_dir)
            build_dir = os.path.join(docs_path, args.build_dir, chips, language, builder)
...
def build_docs(parameters):
    ...
    for parameter in parameters:
        clean_cmd = "rm -rf {TS_DOCS_PATH}/docs/_build/{}/{}".format(ts_docs_path,parameter[0], parameter[1])
        print("Deleting {TS_DOCS_PATH}/docs/_build/{}/{}".format(ts_docs_path,parameter[0], parameter[1]))
        sbp_call(clean_cmd)
