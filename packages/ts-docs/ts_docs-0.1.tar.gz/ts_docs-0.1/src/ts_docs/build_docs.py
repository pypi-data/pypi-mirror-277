...
LANGUAGES = ['en', 'zh_CN']
TARGETS = ['at1k', 'at5k', 'at5050', 'at820', 'at103']
    ...
    for target in targets:
        for language in languages:
            ...
            if len(args.source_dir) == 0:
                ...
                build_info['source_dir'] = os.path.realpath(os.path.join(args.source_dir))
...
def sphinx_call(build_info, builder):
    ...
    args = [sys.executable, '-u', '-m', 'sphinx.cmd.build',
            ...
            '-w', SPHINX_WARN_LOG,
            '-t', build_info['target'],
            '-c', '{}/{}'.format(os.path.abspath(build_info['doxyfile_dir']), build_info['language']),  # conf file path
