        ...
        while True:
            ...
            if len(buffer) >= fs * duration // 1000:
                ...
                if len(recording) % (fs * whisper_duration) < fs * duration // 1000:
                    ...
                    state.enter_state(ProcessLifecycle.States.recording)
...
#            q_status.put_nowait(("cancel", "Stopping the recording"))
            ...
            if state.state == ProcessLifecycle.States.recording:
                async with session.post(
                    ...
                    try:
                        ...
                        q_text.put_nowait(await response.text())
    ...
    async with aiohttp.ClientSession(
        ...
        while True:
            ...
            async with session.post(
                ...
                q_text.task_done()
            if state.state == ProcessLifecycle.States.stopping and q_text.qsize() == 0:
                ## Stop the consumer after the queue is empty and the recording is stopping
                break
