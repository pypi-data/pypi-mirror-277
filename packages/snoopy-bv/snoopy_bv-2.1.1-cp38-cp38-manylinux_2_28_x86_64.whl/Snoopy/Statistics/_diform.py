...
class _DirectIformABC():
    ...
    The univariate approach should be implemented in the ._fit() method of the child class.
    ...
    def point_to_rp( self, point_coordinates, method = "discrete",
                     opt_kwargs = { "method" : "trust-constr" }):
        ...
        method : str
            if method == "discrete" only existing search direction will be looked for. If optimize is used, optimization on the search direction is used. 
        ...
        if method == "optimize": 
            ...
            res_df = pd.DataFrame( index = np.arange(npoints) , columns= ["RP"] + self.dim_names_a ) 

            _array = self._df.loc[ : , self.dim_names_a ].values
            
            # Initialisation from discrete point
            ...
            def proj_data(direction, ipoint):
                ...
                fit_obj = self._fit( se )
                res = -fit_obj.x_to_rp( proj_point )
                logger.debug( f"RP = {-res:} - {np.sum(direction**2)**0.5:}")
                return res
            
            for ipoint in tqdm( np.arange(npoints), desc = "Optimizing RP") :
                # x0 from discrete case
                x0 = self._direction_vector.loc[ _discrete.loc[ipoint , "DIR_ID"], self.dim_names_a  ].values
                res_opt = minimize( lambda x: proj_data(x , ipoint), 
                                    x0,
                                    constraints = [ {"type":"eq" , "fun" : l2_norm_constraint , "jac" : l2_norm_jac }],
                                    **opt_kwargs
                                   )
                res_df.loc[ipoint, "RP"] = -res_opt.fun
                res_df.loc[ipoint, self.dim_names_a] = res_opt.x
            return res_df
            ...
            return pd.DataFrame( data = { "RP" : rp_tab.max(axis=0), "DIR_ID" : self._direction_vector.index[rp_tab.argmax(axis=0)] } )
        else: 
            raise(Exception(f"Unknown method {method:}"))
        

    def drop_inf_nnlf(self):
        """Drop direction for which fit results in NaN or Inf for nnlf.

        By default, this is not used.
        """
        good_i = np.isfinite( self.fit_df["NNLF"] )
    ...
    def rp_projections(self, rp_list, ci_level = None ):
        ...
        if ci_level is not None :
            ...
            for i , rp in enumerate(rp_list) :
                self._res_df[("ci_low",rp)] = res_low[:,i]
                self._res_df[("ci_high",rp)] = res_high[:,i]
    ...
    def extract_contour( self , rp, ci_level = None, low_high = None ):
        ...
        logger.debug(f"Extract {self.ndim:} contour at RP={rp:} using Voronoi cells")
        
        if ci_level is None:
            col = rp
        else :
            col = ( f"ci_{low_high:}" ,rp )
    ...
    def plot_tangent(self, variables, rp, ax = None, scale = "original", transform_dict = {}, **kwargs ) :
        ...
        ax.set(xlabel = variables[0], ylabel = variables[1], xlim = [ f1(np.min(v1List)), f1(np.max(v1List))] , ylim = [ f2(np.min(v2List)), f2(np.max(v2List))] )
