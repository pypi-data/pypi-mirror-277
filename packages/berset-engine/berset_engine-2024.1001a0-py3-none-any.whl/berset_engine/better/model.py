...
class InverseModel:
    ...
    def model_type(self):
        ...
        self.coeffs = {'base': self.base, 'csl': self.csl, 'ccp': self.ccp, 'hsl': abs(self.hsl), 'hcp': self.hcp} # original code
    ...
    def describe_model_html(self, building):
        ...
        if (self.has_fit):
            ...
            model_description_html += '<b>' + self.energy_type + ':</b> <br>'
            model_description_html += '<ul>'
            model_description_html += '<li>'
            model_description_html += '<b>[Baseload]</b>: Your consistent baseload is ' + str(round(self.base, 2)) + ' kWh/(m<sup>2</sup>*day), or ' + str(round(self.base * 1 * constants.Constants.days_in_year, 1)) + ' kWh/(m<sup>2</sup>*yr). '
            model_description_html += '</li>'
            if(self.model_type_str != '3P Heating'):
                model_description_html += '<li>'
                model_description_html += '<b>[Cooling Change Point]</b>: The building is in cooling mode when the outside air temperature is above ' + str(round(self.ccp, 1)) + ' &#176;C. '
                model_description_html += '</li>'
                model_description_html += '<li>'
                model_description_html += '<b>[Cooling Sensitivity]</b>: During cooling, the building daily energy consumption increases by ' + str(round(building.bldg_area * self.csl, 1)) + ' kWh/day for each 1 degree increase in outside air temperature. '
                model_description_html += '</li>'
            if(self.model_type_str != '3P Cooling'):
                model_description_html += '<li>'
                model_description_html += '<b>[Heating Start Point]</b>: The building is in heating mode when the outside air temperature is below ' + str(round(self.hcp, 1)) + ' &#176;C. '
                model_description_html += '</li>'
                model_description_html += '<li>'
                model_description_html += '<b>[Heating Sensitivity]</b>: During heating, the building daily energy consumption increases by ' + str(round(abs(building.bldg_area * self.hsl), 1)) + ' kWh/day for each 1 degree decrease in outside air temperature.'
                model_description_html += '</li>'
            model_description_html += '</p>'
            model_description_html += '</ul>'
        else:
