...
BING_MAP_API_KEY_ENV= os.getenv(
    "BING_MAP_API_KEY"
)


class Building:
    def __init__(self, bldg_id, bldg_name, bldg_address, bldg_type, bldg_area, currency='US Dollar', saving_target=2,**kwargs):
        ...
        BING_MAP_API_KEY = kwargs.get(
            "BING_MAP_API_KEY",None
        )
        if not BING_MAP_API_KEY:
            BING_MAP_API_KEY = BING_MAP_API_KEY_ENV
        self.geocode_address(BING_MAP_API_KEY = BING_MAP_API_KEY)
    ...
    def geocode_address(self,**kwargs):
        ...
        try:
            ...
            if (self.geo_coder.latlng is None):
                self.geo_coder = geocoder.bing(self.bldg_address, key=BING_MAP_API_KEY)
            if (self.geo_coder.latlng is None):
                self.geo_coder = geocoder.osm(self.bldg_address)
            if (self.geo_coder.latlng is None):
                self.geo_coder = geocoder.baidu(self.bldg_address)            
        except:
            try:
                self.geo_coder = geolocator.geocode(self.bldg_address)
                self.coord = [self.geo_coder.latitude,self.geo_coder.longitude]
                self.geo_address = self.geo_coder.address
                self.latitude = self.geo_coder.latitude
                self.longitude = self.geo_coder.longitude
            except:
    ...
    def add_utility(self, utility_e=None, utility_f=None):
        ...
        if (utility_e is None and utility_f is None):
            print(
                "ERROR: No utility data has been found"
            )


    def add_weather(self, cached=True, weather_e=None, weather_f=None, weather_e_daily=None, weather_f_daily = None):
        # cached: True ~ pre-downloaded weather dat, False ~ download the weather on the go.
        # cached: True weather data will be provided by user
        ...
        if (weather_e is not None and hasattr(self.utility_electricity, "df_raw_data")):
            ...
            if cached:
                ...
                self.weather_electricity.download_weather_NOAA(temperature_daily_data = weather_e_daily)
        ...
        if (weather_f is not None and hasattr(self.utility_fossil_fuel, "df_raw_data")):
            ...
            if cached:
                ...
                self.weather_fossil_fuel.download_weather_NOAA(temperature_daily_data = weather_f_daily)
    ...
    def fit_inverse_model(self, threshold=0.1):
        ...
        if (hasattr(self, "weather_electricity")):
            self.im_electricity = model.InverseModel(self.weather_electricity.v_T_C,
                                                     ...
                                                     'Electricity')
            has_fit_e = self.im_electricity.fit_model(threshold=threshold)
            if (has_fit_e):
        ...
        if (hasattr(self, "weather_fossil_fuel")):
            self.im_fossil_fuel = model.InverseModel(self.weather_fossil_fuel.v_T_C,
                                                     ...
                                                     'Fossil Fuel')
            has_fit_f = self.im_fossil_fuel.fit_model(threshold=threshold)
    ...
    def ee_assess(self, use_default=True, df_benchmark_stats_electricity=None,
        ...
        if (not pd.isna(df_assessment_f['site_coefficients']).all()):
            ...
            self.hsl_new_f = -df_new_coeffs_f['beta_hdd'] # original code
    ...
    def assemble_saving_dataframe(self):
        ...
        if (hasattr(self, "v_new_consumption_all_e")):
            ...
            self.df_savings_e = df_savings_e.sort_values(by=['Bill Start Dates'])
        ...
        if (hasattr(self, "v_new_consumption_all_f")):
            ...
            self.df_savings_f = df_savings_f.sort_values(by=['Bill Start Dates'])

    def plot_savings(self, plt_engine='chartjs'):
        self.assemble_saving_dataframe()
        if (hasattr(self, "df_savings_e")):
            self.energy_savings_fig_e = self.plot_helper(self.df_savings_e, 'Electricity',plt_engine=plt_engine)
        if (hasattr(self, "df_savings_f")):
            self.energy_savings_fig_f = self.plot_helper(self.df_savings_f, 'Fossil Fuel',plt_engine=plt_engine)

    @staticmethod
    def plot_helper(df_savings, utility_type, plt_engine = "chartjs"):
        # Prepare
        df_savings['Time'] = pd.to_datetime(df_savings['Bill Start Dates'])
        df_savings['Time'] = df_savings['Time'].dt.strftime('%b %Y')
        ...
        # using plotly
        if plt_engine=='plotly':
            print("Warning: Using plotly as plot engine does not work properly yet, chartjs must be used")
            graph_data = [
                go.Scatter(
                    x = df_savings['Time'],
                    y = df_savings['Original Consumption'],
                    name = f"Current {utility_type} consumption",
                    mode = 'lines+markers',
                    line=dict(dash='dash')
                ),
                go.Scatter(
                    x = df_savings['Time'],
                    y = df_savings['Estimated Consumption with Improvements'],
                    name = f"Target {utility_type} consumption",
                    mode = 'lines+markers',
                    line=dict(dash='dash')
                )
            ]
            ...
            fig.update_layout(
                title = f"Monthly {utility_type} Consumptions",
                legend = dict(
                    orientation='h',
                    font = dict(
                        size = 12
                ...
                xaxis = dict(
                    tickvals = df_savings['Time'].values,
                    tickfont = dict(
                        size = 12
                    ),
                    ticks='outside',
            ...
            consumption_line_chart_html = plt_to_html(
                fig,
