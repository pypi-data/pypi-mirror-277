...
NOTICE.  This Software was developed
under funding from the U.S. Department of Energy and the U.S. Government consequently retains certain rights. As such, the U.S. Government has been granted for itself and others acting on its behalf a paid-up, nonexclusive, irrevocable, worldwide license in the Software to reproduce, distribute copies to the public, prepare derivative works, and perform publicly and display publicly, and to permit other to do so.
...
def process_downloaded_weather_noaa(weather_station_ID, start_year, end_year, temperature_unit = "C"):
        # Helper functions
        def download_sub_hourly_weather(station_ID, year):
            ...
            # using requests
            print(f"Process downloading the data for station: {station_ID}")
            ...
            if response.status_code == 200:
                ...
                with open(file_name_local, 'wb') as file:
                    file.write(response.content)
                print(f"File downloaded successfully to {file_name_local}")
            else:
        ...
        df_new["Temperature"] = (df_new["Temperature"]-32)/1.8
...
class Weather:
    ...
    def find_closest_weather_station(self, df_weather_station_list=Constants.df_noaa_weather_station):
    ...
    def download_weather_NOAA(self,temperature_daily_data = None, temperature_unit = "C"):
        if not isinstance(temperature_daily_data,pd.DataFrame) or temperature_daily_data.empty:
            ...
            if not all([el in temperature_daily_data.columns for el in ["Datetime","Temperature"]]):
                raise Exception(
                    "temperature daily data must contain Datetime and Temperature as column"
                )
            
            if temperature_unit == "C":
                temperature_daily_data.loc[:,"Temperature"] = temperature_daily_data.loc[:,"Temperature"]*1.8+32

            self.v_T_F, self.v_T_C = self.aggregate_weather(temperature_daily_data)
