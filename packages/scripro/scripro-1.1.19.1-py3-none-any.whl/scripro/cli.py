...
def run_scrip_atac(args):
    """调用SCRIP ATAC命令"""
    result = subprocess.run(['SCRIP', 'ATAC'] + args.extra_args, capture_output=True, text=True)
    print(result.stdout)
...
def prepare_argparser():
    ...
    add_scrip_atac_parser(subparsers)  # 新增的SCRIP ATAC子命令
    return argparser
...
def add_scrip_atac_parser(subparsers):
    """Add a parameter parser for the SCRIP ATAC command"""
    parser = subparsers.add_parser("atac", help="Invoke the SCRIP ATAC command")
    parser.add_argument('extra_args', nargs=argparse.REMAINDER, help="Additional arguments passed to the SCRIP ATAC command")
