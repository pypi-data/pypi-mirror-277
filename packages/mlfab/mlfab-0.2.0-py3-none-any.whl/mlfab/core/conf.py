...
from mlfab.utils.text import show_error
...
def parse_dtype(cfg: Device) -> torch.dtype | None:
    if cfg.use_fp64:
...
def user_config_path() -> Path:
    mlfabrc_path_raw = os.environ.get("MLFABRC_PATH", "~/.mlfab.yml")
    mlfabrc_path = Path(mlfabrc_path_raw).expanduser()
    return mlfabrc_path


@functools.lru_cache(maxsize=None)
def _load_user_config_cached() -> UserConfig:
    mlfabrc_path = user_config_path()
    base_cfg = OmegaConf.structured(UserConfig)

    # Writes the config file.
    if mlfabrc_path.exists():
        cfg = OmegaConf.merge(base_cfg, OmegaConf.load(mlfabrc_path))
    else:
        show_error(f"No config file was found in {mlfabrc_path}; writing one...", important=True)
        OmegaConf.save(base_cfg, mlfabrc_path)
        cfg = base_cfg

    # Looks in the current directory for a config file.
    local_cfg_path = Path("mlfab.yml")
...
def load_user_config() -> UserConfig:
    """Loads the ``~/.mlfab.yml`` configuration file.
