...
class Line(Dao):
    ...
    @beartype
    def sync(self) -> dict:
        r = self.connexion.get(f"/sdk/line/{self.id}").json()
        self.refresh(r)
        return r
    ...
    def refresh(self, data: dict) -> LineSchema:
        ...
        self._label = Label(self.connexion, schema.label.model_dump())
        return schema
    ...
    def update(
        ...
        r = self.connexion.patch(
            f"/sdk/line/{self.id}", data=orjson.dumps(payload)
        ).json()
        self.refresh(r)
    ...
    def delete(self) -> None:
        ...
        Examples:
            ...
            ```
        """
        self.connexion.delete(f"/sdk/line/{self.id}")
