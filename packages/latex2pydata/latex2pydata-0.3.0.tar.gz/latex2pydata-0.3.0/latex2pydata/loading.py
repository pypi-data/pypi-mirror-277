...
def load(readable: Union[pathlib.Path, io.BytesIO, io.TextIOBase],
         encoding: Optional[str] = None,
         schema: Optional[dict[str, str]] = None,
         schema_missing: Optional[Union[Literal['error'], Literal['rawstr'], Literal['evalany']]] = None) -> LoadedDataType:
    if isinstance(readable, pathlib.Path):
        encoding = encoding or 'utf-8-sig'
        return loads(readable.read_text(encoding=encoding), schema, schema_missing)
    raw_read: Union[bytes, str] = readable.read()
