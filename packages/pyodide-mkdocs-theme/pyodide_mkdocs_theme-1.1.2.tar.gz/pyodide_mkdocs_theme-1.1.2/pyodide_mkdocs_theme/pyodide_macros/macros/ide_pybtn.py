...
class PyBtn(IdeManager):
    ...
    def make_element(self) -> str:
        ...
        button_html   = Html.button(
            ...
            style = img_size,
            type = 'button',    # https://developer.mozilla.org/en-US/docs/Web/HTML/Element/button#notes
        )
    ...
    def _build_img(self, img_style:str, docs_path:Optional[str]=None):
        lvl_up   = self.env.level_up_from_current_page()
        img_link = f"{lvl_up}/{docs_path}" if docs_path else get_ide_button_png_path(lvl_up,'play')
        return Html.img(
            src = img_link,
    ...
    def _build_icon(self):
        ...
        # File path (relative to "docs to be")
        elif re.fullmatch(r"(?!http)(?!ftp)(?!www).+[.]\w{2,6}", self.icon):
            img = self._build_img(img_size, self.icon)

        # Assume svg raw code or material icon
        else:
            img = self.icon
