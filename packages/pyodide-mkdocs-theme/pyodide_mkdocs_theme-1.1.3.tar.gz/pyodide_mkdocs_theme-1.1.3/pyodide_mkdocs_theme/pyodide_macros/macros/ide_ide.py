...
class Ide(IdeManager):
    ...
    NEEDED_KINDS: ClassVar[Tuple[ScriptKind]] = ScriptKind.pyodide, ScriptKind.terms, ScriptKind.ides
    ...
    DEFAULTS_EXTRACTION: ClassVar[Tuple[Tuple[str,str]]] = (
        ('auto_log_assert', 'show_assertion_code_on_failed_test'),
        ('max_attempts',    'max_attempts_before_corr_available'),
        ('max_size',        'default_ide_height_lines'),
        ('term_height',     'default_height_ide_term'),
    )
    ...
    def _validate_files_config(self, max_attempts: Union[int,float] ):
        ...
        if(
            self.env.forbid_secrets_without_corr_or_REMs and
            self.has_secrets and not self.has_any_corr_rems
        ):
            msg,prop = (
            ...
            self.env.forbid_hidden_corr_and_REMs_without_secrets and
            self.has_any_corr_rems and not self.has_secrets
        ):
            ...
            self.env.forbid_corr_and_REMs_with_infinite_attempts and
            self.has_any_corr_rems and max_attempts==inf
        ):
            elt_msg = self._get_corr_rems_msg()
            msg,prop = (
                f"{ elt_msg } but will never be visible because the number of "
    ...
    def make_element(self) -> str:
        ...
        # The solution_div not inside the other because markdown rendering troubles and useful
        # as "anchor" for IDE extractions/insertions when the "full screen" mode is implemented.
