...
class Terminal(IdeManager):
    ...
    NEEDED_KINDS: ClassVar[Tuple[ScriptKind]] = ScriptKind.pyodide, ScriptKind.terms
    ...
    DEFAULTS_EXTRACTION: ClassVar[Tuple[Tuple[str,str]]] = (
        ('term_height',  'default_height_isolated_term'),
    )
    ...
    def handle_extra_args(self):
        ...
        if( isinstance(self.py_name, int)
            and 'SIZE' not in self.extra_kw and 'TERM_H' not in self.extra_kw
        ):
            self.py_name, self.term_height = '', self.py_name
            logger.warning(
                f"Invalid macro argument in { self.env.location() }:\n"
                f"`{'{{'} terminal({ self.term_height }) {'}}'}` should be replaced with "
                f"`{'{{'} terminal(TERM_H={ self.term_height }) {'}}'}`"
            )
    ...
    def make_element(self) -> str:
        ...
        terminal_div = Html.terminal(
            self.editor_name,
            kls       = f"{ HtmlClass.py_mk_terminal_solo } { HtmlClass.py_mk_terminal }",
