...
from .tools import execute, git_switch


@click.command()
@click.option(
    "-i", "--initial-version", type=str, required=True, default="12.0"
)
@click.option(
    "-t", "--target-version", type=str, required=True, default="16.0"
)
@click.option(
    "-r", "--distant-remote", type=str, required=True, default="grap"
)
@click.option(
    "-l", "--local-remote", type=str, required=True, default="legalsylvain"
)
    ...
    if not modules:
        ...
        for module in modules:
            ...
            if module_path.exists():
                ...
                logger.warning(
                    f"{module} not found in the branch {initial_version}"
                )
    ...
    for module_path in modules_path:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Some pre-commit stuff should be fixed manually."
                )
        ...
        logger.info("Push on github ...")
        command = ["git", "push", local_remote, branch_name]
        if force:
