...
YT_DLP_OPTIONS_DEFAULT = '--extract-audio --audio-format m4a --audio-quality 48k --embed-thumbnail --console-title --embed-metadata --newline --progress-delta 2 --break-on-existing'
...
def download_audio(movie_id: str, folder: str, ytdlprewriteoptions: str):
...
def download_thumbnail(movie_id: str, folder: str):
    folder = pathlib.Path(folder)
    folder.mkdir(parents=True, exist_ok=True)
    thumbnail = folder.joinpath(f'{movie_id}.jpg')
...
def main():
    ...
    parser.add_argument('--folder', type=str, help='Path to folder to save outputs. Default is \".\"', default='.')
    parser.add_argument('--ytdlprewriteoptions', type=str,
                        help=f'Set no a default YT_DPL options in quotes. Default value is \"{YT_DLP_OPTIONS_DEFAULT}\". '
