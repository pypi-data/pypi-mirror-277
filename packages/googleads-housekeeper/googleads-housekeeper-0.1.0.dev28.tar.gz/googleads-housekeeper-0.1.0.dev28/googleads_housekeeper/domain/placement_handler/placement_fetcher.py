...
class PlacementFetcher:
    ...
    def get_placements_for_account(
        ...
        if not (placements := self._get_placement_performance_data(
                account, task_obj)):
            return None
        placements_joined_with_conversion_split = None
        if runtime_options.is_conversion_query and (
            ...
            placements_by_conversion_name = (
                conversion_split_exclusion_specification.apply_specifications(
                    placements_by_conversion_name))
            placements_joined_with_conversion_split = self._join_conversion_split(
                placements, placements_by_conversion_name,
                runtime_options.conversion_name)

        if runtime_options.is_conversion_query:
            return self._aggregate_placements(
                placements_joined_with_conversion_split,
                task_obj.exclusion_level)
        return self._aggregate_placements(placements,
                                                task_obj.exclusion_level)
    ...
    def _aggregate_placements(
        ...
        aggregation_groupby = self._define_aggregation_group_by(
            exclusion_level)
        groupby = [
