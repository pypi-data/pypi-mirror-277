...
class ZohoCrmAPI(BaseAPIWrapper):
    ...
    def update_customer_record(self, module, customer_id, subscriptions_customer_profile=None, traq_profile_url=None):
        ...
        """
        self.logger.info(f"Entering update_customer_record() for module {module}, record ID {customer_id}")
        ...
        if response is None:
            error_msg = f"Failed to update customer in module '{module}' with ID {customer_id}. No response received. Check to ensure your inputs are valid."
            ...
            error_msg = (f"Failed to update customer in module '{module}' with ID {customer_id}. "
    ...
    def coql_query(self, select_query, limit = 2000):
        ...
        """
        self.logger.info(f"Entering execute_coql_query() with query: {select_query}")
