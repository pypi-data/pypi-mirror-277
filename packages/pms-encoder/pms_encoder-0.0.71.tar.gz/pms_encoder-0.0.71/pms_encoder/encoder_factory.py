    ...
    def __init__(
        ...
        if model_key == "SleepAndPassProcessor":
            ...
            if model_info["alias"].startswith("onnx"):
                ...
                raise ValueError(
                    f"""ERROR! model_info["suffix_or_alias"]={model_info["suffix_or_alias"]} is not supported."""
                )
            model_dir = model_manager.get_local_model_dir(model_name=model_key.replace("Processor", ""), alias=model_info["alias"])
            model_path = os.path.join(model_dir, "model.plan")
            processor_kwargs = {
                "model_path": model_path,
                #     os.path.join(
                #     ROOT_DIR,
                #     model_key.replace("Processor", ""),
        ...
        logger.warning("############################")
        os.system(
            """python -c "import pms_tensorrt; import numpy as np; from pms_tensorrt import TRTProxy;p='/tmp/DPIR-trt-a10g-deploy-dev/model.plan'; proc = TRTProxy(p,0)" """
        )    
