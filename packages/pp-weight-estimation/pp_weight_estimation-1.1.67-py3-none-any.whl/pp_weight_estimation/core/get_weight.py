...
def get_final_mask(masked_img,color,val =55,show=False,logger=None):
    """
...
def get_final_mask_filter(masked_image,colors,val=55,show=False):   
    new_image = masked_image.copy()
    for color in colors:
        new_image = get_final_mask(new_image,color,val,show=show)
