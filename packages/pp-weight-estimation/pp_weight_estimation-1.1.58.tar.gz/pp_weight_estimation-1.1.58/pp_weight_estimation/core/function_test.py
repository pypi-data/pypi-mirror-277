...
def process_pipeline(config_path: str, logger: logging.Logger = None, **kwargs) -> Union[pd.DataFrame, List]:
    ...
    for index, (site_id, local_image_path, food_item,local_image_folder,image_dict_name,reference_image,reference_image_gt,rework_g,model_final_name) in enumerate(entries):
        try:
            ...
            if save_plots:
                final_image_path = local_image_folder + image_dict_name + "_" +todays_date  +"_final_mask.png"
            ...
            masked_img_ref = get_seg(reference_image, model, mask_val=mask_model_val, resize=True, new_bg_removal=new_bg_removal, equalize=equalizer)
            final_mask_ref = get_final_mask(reference_image, masked_img_ref, color=color, val=final_mask_vals, logger=logger)
            new_final_img_ref = get_final_img(reference_image, final_mask_ref,mask_val)  ## saving final mask with the image
    ...
    try:
        temp_pd = input_df[['site_id','food_item_label','pred_w2','error','success']]
        if gpt_response:
