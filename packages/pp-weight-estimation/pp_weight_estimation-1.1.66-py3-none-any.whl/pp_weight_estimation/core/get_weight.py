...
def get_seg(img,model,mask_val=0.5,resize=True,new_bg_removal=False,equalize=False,show=False,logger=None):
    """
...
def get_final_mask(ori_img,masked_img,color,val =55,show=False,logger=None):
    """
    ...
    if show:
        ...
        if masked_image.shape[0]==1600:
            masked_image = cv2.resize(masked_image, (1600, 1200), interpolation = cv2.INTER_AREA)
        #masked_image = np.copy(ori_img)
        masked_image[final_img<= 0.5] = 0
        axarr2.imshow(masked_image)
    return final_img

def get_final_mask_filter(ori_img,masked_image,colors,val=55,show=False):   
    new_image = masked_image.copy()
    for color in colors:
        new_image = get_final_mask(ori_img,new_image,color,val)
