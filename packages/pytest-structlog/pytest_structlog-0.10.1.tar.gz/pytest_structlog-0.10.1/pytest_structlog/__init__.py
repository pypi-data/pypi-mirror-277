...
KEEP_DEFAULT: set[str] = {
    "PositionalArgumentsFormatter",
    "ExceptionRenderer",
    "dict_tracebacks",
    "merge_contextvars",
}
_keep = KEEP_DEFAULT.copy()
_evict: set[str] = set()
...
def log(
    ...
    if _evict:
        new_processors += [p for p in orig_processors if _name(p) not in _evict]
    else:
        new_processors += [p for p in orig_processors if _name(p) in _keep]
    ...
    cfg(processors=orig_processors)
...
def pytest_configure(config: pytest.Config) -> None:
    user_keep = {
        *config.getoption("structlog_keep"),
        *config.getini("structlog_keep"),
    }
    user_evict = {
        *config.getoption("structlog_evict"),
        *config.getini("structlog_evict"),
    }
    if user_evict and user_keep:
        raise pytest.UsageError(
            "--structlog-keep and --structlog-evict are mutually "
            "exclusive. Specify one or the other, not both."
        )
    _keep.update(user_keep)
    if user_evict:
        _evict.update(user_evict)


def pytest_unconfigure(config: pytest.Config) -> None:
    _keep.clear()
    _keep.update(KEEP_DEFAULT)
    _evict.clear()
