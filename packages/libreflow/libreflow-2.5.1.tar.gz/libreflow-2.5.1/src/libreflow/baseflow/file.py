...
class OpenRevision(GenericRunAction):
    ...
    def extra_argv(self):
        ...
        if project_settings.enable_publish_read_only.get() is True:
            if self._file.format.get() == 'blend' and not self._revision.is_working_copy():
...
class Revision(CustomEntity):
    ...
    def get_path(self, relative=False):
        ...
        The function will raise an exception if the relative
        path if this revision is undefined (empty string or
        `None`).
        '''
        path = self.path.get()
...
class OpenTrackedFileAction(GenericRunAction):
    ...
    def extra_argv(self):
        ...
        if project_settings.enable_publish_read_only.get() is True:
            if self._file.format.get() == 'blend' and not revision.is_working_copy():
                args.extend(['--python', resources.get("scripts.blender", "disable_save_keymap.py")])
...
class LinkedJob(jobs_flow.Job):
    ...
    def link_jobs(parent, child):
        ...
        child._waited_jobs.add(parent.job_id.get(), child._waited_jobs.len())
    
    def notify_children(self):
        for oid in self._children.get():
            child = self.root().get_object(oid)
            child.notify_done(self.job_id.get())
    
    def notify_done(self, jid):
        if not self._waited_jobs.has(jid):
            return
        
        self._waited_jobs.remove(jid)
...
bpy.ops.lfs.playblast(do_render=%s, filepath='%s', studio='%s', project='%s', sequence='%s', scene='%s', do_simplify=%s, version='%s', template_path='%s')""" % (
            str(do_render),
            self.output_path,
...
class RenderAEPlayblast(flow.Action):
    ...
    def _render_image_sequence(self, revision_name, render_settings, output_module):
        render_image_seq = self._file.render_image_sequence
        render_image_seq.revision.set(revision_name)
