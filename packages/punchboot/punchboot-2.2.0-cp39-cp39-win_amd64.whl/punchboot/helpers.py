...
def pb_id(cmd: str) -> int:
    """Convert a string to a BPAK ID.

    This is used when calling a board specific command or when addressing
    keys.

    The underlying function is zlibs crc32 that mangles strings into checksums
    used in a bunch of different places.

    Keyword Arguments:
    cmd -- String input

    Returns a checksum result as an integer.
    """
    _table = [
        0x00000000,
        0x77073096,
    ...
    value: int = 0
    for ch in cmd:
        value = _table[(value ^ ord(ch)) & 0xFF] ^ (value >> 8)
