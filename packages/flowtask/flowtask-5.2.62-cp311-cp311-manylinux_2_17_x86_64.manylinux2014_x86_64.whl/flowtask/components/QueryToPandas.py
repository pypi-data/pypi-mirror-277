...
from ..exceptions import ComponentError, DataNotFound, NotSupported, FileError
...
class QueryToPandas(DtComponent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(QueryToPandas, self).__init__(loop=loop, job=job, stat=stat, **kwargs)
                ...
                async with await connection.connection() as conn:
                    for key, query in self._query.items():
                        ...
                        if hasattr(self, "raw_result"):
                            ...
                            self._variables[f"{self.TaskName}_{key}_NUMROWS"] = len(
                                result
                            )
                        else:
