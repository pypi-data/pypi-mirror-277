...
from dektools.version import version_sorted, version_is_digits
...
def get_ins(name):
    ...
    return gitea.get_ins(data['url'], data['token'])


@app.command()
def login(url, name=default_name):  # url: {schema}://{host}{port}
    token = getpass(f"Please input token: ")
    get_cfg(name).update(dict(url=url.rstrip('/ '), token=token.strip()))
    ...
    for org_name, org_data in list_repos(path).items():
        for repo_name, repo_data in org_data.items():
            ...
            if mirror:
                ...
                tags_others = []
                for tag in tags:
                    if tag.startswith('v') and version_is_digits(tag):
                        tags_versions.append(tag)
                    else:
                        tags_others.append(tag)
                tags_versions = version_sorted(tags_versions)
                push_tag_count = 2
