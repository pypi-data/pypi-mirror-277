...
def backup(namespace, name, dest, cluster=default_cluster, progress_cls=ProgressTqdm):
...
def restore(namespace, name, src, cluster=default_cluster, progress_cls=ProgressTqdm):
    data = get_cfg(cluster).get()
    k8s = K8s.from_manual(data['host'], data['token'], data['port'])
    pod_name = next(k8s.list_pods_of_deploy(namespace, name)).metadata.name
