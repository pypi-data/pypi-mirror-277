...
class UploaderService:
    def __init__(self, api_url, token, version = 1, semaphore_limit=5, retry_delay = 300):
    ...
    async def create_object(self, session, size_bytes, key):
        url = f"{self.api_url}/api/v{self.version}/SLizAI/CreateObject"
        data = {
    ...
    async def create_part(self, session, signature, part_number, upload_id):
        url = f"{self.api_url}/api/v{self.version}/SLizAI/CreatePart"
        data = {
            "signature": signature,
    ...
    async def complete_part(self, session, signature, part_etags, upload_id):
        url = f"{self.api_url}/api/v{self.version}/SLizAI/CompletePart"
        data = {
            "signature": signature,
    ...
    async def retry_failed_uploads(self, failed_files):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [self.upload_file_multipart(session, file_path, key, self.semaphore) 
    ...
    async def upload_folder(self, folder_path, include_folder, parent_folder_name):
        async with aiohttp.ClientSession() as session:
            tasks = []
            failed_files = []
