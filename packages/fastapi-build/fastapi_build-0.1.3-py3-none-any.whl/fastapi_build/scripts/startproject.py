...
@click.option('--example-api', is_flag=True, help='是否添加api样例', default=False)
...
def main(example_api, project_name: str):
    ...
    for row in base_dir.iterdir():
        if row.name in copy_dirs:
            shutil.copytree(row, target_dir / row.name, dirs_exist_ok=True)
    ...
    if example_api:
        shutil.copytree(base_dir / 'api' / 'example_api', target_dir / 'api' / 'example_api')
    remove_header(target_dir)
    print(f"Successfully created project {project_name}, project dir: {target_dir.parent}, enjoy it...")
