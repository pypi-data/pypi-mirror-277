...
class EAPIClient:
    def __init__(self, access_key: str, secret_key: str, user_account: str, user_password: str, base_url='http://121.40.232.157:8090/api/interfaceInfo/invoke'):
        """
        初始化OpenAPI客户端
    ...
    def _make_request(self, method: str, interfaceInfoId: int, json_payload: Dict[str, Any] = None):
        ...
        if json_userload is not None:
            print(json_userload)
            user_response = requests.request("POST", 'http://121.40.232.157:7529/api/user/login', headers={'Content-Type': 'application/json'}, json=json_userload)
            print(user_response)
        if json_payload is not None:
            print(json_payload)
            response = requests.post(self.base_url, json=json_final_payload, headers=headers, verify=False)
