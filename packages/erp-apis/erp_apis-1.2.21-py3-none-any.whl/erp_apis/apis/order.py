...
# 查询订单操作历史
def OrderEditorNew(o_id_en: str):
    '''
    获取订单操作历史
    :return: 查询结果
    '''
    return Request(
        url='https://www.erp321.com/app/order/order/OrderEditorNew.aspx',
        # params=getDefaultParams({'defaultParams': ["_t", "_c", "owner_co_id"],
        #                          'order_id': o_id_en,
        #                          'fromList': True}),
        params = {
            'order_id': o_id_en,
...
# 清空已设快递
def ClearExpress(oid: Union[str, int]):
    '''
    清空已设快递
    :param oid:  内部订单号
    :return: 执行结果
    ...
    return OrderRequest({
        '__CALLBACKPARAM': dumps(
            {"Method": "SetLcs", "Args": ["{EMPTY}", oid, "false", "false"], "CallControl": "{page}"}
        ),
    }, method='SetLcs')
...
# 合并订单还原到合并前
def ResetMerges(oid: Union[str, int]):
    '''
    合并订单还原到合并前
    :param oid:  内部订单号
    :return: 执行结果
    ...
    return OrderRequest({
        '__CALLBACKPARAM': dumps(
            {"Method": "ResetMerges", "Args": [dumps({"o_id": str(oid)})], "CallControl": "{page}"}
        ),
    }, method='ResetMerges')
