...
def test_autorefs():
    ...
    algs = {
        "ppr.85": pg.PageRank(0.85, preprocessor=pre, tol=1.0e-9, max_iters=1000),
        "ppr.99": pg.PageRank(0.99, preprocessor=pre, tol=1.0e-9, max_iters=1000),
        "hk3": pg.HeatKernel(3, preprocessor=pre, tol=1.0e-9, max_iters=1000),
        "hk5": pg.HeatKernel(5, preprocessor=pre, tol=1.0e-9, max_iters=1000),
        "hk5'": pg.HeatKernel(5, preprocessor=pre, tol=1.0e-9, max_iters=1000),
    }
...
def test_filter_citations():
    ...
    assert (
        pg.HeatKernel(coefficient_type="taylor").cite()
        != pg.HeatKernel(coefficient_type="chebyshev").cite()
    )
    assert (
        pg.HeatKernel(optimization_dict=dict()).cite()
        != pg.HeatKernel(optimization_dict=None).cite()
    )
...
def test_explicit_citations():
    ...
    assert (
        "with parameters tuned \\cite{krasanakis2022autogf}"
        in pg.ParameterTuner(lambda params: pg.PageRank(params[0])).cite()
    )
    ...
    assert ".500 " in pg.Top(0.5).cite()
    assert (
        pg.AbsorbingWalks().cite()
        in pg.BoostedSeedOversampling(pg.AbsorbingWalks()).cite()
    )
    assert (
        "krasanakis2018venuerank"
        in pg.BiasedKernel(converge_to_eigenvectors=True).cite()
    )
    ...
    assert (
        "krasanakis2022pygrank"
        in pg.GenericGraphFilter(optimization_dict=dict()).cite()
    )
...
def test_postprocessor_citations():
    ...
    assert (
        pg.Normalize(pg.PageRank(), "sum").cite()
        != pg.Normalize(pg.PageRank(), "range").cite()
    )
    assert (
        pg.Ordinals(pg.PageRank()).cite() != pg.Normalize(pg.PageRank(), "sum").cite()
    )
    ...
    assert (
        pg.SeedOversampling(pg.PageRank(), method="safe").cite()
        != pg.SeedOversampling(pg.PageRank(), method="top").cite()
    )
    assert (
        pg.BoostedSeedOversampling(pg.PageRank(), objective="partial").cite()
        != pg.BoostedSeedOversampling(pg.PageRank(), objective="naive").cite()
    )
    assert (
        pg.BoostedSeedOversampling(
            pg.PageRank(), oversample_from_iteration="previous"
        ).cite()
        != pg.BoostedSeedOversampling(
