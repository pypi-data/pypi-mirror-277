...
def test_primitive_conversion():
    for _ in supported_backends():
        ...
        assert (
            pg.sum(pg.dot(pg.exp(pg.log(pg.to_array([4, 5]))), pg.to_array([2, 2])))
            == 18
        )
...
def test_fastgraph():
    for graph_api in [pg.fastgraph, nx]:
        ...
        sparse = (
            graph.to_scipy_sparse_array()
            if isinstance(graph, pg.Graph)
            else nx.to_scipy_sparse_array(graph, weight="weight", dtype=float)
        )
...
def test_signal_init():
    for backend in supported_backends():
        ...
        signal = pg.GraphSignal(
            next(pg.load_datasets_graph(["graph9"])), {"A": 1, "B": 2}
        )
...
def test_signal_np_auto_conversion():
    ...
    signal = pg.to_signal(graph, tf.convert_to_tensor([1.0, 2.0, 3.0]))
...
def test_signal_direct_operations():
    for _ in supported_backends():
        graph = nx.DiGraph([(1, 2), (2, 3)])
        signal = pg.to_signal(graph, [1.0, 2.0, 3.0])
        ...
        assert pg.sum(signal / pg.to_signal(graph, [1.0, 2.0, 3.0])) == 3
        ...
        assert pg.sum(signal) == 12
        with pytest.raises(Exception):
            signal + pg.to_signal(graph.copy(), [1.0, 2.0, 3.0])
