...
def test_seed_top():
    ...
    for _ in supported_backends():
        ...
        seed(0)
        training, evaluation = pg.to_signal(
            graph, {v: 1 for v in graph if v in original_training or random() < 0.5}
        ...
        for measure in [pg.AUC, pg.NDCG]:
            ...
            ranks = pg.Top(pg.Sweep(pg.PageRank(0.9, max_iters=1000)), 0.9).rank(
                graph, training
            )
            undersampled_result1 = measure(
                evaluation, list(original_training)
            ).evaluate(ranks)
...
def test_seed_oversampling_arguments():
    ...
    with pytest.raises(Exception):
        pg.BoostedSeedOversampling(
            pg.PageRank(), "naive", oversample_from_iteration="unknown"
        ).rank(graph, {"B": 1})
    ...
    for _ in supported_backends():
        ...
        for measure in [pg.NDCG, pg.AUC]:
            ...
            bso_result = measure(evaluation, training).evaluate(
                pg.BoostedSeedOversampling(pg.PageRank(0.9, max_iters=1000)).rank(
                    graph, training
                )
            )
            assert float(base_result) <= float(so_result)
            assert float(so_result) <= float(bso_result)
        pg.PageRank(0.99, max_iters=1000) >> pg.SeedOversampling("top") | pg.to_signal(
            graph, training
        )
        pg.SeedOversampling("neighbors", pg.PageRank(0.99, max_iters=1000)).rank(
            graph, training
        )
        pg.BoostedSeedOversampling(
...
def test_separate_normalization():
    graph = next(pg.load_datasets_graph(["graph5"]))
    for _ in supported_backends():
        algorithm = pg.PageRank(preserve_norm=False) + pg.SeparateNormalization(
            ["A", "B"]
        )
...
def test_sequential():
    ...
    for _ in supported_backends():
        ...
        assert (
            pg.sum(pg.abs(posterior1 - posterior2)) < pg.epsilon()
        )  # TODO: investigate when not exactly zero
        assert (
            pg.sum(pg.abs(posterior1 - posterior3)) < pg.epsilon()
        )  # TODO: investigate when not exactly zero
    ...
    for _ in supported_backends():
        assert (
            float(
                pg.sum(
                    pg.Normalize("range")
                    .transform(pg.to_signal(nx.Graph([("A", "B")]), [2, 2]))
                    .np
                )
            )
            == 4
...
def test_ordinals():
    ...
    for _ in supported_backends():
        test_result = pg.Ordinals(
            pg.Ordinals(pg.Ordinals(pg.PageRank(normalization="col")))
        ).rank(
            graph, {"A": 1}
        )  # three ordinal transformations are the same as one
...
def test_sweep():
    ...
    for _ in supported_backends():
        ...
        auc1 = pg.AUC({v: 1 for v in evaluation}, exclude=training).evaluate(
            pg.Sweep(pg.PageRank()).rank(graph, {v: 1 for v in training})
        )
        auc2 = pg.AUC({v: 1 for v in evaluation}, exclude=training).evaluate(
            pg.PageRank().rank(graph, {v: 1 for v in training})
        )
        auc3 = pg.AUC({v: 1 for v in evaluation}, exclude=training).evaluate(
            pg.LinearSweep(pg.Transformer(pg.PageRank(), pg.log)).rank(
                graph, {v: 1 for v in training}
            )
        )
        assert auc1 > auc2
...
def test_sweep_streaming():
    ...
    for _ in supported_backends():
        training, evaluation = pg.split(list(group), training_samples=0.1)
        auc1 = pg.AUC({v: 1 for v in evaluation}, exclude=training).evaluate(
            (pg.PageRank() >> pg.Sweep()).rank(graph, {v: 1 for v in training})
        )
        auc2 = pg.AUC({v: 1 for v in evaluation}, exclude=training).evaluate(
            pg.PageRank().rank(graph, {v: 1 for v in training})
        )
        auc3 = pg.AUC({v: 1 for v in evaluation}, exclude=training).evaluate(
            ...
            | pg.to_signal(graph, {v: 1 for v in training})
        )
        assert auc1 > auc2
...
def test_threshold():
    ...
    for _ in supported_backends():
        ...
        cond1 = pg.Conductance().evaluate(
            pg.Threshold(pg.Sweep(algorithm), "gap").rank(
                graph, {v: 1 for v in training}
            )
        )
        cond2 = pg.Conductance().evaluate(
            pg.Threshold(0.3).transform(algorithm.rank(graph, {v: 1 for v in training}))
        )  # try all api types
