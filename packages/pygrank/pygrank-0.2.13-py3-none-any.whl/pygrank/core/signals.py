...
class GraphSignal(MutableMapping):
    ...
    def normalized(self, normalize=True, copy=True):
        ...
        if copy:
            ...
            ).normalized(normalize, copy=False)
        if normalize:
...
class NodeRanking(object):
    ...
    def __or__(self, data) -> GraphSignal:
        if not isinstance(data, GraphSignal):
            raise Exception(
                "Can only apply signals into rankers (use pygrank.to_signal(graph, data)) to create those)"
            )
        return self(data)
    ...
    def propagate(self, graph, features, *args, **kwargs):
        return backend.combine_cols(
            [
                self.rank(graph, col, *args, **kwargs)._np
                for col in backend.separate_cols(features)
            ]
        )
...
class Add(NodeRanking):
    ...
    def rank(
        ...
        **kwargs
        ...
        return self.ranker1.rank(
            graph, personalization, *args, **kwargs
        ) + self.ranker2(graph, personalization, *args, **kwargs)
