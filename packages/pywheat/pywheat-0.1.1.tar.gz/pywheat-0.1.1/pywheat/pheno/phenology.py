...
# 
# version: 0.1.0
# Copyright: (c) November 2023, May 2024
...
def getGDDE(P1V, P1D, VREQ, _doy, _Tn, _Tx, maxSUMDTT, latitude, Vern=0):
    ...
    # Updated 2024-03-05
    #y = 0.0000000x + 6.1999998
    GDDE = 0.2035874 * obsTT - 6.1999998
...
def getPHINT(P1V, P1D, VREQ, _doy, _Tn, _Tx, maxSUMDTT, latitude, Vern=1):
    ...
    # Updated 2024-03-05
    # y = 0.3031310x + 6.1242712
    PHINT = 0.3031310 * obsTT + 6.1242712
...
def getP5(P1V, P1D, VREQ, _doy, _Tn, _Tx, maxSUMDTT, latitude, Vern=0):
    ...
    # Updated 2024-03-05
    # y = 0.9545484x + 11.8990980
    P5 = 0.9545484 * obsTT + 11.8990980
...
# -------------------------------------------
''' Estimate phenological stages using calibrated values '''
def apply_pheno_v3(df_IWIN, arr_final, config, idx_data=39, sz=400):
    params = init_params() #initparams
    # Run model for each site
    res_arr = []
    params_arr = []
    #idx_data = 38 # Count of columns with pheno data
    #['UID', 'CID', 'SID', 'GID' ,'location', 'Occ', 'loc_code', 'country',
    #    'locationname', 'cycle', 'Nursery_Yr', 'Nursery',  'sowing', 'emergence', 'heading',
    #    'maturity', 'Days_To_Heading', 'Days_To_Anthesis', 'Days_To_Maturity',
    #    'lat', 'lon', 'ObsYield', 'Pedigree', 'E', 'G', 'GrandParent', 'genotype',
    #    'ObsEmergDAP', 'ObsHeadingDAP', 'ObsAnthesisDAP', 'ObsMaturityDAP',
    #    'ObsEmergDOY', 'ObsHeadingDOY', 'ObsAnthesisDOY', 'ObsMaturityDOY',
    #    '1000_GRAIN_WEIGHT','PLANT_HEIGHT','TEST_WEIGHT']
    # Warm up
    for i in range(len(arr_final[:100])):
        UID = int32(arr_final[i][0])
        location = int32(arr_final[i][4]) #location
        sowing_date = arr_final[i][12] #sowing
        lat = float32(arr_final[i][19])
        lon = float32(arr_final[i][20])
        ObsEmergenceDAP = int32(arr_final[i][27])
        ObsHeadingDAP = int32(arr_final[i][28])
        ObsAnthesisDAP = int32(arr_final[i][29])
        ObsMaturityDAP = int32(arr_final[i][30])
        wdates = list(np.asarray(arr_final[i][idx_data:sz*1+idx_data], dtype='<U32'))
        DOYs = np.asarray(arr_final[i][sz*1+idx_data:sz*2+idx_data], dtype='int32')
        Tn = np.asarray(arr_final[i][sz*2+idx_data:sz*3+idx_data], dtype='int32')
        Tx = np.asarray(arr_final[i][sz*3+idx_data:sz*4+idx_data], dtype='int32')
    ...
    if (len(res_arr)>0):
        ...
        # Clean some wrong results
        ph.dropna(subset=['UID', 'location'], inplace=True)
        #ph[df_iwinPheno_v1['UID'].isnull()]
        ph.drop_duplicates(inplace=True)
        
        # Try to convert formats
        def convertFormatDates(d):
            try:
                return dt.datetime.strptime(d, "%Y-%m-%d") #pd.to_datetime(str(d), format="%Y-%m-%d")
            except Exception as err:
                return np.nan
        try:
            dates_columns = ['Sowing', 'SimGermination', 'SimEmergence', 'Sim Term Spklt', 'Sim End Veg', 'SimHeading', #'End Ear Gr or Heading'
            ...
            for c in dates_columns:
                ph[c] = ph[c].apply(convertFormatDates )
            
        except Exception as err:
            pass
        # Try to convert to integer
        def convertFormatInteger(v):
            try:
                return int(v)
            except Exception as err:
                return -99 #np.nan
        try:
            #intcols = ['UID','location']
            #ph[intcols] = ph[intcols].astype(np.uint32)
            intcols = ['UID','location', 'g_DOY', 'g_DAP', 'g_AGE', 'g_SUMDTT',
                      'e_DOY', 'e_DAP', 'e_AGE', 'e_SUMDTT',
                      'ts_DOY', 'ts_DAP', 'ts_AGE', 'ts_SUMDTT',
                      'ev_DOY', 'ev_DAP', 'ev_AGE', 'ev_SUMDTT',
                      'h_DOY', 'h_DAP', 'h_AGE', 'h_SUMDTT', #'End Ear Gr or Heading'
            ...
            #ph[intcols] = ph[intcols].astype(np.float32)
            for c in intcols:
                ph[c] = ph[c].apply(convertFormatInteger )
        except Exception as err:
            pass
        ...
        try:
            ...
            obscols = ['ObsEmergDOY','ObsEmergDAP', 'ObsHeadingDOY', 'ObsHeadingDAP', 
                       'ObsAnthesisDOY', 'ObsAnthesisDAP', 'ObsMaturityDAP', 'ObsMaturityDOY']
            for c in obscols:
                df.loc[(df[c]==-99), f'{c}'] = np.nan
...
# -------------------------------------------
# New version - minimum requirements
# -------------------------------------------
''' Transform data into PyWheat format to accelerate the calculations in parallel way 

'''
def prepareDF_InPyWheatFormat(df, weather_data=None, saveArr=True, path_to_save_results='./'):
    if 'UID' not in df.columns:
        print("Column `UID` not found. Please check your dataframe has this renamed column.")
        return 
    if 'location' not in df.columns:
        print("Column `location` not found. Please check your dataframe has this renamed column.")
        return 
    if 'sowing' not in df.columns:
        print("Column `sowing` not found. Please check your dataframe has this renamed column.")
        return 
    if weather_data is None:
        print("Weather data was not found. ")
        return
    if 'location' not in weather_data.columns:
        print("Column `location` not found in weather table. Please check your dataframe has this column.")
        return 

    if 'DATE' not in weather_data.columns: # or 'date' not in weather_data.columns:
        print("Column `DATE` not found in weather table. Please review your dataframe has this column.")
        return 
        
    # Extract unique sowing dates by location
    sw_by_loc = df.groupby(['location','sowing'], as_index=False).agg({'UID':'count'}).drop_duplicates(['location','sowing'])
    
    # Organizar los datos de una forma diferente
    # para cada location, sowing_date, incluir arreglos de 1D de 400 datos consecutivos de clima
    # para tener data suficiente para realizar los calculos durante la estacion de crecimiento
    # Todos los 400 valores de cada variable son escalados o multiplicados por 100 para convertirlos a enteros
    # Esto hace la manipulación mas fácil y agíl, salvando espacio
    
    # Organizando con pandas
    pheno_weather = sw_by_loc[['location', 'sowing']].copy()
    pheno_weather = pheno_weather.values
    pheno_weather[:,1] = [d.strftime('%Y-%m-%d') for d in pheno_weather[:,1]] # sowing
    
    # Extraemos los datos de clima para cada localización y fecha de siembra
    print("Extracting daily weather data for each location and sowing date...")
    # Esto permite evitar duplicar procesos de extracción para sitios iguales y reduce el tiempo de procesamiento
    pw_dict = dict() # Numpy Dictionary
    # Convert to Numba Dict
    dict_clima = Dict.empty(
                 key_type=types.Tuple((types.unicode_type, types.unicode_type)),
                 value_type=types.int32[:],
             )
    pheno_weather_locxsw = np.empty(len(pheno_weather), dtype='object')
    for i in tqdm(range(len(pheno_weather))): 
        #loc = Weather['location'][idx]
        e_date = (dt.datetime.strptime(pheno_weather[i,1], "%Y-%m-%d") + dt.timedelta(days = 400)).strftime("%Y-%m-%d")
        w = weather_data[( (weather_data['location']==pheno_weather[i,0]) 
                     & (weather_data['DATE']>=pheno_weather[i,1]) & (weather_data['DATE']<e_date) )]
        wdates = np.asarray([ d.strftime("%Y-%m-%d") for d in w['DATE']]).astype('<U32')
        DOYs = np.asarray([ d.strftime('%j') for d in w['DATE']]).astype(np.int32)
        tn = np.asarray(w['TMIN']*100).astype(np.int32) # OJO si los valores son string produce errores
        tx = np.asarray(w['TMAX']*100).astype(np.int32)
        #rhx = np.asarray(w['RHUMx']*100).astype(np.int32)
        #pcp = np.asarray(w['PCP']*100).astype(np.int32)
        #srad = np.asarray(w['SolRad']*100).astype(np.int32)
        #pheno_weather_locxsw[i] = np.hstack(( i, pheno_weather[i,:], wdates, DOYs, tn, tx, rhx, pcp, srad ))
        pheno_weather_locxsw[i] = np.hstack(( i, pheno_weather[i,:], wdates, DOYs, tn, tx ))
        # filling dictionary
        pw_dict[( str(pheno_weather[i,0]), str(pheno_weather[i,1]) )] = pheno_weather_locxsw[i] 
        #dict_clima[( str(pheno_weather[i,0]), str(pheno_weather[i,1]) )] = np.hstack(( i, DOYs, tn, tx, rhx, pcp, srad )).astype(np.uint32)
        dict_clima[( str(pheno_weather[i,0]), str(pheno_weather[i,1]) )] = np.hstack(( i, DOYs, tn, tx )).astype(np.uint32)
    #
    # file size
    # print(sys.getsizeof(pheno_weather_locxsw), "bytes", round(sys.getsizeof(pheno_weather_locxsw) / 1024 / 1024, 2), "MB")
    # Save arrays
    if (saveArr is True):
        hoy = dt.datetime.now().strftime('%Y%m%d')
        with open(os.path.join(path_to_save_results, f'data_pheno_weather_arr_{hoy}.dat'), 'wb') as f:
            pickle.dump(pheno_weather_locxsw, f, pickle.HIGHEST_PROTOCOL)
        
        # Save dictionary 
        # Numba not support pickle so we need to save the python dict to file and load it and convert to numba.Dict
        with open(os.path.join(path_to_save_results, f'data_pheno_weather_arr_{hoy}.pickle'), 'wb') as f:
            # Pickle the 'data' dictionary using the highest protocol available.
            pickle.dump(pw_dict, f, pickle.HIGHEST_PROTOCOL)
        
    # 
    # Format date in text format
    df['sowing'] = df['sowing'].apply(lambda x: str(str(x).split(' ')[0]))
    # Join raw pheno data with weather
    print("Joining raw phenology data with daily weather data for each location and sowing date...")
    arr_final = []
    mis_locs = []
    for i in tqdm(range(len(df))):
        _uid = int(df.iloc[i]['UID'])
        loc = int(df.iloc[i]['location'])
        sw = df.iloc[i]['sowing'] #.strftime("%Y-%m-%d")
        w_loc = dict_clima.get((str(loc), str(sw))) # i, DOYs, tn, tx, #rhx, pcp, srad 
        if w_loc is not None and len(w_loc)>0:
            arr_pw = pheno_weather_locxsw[w_loc[0]] # i, location, sowing, wdates, DOYs, tn, tx, #rhx, pcp, srad
            arr_f = np.hstack(( df.iloc[i].to_numpy(), arr_pw[2:] )) # merge
            arr_final.append(arr_f)
        else:
            mis_locs.append({"loc":loc, "sowing":sw})

    # return results
    # missing locations
    mis_locs = pd.DataFrame(mis_locs).drop_duplicates(subset=['loc'])
    return arr_final, mis_locs
    

''' Estimate phenological stages such as CERES using calibrated values 

    Version that use minimum requirements and does not take into account
    the measured values or ground-truth data

    df: table with minimum data. (UID, location, sowing date, latitude and longitude)
    df_in_arr_format: The dataframe or data table in numpy array format 
    idx_data: Number of columns of original dataframe
    sz: Number of daily values taken from sowing date
    
'''
def extract_phenophases(config, df=None, df_in_arr_format=None, weather_data=None, idx_data=6, sz=400,
                       saveArr=True, path_to_save_results='./'):
    if (df is None):
        print("Table with minimum requirements was not found!")
        return 
    if (df_in_arr_format is None):
        try:
            df_in_arr_format, mis_locs = prepareDF_InPyWheatFormat(df=df, weather_data=weather_data, 
                                                saveArr=saveArr, path_to_save_results=path_to_save_results)
        except Exception as err:
            df_in_arr_format = []
            mis_locs = []
            print("Error loading weather data and transforming dataset into PyWheat format")
            return
    #
    print("Estimating phenological stages...")
    params = init_params() #initparams
    # Run model for each site
    res_arr = []
    params_arr = []
    # Warm up
    for i in range(len(df_in_arr_format[:100])):
        # ['UID','location', 'sowing', 'lat', 'lon']
        UID = int32(df_in_arr_format[i][0])
        location = int32(df_in_arr_format[i][1]) #location
        sowing_date = df_in_arr_format[i][2] #sowing
        lat = float32(df_in_arr_format[i][3])
        lon = float32(df_in_arr_format[i][4])

        # These parameters are not needed to run the model
        #ObsEmergenceDAP = int32(df_in_arr_format[i][5])
        #ObsHeadingDAP = int32(df_in_arr_format[i][6])
        #ObsAnthesisDAP = int32(df_in_arr_format[i][7])
        #ObsMaturityDAP = int32(df_in_arr_format[i][8])
        # But the first attempt were included, just send empty array
        ObsEmergenceDAP = int32(-99)
        ObsHeadingDAP = int32(-99)
        ObsAnthesisDAP = int32(-99)
        ObsMaturityDAP = int32(-99)
        
        wdates = list(np.asarray(df_in_arr_format[i][idx_data:sz*1+idx_data], dtype='<U32'))
        DOYs = np.asarray(df_in_arr_format[i][sz*1+idx_data:sz*2+idx_data], dtype='int32')
        Tn = np.asarray(df_in_arr_format[i][sz*2+idx_data:sz*3+idx_data], dtype='int32')
        Tx = np.asarray(df_in_arr_format[i][sz*3+idx_data:sz*4+idx_data], dtype='int32')
        # Extract values from configuration file "configbycoords" using K-Nearest neighbors
        new_params = get_calibrated_params_v2(sowing_date=sowing_date, latitude=lat, longitude=lon, 
                                           params=params, config=config)
        R = numba.typed.List.empty_list(numba.types.string)
        _ = run_pheno_model(i, new_params, wdates, location, sowing_date, 
                                                ...
                                                lat, lon, DOYs, Tn, Tx, R)
    # Run models
    for i in tqdm(range(len(df_in_arr_format))):
        UID = int32(df_in_arr_format[i][0])
        location = int32(df_in_arr_format[i][1]) #location
        sowing_date = df_in_arr_format[i][2] #sowing
        lat = float32(df_in_arr_format[i][3])
        ...
        Tn = np.asarray(df_in_arr_format[i][sz*2+idx_data:sz*3+idx_data], dtype='int32')
        Tx = np.asarray(df_in_arr_format[i][sz*3+idx_data:sz*4+idx_data], dtype='int32')
        #rhx = np.asarray(df_in_arr_format[i][sz*4+idx_data:sz*5+idx_data], dtype='int32')
        #pcp = np.asarray(df_in_arr_format[i][sz*5+idx_data:sz*6+idx_data], dtype='int32')
        #srad = np.asarray(df_in_arr_format[i][sz*6+idx_data:sz*7+idx_data], dtype='int32')
        #
    ...
    if (len(res_arr)>0):
        ...
        try:
            #df = pd.merge(df[['UID','location', 'lat', 'lon', 'sowing']], ph, on=['UID'], how='right')
            df = pd.merge(df, ph, on=['UID', 'location'], how='right')
            df.drop(columns=['Sowing'], inplace=True)
            #obscols = ['ObsEmergDOY','ObsEmergDAP', 'ObsHeadingDOY', 'ObsHeadingDAP', 
            #           'ObsAnthesisDOY', 'ObsAnthesisDAP', 'ObsMaturityDAP', 'ObsMaturityDOY']
            #for c in obscols:
            #    df.loc[(df[c]==-99), f'{c}'] = np.nan
            ...
            hoy = dt.datetime.now().strftime('%Y%m%d')
            df.to_parquet(os.path.join(path_to_save_results, f'pywheat_phenology_{hoy}.parquet'))
            return df
        except Exception as err:
            pass
    return ph
...
# -------------------------------------------
# Add columns for displaying comparisons
# -------------------------------------------
...
# Create columns with names required for the library
def reformatColumns_toDisplay(df_pheno, fields=None, HDAP2AD=5):
    if (fields is None):
        return
    df = df_pheno.copy()

    df["sowing"] = pd.to_datetime(df["sowing"], format="%Y-%m-%d")

    df['ObsEmergDAP'] = df[fields['ObsEmergDAP']] if fields['ObsEmergDAP'] in df.columns else np.nan
    df['ObsHeadingDAP'] = df[fields['ObsHeadingDAP']] if fields['ObsHeadingDAP'] in df.columns else np.nan
    df['ObsAnthesisDAP'] = df[fields['ObsAnthesisDAP']] if fields['ObsAnthesisDAP'] in df.columns else np.nan
    df['ObsMaturityDAP'] = df[fields['ObsMaturityDAP']] if fields['ObsMaturityDAP'] in df.columns else np.nan
        
    if fields['ObsEmergDAP']=='':
        df['emergence'] = ''
        df['ObsEmergDOY'] = np.nan
    else:
        df['emergence'] = df[['sowing','ObsEmergDAP']].apply(lambda row: getTraitDay(row['sowing'], row['ObsEmergDAP']), axis=1)
        df['ObsEmergDOY'] = df['emergence'].dt.dayofyear
        
    if fields['ObsHeadingDAP']=='':
        df['heading'] = ''
        df['ObsHeadingDOY'] = np.nan
    else:
        df['heading'] = df[['sowing','ObsHeadingDAP']].apply(lambda row: getTraitDay(row['sowing'], row['ObsHeadingDAP']), axis=1)
        df['ObsHeadingDOY'] = df['heading'].dt.dayofyear
        
    if fields['ObsAnthesisDAP']=='':
        if fields['ObsHeadingDAP']!='':
            df['ObsAnthesisDAP'] = df['ObsHeadingDAP'] + HDAP2AD
            df['anthesis'] = df[['sowing','ObsAnthesisDAP']].apply(lambda row: getTraitDay(row['sowing'], row['ObsAnthesisDAP']), axis=1)
            df['ObsAnthesisDOY'] = df['anthesis'].dt.dayofyear
        else:
            df['anthesis'] = ''
            df['ObsAnthesisDOY'] = np.nan
    else:
        df['anthesis'] = df[['sowing','ObsAnthesisDAP']].apply(lambda row: getTraitDay(row['sowing'], row['ObsAnthesisDAP']), axis=1)
        df['ObsAnthesisDOY'] = df['anthesis'].dt.dayofyear

    if fields['ObsMaturityDAP']=='':
        df['maturity'] = ''
        df['ObsMaturityDOY'] = np.nan
    else:
        df['maturity'] = df[['sowing','ObsMaturityDAP']].apply(lambda row: getTraitDay(row['sowing'], row['ObsMaturityDAP']), axis=1)
        df['ObsMaturityDOY'] = df['maturity'].dt.dayofyear
    ...
    obscols = ['ObsEmergDOY','ObsEmergDAP', 'ObsHeadingDOY', 'ObsHeadingDAP', 
               'ObsAnthesisDOY', 'ObsAnthesisDAP', 'ObsMaturityDAP', 'ObsMaturityDOY',
               'e_DOY', 'e_DAP', 'h_DOY', 'h_DAP', 'a_DOY', 'a_DAP', 'm_DOY', 'm_DAP'
              ]
    for c in obscols:
        df.loc[(df[c]<=0), f'{c}'] = np.nan
