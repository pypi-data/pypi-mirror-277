...
class GoogleContacts(GenericCRUD, metaclass=MetaLogger,
    ...
    # TODO GOOGLE_PEOPLE_API_TYPE = 16
    PEOPLE_API_ID = 16
    GOOGLE_CONTACTS_DATA_SOURCE_ID = 10
        ...
        if not self.creds or not self.creds.valid:
            if self.creds and self.creds.expired and self.creds.refresh_token:
                ...
                select_result_dict = self.select_one_dict_by_where(
                    ...
                    where="username=%s AND is_refresh_token_valid=TRUE", params=(email,), order_by="user_external_id DESC")
    ...
    def __authorize(self):
        ...
        flow = InstalledAppFlow.from_client_config(client_config, SCOPES, redirect_uri=self.redirect_uris)
    ...
    def pull_people_api(self, email: str):
        ...
        while True:
            try:
                ...
                for index, contact_dict in enumerate(contact_dicts_list):
                    self.logger.info(log_message=f"Index of contact: {index}, Contents of contact: {contact_dict}")
    ...
    def __insert_contact_details_to_db(self, contact_dict: dict, user_external_id: int,
        ...
        try:
            ...
            GoogleContacts.__insert_link_contact_groups(
            ...
            contact_person_result_dict = GoogleContacts.__insert_link_contact_persons(
                contact_dict=contact_dict) or {}
            ...
            contact_profile_info = GoogleContacts.__insert_contact_profiles(
                contact_dict=contact_dict) or {}
            ...
            GoogleContacts.__insert_link_contact_email_addresses(contact_dict=contact_dict)
            ...
            GoogleContacts.__insert_link_contact_phones(contact_dict=contact_dict)

            # inset link contact_user_externals
            GoogleContacts.__insert_link_contact_user_external(
                contact_dict=contact_dict)

            # insert link contact_internet_domains
            GoogleContacts.__insert_link_contact_domains(
                contact_dict=contact_dict)
    ...
    def __insert_data_source_instance_id(self, email: str, user_external_id: int) -> int:
        ...
        data_source_instance_id = self.insert(
            ...
            data_dict={
                ...
                "computer_name": None,
                "user_external_id": user_external_id,
                "start_index": self.contact_seq_start,
                "end_index": self.contact_seq_end,
            }
        )
    ...
    def pull_contacts_with_stored_token(self, email: str) -> None:
        ...
        self.logger.info("Getting token data from DB", object={
            "email": email, "profile_id": self.profile_id})
        user_externals_local = UserExternalsLocal()
        token_data = user_externals_local.get_auth_details(  # TODO: fix typing
            username=email, system_id=self.GOOGLE_CONTACT_SYSTEM_ID, profile_id=self.profile_id)
        if not token_data:
        ...
        if not self.creds.valid:
            self.logger.error("Stored credentials are not valid.")
            self.logger.error(f"Token info: {token_info}")
    ...
    def __sync_organization(self, contact_dict: dict, update_status: UpdateStatus) -> Optional[int]:
        ...
        if update_status == UpdateStatus.UPDATE_CIRCLEZ:
            ...
            upsert_organization_result = organizations_local.upsert_organization(
    ...
    def __insert_organization_profile(self, organization_id: int, profiles_ids_list: list[int]) -> Optional[int]:
    ...
    @staticmethod
    def __insert_link_contact_groups(contact_dict: dict) -> list:
        contact_id = contact_dict.get("contact_id")
        # TODO groups -> group_names
        groups = []
        groups_linked = None
    ...
    @staticmethod
    def __insert_link_contact_persons(contact_dict: dict) -> dict:
        ...
        contact_person_result_dict = contact_persons.insert_contact_and_link_to_existing_or_new_person(
            contact_dict=contact_dict,
    ...
    @staticmethod
    def __insert_link_contact_email_addresses(contact_dict: dict) -> None:
    ...
    @staticmethod
    def __insert_link_contact_phones(contact_dict: dict) -> None:
    ...
    def __insert_link_contact_user_external(contact_dict: dict) -> int:
        contact_id = contact_dict.get("contact_id")
    ...
    @staticmethod
    def __insert_contact_profiles(contact_dict: dict) -> dict:
        contact_profiles = ContactProfilesLocal()
        # TODO We should link the contact to the person and the person to the profiles 
        insert_information = contact_profiles.insert_and_link_contact_profile(contact_dict=contact_dict) or {}

        return insert_information

    @staticmethod
    def __insert_link_contact_domains(contact_dict: dict) -> list[dict]:
        contact_id = contact_dict.get("contact_id")
        ...
        while website_url:
            domain = DomainLocal()
            domain_insert_information = domain.link_contact_to_url(contact_id=contact_id,
    ...
    def __procces_location_of_google_contact(self, contact_dict: dict) -> Optional[dict]:
        """
    ...
    def __conflict_resolution(self, last_modified_timestamp: str, contact_id: int) -> UpdateStatus:
        ...
        update_status: UpdateStatus = sync_conflict_resolution.get_update_status(
            last_modified_timestamp=last_modified_timestamp,
            data_source_instance_id=self.GOOGLE_CONTACTS_DATA_SOURCE_ID,
            entity_type_id=self.contact_entity_type_id,
...
if __name__ == '__main__':
    ...
    fetcher.pull_people_api("valerka.prov@gmail.com")
