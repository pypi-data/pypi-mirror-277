...
def get_passport_user_serializer():
    ...
    """
    try:
        return getattr(importlib.import_module(settings.PASSPORT_USER_DETAILS_SERIALIZER_PATH),
                       settings.PASSPORT_USER_DETAILS_SERIALIZER_CLASS)
    except ValueError:
        raise ImproperlyConfigured("PASSPORT_USER_DETAILS_SERIALIZER_CLASS must be of the form 'app_label.model_name'")
    except LookupError:
        raise ImproperlyConfigured(
            "PASSPORT_USER_DETAILS_SERIALIZER_CLASS refers to model '%s' that has not been installed" % settings.PASSPORT_USER_DETAILS_SERIALIZER_CLASS
        )
