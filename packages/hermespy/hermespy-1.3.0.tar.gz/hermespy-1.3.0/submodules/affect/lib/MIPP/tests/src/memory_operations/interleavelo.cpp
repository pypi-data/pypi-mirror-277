...
void test_reg_interleavelo()
{
	...
	for (auto i = 0; i < mipp::N<T>(); i++)
		if (i % 2)
			REQUIRE(inputs2[i/2] == *((T*)&ri +i));
		else
			REQUIRE(inputs1[i/2] == *((T*)&ri +i));
}
...
void test_reg_interleavelo2()
{
	if (mipp::N<T>() > 2)
	{
		...
		for (auto i = 0; i < mipp::N<T>()/2; i++)
			if (i % 2)
				REQUIRE(*((T*)&ri + mipp::N<T>()/2 +i) == inputs2[mipp::N<T>()/2 + i/2]);
			else
				REQUIRE(*((T*)&ri + mipp::N<T>()/2 +i) == inputs1[mipp::N<T>()/2 + i/2]);
	}
}
	...
	if (mipp::N<T>() > 4)
	{
		...
		for (auto j = 0; j < 4; j++)
			for (auto i = 0; i < mipp::N<T>()/4; i++)
				if (i % 2)
					REQUIRE(*((T*)&ri + j*mipp::N<T>()/4 +i) == inputs2[j*mipp::N<T>()/4 + i/2]);
				else
					REQUIRE(*((T*)&ri + j*mipp::N<T>()/4 +i) == inputs1[j*mipp::N<T>()/4 + i/2]);
	}
}
