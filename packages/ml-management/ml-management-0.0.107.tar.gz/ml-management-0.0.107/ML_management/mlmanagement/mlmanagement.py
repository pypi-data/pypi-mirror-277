...
def log_object_src(
    ...
    if not os.path.exists(os.path.join(model_path, "conda.yaml")):
        raise Exception("There is no conda.yaml file.")
    ...
    try:
        ...
        if os.path.isabs(model_path):
            sys.path.append(str(Path(*parts[:-1])))

            python_model = importlib.import_module(parts[-1]).get_object()  # noqa: F841
        else:
            python_model = importlib.import_module(".".join(parts)).get_object()  # noqa: F841
...
def set_minio_url(url: str) -> None:
    """
    Set minio URL.
    ...
    """
    variables.minio_url = url


def set_minio_credentials(login: str, password: str) -> None:
    """
    Set login and password for minio.

    If you set the credentials using this function,
    it takes precedence over the credentials from the environment variables.
    """
    variables.minio_username = login
    variables.minio_password = password
...
def get_minio_url() -> str:
    ...
    return variables._get_minio_url()
