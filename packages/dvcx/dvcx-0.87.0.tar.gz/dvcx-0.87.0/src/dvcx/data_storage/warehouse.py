...
class AbstractWarehouse(ABC, Serializable):
    ...
    def get_nodes_by_parent_path(
        ...
        dr = dataset_rows
        where_cond = (dr.c.parent == parent_path) & (dr.c.is_latest == true())
        if parent_path == "":
            # Exclude the root dir
            where_cond = where_cond & (dr.c.name != "")
        query = dr.select(*dr.default_columns).where(where_cond)
        query = self.add_node_type_where(query, type)
        query = query.order_by(
            dr.c.source, dr.c.parent, dr.c.name, dr.c.version, dr.c.etag
        )
    ...
    def _get_nodes_by_glob_path_pattern(
        ...
        return self.get_nodes(
            dataset_rows.select(*dataset_rows.default_columns)
            .where(
                (self.path_expr(dr).op("GLOB")(path_glob))
                & (dr.c.is_latest == true())
                & ~self.instr(relpath, "/")
                & (self.path_expr(dr) != dirpath)
            )
    ...
    def get_node_by_path(self, dataset_rows, path: str) -> Node:
        ...
        dr = dataset_rows
        query = dr.select(*dr.default_columns).where(
            (self.path_expr(dr) == path.strip("/")) & (dr.c.is_latest == true())
        )
        if path.endswith("/"):
    ...
    def expand_path(self, dataset_rows: DatasetRow, path: str) -> list[Node]:
    ...
    def select_node_fields_by_parent_path(
        ...
        return self.db.execute(
            dr.select(*(getattr(dr.c, f) for f in fields))
            .where(where_cond)
            .order_by(dr.c.source, dr.c.parent, dr.c.name, dr.c.version, dr.c.etag)
        )
    ...
    def _find_query(
        ...
        if order_by is not None:
            if isinstance(order_by, str):
                order_by = [order_by]
            query = query.order_by(*(getattr(dr.c, col) for col in order_by))
        return query
    ...
    def get_subtree_files(
        ...
        """
        dr = dataset_rows
        query = self._find_query(dr, node, dr.select(*dr.default_columns))
