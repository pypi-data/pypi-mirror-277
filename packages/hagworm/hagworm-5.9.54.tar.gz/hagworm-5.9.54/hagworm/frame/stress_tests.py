...
class Launcher(MainProcessWithIPC):
    ...
    def __init__(
            ...
            shard_table_bins: typing.List[float] = None, *args, **kwargs
    ):
    ...
    def _get_base_table(self) -> str:
        ...
        for _name, _data_frame in self._data_frame.groupby(self._data_frame.name):
            ...
            _groups = _data_frame.groupby(_data_frame.success)

            _success_data_frame = _groups.get_group(True)
            _failure_data_frame = _groups.get_group(False)

            table.add_row(
                [
                    _name,
                    _success_data_frame.size,
                    _failure_data_frame.size,
                    r'{:.2%}'.format(_success_data_frame.size / max(_data_frame.size, 1)),
                    r'{:.3f}ms'.format(_success_data_frame[r'response_time'].mean()),
                    r'{:.3f}'.format(_success_data_frame[r'response_time'].std()),
                    r'{:.3f}ms'.format(_success_data_frame[r'response_time'].min()),
                    r'{:.3f}ms'.format(_success_data_frame[r'response_time'].max()),
                ]
            )
    ...
    def _get_series_table(self) -> str:
        ...
        labels = [f'{v1}-{v2}(ms)' for v1, v2 in itertools.pairwise(self._shard_table_bins)]
