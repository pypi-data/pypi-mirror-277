...
class RotationBuilder:
  ...
  def __init__(self, stats, skill_library, snap_dots_to_server_tick_starting_at=None, enable_autos=False, ignore_trailing_dots=False, fight_start_time=None, downtime_windows=[]):
    self.__stats = stats
  ...
  def add_next(self, skill_name, skill_modifier=SkillModifier(), job_class=None, num_times=1):
  ...
  def __process_non_dot_follow_up_skill(self, follow_up_skill, priority_modifier, parent_snapshot_time, parent_application_time, se_tracker, job_resource_tracker):
  ...
  def _process_follow_up_skills(self, follow_up_skills, parent_snapshot_time, parent_application_time, se_tracker, job_resource_tracker):
    for i in range(0, len(follow_up_skills)):
      ...
      if RotationBuilder.__follow_up_is_dot(follow_up_skill):
        ...
        self.__process_non_dot_follow_up_skill(follow_up_skill, priority_modifier, parent_snapshot_time, parent_application_time, se_tracker, job_resource_tracker)
  ...
  def __process_all_dots(self, last_event_time):
    for follow_up_dot_skill in self._q_dot_skills:
      ...
      for _dot_start_time, dot_end_time, parent_snapshot_time, priority_modifier in consolidated_dot_times:
        ...
        for application_time in range(dot_start_time, dot_end_time, GameConsts.DOT_TICK_INTERVAL):
  ...
  def __process_skill(self, t, skill, skill_modifier, se_tracker, job_resource_tracker, curr_buffs=StatusEffects(), curr_debuffs=StatusEffects()):
      timing_spec = skill.get_timing_spec(skill_modifier)

      cast_time = self.__get_cast_time(timing_spec, skill_modifier, curr_buffs, curr_debuffs)
      ...
      if follow_up_skills:
        self._process_follow_up_skills(follow_up_skills, snapshot_time, application_time, se_tracker, job_resource_tracker)
  ...
  def __process_q_timed(self):
    ...
    while len(q) > 0:
      ...
      self.__q_button_press_timing.append([curr_t,
                                           skill.name,
      ...
      curr_debuffs, skill_modifier_from_debuffs = curr_debuffs_and_skill_modifier[0], curr_debuffs_and_skill_modifier[1]
      ...
      combo_conditional = combo_tracker.compile_and_update_combo(curr_t, skill, skill_modifier)
      ...
      self.__process_skill(curr_t, skill, skill_modifier, se_tracker, job_resource_tracker, curr_buffs, curr_debuffs)
  ...
  def shift_timelines_for_first_damage_instance(self):
    ...
    while not self._q_snapshot_and_applications.is_empty():
      [priority, event_times, skill, skill_modifier, snapshot_status, _, _, _, _] = self._q_snapshot_and_applications.get_next()
  ...
  def remove_damage_during_downtime(self):
    ...
    while not self._q_snapshot_and_applications.is_empty():
      ...
      application_time = primary_time if secondary_time is None else secondary_time
      snapshot_time = primary_time
      if skill.damage_spec is not None and (self.__is_in_a_downtime_range(application_time) or self.__is_in_a_downtime_range(snapshot_time)):
        continue
      res.add(priority, primary_time, secondary_time, skill, skill_modifier, snapshot_status)
    return  res
  ...
  def __get_applicable_status_effects(self, speed_status_effects_timeline, curr_t, skill, skill_modifier):
    ...
    curr_buffs_and_skill_modifier = se_tracker.compile_buffs(curr_t, skill)
    curr_debuffs_and_skill_modifier = se_tracker.compile_debuffs(curr_t, skill)
    return (curr_buffs_and_skill_modifier, curr_debuffs_and_skill_modifier), job_resource_tracker.compile_job_resources(curr_t, skill)
  ...
  def get_cast_periods(self, speed_status_effects_timeline):
    ...
    while len(q) > 0:
      ...
      combo_conditional = combo_tracker.compile_and_update_combo(t, skill, skill_modifier)
      skill_modifier.add_to_condition(combo_conditional)

      cast_time = self.__get_cast_time(skill.get_timing_spec(skill_modifier), skill_modifier, curr_buffs, curr_debuffs)
      if cast_time > 0:
  ...
  def __get_next_auto_time(self, t, cast_periods):
    ...
    intersecting_cast_periods = list(filter(lambda x: (x[0] <= t) and (x[1] >= t), cast_periods))
    return t if len(intersecting_cast_periods) == 0 else intersecting_cast_periods[-1][1]
  ...
  def __add_autos(self, last_event_time=None):
    ...
    snapshot_time = self._q_snapshot_and_applications.get_first_damage_time() #autos do not have cast times, so snapshot time should be immediate
    application_time = snapshot_time + auto_skill.timing_spec.application_delay
    while application_time < last_event_time:
      self._q_snapshot_and_applications.add(Utils.transform_time_to_prio(snapshot_time), snapshot_time, application_time, auto_skill, SkillModifier(), [True, True])
      (curr_buffs_and_skill_modifier, curr_debuffs_and_skill_modifier), job_resource_conditional = self.__get_applicable_status_effects(speed_status_effects_timeline, snapshot_time, auto_skill, SkillModifier())
      ...
      snapshot_time += Utils.truncate_to_digit(weapon_delay*trait_auto_delay_mult*curr_buffs.auto_attack_delay_mult*curr_debuffs.auto_attack_delay_mult,2)
  ...
  def print_button_press_timings_as_csv(self):
    if len(self.__q_button_press_timing) == 0:
      print('You must call "get_skill_timing() before calling this function, in order to compute when buttons are pressed.')
      return
