...
def imssave(imgs, img_path):
    ...
    for i, img in enumerate(imgs):
        ...
        new_path = os.path.join(os.path.dirname(
            img_path), img_name+str('_s{:04d}'.format(i))+'.png')
...
def uint2tensor4(img, max_value=255):
    if img.ndim == 2:
        img = np.expand_dims(img, axis=2)
    return torch.from_numpy(np.ascontiguousarray(img)).permute(2, 0, 1).float().div(float(max_value)).unsqueeze(0)
...
def tensor2uint(img, max_value=255):
    ...
    if max_value <= 255:
        ...
        return np.int16((img*float(max_value)).round())
