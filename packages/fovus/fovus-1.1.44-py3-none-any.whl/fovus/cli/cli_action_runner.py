...
class CliActionRunner:  # pylint: disable=too-few-public-methods
    ...
    def _install_setup(self):
        ...
        mount_storage_version_script_path = os.path.join(
            os.path.expanduser(PATH_TO_CONFIG_ROOT), "mount_storage.version"
        )
...
aws_secret_access_key = {mount_storage_credentials_body["credentials"]["secretAccessKey"]}
    ...
    def _mount_storage(self, credentials_content):
        ...
        mount_command = (
            f"mount-s3 fovus-{user_id}-{workspace_id}-us-east-1 {mount_storage_path}files --prefix=files/ "
            "--allow-delete --file-mode=0770 --dir-mode=0770 --endpoint-url https://s3.amazonaws.com "
            f"> /dev/null 2>&1 && mount-s3 fovus-{user_id}-{workspace_id}-us-east-1 {mount_storage_path}jobs "
            "--prefix=jobs/ --allow-delete --file-mode=0550 --dir-mode=0550 "
            "--endpoint-url https://s3.amazonaws.com > /dev/null 2>&1"
        )
