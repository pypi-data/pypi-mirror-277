...
def get_offset_fit(th, I, dI, dinfo, scanno,  varylist = None, startlist = None, showfit=True, verbose=True):
    """
    ...
    params = AuI_funs.get_params(dinfo['fitflag'])
    
    if dinfo['fitflag'] == 'NN':
        if verbose: 
            print(f'Using flag {dinfo["fitflag"]}: New cell, New multilayer')
        th_refrac = AuI_funs.water_refract(th, Beam_Energy)
        fitfun = AuI_funs.multilayer_ref_new_model
    elif dinfo['fitflag'] == 'OO':
        if verbose: 
            print(f'Using flag {dinfo["fitflag"]}: Old cell, Old multilayer')
            print('Side entry sample cell: No refraction correction.')
        th_refrac = th
        fitfun = AuI_funs.multilayer_model_Ti
    elif dinfo['fitflag'] == 'NO':
        if verbose: 
            print(f'Using flag {dinfo["fitflag"]}: New cell, Old multilayer')
            print('Top entry sample cell: Using refraction correction.')
        th_refrac = AuI_funs.water_refract(th, Beam_Energy)
        fitfun = AuI_funs.multilayer_model_Ti
    else:
        print(f'Not configured for flag {dinfo["fitflag"]}')
        sys.exit()
    ...
    if showfit:    
        ...
        plt.legend()
        if verbose:
            print(f'Angular offset = {thoff:.3f}') 
    return thoff, th_peak, result

def setup_fit(dinfo,mca_sum):
    """
...
def fluor_multifit(N_align, N_fluor, dinfo):
    ...
    for i, (align_scan_number, fluorescence_scan_number) in enumerate(zip(N_align, N_fluor)):
        ...
        if align_scan_number != last_align_scan_number:
            print('Fitting alignment scan for angle offset')
            ...
            print('Repeated alignment scan. No fit required')
        
        last_align_scan_number = align_scan_number

        mca_data, mca_sum, ndset, angles, npt, exposure_time, norm = spec_utils.get_fluor_data(
            dinfo["spec_filename"], dinfo["mca_filename"], fluorescence_scan_number)
        
        mca_amplitudes, mca_errors = get_fluorescence_data(mca_data / exposure_time, dinfo, norm=None, plot_fit=False)
    ...
    return angles_0, mca_amplitudes_0 / nnorm, mca_errors_0 / nnorm, nflist, thoff
...
def plot_ref_and_fluor(th, mca_amplitudes, mca_errors, I, dinfo, fluorescence_scan_number):
    fig, ax1 = plt.subplots()
    color = 'tab:red'
    ax1.set_xlabel('$\\theta$ (deg)')
    ax1.set_ylabel('fluorecence counts', color=color)
    ax1.errorbar(th, mca_amplitudes, mca_errors, fmt = '-ks',  color=color)
    ax1.tick_params(axis='y', labelcolor=color)

    ax2 = ax1.twinx()  
    color = 'black'
    ax2.set_ylabel('Reflectivity (cps)', color=color)
    ax2.plot(th, I, color=color)
    ax2.tick_params(axis='y', labelcolor=color)
    tit =  f' {dinfo["vortex_filename_stub"]:s} scan {fluorescence_scan_number:d}'
    plt.title(tit)
    fig.tight_layout()  
    plt.show()
