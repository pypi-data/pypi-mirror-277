...
def fluor_multifit(N_align, N_fluor, dinfo):
    ...
    for i, (align_scan_number, fluorescence_scan_number) in enumerate(zip(N_align, N_fluor)):
        ...
        if align_scan_number != last_align_scan_number:
            ...
            elif (dinfo['run_date'] == 'Feb24') or (dinfo['run_date'] == 'Nov23'):
                th, I, exposure_time, norm = spec_utils.get_CLS_refl(data)
            rr = (th > 0.4) & (th < 0.6)
            dI = np.sqrt(I)
            theta_offset, theta_peak, result= get_offset_fit(th[rr], I[rr], dI[rr], dinfo, align_scan_number, showfit=False, verbose=True)
            print(f'scan: {align_scan_number:d} theta_offset = {theta_offset:f} at {theta_peak:f}')
        else:
        ...
        mca_data, mca_sum, ndset, angles, npt, exposure_time, norm = spec_utils.get_fluor_data(
            dinfo, fluorescence_scan_number)
...
def plot_ref_and_fluor(th, mca_amplitudes, mca_errors, I, dinfo,  nflist):
    ...
    tit =  f' {dinfo["vortex_filename_stub"]:s} scan {nflist:s}'
    plt.title(tit)
    fig.tight_layout()  
    plt.show()
