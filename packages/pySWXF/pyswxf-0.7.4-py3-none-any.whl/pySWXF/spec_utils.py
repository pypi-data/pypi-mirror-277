...
def readscan_2(specfile, scanno):
    ...
    try:
        with open(specfile, 'r') as fp:
            ...
            for i, line in enumerate(fp):
                ...
                if not start and f'#S {scanno} ' in line:
                    print(f'Found scan {scanno} at line {i}')
                    start = True
                    title = line
                    continue
        ...
        print(f"Error: The file '{specfile}' was not found.")
...
def get_fluor_data(dinfo,  scan_number, quiet=True):
    ...
    if dinfo['run_date'] == 'Feb24':
        ...
        print(f'Not configured for run_date {dinfo["run_date"]}')
        sys.exit() 
    ...
    for line in lines:
        try:
            ...
            if (lE > x_min) and (lE < x_max):
                print(f'{elem:s} K {line:s} E = {lE:5.2f}')
                if nlab == 0:
        ...
        for line in lines:
            try:
                ...
                if (N2*N1 > .02) and (lE > x_min) and (lE < x_max):
                    print(f'{elem:s} L {edge:s} {line:s} E = {lE:5.2f}')
