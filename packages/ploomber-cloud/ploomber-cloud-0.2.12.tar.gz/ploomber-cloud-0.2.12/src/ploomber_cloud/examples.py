...
def _prompt_user_for_location():
    """Prompt user for the location to download example"""
    location = click.prompt(
        "Enter the location (if empty, \
files will be downloaded in the current directory)...",
        default="",
    )
    if not location:
        return ""
...
def _copy_selected_example(location, path_to_example, example_title):
    ...
    location = f"{location}{path_to_example.split('/')[-1]}/"
