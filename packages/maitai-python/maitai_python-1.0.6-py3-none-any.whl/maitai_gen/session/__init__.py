# Generated by the protocol buffer compiler.  DO NOT EDIT!
# sources: session.proto
# plugin: python-betterproto
# This file has been @generated

from dataclasses import dataclass
from typing import List

import betterproto

from .. import chat as _chat__


@dataclass(eq=False, repr=False)
class Session(betterproto.Message):
    session_id: str = betterproto.string_field(1)
    date_created: int = betterproto.int64_field(2)
    application_id: int = betterproto.int64_field(3)
    eval_requests: List["_chat__.EvaluateRequest"] = betterproto.message_field(4)


@dataclass(eq=False, repr=False)
class SessionContext(betterproto.Message):
    id: int = betterproto.int64_field(1)
    application_id: int = betterproto.int64_field(2)
    session_id: int = betterproto.int64_field(3)
    date_created: int = betterproto.int64_field(4)
    context_body: str = betterproto.string_field(5)
