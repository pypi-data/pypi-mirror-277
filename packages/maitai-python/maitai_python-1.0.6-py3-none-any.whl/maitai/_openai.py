...
class Completions:
    ...
    def create(
        ...
        *,
        ...
        maitai_config = config.get_application_action_config(application_ref_name, action_type)
        ...
        if server_side_inference:
            ...
            response = maitai.Inference.infer(session_id, reference_id, action_type, application_ref_name, completion_params, apply_corrections, evaluation_enabled, callback, timeout=response_timeout)
            if stream:
                return _process_inference_stream(response)
            # ChatCompletion only
            ...
            if stream:
                return _process_openai_stream(session_id, reference_id, action_type, application_ref_name, messages, response, evaluation_enabled, callback)
            else:
...
def _process_openai_stream(session_id: Union[str, int],
    ...
    for chunk in stream:
        ...
        if content is not None:
            full_body += content
        yield maitai_chunk
        if maitai_chunk.choices[0].finish_reason and evaluation_enabled:
...
def _process_inference_stream(stream: Iterable[InferenceStreamResponse]) -> Iterable[ChatCompletionChunk]:
