...
class Dataset:
  ...
  def read(cls, base: str) -> 'Dataset':
    ...
    return Dataset(base, meta.images_dataset)
  ...
  def iterate(self, key: str) -> Iterable[bytes]:
    ...
    if (archive := self.archive(key)) is not None:
      if archive.format == 'zip':
        ...
        folders = _glob(archive.archive)
        for folder in folders:
          for file in os.listdir(folder):
            with open(os.path.join(archive.archive, file), 'rb') as f:
              yield f.read()
  ...
  def _len(self, key: str) -> int | None:
    ...
    return file and file.num_images
