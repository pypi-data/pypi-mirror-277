...
class Client:
    ...
    async def _get(self, path: str, query_params: dict = None, path_params: list = None, key_required: bool = True) -> Dict:
        ...
        if response.status == 429:
            ...
            if not query_params.get("key", None):
                raise KeyRequired(path)
            ...
            try:
                ...
                raise APIError(response, "Voxyl API", f"An unexpected error occurred with the Voxyl API: {text}")
    ...
    def get_all_techniques(self, technique: str) -> List[str]:
        return VOXYL_TECHNIQUES
    
    def get_all_games(self) -> Dict[str, Dict[Dict[str, str], Dict[str, float]]]:
        return VOXYL_GAMES
    
    def get_all_ranks(self) -> Dict[str, Dict[str, str | List[int]]]:
        return VOXYL_RANKS
    
    def get_all_stars(self) -> Dict[str, Dict[str, str | int | List[int]]]:
        return VOXYL_STARS