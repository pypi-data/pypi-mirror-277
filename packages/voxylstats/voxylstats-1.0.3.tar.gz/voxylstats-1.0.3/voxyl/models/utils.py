    ...
    if mode == "PLAYER":
        ...
        return {
            i: (
                ...
                if GAME_ALIASES[mode][i] in data
                else model.__dataclass_fields__[i].type(**GAME_STATS[GAME_ALIASES[mode][i]])
            )
            for i in GAME_ALIASES[mode]
        }

    else:
        if GAME_ALIASES[mode] in data:
...
GUILD_MEMBER_ALIASES = {"uuid": "uuid", "role": "role", "time": "join_date_raw"}
