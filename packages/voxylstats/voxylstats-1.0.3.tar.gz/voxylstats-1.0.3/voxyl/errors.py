...
class GuildMemberNotFound(ArgumentError):
    def __init__(self, member: str, guild_name: str = None):
        super().__init__(f"'{member}' was not found in the guild '{guild_name}'" if guild_name else f"'{member}' was not found in the guild")

class GuildNotFound(APIError):
    def __init__(self, api: str, guild: str, message: str = None):
        super().__init__(resp=None, api=api, message=message if message else f"Guild not found: '{guild}'")
...
class InvalidLeaderBoard(VoxylException):
    pass
...
class PlayerNotFound(APIError):
    def __init__(self, api: str, player: str, message: str = None):
        super().__init__(resp=None, api=api, message=message if message else f"Player not found: '{player}'")

class RateLimitError(APIError):
    def __init__(self, api: str, resp: dict):
        super().__init__(resp=resp, api=api, message=f"You are being rate limited ({api})")
