...
def convert_labels(labels):
    ...
    return labels.codes, labels.categories.tolist()
    ...
    if language == 'cn':
        ...
        if cut_type == 'word':
            if keep_punctuation:
                ...
                def fn(s):
                    return cut_fn(re.sub(REMOVE_CHARS, replace_char, s.strip()))
        else:
            if cut_fn is None:
                cut_fn = cut_char
    ...
    if language == 'cn':
        ...
        if cut_type == 'word':
            if keep_punctuation:
                ...
                return list(cut_fn(re.sub(REMOVE_CHARS, replace_char, sentence.strip())))
        else:
            if cut_fn is None:
                cut_fn = cut_char
            ...
            if keep_punctuation:
                ...
                return cut_fn(re.sub(REMOVE_CHARS, replace_char, sentence.strip()))
