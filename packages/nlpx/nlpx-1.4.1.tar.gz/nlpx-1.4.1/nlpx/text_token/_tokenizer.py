...
class BaseTokenizer:
    ...
    def __init__(self, file: str = None, corpus: Iterable[str] = None, cut_corpus: Iterable[Iterable[str]] = None, vocab: Iterable[str] = None,
            min_frequency=5, reserved_token=[], language='cn', cut_type='word', word_frequency=False, cut_fn=None):
        """
        ...
        if vocab is not None:
            ...
            if corpus is not None:
                cut_corpus = batch_cut(corpus, language=language, cut_type=cut_type, keep_punctuation=True, cut_fn=self.cut_fn)
                del corpus
            if cut_corpus is not None:
                ...
                del sorted_token_frequency, counter, cut_corpus
                gc.collect()
            else:
    ...
    def load(cls, path='vocab.txt', encoding=UTF8, cut_fn=None):
        ...
        return cls(vocab=[word.strip() for word in lines[2:]], reserved_token=lines[1].strip().split(Tokenizer.SAVE_SEP),
    ...
    def from_file(cls, file: str, encoding=UTF8, pattern='*', func=read_file, min_frequency=0, reserved_token=[], language='cn', cut_type='word',
            word_frequency=False, cut_fn=None):
        """
        :param file: 语料文件，可以是文件也可以是文件夹， 如：'./train.txt'. 如果是文件夹，会读取文件夹下的所有目录和文件.
        :param encoding: 编码
        ...
        return cls(corpus=corpus, min_frequency=min_frequency, reserved_token=reserved_token, language=language, cut_type=cut_type,
                        word_frequency=word_frequency, cut_fn=cut_fn)

    @classmethod
    def from_corpus(cls, corpus: Iterable[str], min_frequency=0, reserved_token=[], language='cn', cut_type='word', word_frequency=False, cut_fn=None):
        """
        :param corpus: 语料，每个元素是一句话，如：['上课时学生手机响个不停，老师一怒之下把手机摔了。', '家长拿发票让老师赔', ....]
        :param min_frequency: 最小词频，小于词词频的词会被忽略，默认是0，所有的词都保留
        :param reserved_token: 保留token, 如 '<pad>', '<unk>'等
    ...
    @classmethod
    def from_cut_corpus(cls, cut_corpus: Iterable[Iterable[str]], min_frequency=0, reserved_token=[], language='cn', cut_type='word', word_frequency=False, cut_fn=None):
        """
    ...
    @classmethod
    def from_vocab(cls, vocab: Iterable[str], reserved_token=[], cut_fn=None):
        """
        :param vocab: 词表，如：['上课', '学生', '手机', '不停', '，', '老师']
        :param reserved_token: 保留token, 如 '<pad>', '<unk>'等
        :param cut_fn
        """
        return cls(vocab=vocab, reserved_token=reserved_token, cut_fn=cut_fn)
...
class Tokenizer(BaseTokenizer):
    ...
    def __init__(self, file: str = None, corpus: Iterable[str] = None, cut_corpus: Iterable[Iterable[str]] = None, vocab: Iterable[str] = None,
            min_frequency=10, reserved_token=[], language='cn', cut_type='word', word_frequency=False, cut_fn=None):
        """
        ...
        super().__init__(file=file, corpus=corpus, cut_corpus=cut_corpus, vocab=vocab, min_frequency=min_frequency, reserved_token=reserved_token,
                         language=language, cut_type=cut_type, word_frequency=word_frequency, cut_fn=cut_fn)
...
class TokenEmbedding(Tokenizer):
    ...
    def __init__(self, file: str = None, vocab: Iterable[str] = None, embedding: Iterable[Iterable[float]] = None, reserved_token=[],
        ...
        super().__init__(vocab=vocab, reserved_token=reserved_token, language=language, cut_type=cut_type, cut_fn=cut_fn)
    ...
    @classmethod
    def from_file(cls, file: str, func=load_embedding, is_large_file=False, reserved_token=[], language='cn', cut_type='word', cut_fn=None):
        """
        ...
        :param cut_fn
        """
        return cls(file=file, reserved_token=reserved_token, language=language, cut_type=cut_type, func=func, is_large_file=is_large_file, cut_fn=cut_fn)
    ...
    def from_vocab_embedding(cls, vocab: Iterable[str], embedding: Iterable[Iterable[float]], large_file=False, reserved_token=[], language='cn',
        ...
        :param cut_fn
        """
        return cls(vocab=vocab, embedding=embedding, large_file=large_file, reserved_token=reserved_token,
                   language=language, cut_type=cut_type, cut_fn=cut_fn)
