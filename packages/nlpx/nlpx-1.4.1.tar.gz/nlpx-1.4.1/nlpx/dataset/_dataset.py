...
class TextVecCollator:
	...
	def __call__(self, examples):
		...
		if isinstance(self.tokenize_vec, TokenizeVec):
			...
			max_length = get_texts_max_length(texts, cut_type=self.tokenize_vec.cut_type, language=self.tokenize_vec.language)
			if self.max_length is not None:
				max_length = min(max_length, self.max_length)
...
class PaddingTokenCollator:
	...
	def __call__(self, examples):
		...
		return torch.tensor(self.padding_func(tokens, max_length), dtype=torch.long), labels
