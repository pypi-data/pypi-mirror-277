...
class TextVecCollator:
	...
	def __call__(self, examples):
		...
		if isinstance(self.tokenize_vec, TokenizeVec):
			...
			return self.tokenize_vec(texts, self.max_length, **self.kwargs), labels
...
class TokenizeCollator:
	...
	def __call__(self, examples):
		...
		if isinstance(self.tokenizer, BaseTokenizer):
			return torch.tensor(self.tokenizer.batch_encode(texts, self.max_length, **self.kwargs), dtype=torch.long), labels
...
class PaddingTokenCollator:
	...
	def __init__(self, padding_func, max_length: int = None):
	...
	def __call__(self, examples):
		...
		return torch.tensor(self.padding_func(texts, self.max_length), dtype=torch.long), labels
