...
def df_to_span(df:pd.DataFrame):
    spans = pd.DataFrame(df.values[:, :1], columns=['review'])
    for col in df.columns[1:]:
        spans[col] = df.apply(lambda row: column_to_span(row, col), axis=1)
    return spans

def evaluate(spans:pd.DataFrame, true_column:str="annotation"):
    metrics = {}
    for c in spans.columns[1:]:
        metrics[c] = Evaluator(df[true_column].values, df[c].values, tags=['POS', 'NEU', 'NEG']).evaluate()
