...
ROUTESCHEMA_ONE_OF_SCHEMAS = ["Aws", "Azure", "Google", "Http", "Mqtt", "Proxy", "Radresponder", "Slack", "Snowflake", "Thingworx"]
...
class RouteSchema(BaseModel):
    ...
    actual_instance: Optional[Union[Aws, Azure, Google, Http, Mqtt, Proxy, Radresponder, Slack, Snowflake, Thingworx]] = None
    one_of_schemas: Set[str] = { "Aws", "Azure", "Google", "Http", "Mqtt", "Proxy", "Radresponder", "Slack", "Snowflake", "Thingworx" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = RouteSchema.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, Radresponder):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Radresponder`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in RouteSchema with oneOf schemas: Aws, Azure, Google, Http, Mqtt, Proxy, Radresponder, Slack, Snowflake, Thingworx. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in RouteSchema with oneOf schemas: Aws, Azure, Google, Http, Mqtt, Proxy, Radresponder, Slack, Snowflake, Thingworx. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into RouteSchema with oneOf schemas: Aws, Azure, Google, Http, Mqtt, Proxy, Radresponder, Slack, Snowflake, Thingworx. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into RouteSchema with oneOf schemas: Aws, Azure, Google, Http, Mqtt, Proxy, Radresponder, Slack, Snowflake, Thingworx. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Aws, Azure, Google, Http, Mqtt, Proxy, Radresponder, Slack, Snowflake, Thingworx]]:
