...
def plot_predictions(fam_preds, clan_preds, maps, threshold, seq_name=None, save_pth=None):
    ...
    for entry in clan_unique_test:
        ...
        if torch.count_nonzero(idx) != 0:
            ax2.bar(torch.where(idx)[0], clan_pred_vals[idx], width=1.0, color=clan_c_map[entry.item()], label=clan_keys[entry.item()].split(".")[0])
    ...
    if save_pth:
        plt.tight_layout()
        plt.savefig(f"{save_pth}/{seq_name}_PSALM.png")
