...
def plot_predictions(fam_preds, clan_preds, maps, threshold, seq_name=None):
    ...
    clan_c_map[-2] = 'grey' #used to be red
    clan_c_map[-1] = 'black'
    ...
    for entry in fam_unique_test:
        ...
        if torch.count_nonzero(idx) != 0:
            ax1.bar(torch.where(idx)[0], fam_pred_vals[idx], width=1.0, color=fam_c_map[entry.item()], label=fam_keys[entry.item()])

    for entry in clan_unique_test:
        idx = (clan_pred_labels == entry) & (clan_pred_vals >= threshold)
        if torch.count_nonzero(idx) != 0:
            ax2.bar(torch.where(idx)[0], clan_pred_vals[idx], width=1.0, color=clan_c_map[entry.item()], label=clan_keys[entry.item()])

    # Axis labels
    ax2.set_xlabel('Position')
    ax1.set_title(f'Sequence {seq_name}' if seq_name else 'Sequence')
