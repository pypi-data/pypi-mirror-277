...
def number_increasing_along_column_right_angle_triangle(row=ROWS5, starting_point=STARTING_POINT):
    '''
      1
      2   6
      3   7  10
    ...
    To view pattern run: 'print(number_increasing_along_column_right_angle_triangle.__doc__)'
    '''
    if row < MINROW:
        print("right-angled-triangle-Pattern generation failed, number of rows has to be greater than %d..." % MINROW)
    else:
        a = [i for i in range(row - 1, 0, -1)]
