from patternlib.constants.patternlib_constants import ROWS5

MINROW = 2

def triangle_pointing_left(row=ROWS5):
    '''
            *
          * *
        * * *
      * * * *
    * * * * *
      * * * *
        * * *
          * *
            *

    This generates a triangle pointing to the left with the fill character *

    To view pattern run: 'print(triangle_pointing_left.__doc__)'
    '''
    if row < MINROW:
        print("Pattern generation failed, number of rows has to be greater than %d..." % MINROW)
    else:
        for i in range(1, 2 * row + 1):
            print((' *' * i).rjust(2 * row)) if i <= row else print(
                (' *' * (2 * row - i)).rjust(2 * row))