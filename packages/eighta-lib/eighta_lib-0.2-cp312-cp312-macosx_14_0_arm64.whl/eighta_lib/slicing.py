...
def explore_hdf5_file(file_path):
    ...
    Outputs:
        This function prints the structure of the HDF5 file, including paths, dataset shapes, data types,
        compression (if applied) as well as the size of each dataset in gigabytes.
    """
    def explore_hdf5(item, path='/', indent=0):
        ...
        if isinstance(item, h5py.Dataset):
            if '_index' in item.attrs:
                ...
                print(
                    f"{indent_str}{path} is a Dataset with shape {item.shape}, dtype {item.dtype}, and size {dataset_size_gb:.4f} GB")
...
def get_raw_group_size(source_group, row_indices, col_indices, batch_size):
    ...
    if 'X' in source_group.name:
        ...
        total_size += get_matrix_group_size(source_group, row_indices, col_indices, batch_size, is_csr=True)
...
def get_dataset_size(dataset, row_indices, col_indices, parent_encoding_type=None, parent_group_name=None):
    """
...
def get_group_size(source_group, row_indices, col_indices, batch_size):
    ...
    if parent_encoding_type == 'csr_matrix':
        ...
        total_size += get_matrix_group_size(source_group, row_indices, col_indices, batch_size, is_csr=False)
    elif parent_encoding_type == 'categorical':
        total_size += get_categorical_group_size(source_group, row_indices, col_indices)
        ...
        for key in source_group.keys():
            ...
            if isinstance(item, h5py.Dataset):
                total_size += get_dataset_size(item, row_indices, col_indices, parent_encoding_type, source_group.name)
            elif isinstance(item, h5py.Group):
...
def get_slice_size(source_file_path, rows: slice, cols: slice, batch_size=1000):
    ...
    with h5py.File(source_file_path, 'r') as f:
        ...
        row_indices = np.arange(rows.start or 0, rows.stop if rows.stop is not None else num_rows, rows.step or 1)
        col_indices = np.arange(cols.start or 0, cols.stop if cols.stop is not None else num_cols, cols.step or 1)
...
def read_process_csr_matrix(source_group, row_indices, col_indices):
    ...
    # Concatenate lists only once
    if data_list:
        data_array = np.concatenate(data_list)
        indices_array = np.concatenate(indices_list)
    else:
        data_array = np.array([], dtype=source_group['data'].dtype)
        indices_array = np.array([], dtype=source_group['indices'].dtype)

    indptr_array = total_indptr

    return csr_matrix((data_array, indices_array, indptr_array), shape=(len(row_indices), len(col_indices)))
...
def read_process_csc_matrix(source_group, row_indices, col_indices):
    ...
    data_array = np.concatenate(data_list) if data_list else np.array([], dtype=source_group['data'].dtype)
    indices_array = np.concatenate(indices_list) if indices_list else np.array([], dtype=source_group['indices'].dtype)
    indptr_array = total_indptr

    return csc_matrix((data_array, indices_array, indptr_array), shape=(len(row_indices), len(col_indices)))
...
def read_process_categorical_group(source_group, row_indices, col_indices):
    """
    Process an HDF5 group representing a categorical variable, slicing based on the specified row or column indices.
...
def read_process_dataframe_group(source_group, indices, c_slicing, is_obs):
    """
    ...
    for key in source_group.keys():
        ...
        if isinstance(item, h5py.Dataset):
            ...
            sliced_data[key] = read_process_group(item, indices, indices, c_slicing)

    index_key = "_index"

    # Get the original indices from the parent 'obs' or 'var' group
    if is_obs:
        original_indices = source_group.parent['obs'][source_group.parent['obs'].attrs[index_key]][indices]
    else:
        original_indices = source_group.parent['var'][source_group.parent['var'].attrs[index_key]][indices]
...
def read_process_raw_group(source_group, row_indices, c_slicing):
    """
    ...
    if 'var' in source_group:
        # Slice the 'var' dataframe (use all rows with slice(None) since we want to keep the unsliced version)
        sliced_data['var'] = read_process_dataframe_group(source_group['var'], slice(None), c_slicing, is_obs=False)
...
def read_process_varp_group(source_group, col_indices):
    """
    Process an HDF5 group representing a 'varp' group, slicing based on the specified column indices.
...
def read_process_dataset(dataset, row_indices, col_indices, parent_encoding_type=None, parent_group_name=None):
    """
    ...
    # Scalar datasets
        ...
        data = str(dataset[()], 'utf-8') if dataset.attrs['encoding-type'] == 'string' else dataset[()]
    # 1-D datasets
    elif dataset.ndim == 1:
        data = np.array([str(val, 'utf-8') for val in dataset[:]], dtype=object) if dataset.attrs['encoding-type'] == 'string-array' else dataset[:]
    # 2-D datasets
    elif dataset.ndim == 2:
        if 'layers' in parent_group_name:
...
def read_process_group(source_group, row_indices, col_indices, c_slicing=False):
    """
    ...
    if parent_encoding_type == 'csr_matrix':
        ...
        if c_slicing:
            ...
            sliced_data = csr_matrix((result[0], result[1], result[2]), shape=(len(row_indices), len(col_indices)))
        else:
        ...
        sliced_data = read_process_dataframe_group(source_group, row_indices, c_slicing, is_obs=True)
    elif 'var' in source_group.name and parent_encoding_type == 'dataframe':
        sliced_data = read_process_dataframe_group(source_group, col_indices, c_slicing, is_obs=False)
    elif parent_encoding_type == 'raw':
        sliced_data = read_process_raw_group(source_group, row_indices, c_slicing)
    else:
        ...
        for key in source_group.keys():
            ...
            if isinstance(item, h5py.Dataset):
                # Process dataset
                sliced_data[key] = read_process_dataset(item, row_indices, col_indices, parent_encoding_type,
                                                        source_group.name)
            elif isinstance(item, h5py.Group):
                # Recursively process sub-group
                sliced_data[key] = read_process_group(item, row_indices, col_indices, c_slicing)

    return sliced_data


def read_slice_h5ad(file_path, rows: slice, cols: slice, check_size=False, include_raw=False, c_slicing=False):
    """
    ...
    Args:
        ...
        check_size (bool, optional): If True, check if the slice size fits in memory before slicing. Default is False.
        include_raw (bool, optional): If True, include the 'raw' group in the sliced data. Default is False.
    ...
    if check_size:
        ...
        if total_size_gb > available_memory_gb:
            raise MemoryError(
                f"The slice size ({total_size_gb:.2f} GB) exceeds the available physical + swap memory ({available_memory_gb:.2f} GB).")
    ...
    with h5py.File(file_path, 'r') as f:
        ...
        adata = ad.AnnData(X=X, layers=layers, obs=obs, obsm=obsm, obsp=obsp, raw=raw, uns=uns, var=var, varm=varm,
                           varp=varp)
...
def copy_attrs(source, dest, shape=None):
    ...
    Args:
        ...
        shape (tuple, optional): The shape to set as an attribute in the destination. Default is None.
    """
...
def write_process_raw_group(source_group, dest_group, row_indices, batch_size):
    ...
    if 'X' in source_group.name:
        parent_encoding_type = source_group.attrs.get('encoding-type', None)
        is_csr = parent_encoding_type != "csc_matrix"
        write_process_matrix(source_group, dest_group, row_indices, np.arange(source_group.attrs['shape'][1]), batch_size, is_csr)
...
def write_process_obsp_group(source_group, dest_group, row_indices, batch_size):
    """
    Processes and copies data from an 'obsp' group in the source HDF5 file to the destination HDF5 file,
    slicing based on the specified row indices.
...
def write_process_varp_group(source_group, dest_group, col_indices, batch_size):
    """
    Processes and copies data from a 'varp' group in the source HDF5 file to the destination HDF5 file,
...
def write_process_dataset(dataset, dest_group, row_indices, col_indices, parent_encoding_type=None, parent_group_name=None):
    """
    Processes and copies a dataset from a source HDF5 group to a destination HDF5 group, handling different encoding types and structures.
    ...
    if data is not None:
        dset = dest_group.create_dataset(dataset.name.split('/')[-1], data=data, compression=compression)
...
def write_process_group(source_group, dest_group, row_indices, col_indices, batch_size = 1000):
    ...
    parent_encoding_type = source_group.attrs.get('encoding-type', None)
    if parent_encoding_type == 'csr_matrix':
        write_process_matrix(source_group, dest_group, row_indices, col_indices, batch_size, is_csr=True)
    elif parent_encoding_type == 'csc_matrix':
        write_process_matrix(source_group, dest_group, row_indices, col_indices, batch_size, is_csr=False)
    elif parent_encoding_type == 'categorical':
        ...
        for key in source_group.keys():
            ...
            if isinstance(item, h5py.Dataset):
                write_process_dataset(item, dest_group, row_indices, col_indices, parent_encoding_type, source_group.name)
            elif isinstance(item, h5py.Group):
...
def write_slice_h5ad(source_file_path, dest_file_path, rows: slice, cols: slice, memory_fraction: float = 0.1):
    """
    Slices specified rows and columns from an HDF5-stored AnnData object and writes them to a new HDF5 file.
    ...
    Args:
        ...
        rows (slice): The slice object specifying the row indices to extract.
        cols (slice): The slice object specifying the column indices to extract.
        memory_fraction (float, optional): The fraction of available memory to use for the batch size. Defaults to 0.1.
