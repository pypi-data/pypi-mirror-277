...
class DynamicCompose:
    ...
    def dump(self) -> None:
        ...
        if self.compose_file.exists():
            old_compose_content = self.compose_file.read_text(encoding='utf-8')
        else:
            old_compose_content = ''
        if self.dockerfile_file.exists():
            old_dockerfile_content = self.dockerfile_file.read_text(encoding='utf-8')
        else:
            old_dockerfile_content = ''

        if self.compose_content:
            if (not old_compose_content) or (self.SAFELY_COVER_MARK in old_compose_content) or self.force_cover:
                self.compose_file.write_text(self.SAFELY_COVER_MARK + self.compose_content)
            else:
