...
class BuilderModel:
    ...
    def buildTFM(self, output_path: str,  local_library_path: str, TFM_inputs = None, magnet_data = None):
        """
            Building a TFM model
        """
        builder_ledet = BuilderLEDET(path_input_file=self.path_input_file, input_model_data=self.model_data,
                                     input_map2d=self.path_map2d, flag_build=True, input_roxie_data=self.roxie_data, smic_write_path='skip')
    ...
    def buildXYCE(self, sim_name: str, sim_number: Union[int, str], output_path: str, verbose: bool = None):
        ...
        if self.circuit_data.TFM != None:
            ...
            for key, m_TFM in magnets_TFM.items():
                ...
                if m_TFM.field_interp_value is None:
                    m_TFM.field_interp_value = T
                ...
                bM_TFM.buildTFM(output_path=output_path_TFM, local_library_path=local_library_path, TFM_inputs=TFM_general, magnet_data=m_TFM)
                ...
                self.circuit_data.GlobalParameters.global_parameters[f'C_ground_{key}'] = m_TFM.C_ground
                self.circuit_data.GlobalParameters.global_parameters[f'field_interp_value_{key}'] = m_TFM.field_interp_value
                ...
                for key_TFM in TFM.__dict__.keys():
                    if key_TFM != 'magnets_TFM':
                        # Add for each magnet the corresponding flags to the netlist parameters in the format  'flag_PC' = 'flag_PC'
                        if key_TFM != 'M_CB_wedge' or (key_TFM == 'M_CB_wedge' and TFM.M_CB_wedge):
