...
class TestBuilderFiQuS(unittest.TestCase):
    ...
    def test_BuilderFiQuS_attributes(self):
        # arrange
        magnet_names = ['CCT_1', 'HTS1', 'MQXA', 'MCBRD', 'CAC_Strand_hexFilaments']
        for magnet_name in magnet_names:
            ...
            self.assertTrue(bF.data_FiQuS.magnet.type in ['multipole', 'CCT_straight', 'CWS', 'Pancake3D', 'CACStrand'])  # check if magnet type has been set_up to one of the currently supported types
            if bF.data_FiQuS.magnet.type == 'multipole':
                ...
                self.assertTrue(hasattr(bF, 'data_FiQuS_set'))  # check if additional data_FiQuS_set required by FiQuS multipole exists
