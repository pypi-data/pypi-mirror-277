...
from steam_sdk.data.DataModelCosim import DataModelCosim, sim_FiQuS, sim_XYCE, sim_PSPICE, sim_LEDET, CosimPort, \
    CosimPortModel, CosimPortVariable
...
class TestDataCosim(unittest.TestCase):
    ...
    def test_writeToFile_cosim_all_keys(self):
        ...
        data.PortDefinition['port_1'] = CosimPort()
        data.PortDefinition['port_1'].Models['model_1'] = CosimPortModel()
        data.PortDefinition['port_1'].Models['model_1'].inputs['input_1'] = CosimPortVariable()
        data.PortDefinition['port_1'].Models['model_1'].outputs['output_1'] = CosimPortVariable()
        dict_to_yaml(data.model_dump(), generated_file, list_exceptions=['PortDefinition'])
