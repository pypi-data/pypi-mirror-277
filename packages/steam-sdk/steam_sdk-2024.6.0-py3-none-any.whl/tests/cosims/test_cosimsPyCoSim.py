...
class TestCosimPyCoSim(unittest.TestCase):
    ...
    def test_run_PyCoSim_CCT_FiQuS_LEDET(self):
        ...
        if cosim_data.Simulations[FiQuS_set_name].files_to_copy_after.pre_cosim[0].new_file_name_relative_path == 'Input/<<modelName>>_selfMutualInductanceMatrix_<<n>>.csv':
            new_file_name_relative_path = f'Input/{cosim_data.Simulations[FiQuS_set_name].modelName}_selfMutualInductanceMatrix_{sim_number}.csv'
        else:
        ...
        list_nsti = [NSTI(sim_number, 2, 0, 0), NSTI(sim_number, 2, 1, 0), NSTI(sim_number, 2, 1, 1), NSTI(sim_number, 2, 1, 2), NSTI(sim_number, 2, 1, 3), NSTI(sim_number, 2, 2, 0)]
