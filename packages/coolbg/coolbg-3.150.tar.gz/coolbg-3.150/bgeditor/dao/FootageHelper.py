...
def make_footage_video(arr_splited_video=[]):
    ...
    while i < len(arr_splited_video):
        ...
        if os.path.exists(arr_splited_video[i][1]):
            items.append(arr_splited_video[i][1])
        else:
            cnt_err_vid += 1
        if i == len(arr_splited_video)-1:
            if os.path.exists(arr_splited_video[i][2]):
            ...
            if i > 60:
                ...
                if os.path.exists(arr_splited_video[i][2]):
                    ...
                    cnt_err_vid += 1
                if os.path.exists(arr_splited_video[i+1][0]):
                    items.append(arr_splited_video[i+1][0])
                else:
                    cnt_err_vid += 1
            else:
                trans_vid_path = transistion_video(arr_splited_video[i][2], arr_splited_video[i + 1][0])
