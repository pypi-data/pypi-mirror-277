...
class lmc_linked_model():
    ...
    def set_enum_output_parser(self, enum_or_list):
        ...
        if issubclass(enum_or_list, Enum):
    ...
    def __call__(self, prompt=None, **kwargs):
        ...
        其中`__call__`函数是一个特殊的魔术方法，它使类的实例可以像函数一样被调用。
        ...
        for time_idx in range(self.invoke_times):
            ...
            for retry_id in range(self.retry_count):
                all_logs.append(linked_log(f"invoke time:{time_idx}, retry:{retry_id}"))
                middle_result_list = [init_result_list[-1]]

                normal_invoke_result = self._invoke_list_func(kwargs, middle_result_list, all_logs,
                                                              self.norm_func_list)
                ...
                all_logs.append(linked_log(f"invoke time:{time_idx}, retry:{retry_id} failed, run fix..."))

                exception_invoke_result = self._invoke_list_func(kwargs, middle_result_list, all_logs,
                                                                 self.exception_func_list)
        ...
        return result_list, linked_log_collections(all_logs)
