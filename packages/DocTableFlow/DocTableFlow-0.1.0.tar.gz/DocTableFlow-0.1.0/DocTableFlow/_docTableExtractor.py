...
def _docTableExtractor(input_file):
    ...
    for page_number in range(0, total_pages):
        ...
        def draw_bounding_box(box_coodrinates, bbox, color):
            """
            Draws a rectangular bounding box on the specified axes.

            Parameters:
                box_coodrinates (matplotlib.axes.Axes): The matplotlib axes where the rectangle will be drawn.
                bbox (tuple): A tuple containing the coordinates of the bounding box (x_min, y_min, x_max, y_max).
                color (str): The color of the rectangle's border.
            """

            x_min, y_min, x_max, y_max = box_coodrinates
            box_width = x_max - x_min
            box_height = y_max - y_min
            initial_position = (x_min, y_min)
            bbox.add_patch(
                patches.Rectangle(
                    initial_position,
                    box_width,
        ...
        def draw_rect(rectanlge_coodrinates, ax, color="black"):
            x_min, y_min, x_max, y_max = rectanlge_coodrinates.bbox
            draw_bounding_box((x_min, y_min, x_max, y_max), ax, color)
        ...
        for rect in table_rectangle_data:
            draw_rect(rect, ax)

        # Draw rectangles for each character in the characters list, using red color
        for text_char in table_data_chars:
            draw_rect(text_char, ax, "red")
