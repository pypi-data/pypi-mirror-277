...
def fetch_address_by_zip_code(cursor: Cursor, zip_code: str) -> List[Address]:
    cursor.execute(
        "SELECT p.*, c.*, a.* FROM addresses AS a "
...
def fetch_addresses_from_city(cursor: Cursor, city: City) -> List[Address]:
    cursor.execute("SELECT * FROM addresses " "WHERE city_id=?", (city.id,))
    data = cursor.fetchall()
    addresses = []
    for v in data:
...
def search_addresses(
    cursor: Cursor, query: str, prefecture: Prefecture, city: City, type_: str
) -> List[Address]:
    query = [
        f"SELECT p.*, c.*, a.* FROM addresses AS a "
