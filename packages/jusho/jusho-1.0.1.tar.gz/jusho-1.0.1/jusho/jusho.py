...
def get_database_path() -> str:
    file_paths = [
        os.path.join(os.path.dirname(__file__), "data/address.db"),
        os.path.join(sys.prefix, "data/address.db"),
        os.path.join(os.path.dirname(__file__).rsplit("lib", 1)[0], "data/address.db"),
    ]
...
class Jusho:
    ...
    def prefecture_by_id(self, id_: int) -> Prefecture:
        ...
        if table != "prefectures":
            warnings.warn(
                f"id_: {id_} does not seem to be a prefecture id. rather, it's a {table} id",
                RuntimeWarning,
            )
    ...
    def search_cities(
        self, query: str, prefecture: Prefecture = None, type_="kanji"
    ) -> List[City]:
        """
    ...
    def city_by_id(self, id_: int) -> City:
        ...
        if table != "cities":
            warnings.warn(
                f"id_: {id_} does not seem to be a city id. rather, it's a {table} id",
                RuntimeWarning,
            )
    ...
    def search_addresses(
        ...
        return gateway.addresses.search_addresses(
            self.cursor, query, prefecture, city, type_
        )
    ...
    def address_by_id(self, id_: int) -> Address:
        ...
        if table != "addresses":
            warnings.warn(
                f"id_: {id_} does not seem to be a address id. rather, it's a {table} id",
                RuntimeWarning,
            )
