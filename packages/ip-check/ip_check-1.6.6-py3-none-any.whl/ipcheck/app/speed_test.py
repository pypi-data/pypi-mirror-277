...
class SpeedTest:
    ...
    def run(self) -> List[IpInfo]:
        ...
        for i in range(total_num):
            ...
            print('{}:{} loss {}% rtt {} ms 下载速度(max/avg)为 {}/{} kB/s'.format(test_ip_info.ip_str,
                                                                                ...
                                                                                max_speed,
                                                                                avg_speed))
            if max_speed > self.config.download_speed:
    ...
    def __test(self, ip_info : IpInfo):
        ...
        def cal_speed():
            nonlocal size, stop_signal, speeds
            ...
            while not download_exit:
                ...
                if end - start > 0.9:
                    ...
                    if old_speed != speed_now:
                        old_speed = speed_now
                        content = '  当前下载速度为: {} kB/s'.format(speed_now)
                ...
                if self.config.fast_check:
                    cur_max_speed = max(speeds) if speeds else 0
                    if end - original_start > self.config.download_time * 0.5 and cur_max_speed < self.config.download_speed * 0.5:
                        stop_signal = True
                        break
                if end - original_start > self.config.download_time:
                    stop_signal = True
                    break
        ...
        t1.join()
        t2.join()
        return (max(speeds), int(sum(speeds) / len(speeds))) if speeds else (0, 0)
