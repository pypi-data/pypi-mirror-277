...
def recode_missing(input_data):
    ...
    for col in data.columns:
        # Flatten list values and handle missing values
        data[col] = data[col].apply(
            lambda x: x[0] if isinstance(x, list) and len(x) == 1 else (pd.NA if isinstance(x, list) else x))

        # Convert entire column to a consistent type, forcing non-numeric types to NaN if needed
        if pd.api.types.is_numeric_dtype(data[col]):
            data[col] = pd.to_numeric(data[col], errors='coerce')
        else:
            data[col] = data[col].astype(str).replace({'nan': pd.NA})
