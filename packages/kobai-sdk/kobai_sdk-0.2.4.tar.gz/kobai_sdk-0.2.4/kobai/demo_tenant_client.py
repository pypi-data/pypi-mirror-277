...
########################################
    ...
    def __get_tenant_export(self):
        ...
        #tenant_json = json.loads(blob_content)
        #print(tenant_json)
        return blob_content

    def __get_view_sql(self):
        id = self.__get_tenant_solutionid()
        view_df = self.spark_client._SparkClient__get_sql(f"""SELECT  table_name, genie_create_view FROM {self.demo_schema}.demo_tables WHERE demo_id={id}""")
        views = view_df.collect()
        return views
    
    def __get_question_view_sql(self):
        id = self.__get_tenant_solutionid()
        view_df = self.spark_client._SparkClient__get_sql(f"""SELECT  table_name, create_view FROM {self.demo_schema}.demo_questions WHERE demo_id={id}""")
        views = view_df.collect()
        return views
