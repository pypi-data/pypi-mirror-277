...
class SidecarDaemon:
    ...
    def open(self, inst_id: str) -> Selector:
        if not self.running:
            try:
                subprocess.Popen(self.executable, env={"MESH_NAME": "sidecar"})
                self.running = True
...
def open_sidecar(inst_id: str) -> Selector:
    return sidecar.open(inst_id)
