    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            if self.native:
                try:
                    need_authorization = self._call_wx_method("getPrivacySetting").result.result.needAuthorization
    ...
    def _inject(self):
        ...
        if self.platform == "ide" and isinstance(self.conf.mock_native_modal, dict):  
            ...
            if not has_mock_native_modal:
                ...
                not use_native and self.app._mock_wx_js("getSetting", "ideMockAuthSetting")
                ...
                if (
                    not use_native and mini_setting["authSetting"].get("scope.userLocation") is None
                ):  # 没有授权，mock掉使之不弹窗
                    self._mock_get_location(
                        self.conf.mock_native_modal.get("location", {})
                    )
                # mock wx.chooseLocation, 授权过获取位置也应该mock掉，不然会弹窗
                if not use_native and mini_setting["authSetting"].get("scope.userLocation") in (
                    None,
                    True,
                ):
                ...
                if not use_native and mini_setting["authSetting"].get("scope.werun") is None:
        ...
        # conf.mock_request
        self._evaluate_js("mockNetwork", args=["request",])  # 先mock, 再打开network panel才能记录mock的请求
