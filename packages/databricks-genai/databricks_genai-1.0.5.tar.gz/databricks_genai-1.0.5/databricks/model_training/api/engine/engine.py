...
This engine code is mostly copied from mcli, but we should have a better long-term maintenance plan.
- In mcli, refactor the models so that each class owns its own custom display properties.
- In this repo, refactor configure_request and bake into this engine code.
- Unify the engine code so that we maintain 1 source of truth for how to communicate with MAPI. This will
  require refactoring the engine code with environment-level overrides to fork Databricks MAPI from Mosaic MAPI.
"""
...
from databricks.model_training.api.exceptions import (DatabricksGenAIConfigError, DatabricksModelTrainingRequestError,
                                                      MAPIException, MultiMAPIException)
from databricks.model_training.types.common import ObjectList, ObjectType  # pylint: disable=cyclic-import
...
_MAPI_ENDPOINT_SUFFIX = '/api/2.0/genai-mapi/graphql'
...
def configure_request(func: _TCallable) -> _TCallable:
    """
    Decorator that handles exceptions from the MAPI engine

    Args:
        func (Callable[..., Any]): The function whose exceptions we want to catch
    """
    ...
    def setup(*args, **kwargs):
        try:
            ...
            raise DatabricksModelTrainingRequestError('Timeout connecting to Databricks') from e
...
class MAPIConnection:
    ...
    Args:
        api_key: The user's API key, as specified in the user's config.
        endpoint: The MAPI URL to hit for all requests. This uses the host specified in the user's config.
    ...
    def _load_from_environment(self, api_key: Optional[str] = None, endpoint: Optional[str] = None) -> None:
        if api_key is not None and endpoint is not None:
            # allow for override without using WorkspaceClient
            logger.info('Using provided API key and endpoint, not from Databricks config.')
        ...
        if is_running_in_databricks_notebook():
            ...
            config_host = ctx.apiUrl().get()
            config_token = ctx.apiToken().get()
        else:
            config_token = w.config.token
            config_host = w.config.host

        config_endpoint = get_endpoint(config_host)

        self.api_key = api_key or config_token
        self.endpoint = endpoint or config_endpoint

        # edge case: user could be using a different auth system for WorkspaceClient, could make token None
        if not self.api_key:
            raise DatabricksGenAIConfigError(
                "No token found. Please set up your Databricks auth with a Personal Access Token "
                "https://docs.databricks.com/en/dev-tools/auth/pat.html#perform-databricks-personal-access-token-authentication"  # pylint: disable=line-too-long
            )
    ...
    def request_header(self):
        ...
        return {
            ...
            'x-mosaicml-client': 'databricks-genai',
    ...
    def create_websocket_connection(self) -> Client:
        if self._client is None:
            ...
            self._client = asyncio.run_coroutine_threadsafe(self._create_gql_client(), self._loop).result(timeout=10)
...
def _deserialize_response(
    ...
    """
    # error_code is returned if we get an API proxy auth error
    if response.get('error_code'):
        raise MAPIException(status=response.get('error_code'), message=response.get('message'))
...
def get_return_response(response, timeout: Optional[float] = 10.0):
...
def get_endpoint(host: str):
    """
    Get the endpoint for the MAPI service using the host.
    
    Args:
        host: The host of the MAPI service (e.g. https://example.databricks.com)"""
    suffix = _MAPI_ENDPOINT_SUFFIX
    if os.environ.get('DATABRICKS_CONFIG_PROFILE') == 'DBX_DEV_LOCAL':
        suffix = '/graphql'

    # strip any trailing '/' from the hosname
