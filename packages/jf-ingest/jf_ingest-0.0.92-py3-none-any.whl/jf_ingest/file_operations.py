...
class IngestIOHelper:
    ...
    def write_json_data_to_s3(
        ...
        logging_helper.send_to_agent_log_file(f"Compressing {object_name} data...")
        zipped_json_data = gzip.compress(encoded_json_data)
        session = retry_session()
        # When pushing to S3, there's a small chance that we randomly get a 500 error.
        # It is expected that on retry this should be resolved.
        # To be safe, we've implemented an exponential back off algorithm
        max_attempts = 5
        current_attempt = 0
        while True:
            try:
                logging_helper.send_to_agent_log_file(
                    f"Attempting to upload {object_name} data to S3"
                )
                ...
                logging_helper.send_to_agent_log_file(
                    f"Successfully uploaded {object_name} data to S3"
                )
                break
                ...
                current_attempt += 1
                logging_helper.send_to_agent_log_file(
                    f"Exception encountered when attempting to upload {object_name} data to S3. Attempt number {current_attempt} of {max_attempts} Error: {e}"
                )
                if current_attempt >= max_attempts:
                    logger.error(f'Error attempting to upload {object_name} data to S3')
                    logging_helper.send_to_agent_log_file(traceback.format_exc())
                    raise
                else:
                    sleep_time = current_attempt**2
                    logging_helper.send_to_agent_log_file(f'Sleeping for {sleep_time}')
