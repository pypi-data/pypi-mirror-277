...
class ThreadPoolWithTqdm(ThreadPoolExecutor):
    ...
    def get_results(self) -> Generator[Any, None, None]:
        for future in as_completed(self.futures_lookup.values()):
            ...
            try:
                ...
                logger.info('Getting future result in get_results')
                yield future.result()
                logger.info('Finished getting future result in get_results')
            except Exception as e:
                logger.warning(f'Exception encountered in ThreadPoolExecutor: {e}')
    ...
    def __exit__(
        ...
        logger.info(f'__exit__ - {len(self.futures_lookup.keys())}')
        if self.futures_lookup.keys():
            logger.info(f'__exit__ - Getting results')
            for _ in self.get_results():
                # Iterate through the get_results generate to force progress bar load
                pass
        logger.info(f'__exit__ - Got results')
        ...
        self.prog_bar.close()
        logger.info(f'__exit__ - Exited successfully')
    ...
    def update_progress_bar(self, future: Future):
        ...
        """
        logger.info(f'Checking future.exception() in update_progress_bar')
        if future.exception():
            return

        logger.info(f'Getting future.exception() in update_progress_bar')
        result = future.result()

        logger.info(f'Getting tqdm_lock() in update_progress_bar')
        with tqdm.get_lock():
