...
def fetch_id_to_key_for_updated(
    ...
    for proj_id, issues in project_id_to_issue.items():
        if len(issues) == 0:
            last_run_date_with_buffer: datetime = pull_from
        else:
            ...
            if last_run_date == pytz.utc.localize(
                datetime.min
            ):  # It is possible that redownloaded issues have a datetime value of datetime.min, which causes an error if we subtract 2 days from
                last_run_date_with_buffer: datetime = last_run_date
            else:
                last_run_date_with_buffer: datetime = last_run_date - timedelta(days=2)
        last_run_buffered_string = last_run_date_with_buffer.strftime('%Y-%m-%d')
        if last_run_date_with_buffer.year < 1000:
            year_str = f'{last_run_date_with_buffer.year:04}'
            last_run_buffered_string = last_run_buffered_string.replace(
                str(last_run_date_with_buffer.year), year_str
            )
        ...
        jql_expression = (
            f"project = {proj_id} AND updatedDate > '{last_run_buffered_string}' "  # has to be a string in the jql query
            f"ORDER BY id ASC"
        )
...
def _download_issue_page(
    ...
    while True:
        try:
            ...
            logger.info(f'{threading.get_native_id()} | started get_issues_with_post - {start_at=}')
