...
#######################################
    ...
    def get_vector_runs(self, org_id: int):
        with Session(self.engine) as session:
            query = (
                select(Run, Process.name)
                .join(Process, Process.id == Run.process_id)
                .join(RunData, RunData.id == Run.output_id)
                .where(
            ...
            if results is None:
                return []
            runs = []
            for run, process_name in results:
                run_serialized = run.serialize()
                run_serialized['process_name'] = process_name
                runs.append(run_serialized)
            return runs
