...
class PriceWebsocket(BaseWebsocket):
    ...
    def subscribe_options(
        ...
        for underlying in underlyings:
            ...
            if underlying not in self.underlying_options_subscribed:
                self.underlying_options_subscribed[underlying] = []
            old_range = self.underlying_options_subscribed[underlying]
            new_range = list(set(old_range + strike_range))
            self.underlying_options_subscribed[underlying] = new_range
    ...
    def update_strike_range(self):
        for underlying in self.underlying_options_subscribed.keys():
            ...
            if len(new_strikes) >= 0.1 * len(current_strike_range):  # Hardcoded 10%
                logger.info(
