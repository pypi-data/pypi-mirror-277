...
def _safeExtract(tar_path, extract_path):
    # by GPT 
    # 确保目标解压目录存在
        ...
        for member in tar.getmembers():
            if member.isfile():  # 确保是文件
                ...
                if not member_path.startswith(os.path.abspath(extract_path)):
                    raise Exception(f"Attempted Path Traversal in Tar File {tar_path}")
                
                # 使用 extractfile 读取文件内容
                file_content = tar.extractfile(member)
                if file_content:
                    # 将内容写入新文件
                    with open(member_path, 'wb') as outfile:
                        outfile.write(file_content.read())
                    print(f"已成功解压: {member.name}")
    ...
    def getsharefrom(self,username:str):
        ...
        if meta['isdir']:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                _safeExtract(tar_path=localZipFile, extract_path=localPath)
        else:
            localPath = os.path.join( os.getcwd(),meta['filename'] ) 
