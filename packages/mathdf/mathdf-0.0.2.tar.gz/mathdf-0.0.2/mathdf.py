...
print("Товары фирмы Bosh:", bosh_products)
            ...
            "Даны 2 списка: перечень товаров, производимых Концернами BORK и PHILIPS. Создать список товаров, выпускаемых как одной, так и другой фирмой. Результат вывести в веб-интерфейс при помощи фреймворка Flask. Базу данных можно использовать по желанию, дизайн не обязателен: достаточно обычных кнопок/списков.":
                """
                # База данных и Flask приложение
                ...
                def init_db():
                    ...
                    insert_query = "INSERT INTO products (name, brand) VALUES (%s, %s)"
                    try:
                ...
                def get_common_products():
                    ...
                    query = """"""
                        ...
                        WHERE brand = 'BORK'
                        AND name IN (SELECT name FROM products WHERE brand = 'PHILIPS')
                    """"""
                ...
                @app.route('/show')
                def show():
                    common_products = get_common_products()
                    return render_template('show.html', products=common_products)
                ...
                <body>
                    <h1>Список товаров, выпускаемых BORK и PHILIPS</h1>
                    <ul id="product-list">
            ...
            "Даны 2 списка: перечень товаров и фамилии покупателей. Каждый n-й покупатель покупает m-й товар. Вывести список покупок.":
                """
                # Пример списков товаров и покупателей
                products = ["Телевизор", "Холодильник", "Стиральная машина", "Микроволновка", "Пылесос", "Блендер"]
                customers = ["Иванов", "Петров", "Сидоров", "Смирнов", "Кузнецов", "Попов", "Соколов", "Лебедев"]
        
                n = 2  # Каждый 2-й покупатель
                m = 3  # Покупает каждый 3-й товар
        
                # Функция для формирования списка покупок
                def generate_purchases(customers, products, n, m):
                    purchases = []
                    for i in range(n - 1, len(customers), n):
                        product_index = (i // n * m) % len(products)  # Цикличность в списке товаров
                        purchases.append((customers[i], products[product_index]))
                    return purchases
        
                # Формирование списка покупок
                purchases = generate_purchases(customers, products, n, m)
        
                print("Список покупок:")
                for customer, product in purchases:
                    print(f"{customer} покупает {product}")
                """,
            ...
            "Вершины дерева вещественные числа. Описать функцию, которая вычисляет среднее арифметическое всех вершин дерева. Результат вывести в веб-интерфейс при помощи фреймворка Django. Базу данных можно использовать по желанию, дизайн не обязателен: достаточно обычных кнопок/списков.":
                """
                # models.py
                # Создайте файл models.py в вашем приложении Django и вставьте следующий код:
        
                from django.db import models
        
                class TreeNode(models.Model):
                    value = models.FloatField()  # Вещественное число, представляющее вершину дерева
                ...
                # templates/tree_average.html
                # Создайте директорию templates в вашем приложении.
                # Внутри директории templates создайте файл tree_average.html и вставьте следующий HTML-код:
                ...
                # urls.py
                # Создайте файл urls.py в вашем приложении и вставьте следующий код:
                ...
                # views.py
                # Создайте или обновите файл views.py в вашем приложении и вставьте следующий код:
                ...
                def index(request):
                    return render(request, 'tree_average.html')
        
                def calculate_average(request):
                    average = TreeNode.objects.all().aggregate(Avg('value'))['value__avg']
                    return render(request, 'tree_average.html', {'average': average})
        
        
                # settings.py
                # Убедитесь, что в settings.py указана конфигурация для MySQL:
                ...
                # Перейдите на http://127.0.0.1:8000/, чтобы проверить работу.
                """,
        }
