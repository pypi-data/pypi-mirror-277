...
def important_targets_from_search(
    ...
    targets = targets[targets["stage"] != "land"]
    ...
    with db.connect() as con:
        with con.begin():
            ...
            # ok also what to update company.source with from_search.label

            company_updates = [
                {
                    "domain": domain,
                    "source": from_search.label,
                }
                for domain in domains
            ]
...
def important_targets_from_event(event: ts.models.Event) -> None:
    ...
    def get_stage(domain):
        ...
        return ts.constants.LABEL_MAP.get(stage_key, "Unknown")
