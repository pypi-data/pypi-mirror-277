    ...
    *,
    ...
    Args:
        ...
        project: a Project or project name/alias to get objects for. A query can only be for a single
                 ...
                 Will be removed in v1.0.0. Use `projects` instead.
    ...
    if projects is None:
        ...
        projects = [_get_project(project)]
    else:
        projects = [_get_project(p) for p in projects]
        if project is not None:
            raise ValueError(
                "cannot specify both project and projects. project will be deprecated, use projects instead"
            )
    if projects == []:
...
class Object(_APIObject, metaclass=ObjectResolver):
    ...
    xid = _StringDescr(
        ...
        A typical example is an RDF IRI including the namespace prefix (e.g. https;//schema.org/Organization/SeerAI)"
    )
