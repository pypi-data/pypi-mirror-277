...
def scroll_calendar_container(driver: webdriver, calendar: WebElement, container_header: WebElement,
                              month: WebElement) -> dict:
    header_height = container_header.size['height']
    js_str = """
            ...
            // 获取目标元素相对于文档的整体位置
            var targetDocTop = targetRect.top + window.pageYOffset;
            var containerDocTop = containerRect.top + window.pageYOffset;
            // 计算目标元素距离滚动容器顶部的距离，并考虑星期标题的高度
            var offset = targetDocTop - containerDocTop;
            // 计算目标元素距离滚动容器顶部的滚动距离，同时保留星期标题的高度
            var scrollDistance = container.scrollTop + offset - headerHeight;
            if (scrollDistance < 0) {
                scrollDistance = 0; // 确保不会滚动到负值
            }
            // 设置滚动容器的滚动位置
            container.scrollTop = scrollDistance;
            """
    return driver.execute_script(js_str, calendar, month, header_height)
