...
def get_all(db: Session, query: Query, schema: T.Schema) -> typing.List[T.Schema]:
    ...
    return [schema.model_validate(model) for model in query.with_session(db).all()]
...
def review(
    ...
    if changes is not None:
        for change in changes:
            db.add(M.Review(ingest_id=ingest_id, **change.model_dump()))
    db.add(
        M.Task(
            ingest_id=ingest_id,
...
def next_task(db: Session, worker: str) -> typing.Optional[T.TaskOut]:
    ...
    """
    query = _for_update(
        db.query(M.Task).order_by(
            sqla.case(
                [
                    (M.Task.type == 'configure', 1),
                    (M.Task.type == 'finalize', 2),
                    (M.Task.type == 'upload', 3),
                    (M.Task.type == 'prepare', 4),
                    (M.Task.type == 'prepare_sidecar', 5),
                    (M.Task.type == 'detect_duplicates', 6),
                    (M.Task.type == 'resolve', 7),
                    (M.Task.type == 'extract_uid', 8),
                    (M.Task.type == 'scan', 9),
                ],
                else_=10
            )
        ).filter(M.Task.status == T.TaskStatus.pending),
        skip_locked=True,
    )
    task = query.first()
    if task is None:
...
def get_summary(db: Session, ingest_id: UUID) -> T.Summary:
    ...
    """
    TODO this query needs work if the original (DB) message
    will be needed e.g.:ProjectDoesNotExistError
    """
    errors_by_type = (
...
def load_subject_csv(
    ...
    header = subject_csv.readline().strip()
...
def one_container_multiple_uid_item_ids(
    ...
    query = (
        ...
        .filter(M.UID.ingest_id == ingest_id, sub_query.columns.count > 1)
    )
...
def uids_in_new_session_container(
    ...
    query = (
        ...
        .filter(M.UID.ingest_id == ingest_id, M.Container.existing == False)  # noqa
        .group_by(uid_type_column)
    )
...
def _delete(db: Session, model: M.Base, *conditions):
    ...
    # Disabling trigger
    disable_statement = "ALTER TABLE {} DISABLE TRIGGER ALL"
    if db.bind.name != "sqlite":
        db.execute(disable_statement.format(model.__table__))

    # Delete DB records without fetching them
    db.execute(model.__table__.delete().where(*conditions))

    # Enabling trigger
    enable_statement = "ALTER TABLE {} ENABLE TRIGGER ALL"
    if db.bind.name != "sqlite":
        db.execute(enable_statement.format(model.__table__))
