...
def enable(args):
    ...
    while os.path.exists(refreshMessage):
        time.sleep(1)
    print('Long connection has been reconnected')
    ...
    for root,dirs,files in os.walk(script_folder):
        for file in files:
            ...
            if ext in [ ".json", ".txt", ".md" ]:
                dir_path = os.path.relpath(root, script_folder)
...
def installWidget(args):
    ...
    downloadpath = path + f'/download/{widgetName}'
    widget_template = os.path.join(os.path.dirname(os.path.abspath(__file__))) + r'\widget_template'


    h5_path = widget_dir + r'\h5'
    scr_path = widget_dir + r'\script'
    cmd_str = f'pip install -U {widgetName} -i https://mvn.miyachat.com/nexus/repository/mecord/simple -t {downloadpath}'
    ...
    widgetid = xy_pb.CreateWidgetUUID()
    # setWidgetData(widget_dir, widgetid)
    data = GetWidgetConfig(root)
