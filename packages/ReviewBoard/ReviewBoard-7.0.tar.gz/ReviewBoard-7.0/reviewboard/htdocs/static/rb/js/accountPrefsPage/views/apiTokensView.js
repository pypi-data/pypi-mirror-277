...
(function () {
  ...
  const APITokenItem = RB.Config.ResourceListItem.extend({
    ...
    syncAttrs: ['deprecated', 'expired', 'expires', 'id', 'invalidReason', 'invalidDate', 'lastUsed', 'note', 'policy', 'tokenValue', 'valid'],
    /**
     * Initialize the item.
     *
  ...
  const PolicyEditorView = Backbone.View.extend({
    ...
    template: _.template(['<p><%= instructions %></p>', '<textarea></textarea>'].join('')),
    /**
    ...
    render() {
      ...
      this.$el.modalBox({
        ...
        }, gettext("Save and continue editing")), Ink.paintComponent('Ink.Button', {
          class: 'save-button',
    ...
    async save(closeOnSave) {
      ...
      try {
        ...
        if (err.xhr.errorPayload.err.code === 105 && err.xhr.errorPayload.fields.policy) {
  ...
  const APITokenItemView = Djblets.Config.ListItemView.extend({
    ...
    template: _.template(gettext("<div class=\"rb-c-config-api-tokens__main\"> <div class=\"rb-c-config-api-tokens__value\"> <input readonly=\"readonly\" value=\"<%- tokenValue %>\"> </div> <span class=\"fa fa-clipboard js-copy-token\" title=\"Copy to clipboard\"></span> </div> <div class=\"rb-c-config-api-tokens__info\"> <% if (deprecated) { %> <p class=\"rb-c-config-api-tokens__deprecation-notice\"> This token uses a deprecated format. You should remove it and generate a new one. </p> <% } %> <% if (valid) { %> <% if (lastUsed) { %> <p class=\"rb-c-config-api-tokens__usage -has-last-used\"> Last used <time class=\"timesince\" datetime=\"<%= lastUsed %>\"></time>. </p> <% } else { %> <p class=\"rb-c-config-api-tokens__usage\">Never used.</p> <% } %> <% if (expired) { %> <p class=\"rb-c-config-api-tokens__token-state -is-expired\"> <span>Expired <%= expiresTimeHTML %>.</span> </p> <% } else if (expires) { %> <p class=\"rb-c-config-api-tokens__token-state -has-expires\"> <span>Expires <%= expiresTimeHTML %>.</span> </p> <% } else { %> <p class=\"rb-c-config-api-tokens__token-state\"> <span>Never expires.</span> </p> <% } %> <% } else { %> <p class=\"rb-c-config-api-tokens__token-state -is-invalid\"> Invalidated <time class=\"timesince\" datetime=\"<%= invalidDate %>\"></time>: <%= invalidReason %> </p> <% } %> </div> <div class=\"rb-c-config-api-tokens__actions\"></div> <span class=\"rb-c-config-api-tokens__note\"></span>")),
    events: {
    ...
    render() {
      ...
      this._$expires = this._$tokenState.not('.is-invalid').find('span');
      ...
      this.listenTo(noteEditor, 'complete', value => this.model.saveNote(value));
      const expires = moment(this.model.get('expires')).local().format('YYYY-MM-DDTHH:mm');
...
<time class="timesince" datetime="${value}"></time>.`;
    ...
    getRenderContext() {
      ...
      return {
        expiresTimeHTML: `<time class="timesince" datetime="${expires}"></time>`
      };
    },
    /**
    ...
    _onCopyClicked(e) {
      e.preventDefault();
      e.stopPropagation();
      this.$('.rb-c-config-api-tokens__value input').focus().select();
    ...
    _onRemoveClicked() {
      $('<p>').html(gettext("This will prevent clients using this token when authenticating.")).modalBox({
        ...
        buttons: [$('<input type="button">').val(gettext("Cancel")), $('<input type="button" class="danger">').val(gettext("Remove")).click(() => this.model.resource.destroy())]
      });
    }
  });
...
<div class="api-tokens">
    ...
    generateTokenTemplate: _.template(`<li class="generate-api-token djblets-c-config-forms-list__item">
 <a href="#"><%- generateText %></a>
    ...
    render() {
      ...
      this._$generateTokenItem = $(this.generateTokenTemplate({
    ...
    async _onGenerateClicked(e) {
      ...
      this._$generateTokenItem.detach().appendTo(this._listView.getBody());
    }
  });
  ...
  RB.APITokensView = Backbone.View.extend({
    template: _.template(`<div class="api-tokens-list djblets-l-config-forms-container
