...
class Instrumentations(ObsinfoClassList):
    ...
    """
   # def __init__(self, instrumentations_list):
   #     """
   #     Args:
   #         instrumentations_list: (list of :class:`Instrumentations`):
   #     """
   #     if instrumentations_list is None:
   #         super().__init__([], Instrumentation)
   #     elif isinstance(instrumentations_list, list):
   #         super().__init__(instrumentations_list, Instrumentation)
   #     else:
   #         raise TypeError('instrumentation_list is neither a list nor None')
    def __init__(self, attributes_dict_list, station_locations,
        ...
        Args:
            ...
            station_locations (:class:`Locations`):  list of Locations
...
class Instrumentation(object):
    ...
    """
    def __init__(self, attributes_dict, station_locations, station_location_code,
                 station_start_date, station_end_date):
        """
        Constructor
        ...
        for das_label, ch_attributes in channels_dict.items():
            ...
            ch_modifs = self._select_channel_modifs(ch_attributes, channel_modifs)
            ...
            # Select location
            location = station_locations.get_by_loc_code(ch_dict.pop('location_code'))
    ...
    def __repr__(self):
        s = (f'Instrumentation(attributes_dict={self.input["attributes_dict"]}, '
             f'station_locations={self.input["station_locations"]}, '
    ...
    def _select_channel_modifs(chan_dict:dict, channel_modifs:dict):
        ...
        orientation_code = list(chan_dict['orientation'].keys())[0]
        location_code = chan_dict['location_code']
        id_code = orientation_code + "-" + location_code
        
        # Get modifications, classed by specification type
        mods_specific = channel_modifs.get(id_code, channel_modifs.get(id_code[:1], {}))
        mods_orient = channel_modifs.get("*-" + location_code, {})
        mods_loc = channel_modifs.get(orientation_code + "-*", {})
        mods_default = channel_modifs.get("*", channel_modifs.get("*-*", {}))
