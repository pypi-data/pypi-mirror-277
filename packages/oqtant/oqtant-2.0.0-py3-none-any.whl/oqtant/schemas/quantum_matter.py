...
from bert_schemas import timing
...
class QuantumMatter(BaseModel):
    ...
    def show_potential(
        ...
        ylimits: list = [0.0 - 1, pset.max_energy + 1],
        ...
        term_radius_1e2 = projected.TERMINATOR_WIDTH
        full_width_half_max = GAUSSIAN_1e2_DIAM_TO_FULL_WIDTH_HALF_MAX * term_radius_1e2

        positions = np.arange(
            pset.min_position, pset.max_position + 0.1, 0.1, dtype=float
        )
        fig, ax = plt.subplots()
    ...
    def show_barrier_dynamics(self) -> None:
        ...
        for indx, barrier in enumerate(self.barriers):
            ...
            ax2.plot(barrier_times, barrier.get_heights(barrier_times), ".", color=color,)
        ...
        if self.lasers:
            for laser in self.lasers:
                ...
                for pulse in laser.pulses:
                    ...
                    det_idx = np.argwhere(
                        (times >= timing.decimal_to_float(pulse.times_ms[0])) & (times <= timing.decimal_to_float(pulse.times_ms[-1]))
                    )
...
class QuantumMatterFactory:
    ...
    @staticmethod
    def create_rf_shield(power: float = 0, frequency: float = 0, lifetime: timing.EndTimeMs = 1.0) -> RfShield:
