...
class Barrier(job_schema.Barrier):
    ...
    def show_dynamics(self) -> None:
        ...
        tstart = decimal_to_float(min(self.times_ms))
        tstop = decimal_to_float(max(self.times_ms))
        times = np.linspace(
            tstart,
            tstop,
...
class Pulse(job_schema.Pulse):
    ...
    def new(
        ...
        Args:
            ...
            interpolation (job_schema.InterpolationType) : interpolation in time for intensity, default = job_schema.InterpolationType.OFF
        Returns:
...
class Laser(job_schema.Laser):
    ...
    def is_on(self, time_ms: float) -> bool:
        for pulse in self.pulses:
            if (time_ms >= decimal_to_float(pulse.times_ms[0])) & (time_ms <= decimal_to_float(pulse.times_ms[-1])):
                return True
        return False
