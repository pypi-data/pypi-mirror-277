...
def register_data_asset(
    ...
    if source_type in SCHEMA_SOURCE_TYPE_VALUES:
        ...
        validate_input("register", bucket, lookback_days, flattened_include, history)
...
def check_data_asset(
    ...
    if source_type == SourceType.PYTHON.value:
        ...
        validate_input("check", bucket, lookback_days, flattened_include, history)
        ...
        results = check_compliance_s3_data_assets(
            ...
            bucket,  # type: ignore (input validation ensures bucket is not None, this quashes linter)
            lookback_days,
            flattened_include,  # TODO - revisit this to see if we can avoid nested tuples
            skip_profiling,
            row_sample_count,
        )
    else:
