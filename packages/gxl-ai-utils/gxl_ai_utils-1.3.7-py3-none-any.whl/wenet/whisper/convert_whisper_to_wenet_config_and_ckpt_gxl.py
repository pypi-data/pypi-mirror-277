...
def convert_to_wenet_units(tokenizer, units_txt_path):
    """ NOTE(xcsong):
        The "units_whisper.txt" file is solely for adapting to the training API of Wenet
...
def get_args(target_pt, source_pt, whisper_model):
    whisper_ckpt = os.path.join(source_pt, f"{whisper_model}.pt")
    parser = argparse.ArgumentParser(description='load and parse whisper')
    parser.add_argument('--whisper_ckpt', default=whisper_ckpt,
    ...
    parser.add_argument('--output_dir', default=target_pt,
                        help='output file in wenet\'s style: ' +
                             'units_whisper.txt, train.yaml, model.pt')
    args = parser.parse_args()
    return args


def main(target_pt_dir=None, source_pt_dir=None, whisper_model=None):
    args = get_args(target_pt_dir, source_pt_dir, whisper_model)
    # checkpoint = torch.load(args.whisper_ckpt, map_location="cpu")
    checkpoint = torch.load(
        "/home/work_nfs5_ssd/yzli/workspace/wenet/examples/aishell/s0/exp/whisper_large_v1/avg_3.pt",
        map_location="cpu")
    ...
    convert_to_wenet_state_dict(
        ...
        os.path.join(args.output_dir, f'wenet_whisper_{whisper_model}.pt')
    )
    convert_to_wenet_units(
        tokenizer,
        os.path.join(args.output_dir, f'units_whisper_{whisper_model}.txt')
    )
    convert_to_wenet_yaml(
        tokenizer, checkpoint["dims"],
        os.path.join(args.output_dir, f'train_{whisper_model}.yaml')
    )


if __name__ == "__main__":
    from gxl_ai_utils.utils import utils_file
    utils_file.makedir_sil("./output/whisper")
    main(source_pt_dir="./output",target_pt_dir="./output/whisper", whisper_model="large_v2")
