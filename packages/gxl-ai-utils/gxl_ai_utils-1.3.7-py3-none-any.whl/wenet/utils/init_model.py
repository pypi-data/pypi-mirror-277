...
from wenet.utils.checkpoint import load_checkpoint, load_trained_modules, load_whisper_encoder_origin_param
...
def init_model(args, configs):
    ...
    if configs.get('en_cn', False) and configs.get('whisper_checkpoint4stu', None) is not None:
        load_whisper_encoder_origin_param(model.encoder, configs)
    # If specify checkpoint, load some info from checkpoint
    if "student_checkpoint" in configs and configs["student_checkpoint"] is not None:
        logging.info("耿雪龙： 加载学生模型的checkpoint: {}".format(configs["student_checkpoint"]))
        infos = load_checkpoint(model, configs["student_checkpoint"])
    else:
        infos = {}
    ...
    if configs.get('frozen_encoder', False):
        logging.info("耿雪龙： 开始冻结encoder")
        for param in model.encoder.parameters():
            param.requires_grad = False
    else:
        logging.info("耿雪龙： 不去冻结encoder")
        for param in model.encoder.parameters():
