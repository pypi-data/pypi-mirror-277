...
def do_inference_paraformer(input_wav_scp, output_dir, true_text_path=None, gpuid=0):
    ...
    for key, path in tqdm(wav_dict.items(), total=len(wav_dict)):
        ...
        try:
            ...
            logging_print(f'{key} {text_res["text"]}')
            res_text_list.append(f'{key} {text_res["text"]}')
        except Exception as e:
            print(e)
            continue
    write_list_to_file(res_text_list, os.path.join(output_dir, 'text_hyp'))
    if true_text_path is not None:
        do_compute_wer(true_text_path, os.path.join(output_dir, 'text_hyp'), output_dir)
