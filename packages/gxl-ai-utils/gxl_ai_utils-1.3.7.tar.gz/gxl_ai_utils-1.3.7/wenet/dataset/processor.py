...
def compute_log_mel_spectrogram_and_fbank(data, fbank_conf, mel_conf):
    """"""
    n_fft = mel_conf['n_fft']
    hop_length = mel_conf['hop_length']
    num_mel_bins_1 = mel_conf['num_mel_bins']
    padding = fbank_conf['padding']

    num_mel_bins_2 = fbank_conf['num_mel_bins']
    frame_shift = fbank_conf['frame_shift']
    frame_length = fbank_conf['frame_length']
    dither = fbank_conf['dither']

    assert num_mel_bins_1 == num_mel_bins_2, f"num_mel_bins_1: {num_mel_bins_1}, num_mel_bins_2: {num_mel_bins_2}"
    num_mel_bins = num_mel_bins_1
    for sample in data:
        assert 'sample_rate' in sample
        assert 'wav' in sample
        assert 'key' in sample
        ...
        log_spec = torch.maximum(log_spec, log_spec.max() - 8.0)
        log_spec = (log_spec + 4.0) / 4.0
        feat4wh = log_spec.transpose(0, 1)
...
def padding(data):
    ...
    for sample in data:
        ...
        feats_lengths4wh = torch.tensor(
            [sample[i]['feat4wh'].size(0) for i in order], dtype=torch.int32)
        sorted_feats = [sample[i]['feat'] for i in order]
        sorted_feats4wh = [sample[i]['feat4wh'] for i in order]
        ...
        padded_feats = pad_sequence(sorted_feats,
                                    batch_first=True,
                                    padding_value=0)
        padded_feats4wh = pad_sequence(sorted_feats4wh,
                                       batch_first=True,
                                       padding_value=0)
        ...
        yield {
            ...
            "feats4wh": padded_feats4wh,
            "feats_lengths4wh": feats_lengths4wh
        }
