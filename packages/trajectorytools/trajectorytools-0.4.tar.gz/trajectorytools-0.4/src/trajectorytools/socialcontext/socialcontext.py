...
def _in_convex_hull(positions: np.ndarray) -> np.ndarray:
    hull = scipy.spatial.ConvexHull(positions)
    convex_hull = np.zeros(positions.shape[0], dtype=bool)
    convex_hull[hull.vertices] = True
    return convex_hull


def in_convex_hull(positions) -> np.ndarray:
    convex_hull_list = [
...
def circumradius(points: np.ndarray) -> np.ndarray:
...
def _in_alpha_border(positions: np.ndarray, alpha=5) -> np.ndarray:
    ...
    points_in_rejected_triangles = [
        p for triangle in delaunay.simplices[rejected_triangles] for p in triangle
    ]
    in_border = np.zeros(num_individuals, bool)
...
def in_alpha_border(
    positions: Iterable[np.ndarray] | np.ndarray, alpha=5
) -> np.ndarray:
    alpha_border_list = [
...
def neighbour_indices_in_frame(
    ...
    """
    nbrs = NearestNeighbors(n_neighbors=num_neighbours + 1, algorithm="ball_tree").fit(
        positions
    )
    return nbrs.kneighbors(positions, return_distance=False)


def neighbour_indices(positions: np.ndarray, num_neighbours: int) -> np.ndarray:
    ...
    next_neighbours = np.empty(
        [total_time_steps, individuals, num_neighbours + 1], dtype=int
    )
...
def adjacency_matrix(
    ...
    if mode == "connectivity":
        adjacency_m = np.empty([total_time_steps, individuals, individuals], dtype=bool)
    elif mode == "distance":
...
def interindividual_distances(positions: np.ndarray) -> np.ndarray:
    return adjacency_matrix(positions, mode="distance")


def restrict(data: np.ndarray, indices, individual=None) -> np.ndarray:
    """restrict_with_delay
