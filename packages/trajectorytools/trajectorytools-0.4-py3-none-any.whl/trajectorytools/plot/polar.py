...
def remove_not_finite_from_args(f_unwrapped: T) -> T:
    ...
    def wrapped(*args, **kwargs):
        ...
        # Remove nans, if any
        finite_values = np.logical_and.reduce([np.isfinite(arg) for arg in args])
...
BinnedStatisticPolarResult = namedtuple(
    "BinnedStatisticPolarResult", ("statistic", "r_edge", "theta_edge", "binnumber")
)
...
def binned_statistic_polar(
    ...
    bins: Tuple[int, int] | int = (7, 12),
    range_r: Tuple[float, float] | float = 5,
...
def interpolate_polarmap_angles(
    values: np.ndarray, theta_edges: np.ndarray, r_edges: np.ndarray, factor: int = 1
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
