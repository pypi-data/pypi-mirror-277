...
class Fish:
    ...
    def __init__(
        ...
        self.velocity_line = Line2D(
            [xy[0], self.xy_vel[0]], [xy[1], self.xy_vel[1]], color=color, linewidth=0.4
        )
    ...
    def velocity(self, v):
        ...
        self.velocity_line.set_data(
            [self.position[0], self.xy_vel[0]], [self.position[1], self.xy_vel[1]]
        )
        self.body.stale = True
...
def get_spaced_colors(
    ...
    RGB_tuples = Colormap(cmap)
    return [RGB_tuples(i / n) for i in range(n)]


def subplots_row_and_colums(number_of_individuals) -> tuple[int, int]:
...
def with_ordering(old_func) -> Callable:
...
def plot_individual_distribution_of_vector(vector, indices, nbins=10, ticks=False):
    ...
    for i, identity in enumerate(indices):
        ...
        H.append(
            np.histogram2d(
                vector[:, identity, 0][~np.isnan(vector[:, identity, 0])].flatten(),
                vector[:, identity, 1][~np.isnan(vector[:, identity, 1])].flatten(),
                bins=(binsX, binsY),
            )[0]
        )
