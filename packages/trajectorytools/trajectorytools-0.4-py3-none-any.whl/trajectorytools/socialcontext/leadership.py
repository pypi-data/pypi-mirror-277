...
def sweep_delays(data, indices, max_delay, individual=None):
    ...
    if individual is None:
        output = np.empty(
            [max_delay, total_time_steps, num_individuals, num_restricted, coordinates],
            dtype=data.dtype,
        )
    else:
        output = np.empty(
            [max_delay, total_time_steps, num_restricted, coordinates], dtype=data.dtype
        )
...
def sliding_average_dot_product_with_delays(
    data, indices, sweep_delayed_e, start_frame=0, end_frame=None, window_size=50
):
    """
    ...
    fleshout_list = [
        dot_product_per_frame_with_delays(data, indices, sweep_delayed_e, frame)
        for frame in frames
    ]
    return [
