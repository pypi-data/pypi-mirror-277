...
def interpolate_nans(t: np.ndarray):
...
def resample(x: np.ndarray, up, down, params: dict | None = None):
    if params is None:
        params = {}
    # Temporary
    ...
    background_in = np.stack(
        [background_line[0] + background_line[1] * length for length in rel_len],
        axis=axis,
    )
...
def find_enclosing_circle_simple(t: np.ndarray):
...
def find_enclosing_circle(t: np.ndarray):
    ...
    try:
        ...
        logging.warning('Please, install "pip install MiniballCpp"')
