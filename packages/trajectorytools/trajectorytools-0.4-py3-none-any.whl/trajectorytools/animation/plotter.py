...
def ellipse(color="c", size: list | None = None) -> Plotter:
    if size is None:
        size = [0.04, 0.02]

    def plot_function(xy, ax) -> tuple[Ellipse, ...]:
        patches = tuple(
            Ellipse(
                x[:2],
                size[0],
                size[1],
                angle=np.degrees(np.arctan2(x[3], x[2])),
                color=color,
                animated=True,
            )
            for x in xy
...
def simple(marker="o", color=None):
    if color is None:
        color = [1, 0, 0]
...
def circle(color="k", radius=1):
    def plot_function(xy, ax):
        patches = tuple(
            Circle((x[0], x[1]), radius, fill=False, animated=True) for x in xy
        )
        for patch in patches:
            ax.add_patch(patch)
        return patches
...
def labels(labels: np.ndarray, colors=None, **kwargs):
    def plot_function(xy, ax):
        if len(labels.shape) == 2:
            label_list = labels[0].tolist()
        else:
            label_list = labels.tolist()
        patches = tuple(ax.text(x[0], x[1], label) for label, x in zip(label_list, xy))
    ...
    def update_function(xy, ax, lines, frame=None):
        ...
        if colors is not None and isinstance(colors[0], list):
