...
def _find_interlaced_peaks(signal: np.ndarray, find_dict: dict[str, Any]):
...
def find_bouts_individual(
    speed: np.ndarray,
    find_min_dict: dict[str, Any] | None = None,
    find_max_dict: dict[str, Any] | None = None,
) -> np.ndarray:
...
def compute_bouts_parameters(tr, bouts, focal):
    ...
    bouts_dict = {
        var.__name__: [var(tr, bout, focal) for bout in bouts] for var in variables
    }
