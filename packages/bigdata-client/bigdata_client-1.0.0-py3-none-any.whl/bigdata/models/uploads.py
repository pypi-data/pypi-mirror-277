...
from bigdata.api.uploads import UploadsConnectionProtocol
from bigdata.connection import get_chunks_from_presigned_url
...
class File:
    ...
    # def share_with_company(self):  # SHARE FILE WITH COMPANY
    #     """
    #     Shares a file with the whole company.
    #     """
    #     if self.id is None:
    #         raise ValueError("File not uploaded")
    #     response = self._api.share_file_with_company(self.id)
    ...
    #     if self.id is None:
    #         raise ValueError("File not uploaded")
    #     response = self._api.unshare_file_with_company(self.id)
    #     return response.model_dump()
