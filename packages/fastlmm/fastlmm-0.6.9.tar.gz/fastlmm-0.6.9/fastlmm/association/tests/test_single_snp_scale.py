...
class TestSingleSnpScale(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.bed = Bed(
            os.path.join(self.pythonpath, "tests/datasets/synth/all.bed"), count_A1=True
        )[:, ::10]
    ...
    def test_multipheno(self):
        ...
        for pheno_count in [2, 5, 1]:
            ...
            frame = single_snp_scale(
                test_snps=self.bed, pheno=pheno_multi, covar=self.cov_fn
            )
    ...
    def test_local_distribute(self):
        ...
        test_snps = DistributedBed.write(
            test_storage, self.bed, piece_per_chrom_count=2
        )
        ...
        results_df = single_snp_scale(
            ...
            covar=self.cov_fn,
            G0=self.bed,
            cache=self._cache_dict(storage, clear_cache=True),
            output_file_name=output_file,
        ...
        results_df = single_snp_scale(
            ...
            covar=self.cov_fn,
            G0=self.bed,
            cache=self._cache_dict(storage, clear_cache=False),
            output_file_name=output_file,
        )
        self.compare_files(results_df, "old")
    ...
    def test_one_fast(self):
        ...
        test_storage.rmtree()
        test_snps3 = self.bed[:, self.bed.pos[:, 0] == 3]  # Test only on chromosome 3
        test_snps3_dist = DistributedBed.write(
            test_storage, test_snps3, piece_per_chrom_count=2
        )

        results_df = single_snp_scale(
            test_snps=test_snps3_dist,
            pheno=self.phen_fn,
            covar=self.cov_fn,
            G0=self.bed,
            output_file_name=output_file,
    ...
    def test_doctest(self):  # Can't get doc test to work so marked out.
        ...
        result = doctest.testmod(
            sys.modules["fastlmm.association.single_snp_scale"],
...
def getTestSuite():
    ...
    test_suite.addTests(
        unittest.TestLoader().loadTestsFromTestCase(TestSingleSnpScale)
    )  # Tests Ludicrous Speed GWAS
    return test_suite
