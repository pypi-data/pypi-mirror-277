...
def work_item2(
    ...
    if isinstance(spatial_coor, str):
        assert (
            spatial_iid is None
        ), "if spatial_coor is a str, then spatial_iid should be None"
    ...
    pheno = pheno.read()
    pheno = pheno[
        pheno.val[:, 0] == pheno.val[:, 0], :
    ]  # Excludes NaN because NaN is not equal to NaN
    ...
    if jackknife_index >= 0:
        assert (
            jackknife_count <= G_kernel.iid_count
        ), "expect the number of groups to be less than the number of iids"
        assert (
            jackknife_index < jackknife_count
        ), "expect the jackknife index to be less than the count"
        m_fold = model_selection.KFold(
            n_splits=jackknife_count,
            shuffle=True,
            random_state=jackknife_seed % 4294967295,
        ).split(list(range(G_kernel.iid_count)))
    ...
    if not do_uncorr:
        ...
        logging.info(
            "just G: h2uncorr: {0}, nLLuncorr: {1}".format(h2uncorr, nLLuncorr)
        )
    ...
    if a2 is None:
        ...
        if not just_testing:
            ...
            h2corr, e2, a2, nLLcorr, h2corr_raw = 0, 0, 0.5, 0, 0
        logging.info(
            "G plus E mixture: h2corr: {0}, e2: {1}, a2: {2}, nLLcorr: {3} (h2corr_raw:{4})".format(
                h2corr, e2, a2, nLLcorr, h2corr_raw
            )
        )
    else:
    ...
    if not do_gxe2:
        ...
        GplusE_kernel = KernelData(
            iid=G_kernel.iid, val=val, name="{0} G + {1} E".format(1 - a2, a2)
        )
        ...
        if not just_testing:
            ...
            gxe2, a2_gxe2, nLL_gxe2 = 0, 0.5, 0
        logging.info(
            "G+E plus GxE mixture: gxe2: {0}, a2_gxe2: {1}, nLL_gxe2: {2}".format(
                gxe2, a2_gxe2, nLL_gxe2
            )
        )
...
def heritability_spatial_correction(
    G_kernel,
    spatial_coor,
    spatial_iid,
    alpha_list,
    ...
    with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
        ...
        alpha_table_fn = "{0}/alpha_table.{1}.txt".format(
            cache_folder, pheno.sid_count
        )  # create a name for the alpha_table cache file
        ...
        if cache_folder is not None and os.path.exists(alpha_table_fn):
            alpha_table = pd.read_csv(
                alpha_table_fn, delimiter="\t", index_col=False, comment=None
            )
        else:
            ...
            return_list = (
                map_function(work_item, arg_list)
                if len(arg_list) > 1 or always_remote
                else list(map(work_item, arg_list))
            )
            return_list = [
                line for line in return_list if line is not None
            ]  # Remove 'None' results
        ...
        for phen, phen_table in grouped:
            best_index_corr = phen_table[
                "nLLcorr"
            ].idxmin()  # with Pandas, this returns the index in the parent table, not the group table
        ...
        if cache_folder is not None and os.path.exists(jackknife_table_fn):
            jackknife_table = pd.read_csv(
                jackknife_table_fn, delimiter="\t", index_col=False, comment=None
            )
        else:
            arg_list = []
            for phen_target in phen_target_array:
        ...
        jackknife_table["diff"] = (
            jackknife_table.h2uncorr - jackknife_table.h2corr
        )  # Compute the diff = h2uncorr-h2corr column
        results_both = jackknife_table[
            jackknife_table.jackknife_index == -1
        ]  # Create a table of the real (non-jackknifed) results for both alphas (which may be the same)
        del results_both["jackknife_index"]
        results_corr = results_both[
            results_both.alpha == [alpha_dict[phen][0] for phen in results_both.phen]
        ]  # Create version for g+e's alpha
        results_gxe2 = results_both[
            results_both.alpha == [alpha_dict[phen][1] for phen in results_both.phen]
        ...
        results_corr["from SE, one-sided, P (e2=0)"] = stats.t.sf(
            results_corr["e2"] / results_corr["e2 SE"], df=jackknife_count_actual - 1
        )
        ...
        if cache_folder is not None and os.path.exists(permplus_table_fn):
            permplus_table = pd.read_csv(
                permplus_table_fn, delimiter="\t", index_col=False, comment=None
            )
        else:
            arg_list = []
            for phen_target in phen_target_array:
        ...
        real_result_permplus = permplus_table[permplus_table.permute_plus_index == -1][
            ["phen", "nLLcorr"]
        ]
        ...
        for phen_target in permtimes_phenotypes:
            ...
            if cache_folder is not None and os.path.exists(permtimes_table_fn):
                permtime_results = pd.read_csv(
                    permtimes_table_fn, delimiter="\t", index_col=False, comment=None
                )
            else:
                arg_list = []
                pheno_one = pheno[
        ...
        if permtimes_table_list:  # not empty
            ...
            real_result_permtimes = permtimes_table[
                permtimes_table.permute_times_index == -1
            ][["phen", "nLL_gxe2"]]
            real_result_permtimes.rename(
                columns={"nLL_gxe2": "nLL_gxe2_real"}, inplace=True
            )
            ...
            summary_permtimes_table_fn = (
                "{0}/summary.permutation.GxE.{1}.count{2}.txt".format(
...
if __name__ == "__main__":
    ...
    sd = distance_between_centers / 4.0
    ...
    spatial_coor_gen_original = SnpData(
        ...
        sid=["x", "y"],
        val=centers
        + np.random.multivariate_normal([0, 0], [[1, 0], [0, 1]], size=len(centers))
        * sd,
    ...
    if do_plot:
        ...
        plt.axis("equal")
        plt.scatter(
            spatial_coor_gen_original.val[:, 0],
    ...
    np.random.seed(seed)
    pheno_causal = SnpData(
        iid=iid,
        sid=["causal"],
        val=np.random.multivariate_normal(np.zeros(iid_count), K_causal.val).reshape(
            -1, 1
        ),
    ...
    pheno_noise = SnpData(
        iid=iid,
        sid=["noise"],
        val=np.random.normal(size=iid_count).reshape(-1, 1),
        parent_string="noise",
    )
    np.random.seed(seed ^ 12230302)
    pheno_loc_original = SnpData(
        iid=iid,
        sid=["loc_original"],
        val=np.random.multivariate_normal(np.zeros(iid_count), K_loc.val).reshape(
            -1, 1
        ),
    ...
    print(
        "pop0 mean={0}. pop1 mean={1}".format(
            pheno.val[is_pop0].mean(), pheno.val[is_pop1].mean()
        )
    )
    ...
    if do_plot:
        ...
        plt.plot(
            list(range(sum(is_pop0))),
            pheno.val[is_pop0],
            "r.",
            list(range(sum(is_pop1))),
            pheno.val[is_pop1],
            "b.",
        )
        plt.show()
    ...
    def generate_and_analyze(
        seed, N, do_shuffle, just_testing=True, map_function=None, cache_folder=None
    ):
        # Generate SNPs
        snpdata = snp_gen(
            fst=0.1,
        ...
        pheno_causal = SnpData(
            ...
            val=np.random.multivariate_normal(
                np.zeros(iid_count), K_causal.val
            ).reshape(-1, 1),
            parent_string="causal",
        )
        np.random.seed(seed ^ 998372)
    ...
    def loop_generate_and_analyze(
        N, seed_count, do_shuffle, just_testing, map_function, cache_folder=None
    ):
        """
        ...
        for seed in range(seed_count):
            ...
            df = generate_and_analyze(
                seed=seed,
                N=N,
                do_shuffle=do_shuffle,
                just_testing=just_testing,
                map_function=map_function,
                cache_folder=cache_folder_per_seed,
            )
            df_list.append(df)
    ...
    df = loop_generate_and_analyze(
        N=100, seed_count=7, do_shuffle=True, just_testing=True, map_function=map
    )
    df = loop_generate_and_analyze(
        N=50, seed_count=1, do_shuffle=False, just_testing=False, map_function=map
    )
    df = loop_generate_and_analyze(
        N=50, seed_count=1, do_shuffle=True, just_testing=False, map_function=map
    )
    df
