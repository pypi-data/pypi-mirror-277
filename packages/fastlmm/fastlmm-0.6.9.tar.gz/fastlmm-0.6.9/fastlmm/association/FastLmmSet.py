...
class FastLmmSet:  # implements IDistributable
    ...
    def __init__(self, **entries):
        ...
        self.nlocalperm = (
            None  # same as nperm, but local to each test (doesn't do pooling)
        )
        ...
        if self.hasNonNoneAttr("alt_snpreader") and self.hasNonNoneAttr("bedfilealt"):
            raise Exception("alt_snpreader or bedfilealt may be given, but not both")
        if self.hasNonNoneAttr("bedfilealt"):
            warnings.warn(
                "bedfilealt is deprecated. alt_snpreader instead", DeprecationWarning
            )
        ...
        if not hasattr(self, "test"):
            raise Exception("FastLmmSet must have 'test' set")
    ...
    def work_count(self):
        ...
        if self.altset_list2 is None:  # singleton sets
            ...
            return (
                len(self.altsetlist_filtbysnps)
                * len(self.__altsetlist2_filtbysnps)
                * (1 + self.nperm)
            )
    ...
    def work_sequence(self):
        ...
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            if self.altset_list2 is None:  # singleton sets
                    ...
                    for iperm in range(
                        ...
                        if self.genphen is None:
                            ...
                            if (
                                ...
                                y_back = loadPhen(
                                    filename=self.genphen["varBackNullPhenGen"]
                                )["vals"]
                            elif y_back is None:
                                y_back = 0

                            if (
                                self.genphen["once"] and y is None
                            ):  # only generate the phenotype once for entire run
                                newseed = self.mainseed ^ self.genphen["seed"]
                                assert (
                                    self.genphen["varG"] == 0
                                ), "doesn't make sense to have varG>0 and only one phen--not sure why I put this code there"
                                if (
                                    self.genphen["varG"] > 0
                                    and nInd <= SNPsalt["snps"].shape[1]
                                ):
                                    Kall = self.KfromAltSnps(
                                        nInd
                                    )  # useful for full rank
                                    y = gp.genphen(
                                        y_G0=y_G0 + y_back,
                                        G1=None,
                                        covDat=self.__X,
                                        options=self.genphen,
                                        nInd=nInd,
                                        K1=Kall,
                                        randseed=newseed,
                                    )
                            ...
                            elif not self.genphen[
                                "once"
                            ]:  # generate for each set in turn
                                eachseed = utilx.combineseeds(
                                    iset, self.genphen["seed"]
                                )
                        ...
                        yield lambda altset=altset, iset=iset, iperm=iperm, y=y, ichrm=ichrm, iposrange=iposrange: self.run_test(
                            SNPs1=SNPsalt,
                            G1=G1,
                            y=y,
                            altset=altset,
                ...
                raise Exception(
                    "not implemented, started a long time ago and never finished"
                )
    ...
    def check_for_None(self, alteqnull, alteqnullperm):
        ...
        if len(none_ind) > 0:
            raise Exception(
                "found None entries in alteqnull during reduce, which means some Result objects were missing"
            )
        if len(none_ind_perm) > 0:
            raise Exception(
                "found None entries in alteqnullperm during reduce, which means some Result objects were missing"
            )
    ...
    def reduce(self, result_list_sequence):
        ...
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            if self.datestamp == "auto":
                self.outfile = utilx.appendtofilename(self.outfile, "tab")
            ...
            if self.write_lrtperm and self.nperm > 0:
                self._saveArray(
                    "lrtperm",
                    ["2*(LL(alt)-LL(null))", "alteqnull", "setsize"],
                    (lrtperm, alteqnullperm, setsizeperm),
                )

            pv_adj, ind = self.test.pv_adj_and_ind(
                self.nperm,
                pv_adj,
                self.nullfit,
                lrt,
            ...
            if (
                not self.show_pvalue_5050
            ):  # Remove the unwanted column if requested and necessary
                dataframe = pd.read_csv(
                    outfiletab, delimiter="\t", comment=None
    ...
    def TESTBEFOREUSINGKfromAltSnps(self, N, SNPsalt=None):
        ...
        if self.alt_snpreader.snp_count < N:
            raise Exception("need to adjust code to handle low rank")
    ...
    def copyinputs(self, copier):
        ...
        if (
            self.genphen is not None
            and "varBackNullPhenGen" in self.genphen
            and self.genphen["varBackNullPhenGen"] is not None
        ):
    ...
    def _check_entries(self, entries):
        ...
        assert all(
            [
                (v in set(["effect", "link", "penalty", "approx"]))
                for v in entries["nullModel"]
            ]
    ...
    def _check_params_after(self):
        ...
        if self.minsetsize is not None and self.minsetsize < 1:
            raise Exception("minsetsize must be 1 or greater")
        if self.nullfitfile is not None:
            assert (
                self.nperm is None or self.nperm == 0
            ), "if using nullfitfile then nperm should be 0"
        assert ((self.nlocalperm is None) or self.nlocalperm == 0) or (
            (self.nperm is None) or self.nperm == 0
        ), "cannot use both nperm and nlocalperm"
        assert (
            self.nullfitfile is None or self.nlocalperm == 0
        ), "cannot use nullfitfile with nlocalperm"

        if self.genphen is not None:
            if "casefrac" not in list(self.genphen.keys()):
                self.genphen["casefrac"] = None
            if "fracCausal" not in list(self.genphen.keys()):
                self.genphen["fracCausal"] = 1.0
            if "numBackSnps" not in list(self.genphen.keys()):
                self.genphen["numBackSnps"] = 0
            if "varBackNullFileGen" not in list(self.genphen.keys()):
                self.genphen["varBackNullFileGen"] = None
    ...
    def pv_from_localperm(self, iperm, result, t0, varcomp_test, y):
        ...
        assert (
            iperm == -1
        ), "should not be permuting for global null fit here, only local"
        assert (
            str(self.test) == "lrt"
        ), "can only use local permutations with lrt, not with " + str(self.test)
        assert (
            self.altModel["effect"] == "mixed" and self.altModel["link"] == "linear"
        ), "nlocalperm only set up for linear lrt at the moment"
        assert (
            self.nullModel["effect"] == "fixed"
        ), "nlocalperm only set up for one-kernel"
    ...
    def run_test(
        ...
        logging.info(
            "Working on permutation index #{0} (where -1 means no permutation) out of {1} total permutations...".format(
                iperm, self.nperm
            )
        )
        logging.info(
        ...
        if varcomp_test is None or not self.cache_from_perm:
            ...
            if self.filenull is not None:
                ...
                i_exclude = utilx.excludeinds(
                    pos0, SNPs1["pos"], mindist=self.mindist, idist=self.idist
                )
        ...
        if self.permute is not None:  # permute the data to test for type I error
            permutationIndex = utilx.generate_permutation(
                SNPs1["snps"].shape[0], self.mainseed ^ self.permute
            )
        if (
            iperm >= 0
        ):  # permute the data to create Null-only P-values for lrt null fitting
        ...
        if varcomp_test is None or not self.cache_from_perm:
            ...
            if (
                ...
                or ((str(self.test) != "lrt_up") and result.nexclude > 0)
                or self.__varcomp_test is None
            ):
            ...
            result.test = varcomp_test.testG(
                G1, self.test, i_exclude=i_exclude, G_exclude=G_exclude
            )
            ...
            assert (
                self.altModel["effect"] == "mixed" and self.altModel["link"] == "linear"
            ), "caching only set up for linear lrt at the moment"
            assert (
                self.nullModel["effect"] == "fixed"
            ), "caching only set up for one-kernel"
            assert (
                self.__G0 is None
            ), "caching only 1-kernel implemented, otherwise need to permute G0 as well here"
    ...
    def setSNPs0(self):
        ...
        if self.autoselect:
            ...
            assert (
                not self.forcefullrank
            ), "cannot currently force full rank with autoselect=True"
            if self.extractSim is not None:
                raise Exception("extractSim not supported with autoselect")
            ...
            if ext.lower() == ".bed":
                ...
                if self.extractSim is not None:
                    raise Exception(
                        "extractSim not supported with filenull in ped format, please use bed format"
                    )
                self.__SNPs0 = {"data": readPED(root), "filename": root}
                self.__SNPs0["original_iids"] = self.__SNPs0["data"]["iid"]
                self.__SNPs0["num_snps"] = self.__SNPs0["data"]["snps"].shape[1]
    ...
    def varcomp_test_setup(self, y, SNPs0=None, i_exclude=None):
        ...
        if self.filenull is not None and (
            not excluded_all_snps
        ):  # aka two-kernel after exclusion has been done
            varcomp_test = self.test.construct(
                y,
                self.__X,
                forcefullrank=self.forcefullrank,
                SNPs0=SNPs0,
                i_exclude=i_exclude,
                nullModel=self.nullModel,
                altModel=self.altModel,
                scoring=self.scoring,
                greater_is_better=self.greater_is_better,
            )
        else:  # no background kernel
            varcomp_test = self.test.construct_no_backgound_kernel(
                y,
                self.__X,
                forcefullrank=self.forcefullrank,
                nullModel=nullModelTmp,
                altModel=self.altModel,
                scoring=self.scoring,
                greater_is_better=self.greater_is_better,
            )
    ...
    def run_once(self):
        ...
        if self.genphen is not None:
            ...
            if self.genphen["varBackNullFileGen"] is not None:
                ...
                self.__varBackNullSnpsGen["snps"] = util.standardize(
                    self.__varBackNullSnpsGen["snps"]
                )
            else:
        ...
        if self.filenull is not None:
            ...
            if self.extractSim is not None:
                raise Exception(
                    "extractSim should be specified only if filenull is specified."
                )
            self.__SNPs0 = None
            # self.__G0=None
        ...
        if self.__SNPs0 is not None:
            logging.info(
                "Found " + str(self.__SNPs0["num_snps"]) + " SNPs for null kernel"
            )
        if len(self.__y.shape) > 1:
            nPhen = self.__y.shape[1]
        else:
            nPhen = 1
        ...
        if self.filenull is not None:
            ...
            logging.info(
                "Running test " + str(self.test) + " with no background kernel"
            )
        ...
        logging.info(
            "Found "
            + str(self.work_count / (self.nperm + 1))
            + " tests to run ("
            + str(self.work_count)
        ...
        if self.minsetsize is not None and self.maxsetsize is not None:
            logging.info(
                "min set size=%i, max set size=%i", self.minsetsize, self.maxsetsize
            )
        if self.mpheno is not None:
            logging.info("mpheno=%i", self.mpheno)
        if self.extractSim is not None:
            logging.info("extractSim=%s", self.extractSim)
        logging.info("------------------------------------------------")
    ...
    def intersect_data(self, covar, pheno):
        if self.check_id_order(covar, pheno):
            ...
            indarr = utilx.intersect_ids(
                [
                    pheno["iid"],
                    self.alt_snpreader.original_iids,
                    covarids,
                    nullsnpids,
                    varbacknullsnp,
                ]
            ...
            if (
                hasattr(self, "_varBackNullSnpsGen")
                and self.__varBackNullSnpsGen is not None
            ):
                self.__varBackNullSnpsGen["iid"] = self.__varBackNullSnpsGen["iid"][
                    indarr[:, 4]
                ]
                self.__varBackNullSnpsGen["snps"] = self.__varBackNullSnpsGen["snps"][
                    indarr[:, 4]
                ]
    ...
    def create_altsetlist_filtbysnps(self, altset_list, altsetbed):
        ...
        ):  # filter by those sets specified on the command line, stored in self.sets
            altset_list = Subset(altset_list, self.sets)

        if self.minsetsize is not None or self.maxsetsize is not None:
            altset_list = MinMaxSetSize(
                altset_list, minsetsize=self.minsetsize, maxsetsize=self.maxsetsize
            )  # would be nice if applying 1,None wouldn't wrap at all (becuase that is the identity)

        altsetlist_filtbysnps = altset_list.addbed(altsetbed)

        if len(altsetlist_filtbysnps) == 0:
            raise Exception("Expect altset_list to contain at least one set")
    ...
    def __repr__(self):
        ...
        for f in dir(self):
            if f.startswith("_"):  # remove items that start with '_'
                continue
            if (
                type(self.__class__.__dict__.get(f, None)) is property
            ):  # remove @properties
                continue
...
if __name__ == "__main__":
    ...
    if not os.path.exists(infile):
        raise Exception("Fatal Error: " + infile + " does not exist")
