...
class _SnpWholeWithTrain(KernelReader):
    ...
    def _read(
        ...
        if row_index_or_none is None and col_index_or_none is None:
            ...
            if self.block_size is None or (
                self.whole.sid_count <= self.block_size
                or self.whole.sid_count <= self.whole.iid_count
            ):
                # read blocks of whole
                whole = self.whole.read(
                    dtype=dtype, force_python_only=force_python_only
                )
                # train standardizer on complement of test_idx
                train = whole[self.train_idx, :].read(
                    force_python_only=force_python_only
                )
                _, trained_std = self.standardizer.standardize(
                    train, return_trained=True, force_python_only=force_python_only
                )
                ...
                if order == "A":
                    order = "C"
                k_val = np.zeros(
                    [self.whole.iid_count, self.whole.iid_count],
                    dtype=dtype,
                    order=order,
                )
                ct = 0
                ...
                for start in range(0, self.whole.sid_count, self.block_size):
                    ct += self.block_size
                    # read blocks of whole
                    whole = self.whole[:, start : start + self.block_size].read(
                        dtype=dtype, force_python_only=force_python_only
                    )
                    # train standardizer on complement of test_idx
                    train = whole[self.train_idx, :].read(
    ...
    def __repr__(self):
        s = "_SnpWholeWithTrain(whole={0},train_idx=[...],standardizer={1}".format(
            self.whole, self.standardizer
        )
...
def _kfold(iid_count, n_folds, seed, end_with_all=False, iid_to_index=None):
    ...
    if n_folds < 0:
        ...
        result = list(
            enumerate(
                KFold(n_splits=-n_folds, random_state=seed, shuffle=True).split(
                    list(range(iid_count))
                )
            )
        )[0:1]
        if end_with_all:
...
def single_snp_all_plus_select(
    ...
    :param G: SNPs from which to create a similarity matrix of the top *k* SNPs. If not given, will use test_snps.
           Can be any `SnpReader <http://fastlmm.github.io/PySnpTools/#snpreader-snpreader>`_. If you give a string, it should be the base name of a set of PLINK Bed-formatted files.
    :type G: `SnpReader <http://fastlmm.github.io/PySnpTools/#snpreader-snpreader>`_ or a string
    ...
    with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
        ...
        def _best_snps_for_each_chrom(
            ...
            def mapper_find_best_given_chrom(test_chr):
                ...
                def mapper_gather_lots(i_fold_and_pair):
                    ...
                    logging.info(
                        "Working on GWAS_1K and k search, chrom={0}, i_fold={1}".format(
                            test_chr, i_fold
                        )
                    )
                    ...
                    from fastlmm.association.single_snp import (
                        _internal_determine_block_size,
                        _block_size_from_GB_goal,
                    )

                    min_count = _internal_determine_block_size(
                        G_for_chrom, None, None, force_full_rank, force_low_rank
                    )
                    block_size = _block_size_from_GB_goal(
                        GB_goal, G_for_chrom.iid_count, min_count
                    )
                    ...
                    assert np.array_equal(
                        K_whole_unittrain.iid, G_for_chrom.iid
                    ), "real assert"
                    ...
                    single_snp_result = single_snp(
                        test_snps=G_train,
                        K0=K_train,
                        pheno=pheno,  # iid intersection means when can give the whole covariate and pheno
                        covar=covar,
                    ...
                    k_list_in = [0] + [
                        int(k) for k in k_list if 0 < k < len(single_snp_result)
                    ]
                    ...
                    if i_fold == n_folds:
                        ...
                        for k in k_list_in:
                            top_k = G_for_chrom[
                                :, G_for_chrom.sid_to_index(single_snp_result.SNP[:k])
                            ]
                            logging.info(
                ...
                def reducer_find_best(top_snps_and_k_index_to_nLL_sequence):
                    ...
                    if do_plot:
                        _nll_plot(k_list_in_all, k_index_to_sum_nll)
        ...
        def _gwas_2k_via_loo_chrom(
            test_snps,
            chrom_list,
            input_files,
            runner,
            G,
            chrom_index_to_best_sid,
            pheno,
            covar,
            force_full_rank,
            force_low_rank,
            ...
            logging.info(
                "Doing GWAS_2K for each chrom. Work_count={0}".format(len(chrom_list))
            )
...
if __name__ == "__main__":
    logging.basicConfig(
        level=logging.WARN
    )  # Needs to be WARN and not INFO to stop Doctest from putting out extra messages.
