...
def compare_files(file1, file2, tol=1e-8, delimiter="\t"):
    ...
    try:
        ...
        return np.all(
            dat1 == dat2
        ), "single column result doesn't match exactly ('{0}')".format(file1)
    # logging.warning("DO headers match up? (file='{0}', '{1}' =?= '{2}')".format(file1, head1,head2))
    if not np.all(head1 == head2):
        return False, "headers do not match up (file='{0}', '{1}' =?= '{2}')".format(
            file1, head1, head2
        )
        ...
        for c in range(c_count):
            ...
            if val1 != val2:
                ...
                if abs(f1 - f2) > tol:
                    return (
                        False,
                        "Values too different (file='{0}', '{1}' =?= '{2}')".format(
                            file1, val1, val2
                        ),
                    )
    return True, "files are comparable within abs tolerance=%e" % tol
...
def combineseeds(seed1, seed2):
    ...
    )  # as of numpy 1.9, seeds must be 32-bit, so keep only the 8 right-most hex digits
    return seed
...
def extractcols(filein, colnameset=None, dtypeset=None):
    if colnameset is None:
        raise Exception("must specify column names to read")
    import pandas as pd
...
def dotDotRange(dotDotString):
    ...
    for intervalString in dotDotString.split(","):
        ...
        if len(parts) > 2:
            raise Exception(
                "Expect at most one '..' between commas. (see {0})".format(
                    intervalString
                )
            )
        start = int(parts[0])
...
def _color_list(chr_list, rle):
    ...
    result = [
        index_to_color[chr_to_index[chr] % len(index_to_color)] for chr in chr_list
    ]
    return result
...
def manhattan_plot(
    ...
    :Example:
        ...
        >>> results_dataframe = single_snp(test_snps=test_snps, pheno=pheno_fn, h2=.2, count_A1=False)
