...
def recalibrate(
    ...
    """
    assert (
        lrtpermfile is not None
    ), "must provide lrtpermfile (output by FastLmmSet.py with lrt)"
...
def pairedpvalsin_onefile(
    dirin,
    filepattern="*.txt",
    pnames=["P-value(50/50)", "P-value_adjusted"],
    rownames=rownames(),
    logspace=True,
):
    """
        ...
        if filepattern[s] is not None:
            ...
            if Norig is None:
                ...
                if not (N == Norig):
                    ...
                    raise Exception(
                        "number of tests has changed, look for missing files"
                    )
...
def type1errdir(
    ...
    for f in myfiles:
        if "info.txt" not in f:
            ...
            if auditind in auditdic:
                if auditrange is not None:
                    duplicates.append(int(auditind))
            elif auditrange is not None and auditind not in auditrange:
                extras.append(int(auditind))
            else:
                if auditrange is not None:
                    auditdic[auditind] = True
                pvtmp, rowids, garb1, garb2 = extractpvals(
                    f, pnames, rownames, includefilename=False, verbose=False
                )
                pvtmp.sort()
                pv.extend(pvtmp)
    ...
    if verbose:
        ...
        if not dopower:
            print(
                "Threshold".ljust(padl, " ")
                + "\t"
                + "Obs. Error".ljust(padl, " ")
                + "\t"
...
def qqplotdir(
    ...
    for f in myfiles:
        ...
        if ycoord > maxycoord:
            ycoord = ycoord - ycoordshift + 10
        if savefiles:
            fileout = f[0:-4] + ".out.png"
        else:
            fileout = None
        if not aggregate:
            qqplotfile(
