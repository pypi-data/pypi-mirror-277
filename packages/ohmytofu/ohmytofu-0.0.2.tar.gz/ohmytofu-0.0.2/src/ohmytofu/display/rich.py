...
class RichDisplay(Display):
    ...
    def progress(self, total: int, name: str) -> Iterator[Progress]:
        with rich_progress(self.console, name=name) as progress:
...
class RichProgress(Progress):
    ...
    @override
    def update(self, n: float = 1, name="") -> None:
        advance = (n / self.total) * 100
        self.name = name
        self.progress.update(task_id=self.task_id, advance=advance, refresh=True, description=name)
        if self.on_update:
            self.on_update()
...
class RichTaskDisplay(TaskDisplay):
    def __init__(
        ...
        self.theme = theme
        self.progress_ui = rich_progress(console, name="TofuAI")
        self.name = ""
        self.render = render
        self.on_update()
    ...
    @override
    def summary(self, results: EvalResults, stats: EvalStats, batch_id: str) -> None:
        panel = self.task_panel(
            body=task_stats(self.profile, stats, self.theme),
            config=None,
            footer=task_results(results, self.theme, batch_id),
    ...
    def on_update(self) -> None:
        panel = self.task_panel(
            ...
            footer=live_task_footer(self.theme, self.profile.batch_id),
            log_location=None,
        )
        self.render(panel)
...
def task_targets(profile: TaskProfile) -> str:
    return "   " + "\n   ".join(
        [str(str(profile.model).replace('openai', 'tofu')), f"dataset: {profile.dataset}", f"scorer: {profile.scorer}"]
    )
...
def task_resources() -> str:
    ...
    for model, resource in concurrency_status().items():
        model = model.replace("openai", "tofu")
        resources[model] = f"{resource[0]}/{resource[1]}"
    return task_dict(resources)


def live_task_footer(theme: Theme, batch_id: str) -> tuple[RenderableType, RenderableType]:
    return (
        f"[{theme.light}]{task_resources()}[/{theme.light}]",
        Text(task_batch_id(batch_id), style=theme.light),
    )


def task_results(
    results: EvalResults, theme: Theme, batch_id: str
    ...
    return (metrics, Text(task_batch_id(batch_id), style=theme.light))
...
def task_stats(profile: TaskProfile, stats: EvalStats, theme: Theme) -> RenderableType:
    ...
    for model, usage in stats.model_usage.items():
        table.add_row(
            Text(model.replace("openai", "tofu"), style="bold"),
...
def task_batch_id(batch_id) -> str:
    return f"Batch ID: {batch_id}"
...
def rich_progress(console: Console, name: str) -> RProgress:
    return RProgress(
