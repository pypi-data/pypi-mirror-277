...
def createHU_matrix(directory, inp_file):
    ...
    try:
        ...
        TablePos = dc.ImagePositionPatient[-1]

    try:
        RescaleInt = dc.RescaleIntercept
    except:
        RescaleInt = 0
    ...
    for file in dcms:
        ...
        if zrange0 <= slicelocation <= zrange1:
            ...
            dataframe_list.append([image_array, (dc.ImagePositionPatient)[0], (dc.ImagePositionPatient)[1], round(slicelocation,3), x_space, y_space, RescaleInt])
...
@jit
def get_voxel_value(x: float, y: float, voxeldata, IPx: float, IPy: float, shapex: float, shapey: float, xw: float, yw: float, rescaleint: float):
    ...
    px, py = x2/xtw, y2/ytw
    ix, iy = int(np.floor(px*shapex).item()), int(np.floor(py*shapey).item())
    voxelvalue = (voxeldata[iy, ix]) + rescaleint
...
def find_matprops(centroids, dataframe_list):
    ...
    for i in centroids:
        ...
        x, y = i[1], i[2]
        rescaleint = dataframe_list[nearestz_index][6]
        ...
        #    print(get_voxel_value(x, y, voxeldata, IPx, IPy, shapex, shapey, xw, yw, rescaleint))
        #    exit()
        #t = t+1

        cell_value = get_voxel_value(x, y, voxeldata, IPx, IPy, shapex, shapey, xw, yw, rescaleint) 
...
def write_inp(element_modulus, elements, inp_file, output_filename, numberofmaterials, poisson):
    ...
    for i in range(numberofmaterials):
        ...
        # Add the footer text
        result_lines.append(f"*SOLID SECTION, ELSET={binned_data[i][0]}, MATERIAL={name}_material{i}\n")

    for i in range(numberofmaterials):
        # Add the header for each new material
        result_lines.append(f"*MATERIAL, NAME={name}_material{i}\n*DENSITY\n")
...
def extract_elementdata(dicomdirectory: str, inp_file: str, outputscale: str, phantom1HU=None, phantom2HU=None, phantom1density=None, phantom2density=None, output_filename = "output.inp", number_of_materials=20, poisson_ratio=0.3):
    """extrancts an array of elements [element number, centroid-x, centroid-y, centroid-z, value] which indicates the calibrated HU, density, or modulus values of those elements. This process may take > 5 mins.
