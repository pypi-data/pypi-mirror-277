...
def configure_git_credentials(repo_path, url, username, password):
    ...
    credential_store_command = ['git', 'credential-store']
    credential_store_path = f"{repo_path}/.git-credentials"
    # 使用subprocess模块执行命令，将凭证信息传递给git credential-store命令
    process = subprocess.Popen(credential_store_command + [credential_store_path], stdin=subprocess.PIPE)
