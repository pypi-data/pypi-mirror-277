...
def is_latest(repo_path, branch_name='master', url=None, user=None, pwd=None):
    # 打开本地仓库
    repo = Repo(repo_path)

    # 设置Git配置，指定用户名和密码
    if user and pwd is not None:
        repo.git.config(f'credential.{url}.username', user)
        repo.git.config(f'credential.{url}.password', pwd)
        # repo.config_writer().set_value('http', url, 'username', user)
        # repo.config_writer().set_value('http', url, 'password', pwd)
