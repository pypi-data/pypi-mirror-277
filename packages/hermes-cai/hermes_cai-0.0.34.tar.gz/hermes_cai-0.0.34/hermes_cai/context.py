...
class Context:
    ...
    def __init__(
        ...
        self.num_total_message_tokens = 0
    ...
    def _observe_context_metrics(self):
        ...
        Metrics().TOKEN_METRICS.labels(
            metric_type="total_rendered_message_tokens"
        ).observe(self.num_total_message_tokens)
    ...
    def _maybe_extract_metadata_and_clean(self, idx: int, prompt_part: PromptPart):
        ...
        if prompt_part.name.startswith("message"):
            ...
            self.num_total_message_tokens += len(prompt_part.tokens)
