...
class Context:
    ...
    def _observe_context_metrics(self):
        ...
        Metrics().MESSAGE_METRICS.labels(metric_type="total_messages").observe(
        ...
        if self.num_total_pinned_messages > 0:
            Metrics().MESSAGE_METRICS.labels(
                metric_type="total_messages_with_pinned"
            ).observe(self.num_total_messages)
            Metrics().MESSAGE_METRICS.labels(
                metric_type="truncated_messages_with_pinned"
            ).observe(self.num_messages_truncated)
            Metrics().MESSAGE_METRICS.labels(
                metric_type="total_pinned_messages"
            ).observe(self.num_total_pinned_messages)
