...
class PipelineClassification:
    ...
    @staticmethod
    def feature_importance(model_input, all_features: list, polars: bool = False) -> pl.DataFrame:
    ...
    def rf(self, report_output: bool = True, params: dict = None):
        # train
        self.rf_model = RandomForestClassifier()
        self.rf_model.fit(self.x_train, self.y_train)
        # predict
        self.pred = self.rf_model.predict(self.x_test)
        # report
        report = None
        if report_output:
            report = self.report()
        return self.rf_model, report
