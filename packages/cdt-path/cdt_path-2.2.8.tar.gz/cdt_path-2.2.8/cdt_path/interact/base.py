...
class Interact:
	...
	def updata_path(self, start_point, goal_point):
		...
		came_from, cost_so_far = a_star_P(self.triang, start_point, goal_point,start, goal)
		L=[goal]
		val = came_from[goal]
		...
		Pr = np.concatenate((Pr, [goal_point]))
		Px = self.triang.x[Ll]
		Py = self.triang.y[Ll]
		...
		Pl = np.concatenate((Pl[::-1], [goal_point]))
		Ln = funnel(start_point, Pl, Pr, Li)
		Ln = [start_point] + Ln + [goal_point]
		Ln_np = np.array(Ln)
