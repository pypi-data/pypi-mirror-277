...
from .core.client.model.embed_inputs import EmbedInputs
from .core.client.model.inline_object3 import InlineObject3
...
class Inference(PineconePlugin):
    ...
    def embed(
        ...
        """
        embeddings_inputs = [EmbedInputs(text=i) for i in inputs]

        if parameters:
            embeddings_parameters = build_parameters_dict_for_inference(parameters)
            request_body = InlineObject3(model=model, inputs=embeddings_inputs, parameters=embeddings_parameters)
        else:
            request_body = InlineObject3(
                model=model,
                inputs=embeddings_inputs,
            )

        embeddings_list = self.inference_api.embed(inline_object3=request_body, async_req=async_req)
