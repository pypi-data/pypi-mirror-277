...
def pt_metrics(
  ...
  *,
  ...
  return {
    f'uci-{k}': v for k, v in uci.pt_metrics(uci_preds, true_ucis, ignore_idx=ignore_idx).items()
  } | {
    f'fen-{k}': v for k, v in fen.pt_metrics(fen_preds, true_fens, ignore_idx=ignore_idx).items()
  } # type: ignore