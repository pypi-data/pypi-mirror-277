...
def arquivo_no_root(nome_site, nome_arquivo, aba_sheet=None):
...
def arquivo_em_pasta_root(info_url, aba_sheet=None):
...
def arquivo_em_pastas(info_url, aba_sheet=None):
...
# Função para retornar uma planilha no SharePoint
def obter_planilha(client_id, client_secret, tenant_id, url, aba_sheet=None):
  ...
  if(len(info_url) == 3):
    return arquivo_no_root(info_url[0], info_url[2], aba_sheet)
  elif(len(info_url) == 4):
    return arquivo_em_pasta_root(info_url, aba_sheet)
  else:
    return arquivo_em_pastas(info_url, aba_sheet)
