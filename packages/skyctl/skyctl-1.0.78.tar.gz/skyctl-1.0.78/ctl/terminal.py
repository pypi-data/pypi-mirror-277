...
def check():
    ...
    if authority(user):
        ...
        click.echo('User permission verification failed, please log in again or register on the official website!')
...
def file(space: Optional[str]):
    ...
    user = decryption()
    if authority(user):
        upload_file.execute(int(user.uid), upload_url, space)
    else:
        click.echo('User permission verification failed, please log in again or register on the official website!')
...
def create_space(create: Optional[str], ls: bool):
    if create:
        ...
        user = decryption()
        if authority(user):
            name_space.create(create_url, user.uid, create)
        else:
            click.echo('User permission verification failed, please log in again or register on the official website!')
    ...
    if ls:
        user = decryption()
        if authority(user):
            name_space.get_list(list_url, user.uid)
        else:
            click.echo('User permission verification failed, please log in again or register on the official website!')
