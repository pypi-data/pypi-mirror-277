...
def create_file_from_template(
    template_filename: str, inputs: Dict[str, str], output_filename: str
):
    """
...
def create_requirements_txt(target_dir_name: str):
    """
...
def create_makefile(target_dir_name: str):
    ...
    if not exists(output_file):
        create_file_from_template(
            template_filename="Makefile.jinja", inputs={}, output_filename=output_file
        )
...
def create_manifest(
    spec: PluginSpecTypes.Spec, bin_dir_name: str, prefix: Optional[str] = "icon"
):
    """
    ...
    create_file_from_template(
        ...
        inputs={
            ...
            "description": spec.get(Constants.DESCRIPTION, ""),
...
def create_trigger_py(
    ...
    trigger_spec_input = (
        spec.get(Constants.TRIGGERS, {}).get(trigger_name, "").get(Constants.INPUT)
    )
    trigger_spec_output = (
        spec.get(Constants.TRIGGERS, {}).get(trigger_name, "").get(Constants.OUTPUT)
    )
...
def create_trigger_schema(
    spec: PluginSpecTypes.Spec, trigger_name: str, trigger_dir_name: str
):
    """
    ...
    create_file_from_template(
        ...
        inputs={
            "base_module": BASE_MODULE,
            "trigger": trigger_name,
            "description": trigger_spec.get(Constants.DESCRIPTION, ""),
...
def create_connection_py(spec: PluginSpecTypes.Spec, connection_dir_name: str):
    """
...
def create_connection(spec: PluginSpecTypes.Spec, connection_dir_name: str):
    """
    ...
    create_connection_py(spec, connection_dir_name)
...
def create_action_py(
    ...
    action_spec_output = (
        spec.get(Constants.ACTIONS, {}).get(action_name, "").get(Constants.OUTPUT)
    )
...
def create_action_schema(
    spec: PluginSpecTypes.Spec, action_name: str, action_dir_name: str
):
    """
...
def create_task_py(task_name: str, task_description: str, task_dir_name: str):
    """
...
def create_task_schema(spec: PluginSpecTypes.Spec, task_name: str, task_dir_name: str):
    ...
    create_file_from_template(
        ...
        inputs={
            "base_module": BASE_MODULE,
            "task": task_name,
            "description": task_spec.get(Constants.DESCRIPTION, ""),
...
def create_tasks(spec: PluginSpecTypes.Spec, tasks_dir_name: str):
    """
...
def get_prefix(spec: PluginSpecTypes.Spec, target_dir_name: str) -> str:
    ...
    prefix = (
        ...
        if os.path.isdir(
            os.path.join(
                target_dir_name, f'{BASE_PREFIX}_{spec.get(Constants.NAME, "")}'
            )
        )
        else KOMAND_PREFIX
    )
...
def create_files_create(spec: PluginSpecTypes.Spec, dir_name: str, prefix: str):
    """
...
def create_files_refresh(
    spec: PluginSpecTypes.Spec, dir_name: str, prefix: str, ignore: list
):
    """
    This handles all the other 'create_{file}' functions and merges them into one.
    :param spec: The plugin spec dictionary
...
def handle_refresh_create(
    spec: PluginSpecTypes.Spec, target_dir_name: str, is_create: bool, ignore: list
):
    """
    ...
    if is_create:
        ...
        create_files_create(spec=spec, dir_name=dir_name, prefix=prefix)
    else:
