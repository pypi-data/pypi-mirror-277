...
def load_revenue_auto(directory, skip_rows=13, lhnv = None, return_all_dfs = False):
    ...
    if return_all_dfs:
        ...
        """ CONCAT INTO ONE DF"""
        all_dataframes = {name: globals()[name] for name in global_df_names if name in globals()}
        df_rev = pd.concat(all_dataframes.values(), ignore_index=True)
