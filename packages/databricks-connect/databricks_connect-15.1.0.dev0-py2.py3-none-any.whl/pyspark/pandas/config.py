...
def get_option(key: str, default: Union[Any, _NoValueType] = _NoValue) -> Any:
    ...
    # BEGIN-EDGE
    # As a part of fixing "[SERV-208] pandas API for Spark not working on serverless",
    # we should return the default value when default mode on.
    if (
        spark_session.conf.get(
            "spark.databricks.pyspark.pandas.serverless.block.configurations", "false"
        ).lower()
        == "true"
    ):
        # "compute.ops_on_diff_frames" is True when default mode on.
        if key == "compute.ops_on_diff_frames":
            return True
        return default
    else:
...
def set_option(key: str, value: Any) -> None:
    ...
    # BEGIN-EDGE
    # As a part of fixing "[SERV-208] pandas API for Spark not working on serverless",
    # we should disallow setting the option when default mode on.
    if (
        spark_session.conf.get(
            "spark.databricks.pyspark.pandas.serverless.block.configurations", "false"
        ).lower()
        ...
        raise PySparkRuntimeError(
            error_class="NOT_SUPPORTED_WITH_SERVERLESS",
            message_parameters={"feature": "set_option"},
        )
    else:
...
def reset_option(key: str) -> None:
    ...
    # BEGIN-EDGE
    # As a part of fixing "[SERV-208] pandas API for Spark not working on serverless",
    # we should disallow resetting the option when default mode on.
    if (
        spark_session.conf.get(
            "spark.databricks.pyspark.pandas.serverless.block.configurations", "false"
        ).lower()
        ...
        raise PySparkRuntimeError(
            error_class="NOT_SUPPORTED_WITH_SERVERLESS",
            message_parameters={"feature": "reset_option"},
        )
    else:
