...
from pyspark.sql.connect.profiler import ConnectProfilerCollector, detect_client_context
...
def _safe_enable_large_protobufs_if_available():
    try:
...
class ChannelBuilder:
    ...
    def userAgent(self) -> str:
        ...
        return " ".join(
            [
                user_agent,
                f"dbconnect/{__dbconnect_version__}",
                f"spark/{__version__}",
...
class SparkConnectClient(object):
    ...
    def __init__(
        ...
        add_client_call_context: bool = True,  # EDGE
        client_call_context_exclusions: List[str] = ["/pyspark/sql/", "/dbruntime/"],  # EDGE
        send_release_until: bool = False,  # EDGE
    ):
        """
        ...
        add_client_call_context: bool
            Adds client-side information to the request and allows for better insights into what
            queries were run on the cluster.
        client_call_context_exclusions: List[str]
            Skip stack trace frames whose filename contains one of the given substrings.
        send_release_until: bool
            Whether to send ReleaseUntil requests to server. Default: False
            Note: If the environment variable `SPARK_CONNECT_SEND_RELEASE_UNTIL` exists, its value
            has higher precedence than the parameter here.
            If true, ReleaseUntil requests are sent to the server to confirm every server response
            in a reattachable ExecutePlanResponse stream. The confirmation would allow the server
            to process/clean up consumed responses in any way it sees fit.
            Note: This configuration will take no effect if reattachable execution is disabled.
        """
        # BEGIN-EDGE
        ...
        # BEGIN-EDGE
        retry_unknowns_policy = DatabricksRetryUnknownsPolicy()
        cluster_wait_policy = DatabricksClusterWaitPolicy()
        # END-EDGE

        self.set_retry_policies(
            [cluster_wait_policy, retry_unknowns_policy, default_policy]  # Edge
        )
        ...
        self._artifact_manager = ArtifactManager(
            self._user_id, self._session_id, self._channel, self.metadata()
        )
        self._use_reattachable_execute = use_reattachable_execute
        self._add_client_call_context = add_client_call_context  # EDGE
        self._client_call_context_exclusions = client_call_context_exclusions  # EDGE
        ...
        # BEGIN-EDGE
        if "SPARK_CONNECT_SEND_RELEASE_UNTIL" in os.environ:
            self._send_release_until = (
                os.environ["SPARK_CONNECT_SEND_RELEASE_UNTIL"].lower() == "true"
            )
        else:
            self._send_release_until = send_release_until
        # END-EDGE

    def metadata(self) -> Iterable[Tuple[str, str]]:
        """
        Builds the GRPC specific metadata list to be injected into the request.
        Includes builder's metadata and thread local extra metadata.
        Returns
        -------
        A list of tuples (key, value)
        """
        return self._builder.metadata() + self.thread_local.extra_request_metadata

    def rebuild_connection_for_testing(self):
        self.close()
    ...
    # BEGIN-EDGE
    def disable_send_release_until(self) -> "SparkConnectClient":
        self._send_release_until = False
        return self

    def enable_send_release_until(self) -> "SparkConnectClient":
        self._send_release_until = True
        return self

    def disable_client_call_context(self) -> "SparkConnectClient":
        self._add_client_call_context = False
        return self

    def enable_client_call_context(self) -> "SparkConnectClient":
        self._add_client_call_context = True
        return self

    def set_client_call_context_exclusions(self, exclusions: List[str]) -> "SparkConnectClient":
        self._client_call_context_exclusions = exclusions
        return self
    ...
    def experimental_to_cloudfetch(
        self, plan: pb2.Plan, format: str = "json", compression: bool = True
    ) -> Tuple[Optional[StructType], List[CloudFetchResult]]:
        """
        EXPERIMENTAL FEATURE USE WITH CARE

        This is an experimental feature that allows the caller to indirectly fetch the result of
        the query using the Databricks Cloud Fetch Capabilities. As of right now there is very
        little control over how the results URLs are generated and there is only a default
        expiry time on the URLs of 1h.

        When compression is set to True, the results are compressed using LZ4. Compression is
        only supported for Arrow format.

        Attention: This method does not support passing in observations.
        """
        logger.info(f"Executing plan {self._proto_to_string(plan)}")
        req = self._execute_plan_request_with_metadata()
        req.plan.CopyFrom(plan)

        # Depending on the format type set the proper value:
        if format == "json":
            proto_format = pb2.ResultOptions.CloudOptions.FORMAT_JSON
            if compression:
                raise ValueError("Compression not supported for JSON format.")
        elif format == "csv":
            proto_format = pb2.ResultOptions.CloudOptions.FORMAT_CSV
            if compression:
                raise ValueError("Compression not supported for CSV format.")
        elif format == "arrow":
            proto_format = pb2.ResultOptions.CloudOptions.FORMAT_ARROW
        else:
            raise ValueError(
                f"Invalid format for CloudFetch request: {format}. "
                "Allowed values are: json, csv, arrow."
            )

        ro = pb2.ResultOptions(
            type=pb2.ResultOptions.TYPE_CLOUD,
            cloudOptions=pb2.ResultOptions.CloudOptions(
                format=proto_format,
        ...
        for response in self._execute_and_fetch_as_iterator(req, {}, []):
            if isinstance(response, StructType):
                schema = response
            elif isinstance(response, any_pb2.Any) and response.Is(pb2.CloudResultBatch.DESCRIPTOR):
                batch = pb2.CloudResultBatch()
                response.Unpack(batch)
                cloud_fetch_results += [
                    CloudFetchResult(
                        url=b.url,
        ...
        return schema, cloud_fetch_results
    ...
    def execute_command(
        ...
        extra_request_metadata: Optional[Iterable[Tuple[str, str]]] = None,  # EDGE
    ...
    def execute_command_as_iterator(
        ...
        req.plan.command.CopyFrom(command)
        for response in self._execute_and_fetch_as_iterator(
            req, observations or {}, extra_request_metadata or []
        ):
    ...
    # BEGIN-EDGE
    def _update_request_with_client_call_context(
        self,
        req: Union[
        ...
        client_call_context = detect_client_context(self._client_call_context_exclusions)
        if client_call_context is not None:
            extension = any_pb2.Any()
            extension.Pack(client_call_context)
    ...
    def _execute_plan_request_with_metadata(self) -> pb2.ExecutePlanRequest:
        ...
        if self._user_id:
            req.user_context.user_id = self._user_id
        # BEGIN-EDGE
        if self._add_client_call_context:
            self._update_request_with_client_call_context(req)
    ...
    def _execute(self, req: pb2.ExecutePlanRequest) -> None:
        ...
        try:
            if self._use_reattachable_execute:
                ...
                for attempt in self._retrying():
                    with attempt:
                        for b in self._stub.ExecutePlan(req, metadata=self.metadata()):
                            handle_response(b)
        except Exception as error:
            self._handle_error(error)
    ...
    def _execute_and_fetch_as_iterator(
        ...
        extra_request_metadata: Iterable[Tuple[str, str]],  # EDGE
        ...
        try:
            metadata = self.metadata() + extra_request_metadata  # EDGE
            if self._use_reattachable_execute:
                # Don't use retryHandler - own retry handling is inside.
                generator = ExecutePlanResponseReattachableIterator(
                    req,
    ...
    def interrupt_all(self) -> Optional[List[str]]:
        ...
        try:
            for attempt in self._retrying():
                with attempt:
                    resp = self._stub.Interrupt(req, metadata=self.metadata())
                    self._verify_response_integrity(resp)
