...
class DatabricksChannelBuilder(ChannelBuilder):
    def __init__(
        ...
        params = {
            "x-databricks-cluster-id": config.cluster_id,
        ...
        if headers is not None:
            ...
            if "x-databricks-session-id" in headers:
                ...
                del params['x-databricks-cluster-id']
                # use the session id provided in the header as the spark session id
                params[ChannelBuilder.PARAM_SESSION_ID] = headers["x-databricks-session-id"]
