...
class Relationship:
    ...
    from_space_source: SpaceSource
    to_space_source: SpaceSource

    fkey_relationship: FkeyRelationship

    @property
    def from_source(self) -> Source:
        return self.from_space_source.source

    @property
    def to_source(self) -> Source:
        return self.to_space_source.source
...
@dataclasses.dataclass(kw_only=True)
class SpaceRelationshipsMetadata(DataclassAsTypedMetadataMixin):
    """Metadata attached to relationship path for space edge tables."""
...
class Space(WrappedOrmObject[SpaceID, orm.Space]):
    ...
    source_id_to_space_source: dict[SourceID, SpaceSource]
    ...
    def sources(self) -> list[Source]:
        return [
            space_source.source
            for space_source in self.source_id_to_space_source.values()
        ]
    ...
    def output_edge_tables(self) -> list[Table]:
        ...
        for path in paths_between_outputs:
            ...
            source_id_map = {
                source.source.id: source.source.name for source in self.space_sources
            }
            relationship_path = {
                source_id_map[p.fkey_relationship.source_with_fkey] for p in path
            }
            ...
            table = table.update_typed_metadata(
                ...
                SpaceRelationshipsMetadata(relationship_path=list(relationship_path)),
            )
    ...
    def space_sources(self) -> list[SpaceSource]:
        return list(self.source_id_to_space_source.values())
    ...
    def with_source(
        ...
        *,
        ...
        Args:
            source_id: The id of the source to be considered in the space
        ...
        source_id_to_space_source = self.source_id_to_space_source.copy()
        source_id_to_space_source.update(
            {
        ...
        return dataclasses.replace(
            ...
            source_id_to_space_source=source_id_to_space_source,
    ...
    def _check_or_infer_foreign_keys(  # noqa: C901
        ...
        frm = self.source_id_to_space_source[from_source_id]
        to = self.source_id_to_space_source[to_source_id]
    ...
    def with_relationship(
        ...
        *,
        ...
        """
        from_space_source = self.source_id_to_space_source.get(from_source_id)
        if not from_space_source:
            raise BadArgumentError(
                "from_source_id does not match any source in this space",
                from_source_id=str(from_source_id),
            )
        to_space_source = self.source_id_to_space_source.get(to_source_id)
        if not to_space_source:
            raise BadArgumentError(
        ...
        if from_foreign_key:
            pk = self.source_id_to_space_source[
                to_source_id
        ...
        relationships.append(
            Relationship(
                from_space_source=from_space_source,
                to_space_source=to_space_source,
        ...
        if not directional:
            relationships.append(
                Relationship(
                    from_space_source=to_space_source,
                    to_space_source=from_space_source,
                    fkey_relationship=fkey_relationship,
                )
            )
