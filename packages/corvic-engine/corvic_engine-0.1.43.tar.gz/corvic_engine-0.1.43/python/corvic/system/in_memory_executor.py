...
class _InMemoryExecutionContext:
    ...
    # once
    output_table_to_batches: dict[op_graph.Op, _SchemaAndBatches] = dataclasses.field(
    ...
    def make(
        ...
        tables = [
            InMemoryTableComputeResult(
                storage_manager,
                in_memory_context.output_table_to_batches[table_context.table_op_graph],
                table_context,
            )
...
class InMemoryExecutor:
    ...
    def _execute(
        ...
        with structlog.contextvars.bound_contextvars(
            ...
            if op in context.output_table_to_batches:
                _logger.info("using previously computed table for op")
                return context.output_table_to_batches[op]
            ...
            if op in context.output_tables:
                context.output_table_to_batches[op] = batches
            return batches
    ...
    def execute(self, context: ExecutionContext) -> InMemoryExecutionResult:
        ...
        for op in in_memory_context.output_tables:
            if op not in in_memory_context.output_table_to_batches:
