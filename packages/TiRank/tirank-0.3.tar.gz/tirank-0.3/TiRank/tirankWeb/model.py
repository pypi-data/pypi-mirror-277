...
def initial_model_para_(
    ...
    save_path_data2train = os.path.join(save_path, "data2train")
    ...
    # Load patholabels
    f = open(os.path.join(save_path_data2train, 'patholabels.pkl'), 'rb')
    ...
    with open(os.path.join(save_path, 'model_para.pkl'), 'wb') as f:
...
class TiRank(nn.Module):
    def __init__(self, n_features, nhead, nhid1, nhid2, nlayers, n_output, n_pred=1, n_patho=0, dropout=0.5, mode="Cox",
                 encoder_type="MLP"):
