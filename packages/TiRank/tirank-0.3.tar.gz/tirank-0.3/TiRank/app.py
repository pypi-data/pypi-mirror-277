...
app.title = 'TiRank'
app._favicon = './assets/TiRank_white.png'  # Ensure the path to the favicon is correct
...
def update_preprocess_res(bt1, bt2, bt3, is_log, is_norm,
    ...
    if triggered_id == 'preprocess-perform':
        ...
        return dbc.Col(
            html.Img(
                src='./assets/bulk_gene_pair_heatmap.png',
                style={
                    'width': '700px',
                    'height': '600px',
                    'margin': 'auto',
                    'display': 'flex',
                    'align-items': 'center',
                    'justify-content': 'center'  # Ensure the image is centered inside the column
                }
            ), style={
                'display': 'flex',
                'align-items': 'center',
        ...
        return dbc.Col(
            html.Img(
                src='./assets/sc_gene_pair_heatmap.png',
                style={
                    'width': '700px',
                    'height': '600px',
                    'margin': 'auto',
...
def preprocess(is_log, is_norm):
    ...
    with open('./data/anndata.pkl', 'rb') as f:
...
def train(device, advanced, nhead, n_output, nhid2, nhid1, nlayers, dropout, n_trails, n_clicks):
    ...
    with open('./data/data2train/patholabels.pkl', 'rb') as class_f:
        class_info = pickle.load(class_f)
    value_list = list(class_info.values())
    ...
    if n_clicks == 1:
        if advanced is None:
            ...
            nhead_ = nhead if nhead is not None else 2
            nhid1_ = nhid1 if nhid1 is not None else 96
            nhid2_ = nhid2 if nhid2 is not None else 8
            n_output_ = n_output if n_output is not None else 32
            nlayers_ = nlayers if nlayers is not None else 3
            n_trails_ = n_trails if n_trails is not None else 1
            dropout_ = dropout if dropout is not None else 0.5
...
def predict(reject, advanced, tolerance, reject_mode, n_clicks):
    ...
    if n_clicks == 1:
        if 1 in reject:
            predict_('./data/', mode=mode_global, do_reject=True, tolerance=tolerance or 0.05, reject_mode=reject_mode or 'GMM')
        else:
...
def deg_plot(logfc, pvalue, n_clicks):
    fc_threshold = eval(logfc) if logfc is not None else 2
    p_value_threshold = eval(pvalue) if pvalue is not None else 0.05
    if n_clicks == 1:
