...
Это мир, ради которого стоит работать, один маленький акт доброты за раз.""", language="ru")
	...
	def tokenizer(self, data):
		...
		for word in data.split():
			words.append(word)
		return words
	def train(self, data, language="system", controlled=False, temperature=0.2):
		qq = time.time()
		datasets = 0
		words = 0
		ww = 0
		...
		for word in self.tokenizer(data):
			if word in self.brain["languages"][language]["vocab"]:
				...
				self.brain["languages"][language]["vocab"] = [word.lower()]+self.brain["languages"][language]["vocab"]
				words += 1
				ww += 1
			ee += 1
		if ww == 0:
	...
	def information(self):
		...
		langs = ", ".join(self.brain["languagesText"])
		wordcount = 0
		datacount = 0
