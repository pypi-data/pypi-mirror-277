        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if not hasattr(p, 'manifold') or p.manifold is None:
                    ...
                    grad_grad = inner(grad, grad)
                    if abs(grad_grad) > 0:
                        orth_grad = inner(old_grad, grad) / grad_grad
                    else:
                        orth_grad = None
                    desc_dir = -1 * old_grad
                    ...
                    if ((orth_grad is not None and
                       ...
                       not abs(ograd_ograd) > 0):
                        ...
                        if beta_type == self.BETA_TYPES[0]:  # FletcherReeves
                            ...
                            diff = grad - old_grad
                            deno = inner(diff, desc_dir)
                            numo = inner(diff, grad)
                            if abs(deno) > 0:
                                numo -= (2 * inner(diff, diff) *
                                         inner(grad, desc_dir) / deno)
                                beta = numo / deno
                            else:
                        ...
                        df0 = inner(grad, desc_dir)
                        if df0 >= 0:
                            desc_dir = -1 * grad
                    p.data.add_(group['lr'],desc_dir)
                    state['old_grad'].mul_(0).add_(grad)
                    state['ograd_ograd'] = inner(grad, grad).item()
                    ...
                    if ((orth_grad is not None and abs(orth_grad) >= orth_value) or not abs(ograd_ograd) > 0):
                        beta = 0
                        desc_dir = -1 * grad
