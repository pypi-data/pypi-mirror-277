...
class GnomesAtNightEnv5A(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.token_pos = np.array((1, 1), dtype=int)
        self.treasure = {'onWhichSide': 0, 'pos': np.array((4, 0))}

        # self.token_pos = np.array((1, 1), dtype=int)
        # self.treasure = {'onWhichSide': 0, 'pos': np.array((2, 2))}
    ...
    def _get_observations(self):
        ...
        for player in range(self.n_agents):
            _obs.append(self._get_player_observation(player))
        return tuple(_obs)
    ...
    def _get_player_observation(self, player):
        ...
        #     "token": np.array(self.token_pos, dtype=np.int32),
        #     "walls": flattened_walls,
        #     "treasure": np.array(treasure_pos, dtype=np.int32)
        # }
    ...
    def _get_infos(self):
        return {
            "distance": np.linalg.norm(
                self.token_pos - self.treasure['pos'], ord=1
            )
        }
