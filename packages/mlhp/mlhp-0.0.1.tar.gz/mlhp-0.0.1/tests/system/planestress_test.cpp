...
auto computePlaneStress( const AbsBasis<2>& basis, double E, double nu, std::string name )
{
    ...
    auto quadratureRight = boundary::QuadratureOnMeshFaces<D>( mesh, { boundary::right }, degree + 1 );
    ...
    // Solve
    auto reducedDofs = linalg::makeCGSolver( 1e-12 )( K, F );
    ...
    auto postmesh = createGridOnCells( array::make<D>( degree ) );//, PostprocessTopologies::Faces );
...
TEST_CASE( "plane_stress_test" )
{
    ...
    // Square energy
    auto expectedEnergy = ( ( 117403.0 * nu - 1378393.0 ) * E ) / 
    ...
    SECTION( "unstructured mesh" )
    {
        ...
        auto energy = computePlaneStress( basis, E, nu, "unstructured" );

        //std::cout << energy << " vs. " << expectedEnergy << std::endl;
        //std::cout << std::scientific << std::setprecision(12) << energy<< std::endl;

        CHECK( energy == Approx( 1.244586536798 ).epsilon( 1e-8 ) );
    }
}
