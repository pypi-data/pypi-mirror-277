                                   ...
                                   size_t ifield )
{
    ...
    #pragma omp parallel
    {
        ...
        for( std::int64_t ii = 0; ii < static_cast<std::int64_t>( faces.size( ) ); ++ii )
        {
            ...
            for( size_t i = 0; i < rs.size( ); ++i )
            {
                ...
                auto J = spatial::concatenateJacobians<D, D, D - 1>( Jg, Jf );

                rst.push_back( coords );
                weights[i] *= spatial::computeDeterminant<D, D - 1>( J );
            }

            initializeLocalAssemblyTargets( localSystem, globalSystem, localMap.size( ) );
    ...
    auto solution = std::vector<double>( matrix.size1( ) );
    auto preconditioner = linalg::makeDiagonalPreconditioner( matrix );

    linalg::cg( linalg::makeDefaultMultiply( matrix ), vector, 
        solution, preconditioner, 2 * matrix.size1( ), 1e-14 );

    return { std::move( indices ), std::move( solution ) };
}
...
DofIndicesValuesPair boundaryDofs( const spatial::ScalarFunction<D>& function,
                                   ...
                                   size_t ifield )
{
    return boundaryDofs( function, basis, faces, makeIntegrationOrderDeterminor<D>( ), ifield );
}

template<size_t D> MLHP_EXPORT
DofIndicesValuesPair boundaryDofs( const spatial::VectorFunction<D, D>& function,
                                   const AbsBasis<D>& basis,
                                   const std::vector<size_t>& faces,
                                   const QuadratureOrderDeterminor<D>& determinor )
{
    ...
    {
        auto sliced = [=]( auto xyz ){ return function( xyz )[ifield]; };
...
template<size_t D>
DofIndicesValuesPair boundaryDofs( const spatial::ScalarFunctions<D>& functions,
                                   const AbsBasis<D>& basis,
                                   const std::vector<size_t>& faces,
                                   const QuadratureOrderDeterminor<D>& determinor )
{
    ...
    for( size_t ifield = 0; ifield < functions.size( ); ++ifield )
    {
        results.push_back( boundaryDofs<D>( functions[ifield], 
            basis, faces, determinor, ifield));
    }
...
std::vector<double> inflate( const std::vector<double>& interiorDofs,
...
template<size_t D>
std::any QuadratureOnMeshFaces<D>::initialize( ) const
{
    return Cache { };
}
                                           ...
                                           std::any& anyCache ) const
{
    if constexpr( D > 1 )
    {
        ...
        auto& cache = std::any_cast<Cache&>( anyCache );
        ...
        for( size_t index = indices_[mapping.icell]; index < indices_[mapping.icell + 1]; ++index )
        {
            ...
            if( mapping.type == CellType::NCube )
            {
                tensorProductQuadrature( array::make<D - 1>( order_ ), 
            ...
            else
            {
                ...
                simplexQuadrature( array::make<D - 1>( order_ ), cache.rs, weights, cache.quadrature );
            }
            ...
            for( size_t ipoint = 0; ipoint < npoints; ++ipoint )
            {
                ...
                auto J = spatial::concatenateJacobians<D, D, D - 1>( J1, J0 );
                auto detJ = spatial::computeDeterminant<D, D - 1>( J );
                
                rst[rsize + ipoint] = coords;
                weights[wsize + ipoint] *= detJ;
                normals[nsize + ipoint] = linalg::mvproduct<D, D>( J1, face.normal( ) );
...
#define MLHP_INSTANTIATE_DIM( D )                                                        \
    ...
    DofIndicesValuesPair boundaryDofs( const spatial::ScalarFunction<D>& function,       \
                                       ...
                                       size_t ifield );                                  \
                                                                                         \
    template MLHP_EXPORT                                                                 \
    DofIndicesValuesPair boundaryDofs( const spatial::VectorFunction<D, D>& functions,   \
                                       const AbsBasis<D>& basis,                         \
                                       const std::vector<size_t>& faces,                 \
                                       const QuadratureOrderDeterminor<D>& determinor ); \
                                                                                         \
