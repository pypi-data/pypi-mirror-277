                                  ...
                                  auto&& success, auto&& outside )
{
    ...
    return [=]( std::array<double, D> xyz )
    {
        ...
        if( auto result = cache.backwardMapping->map( xyz ); result )
        {
            ...
            return success( cache.shapes, cache.locationMap );
        }
...
{
    MLHP_CHECK( basis->nfields( ) == nfields, "Value of nfields template parameter does not match basis->nfields( )." );

    auto success = [=]( auto& shapes, auto& locationMap )
    {
        return evaluateSolution<nfields, D>( shapes, locationMap, dofs );
    };

    return makeBackwardMappedEvaluator( basis, std::move( success ), utilities::returnValue( array::make<nfields>( 0.0 ) ) );
}
