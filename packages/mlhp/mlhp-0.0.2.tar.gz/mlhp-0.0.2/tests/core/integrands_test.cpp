...
{ 
    ...
    SECTION( "Poisson" )
    {
        ...
        CHECK( integrand.diffOrder( ) == DiffOrders::FirstDerivatives );

        auto cache = integrand.createCache( );
    
        integrand.prepare( cache, 0, { } );
    ...
    SECTION( "L2" )
    {
        auto integrand = makeL2ProjectionIntegrand<3>( function1, function2 );

        CHECK( integrand.diffOrder( ) == DiffOrders::Shapes );
