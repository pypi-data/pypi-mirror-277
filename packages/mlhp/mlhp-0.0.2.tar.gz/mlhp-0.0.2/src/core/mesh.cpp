                                     ...
                                     MeshMapping<D>& mapping ) const
{
    ...
    auto& cache = mapping.cache.template cast<std::shared_ptr<RefinedGridMappingCache<D>>>( );
                                          ...
                                          MeshMapping<D>& mapping ) const
{
    ...
    auto& cache = mapping.cache.template cast<std::variant<NCubeMapping<D>, SimplexMapping<D>>>( );
...
struct BackwardMapping : public AbsBackwardMapping<D>
{
    ...
    void mapInternal( std::array<double, D> xyz, BackwardMapVector<D>& target, double epsilon ) override
    {
        ...
        for( auto ii : cells )
        {
            ...
            {
                auto [xi, J] = map::withJ( forwardMapping, rst );
