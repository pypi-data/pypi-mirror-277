...
template<size_t I, size_t O, typename... Args> inline
auto constantFunction( std::array<double, O> value )
{
    return std::function( [=]( std::array<double, I>, Args... ) noexcept
    {
        return value;
    } );
}
                            ...
                            std::array<double, D> origin  )
{
    ...
    for( size_t axis = 0; axis < D; ++axis )
    {
        if( npoints[axis] > 1 )
        {
            dx[axis] = lengths[axis] / ( npoints[axis] - 1 );
        }
        else
        {
...
auto triangleNormal( std::array<double, D> vertex0,
                     ...
                     std::array<double, D> vertex2 )
{
    return spatial::normalize( spatial::cross( vertex1 - vertex0, vertex2 - vertex0 ) );
}
