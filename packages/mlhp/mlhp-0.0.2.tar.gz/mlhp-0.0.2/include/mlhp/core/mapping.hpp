...
struct MeshMapping final : public AbsMapping<G, L>
{
    ...
    utilities::UniqueTypeErasedCache<MeshMapping> cache;
...
private:
    void mapInternal( map::Result<D>& result ) const override
    {
        ...
        {
            result.xyz = result.rst * halflengths_ + center_;
        }
        ...
        {
            std::fill( result.J.begin( ), result.J.end( ), 0.0 );

            for( size_t axis = 0; axis < D; ++axis )
            {
                result.J[axis * D + axis] = halflengths_[axis];
            }
        }
    }
...
class NCubeCollapsedToSimplex final : public AbsMapping<D, D>
{
    void mapInternal( map::Result<D>& result ) const override
    {
        ...
        if( result.evaluate & map::J || result.evaluate & map::DetJ )
        {
            ...
            linalg::lu( J.data( ), P.data( ), D );

            result.detJ = linalg::luDeterminant( J.data( ), D );
        }
    }
};
...
struct ConcatenatedMapping final : public AbsMapping<G, L>
{
    ...
    explicit ConcatenatedMapping( ) = default;
...
private:
    void mapInternal( map::Result<G, L>& result ) const override
    {
        ...
        if( result.evaluate & map::J )
        {
            result.J = spatial::concatenateJacobians<G, I, L>( result1.J, result0.J );
        }
        if( result.evaluate & map::DetJ )
        {
            ...
            {
                result.detJ = result1.detJ * result0.detJ;
            }
        }
    }
};
...
public:
    FaceMapping( CellType cellType, size_t iface )
    {
        ...
        if( cellType == CellType::NCube )
        {
            ...
            normal_ = array::setEntry<double, L>( { }, normal, 2.0 * side - 1.0 );

            for( size_t axis = 0; axis < L; ++axis )
            {
                vertices[axis + (axis < normal)][axis] = 1.0;
                vertices[axis][normal] = normal_[normal];
            }
        }
