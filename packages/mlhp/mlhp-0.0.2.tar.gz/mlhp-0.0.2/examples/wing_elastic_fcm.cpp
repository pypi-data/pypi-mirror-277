...
int main( )
{
    ...
    //SpaceTreeIntegrationPartitioner<D> partitioner( domain, penaltyFCM, integrationSpaceTreeDepth );
    ...
    auto integrand = makeIntegrand<D>( kinematics, material, force );
    ...
    linalg::cg( multiplyM, rhs, interiorSolution, multiplyP, matrix.size1( ), 1e-6 );
    ...
    auto postmesh = createMarchingCubesVolume( domain, resolution );
