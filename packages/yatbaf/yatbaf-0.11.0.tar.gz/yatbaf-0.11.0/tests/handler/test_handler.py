...
def test_new_hander(handler_fn):
    handler = Handler(update_type=UpdateType.MESSAGE, fn=handler_fn)
...
@pytest.mark.parametrize(
    ...
    [
        (
            Handler(fn=fn, update_type=UpdateType.MESSAGE),
            Handler(fn=fn, update_type=UpdateType.MESSAGE),
        ),
        (
            Handler(
...
@pytest.mark.parametrize("h1", [Handler(fn=fn, update_type=UpdateType.MESSAGE)])
@pytest.mark.parametrize(
    "h2",
    [
        Handler(fn=fn, update_type=UpdateType.EDITED_MESSAGE),
        Handler(fn=fn, update_type=UpdateType.MESSAGE, middleware=[object()]),
        Handler(fn=fn2, update_type=UpdateType.MESSAGE),
    ]
)
