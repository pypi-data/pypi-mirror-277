...
def test_add_handler_fn(handler_fn):
    ...
    assert router._handlers == [Handler(UpdateType.MESSAGE, fn=handler_fn)]


def test_add_handler_obj(handler_fn):
    router = OnMessage()
    router.add_handler(Handler(UpdateType.MESSAGE, fn=handler_fn))
    assert router._handlers == [Handler(UpdateType.MESSAGE, fn=handler_fn)]


def test_add_handler_type_error(handler_fn):
    router = OnMessage()
    handler = Handler(UpdateType.POLL, fn=handler_fn)
    with pytest.raises(ValueError):
        router.add_handler(handler)


def test_add_handler_registered(handler_fn):
    handler = Handler(UpdateType.MESSAGE, fn=handler_fn)
...
def test_init_duplicate(handler_fn):
    with pytest.warns(BotWarning):
        router = OnMessage(
            handlers=[
                handler1 := Handler(UpdateType.MESSAGE, fn=handler_fn),
                Handler(
                    fn=handler_fn,
...
def test_decorator_fallback():
    ...
    assert router._handlers == [Handler(UpdateType.MESSAGE, fn=handler)]
