    ...
    **kwargs
    ...
    if(is_pandas):
        if(method == 'clip'):
            ...
            if(gr is None):
                df[vr] = df[vr].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
            else:
                _tmp = df.groupby(by = gr)[vr].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
                _tmp = _tmp.reset_index(drop = False)
                _tmp = _tmp.drop(columns = gr)
                _tmp = _tmp.set_index('level_1')
                df = df.drop(columns = vr)
                df = df.merge(_tmp, left_index = True, right_index = True)
            return(df)
            
        elif(method == 'zscore'):
            
            if(gr is None):
                df[vr] = df[vr].apply(lambda x: (x - x.mean()) / (x.std()))
            else:
                _tmp = df.groupby(by = gr)[vr].apply(lambda x: (x - x.mean()) / (x.std()))
                _tmp = _tmp.reset_index(drop = False)
                _tmp = _tmp.drop(columns = gr)
                _tmp = _tmp.set_index('level_1')
                df = df.drop(columns = vr)
            ...
            _ge_zero_cols = list(df[vr].loc[:, df[vr].gt(0).all()].columns)
            vr = list_inter(vr, _ge_zero_cols)
        
            # compute log base n of columns    
            df[vr] = numpy.emath.logn(n, df[vr])
            ...
            if(gr is None):
                df[vr] = df[vr].transform(scipy.special.softmax)
            else:
                df[vr] = df.groupby(by = gr)[vr].transform(scipy.special.softmax)
