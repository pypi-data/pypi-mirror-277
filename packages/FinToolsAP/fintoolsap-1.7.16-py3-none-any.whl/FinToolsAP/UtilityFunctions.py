...
def df_normalize(df: typing.Union[pandas.DataFrame, polars.DataFrame],
    ...
    vr : typing.Optional[typing.Union[str, list[str]]] = None
        ...
        of column names. If None, all numeric columns will be normalized.
    ...
    **kwargs
    ...
    def _valid_columns(df, vr):
        
        _cols = df.columns if(vr == None) else vr
            
        # only appy to numeric columns
        _is_number = numpy.vectorize(lambda x: numpy.issubdtype(x, numpy.number))
        _numeric_cols = df.columns[_is_number(df.dtypes)]
        
        return(list_inter(_cols, _numeric_cols))
    ...
    if(is_pandas):
        if(method == 'clip'):
            ...
            for _col in _cols:
                ...
                if(gr is None):
                    ...
                    if('bounds' in kwargs):
                        ...
                        _lower_bound_val = numpy.quantile(df[_col], q = kwargs['quantiles'][0])
                        _upper_bound_val = numpy.quantile(df[_col], q = kwargs['quantiles'][1])
                        
                    # condtions
                    _upper_condition = df[_col] > _upper_bound_val
                    _lower_condition = df[_col] < _lower_bound_val
                    
                    # clip
                    df[_col] = numpy.where(_upper_condition, _upper_bound_val, df[_col])
                    df[_col] = numpy.where(_lower_condition, _lower_bound_val, df[_col])
                        
                else:
                    
                    try:
                        _qtiles = [kwargs['quantiles'][0], kwargs['quantiles'][1]]
                    except:
                        raise ValueError('using a groupby requires the use of quantiles')
                    
                    _dfs_to_concat = []
                    for name, group in df.groupby(by = gr):
                        
                        _lower_bound_val = numpy.quantile(group[_col], q = _qtiles[0])
                        _upper_bound_val = numpy.quantile(group[_col], q = _qtiles[1])
                        ...
                        # clip
                        group[_col] = numpy.where(_upper_condition, _upper_bound_val, group[_col])
                        group[_col] = numpy.where(_lower_condition, _lower_bound_val, group[_col])
                        
                        _dfs_to_concat.append(group)
                    
                    df = pandas.concat(_dfs_to_concat)
            ...
            _cols = _valid_columns(df, vr)
            if(gr is None):
                df[_cols] = df[_cols].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
            else:
                _tmp = df.groupby(by = gr)[_cols].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
            ...
            if(gr is None):
                df[_cols] = df[_cols].apply(lambda x: (x - x.mean()) / (x.std()))
            else:
                _tmp = df.groupby(by = gr)[_cols].apply(lambda x: (x - x.mean()) / (x.std()))
                _tmp = _tmp.reset_index(drop = True)
                df[_cols] = _tmp
            return(df)
            ...
            _ge_zero_cols = list(df[_cols].loc[:, df[_cols].gt(0).all()].columns)
            _cols = list_inter(_cols, _ge_zero_cols)
        
            # compute log base n of columns    
            df[_cols] = numpy.emath.logn(n, df[_cols])
            ...
            if(gr is None):
                df[_cols] = df[_cols].transform(scipy.special.softmax)
            else:
                df[_cols] = df.groupby(by = gr)[_cols].transform(scipy.special.softmax)
