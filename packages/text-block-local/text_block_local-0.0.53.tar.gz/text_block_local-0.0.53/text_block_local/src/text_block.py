...
class TextBlocks(GenericCRUD, metaclass=MetaLogger, object=object1):
    ...
    def get_fields(self) -> dict:
        ...
        fields = dict(self.select_multi_tuple_by_where(
            schema_name="field", view_table_name="field_view",
    ...
    def get_text_block_details(self, text_block_id: int) -> tuple:
        ...
        result = self.select_one_tuple_by_column_and_value(
            ...
            column_name="text_block_id", column_value=text_block_id)
        if not result:
    ...
    # TODO Can we use table_definition_table generated using Sql2Code here because of performance?
    # TODO Can we generate SQL on logger_table to see how often we call this function? - We should call it one time in the lifetime of each process 
    def process_and_update_field(self, fields_dict: dict, field_id: int, index: int) -> int or None:
        ...
        field_info = self.select_one_tuple_by_column_and_value(
            schema_name="field", view_table_name='field_view', select_clause_value=select_clause_value,
    ...
    def insert_information_from_extracted_fields(
        ...
        # TODO: there are 3 such sections. So A, can we reuse the code? B: orininal version inserted twice (probably mistake?):
    ...
    # TODO Move this methods to field-local, unless we already have such method there which we can use
    @lru_cache
    def __get_field_name_by_field_id(self, field_id: int) -> str:
        """Gets the field name by field ID."""
        field_name = self.select_one_value_by_column_and_value(
            schema_name="field", view_table_name="field_view",
            select_clause_value="name", column_name="field_id", column_value=field_id)
        return field_name
