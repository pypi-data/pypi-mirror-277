...
from bigeye_sdk.generated.com.bigeye.models.generated import (
    ...
    DataNodeType,
    DataNode,
    LineageRelationship, LineageNavigationNodeV2Response,
)
...
class LineageController:
    ...
    def process_all_edge_requests(self, purge_lineage: bool = False):
        ...
        for r in track(self.edge_requests, description=loading_text):
            try:
                ...
                elif existing_relations is None or r.downstream.data_node_id not in existing_relations:
                    "If we don't know the downstream dependencies of the upstream node, OR if this specific dependency"
                    "has not been added, then submit a create edges request - else skip it."
                    self.create_edges(
                        upstream=r.upstream,
    ...
    def _get_existing_relations_for_schema(self, schemas: List[Schema]) -> None:
        ...
        for s in schemas:
            if self.existing_relations.get(s.data_node_id, None) is None:
                ...
                for node_dict in downstream_nodes.nodes:
                    node = LineageNavigationNodeV2Response().from_dict(node_dict["value"])
                    self.existing_relations[node.lineage_node.id] = [d.downstream_id for d in node.downstream_edges]
    ...
    def infer_column_level_lineage_from_file(
            ...
            if r.table_overrides is not None:
                for t_override in r.table_overrides:
                    try:
                        ...
                        matching_tables.append((u_table, d_table))
                    except KeyError as e:
        ...
        self.infer_column_level_lineage_from_tables(tables=matching_tables, purge_lineage=purge_lineage)
    ...
    def infer_column_level_lineage_from_tables(self, tables: List[Tuple[Table, Table]], purge_lineage: bool = False):
        for upstream, downstream in tables:
            matching_columns: List[Tuple[TableColumn, TableColumn]] = self.infer_relationships_from_lists(
            ...
            for up_column, down_column in matching_columns:
                if up_column.data_node_id != down_column.data_node_id:
                    self.edge_requests.append(
                        SimpleLineageEdgeRequest(
                            upstream=up_column,
