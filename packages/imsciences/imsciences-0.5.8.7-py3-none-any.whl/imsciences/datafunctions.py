...
class datapull:
    ...
    def help(self):
        ...
        print("   - Usage: pull_boe_data(week_commencing='mon', max_retries=30, delay=5)")
    ...
    def pull_macro(self, country : str = "GBR", week_commencing : str = "mon"):
        ...
        macroeconomic_weekly_df.rename(columns={'week_start': 'OBS'}, inplace=True)
    ...
    def pull_weather(self,week_commencing,country) -> pd.DataFrame:
        ...
        # Function to flattena list nested list into a list
        ...
        if country == "GB__ASOS":
            ...
            institute_vector = ["CA_NB_ASOS", "CA_NF_ASOS", "CA_NT_ASOS", "CA_NS_ASOS",
                                "CA_NU_ASOS"]
            stations_list = [[] for i in range(5)]
            # stations_list = []
        ...
        if country == "GB__ASOS" or country == "AU__ASOS" or country == "DE__ASOS"  or country == "FR__ASOS":
            ...
            for i in range(0,len(institute_vector)):
        ...
        # Rainfall
        if country == "GB__ASOS":
            # Downloads all the data for rainfall in individual regions
            # Get South East England
            Rainfall_SEE = urllib.request.urlopen("https://www.metoffice.gov.uk/hadobs/hadukp/data/daily/HadSEEP_daily_totals.txt")
            Rainfall_SEE = pd.read_csv(Rainfall_SEE,sep=" ")
            Rainfall_SEE = Rainfall_SEE.iloc[2:,[0,3]]
            Rainfall_SEE.rename(columns={"Daily":"day","England":"Rainfall_SEE"},inplace=True)
            # Get South West England & Wales
            Rainfall_SWEW = urllib.request.urlopen("https://www.metoffice.gov.uk/hadobs/hadukp/data/daily/HadSWEP_daily_totals.txt")
            Rainfall_SWEW = pd.read_csv(Rainfall_SWEW,sep=" ")
            Rainfall_SWEW = Rainfall_SWEW.iloc[2:,[0,3]] 
            Rainfall_SWEW.rename(columns={"Daily":"day","England":"Rainfall_SWEW"},inplace=True)   
            # Get Central England
            Rainfall_CE = urllib.request.urlopen("https://www.metoffice.gov.uk/hadobs/hadukp/data/daily/HadCEP_daily_totals.txt")
            Rainfall_CE = pd.read_csv(Rainfall_CE,sep=" ")
            Rainfall_CE = Rainfall_CE.iloc[2:,[0,3]]
            Rainfall_CE.rename(columns={"Daily":"day","precipitation":"Rainfall_CE"},inplace=True)  
            # Get North West England & Wales
            Rainfall_NWEW = urllib.request.urlopen("https://www.metoffice.gov.uk/hadobs/hadukp/data/daily/HadNWEP_daily_totals.txt")
            Rainfall_NWEW = pd.read_csv(Rainfall_NWEW,sep=" ")
            Rainfall_NWEW = Rainfall_NWEW.iloc[2:,[0,3]]
            Rainfall_NWEW.rename(columns={"Daily":"day","England":"Rainfall_NWEW"},inplace=True)   
            # # Get North East England
            Rainfall_NEE = urllib.request.urlopen("https://www.metoffice.gov.uk/hadobs/hadukp/data/daily/HadNEEP_daily_totals.txt")
            Rainfall_NEE = pd.read_csv(Rainfall_NEE,sep=" ")
            Rainfall_NEE = Rainfall_NEE.iloc[2:,[0,3]]
            Rainfall_NEE.rename(columns={"Daily":"day","England":"Rainfall_NEE"},inplace=True) 
            # # Get South Scotland
            Rainfall_SS = urllib.request.urlopen("https://www.metoffice.gov.uk/hadobs/hadukp/data/daily/HadSSP_daily_totals.txt")
            Rainfall_SS = pd.read_csv(Rainfall_SS,sep=" ")
            Rainfall_SS = Rainfall_SS.iloc[2:,[0,3]]
            Rainfall_SS.rename(columns={"Daily":"day","precipitation":"Rainfall_SS"},inplace=True) 
            # # Get Northern Ireland	
            Rainfall_NI = urllib.request.urlopen("https://www.metoffice.gov.uk/hadobs/hadukp/data/daily/HadNIP_daily_totals.txt")
            Rainfall_NI = pd.read_csv(Rainfall_NI,sep=" ")
            Rainfall_NI = Rainfall_NI.iloc[2:,[0,3]]
            Rainfall_NI.rename(columns={"Daily":"day","precipitation":"Rainfall_NI"},inplace=True) 

            merged_data = pd.concat([Rainfall_SEE.set_index("day"),
                                    Rainfall_SWEW.set_index("day"),
                                    Rainfall_CE.set_index("day"),
                                    Rainfall_NWEW.set_index("day"),
                                    Rainfall_NEE.set_index("day"),
                                    Rainfall_SS.set_index("day"),
                                    Rainfall_NI.set_index("day")],
                                    axis=1,join="inner")

            # Only select data for beyond start date
            merged_data.reset_index(inplace=True)
            merged_data["day"]  = pd.to_datetime(merged_data["day"],format="%Y-%m-%d")
            filtered_data = merged_data[merged_data["day"] >= formatted_date]

            # Replace NAs
            filtered_data = filtered_data.fillna(0)

            # Create mappings 
            data = {"city": ["Manchester", "Leeds", "Birmingham", "Norwich", "Cardiff", "Southampton", "London", "Newquay", "Belfast", "Glasgow", "Bristol", "Inverness", "Newcastle"],
                "station":["EGCC","EGNM","EGBB","EGSH","EGFF","EGHI","EGLC","EGHQ","EGAC","EGPF","EGGD","EGPE","EGNT"],
                "rainfall_regions":["Rainfall_NWEW","Rainfall_NEE","Rainfall_CE","Rainfall_CE","Rainfall_SWEW","Rainfall_SEE","Rainfall_SEE","Rainfall_SWEW","Rainfall_NI","Rainfall_SS","Rainfall_SWEP","Rainfall_NSP","Rainfall_NEEP"]}
            mappings_data = pd.DataFrame(data)

            # Merge data by station
            final_merged_data = weather.merge(mappings_data,on="station")
            final_merged_data["day"] = pd.to_datetime(final_merged_data["day"],format="%Y-%m-%d")

            # Convert the data from wide to long format
            long_data = pd.melt(filtered_data,var_name="Location",value_name="Rainfall",value_vars=["Rainfall_SEE","Rainfall_SWEW","Rainfall_CE","Rainfall_NWEW","Rainfall_NEE","Rainfall_SS","Rainfall_NI"],id_vars=["day"])
            long_data.rename(columns={"Location":"rainfall_regions"},inplace=True)

            # Merge by day by rainfall_regions
            merged_df = final_merged_data.merge(long_data,on = ["day","rainfall_regions"])

            # Create dummy variables for rain > 3? 
            merged_df.loc[merged_df["Rainfall"].apply(pd.to_numeric)>3,"rainfall_dummy"] = 1
            merged_df.loc[merged_df["Rainfall"].apply(pd.to_numeric)<=3,"rainfall_dummy"] = 0
            # merged_df.loc[merged_df["rainfall_dummy"].apply(pd.to_numeric)==1,"not_rain"] = 0
            # merged_df.loc[merged_df["rainfall_dummy"].apply(pd.to_numeric)!=1,"not_rain"] = 1

            output_gb = merged_df.groupby("week_starting")

            # Group by week_starting and summarize
            output_gb = merged_df.groupby("week_starting").mean()
            output_gb.rename(columns={"Rainfall":"avg_rainfall",
                                    "rainfall_dummy":"rain_dummy",
            ...
            output_gb.index = pd.to_datetime(output_gb.index)
            ...
            for city in cities:
                ...
                # Add city name as a column in the data
                daily_data[str(city)] = precipitation_sum
            ...
            df = pd.concat(weather_data_list,axis=0)
            all_weather_data = df.loc[:,~df.columns.duplicated()].copy()

            # Change data from wide to long
            all_weather_data = pd.melt(all_weather_data,id_vars="date",value_vars=all_weather_data.columns)
            all_weather_data.rename(columns={"date":"day","variable":"station","value":"rainfall"},inplace=True)

            # Replace nas
            all_weather_data.fillna(0,inplace=True)
            all_weather_data

            # Convert the date column to a Date type 
            all_weather_data["day"] = pd.to_datetime(all_weather_data["day"])

            # Set week commencing col up
            all_weather_data['week_starting'] = all_weather_data["day"].apply(lambda x: x - pd.Timedelta(days=(x.weekday() - day_dict[week_commencing]) % 7))

            # Change format of date columns
            all_weather_data["day"] = all_weather_data["day"].dt.strftime("%Y-%m-%d")
            all_weather_data["week_starting"] = all_weather_data["week_starting"].dt.strftime("%Y-%m-%d")
        ...
        if country == "GB__ASOS":
            merged_df = output_gb
        elif country == "AU__ASOS" or country == "DE__ASOS" or country == "FR__ASOS":
