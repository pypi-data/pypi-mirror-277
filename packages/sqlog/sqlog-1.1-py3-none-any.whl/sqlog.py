...
class HeaderFormatParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag in {'d', 'delim'}:
            self._format_strings.append({
                'type': 'delimiter',
                'style': self._attrs_to_style(attrs_dict)
            })
            return
    ...
    def _style_from_tag(self, tag_name):
        if tag_name == 'b': return 'bold'
        if tag_name == 'i': return 'italic'
        if tag_name == 'u': return 'underline'
        if tag_name == 's': return 'strike'
        if tag_name in {'pre', 'code'}: return 'code'
    ...
    def handle_data(self, data):
        if not self._tag_stack:
            ...
            if tag_style: style['format'] = tag_style
    ...
    def handle_endtag(self, tag):
        if not self._tag_stack: return
    
        corresponding_tags = [t for t in self._tag_stack if tag == t['tag']]
        if not corresponding_tags: return
    
        start_tag = corresponding_tags[-1]
        index = self._tag_stack.index(start_tag)
...
class SqSection:
    def __init__(self, handler, connection, parent):
        ...
        with self._conn:
            ...
            cur.execute('INSERT INTO Sections(parent_id, level) VALUES(?, ?)', (parent_id, self._level))
    ...
    def add_header_str(self, s: str, html=True):
        if not html:
            with self._conn:
                cur = self._conn.cursor()
                cur.execute('INSERT INTO Headers(section_id, text, type) VALUES(?, ?, ?)', (self._section_id, s, 'text'))
        else:
            ...
            'frames': [
                {
                    'code': {
                        ...
                        'posonlyargcount': getattr(frame.f_code, 'co_posonlyargcount', None),
    ...
    def add_log(self, record):
        arguments = (
            ...
            datetime.fromtimestamp(record.created).strftime('%Y-%m-%d %H:%M:%S,%f'), # created
...
class SqFileHandler(logging.Handler):
    ...
    def __init__(self, filename: str, top_header: str = 'Start Logging at {datetime}'):
        super().__init__()
        start_logging_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S,%f')
...
def get_sections(connection, with_level=1):
    ...
    cur.execute('SELECT id, parent_id, level FROM Sections WHERE level == ? ORDER BY id ASC', [with_level])
    return cur.fetchall()

def get_child_sections(connection, with_parent_id):
    cur = connection.cursor()
    cur.execute('SELECT id, parent_id, level FROM Sections WHERE parent_id == ? ORDER BY id ASC', [with_parent_id])
    return cur.fetchall()

def get_headers(connection, by_section_id):
    cur = connection.cursor()
    cur.execute('SELECT id, section_id, text, type, style FROM Headers WHERE section_id == ? ORDER BY id ASC', [by_section_id])
    return [
        (*item[:4], item[4] and json.loads(item[4]))
...
def header_to_org_header(header):
    ...
    if style is None: return text
    if 'format' not in style: return text

    if style['format'] == 'bold': return f'*{text}*'
    if style['format'] == 'italic': return f'/{text}/'
    if style['format'] == 'underline': return f'_{text}_'
    if style['format'] == 'code': return f'~{text}~'
    if style['format'] == 'strike': return f'+{text}+'
...
def sqlog_to_org(connection, out_io, format='- %(levelname)s :: %(name)s :: %(message)s', start_level=1):
...
def header_to_html_header(header):
    ...
    if type == 'delimiter': return f'<span class="delimiter"{style_str}>|</span>'
    if style is None: return text
    if 'format' not in style:
        return f'<span{style_str}>{text}</span>' if style_str else text

    if style['format'] == 'bold': return f'<b{style_str}>{text}</b>'
    if style['format'] == 'italic': return f'<i{style_str}>{text}</i>'
    if style['format'] == 'underline': return f'<u{style_str}>{text}</u>'
    if style['format'] == 'code': return f'<code{style_str}><pre>{text}</pre></code>'
    if style['format'] == 'strike': return f'<s{style_str}>{text}</s>'
...
def get_convert_func(args):
    if args.format == 'org': return sqlog_to_org
    if args.format == 'html': return sqlog_to_html
    ...
    if output.suffix == '.org': return sqlog_to_org
    if output.suffix in {'.html', '.htm', '.xhtml'}: return sqlog_to_html
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(
        'sqlog', description='conver .sqlog file',
    ...
    arg_parser.add_argument(
        '-f', '--format',
        choices=['org', 'html'], metavar='string',
        help='output file format (org or html)'
    )
    arg_parser.add_argument('-o', '--output', metavar='path', help='output file')
    arg_parser.add_argument('-s', '--start_level', metavar='int', type=int, help='start level (for org-mode)')
    arg_parser.add_argument('-l', '--log_format', metavar='string', help='format logs (as in logging.Formatter)')
    args = arg_parser.parse_args()

    convert_func = get_convert_func(args)
    
    kwargs = {}
    if convert_func is sqlog_to_org and args.start_level: kwargs['start_level'] = args.start_level
    if args.log_format: kwargs['format'] = args.log_format
    ...
    try:
        if not args.output:
            ...
            if str(outdir) == '.': outdir = outdir.resolve()
            if not outdir.is_dir():
                print(f'Error: no such directory {outdir}')
                sys.exit(126)

            with output.open('w', encoding='utf-8') as io, sqlite3.connect(logfile) as conn:
