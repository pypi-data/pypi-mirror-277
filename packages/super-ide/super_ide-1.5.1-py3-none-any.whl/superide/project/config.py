...
from superide.compat import MISSING, string_types
from superide.project import exception
from superide.project.options import ProjectOptions

CONFIG_HEADER = """
; superide Project Configuration File
;
...
; https://docs.superide.org/page/projectconf.html
"""
...
class ProjectConfigBase:
    ...
    def get_default_path():
        from superide import app  # pylint: disable=import-outside-toplevel

        return app.get_session_var("custom_project_conf") or os.path.join(
            os.getcwd(), __configfile__
        )
    ...
    def read(self, path, parse_extra=True):
        ...
        # load extra configs
        for pattern in self.get("superide", "extra_configs", []):
            ...
            if option == "__env__":
                if not parent_section.startswith("env:"):
                    raise exception.ProjectOptionValueError(
                        ...
                        "section that is not valid superide environment, see",
                        option,
                        " ",
                        section,
                    )
    ...
    def default_envs(self):
        return self.get("superide", "default_envs", [])
        ...
        for env in known_envs:
            ...
            if self.get(f"env:{env}", "monitor_raw", False) and self.get(
                ...
                self.warnings.append(
                    ...
                    "`SuperIDE.ini` file. The `monitor_filters` option will "
...
class ProjectConfigDirsMixin:
    def get_optional_dir(self, name):
        """
        Deprecated, used by superide-node-helpers.project.observer.fetchLibDirs
        superide IDE for Atom depends on superide-node-helpers@~7.2.0
        PIO Home 3.0 Project Inspection depends on it
        """
        return self.get("superide", f"{name}_dir")
