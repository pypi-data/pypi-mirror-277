...
class NieuwbouwprojectspiderSpider(scrapy.Spider):
    name = "Nieuwbouwprojectspider"
    base_url = 'https://www.nieuwbouw-nederland.nl/projecten/?hk[]=k&sort=titel_asc&p={}'
    ...
    def __init__(self, start_page=1, page_limit=1, *args, **kwargs):
        super(NieuwbouwprojectspiderSpider, self).__init__(*args, **kwargs)
        self.start_page = start_page
        self.page_limit = page_limit
        self.current_page = start_page

    def parse(self, response):
        # Mimic human behavior by visiting this URL first
        yield scrapy.Request(url='https://www.nieuwbouw-nederland.nl/projecten/', callback=self.get_to_sell_project_page)

    def get_to_sell_project_page(self, response):
        # Start scraping from the desired start page
        start_url = self.base_url.format(self.start_page)
        yield scrapy.Request(url=start_url, callback=self.parse_projects)

    def parse_projects(self, response):
        projects = response.css('section#list article h2 a[href^="https://www.nieuwbouw"]::attr(href)').getall()
        ...
        for project in projects:
            ...
            if project.startswith('https://www.nieuwbouw'):
                yield scrapy.Request(url=project, callback=self.parse_project_page, cb_kwargs={
                    'projects': projects,
                    'project': project,
        ...
        if self.current_page < self.start_page + self.page_limit:
            next_page_url = self.base_url.format(self.current_page)
            yield scrapy.Request(url=next_page_url, callback=self.parse_projects)
...
#set up a check point for the situation if there is no sepicfic information for house type collections
    ...
    def get_city_from_url(self, url):
        ...
        match = re.search(r'nieuwbouw(?:-in)?-(.*?)\.nl', url)
        if match:
