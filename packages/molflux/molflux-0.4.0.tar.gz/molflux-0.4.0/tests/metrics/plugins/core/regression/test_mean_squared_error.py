...
from molflux.metrics.regression.mean_squared_error import MeanSquaredError


@pytest.fixture()
def fixture_metric():
    return load_metric("mean_squared_error")
...
def test_metric_in_catalogue():
    ...
    assert "mean_squared_error" in all_metric_names
...
def test_multioutpout_raw_values(fixture_metric):
    ...
    assert result
    np.testing.assert_allclose([0.416667, 1], result["mean_squared_error"], rtol=1e-6)
...
def test_multioutpout_array_like(fixture_metric):
    ...
    assert result
    assert result["mean_squared_error"] == 0.825
