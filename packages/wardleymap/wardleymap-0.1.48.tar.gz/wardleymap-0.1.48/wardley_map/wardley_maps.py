...
class WardleyMap:
    ...
    def __init__(self, owm: str):
        ...
        print(f"DEBUG: Initializing with input: {owm}")  # Debug print

        if self.is_map_id(owm):
            print("DEBUG: Input is recognized as a map ID.")  # Debug print
            self.map_id = owm
            self.owm = self.fetch_map_text()
        else:
            print("DEBUG: Input is recognized as OWM syntax.")  # Debug print
            self.map_id = None
            self.owm = owm

        print(f"DEBUG: Fetched or assigned OWM syntax: {self.owm}")  # Debug print
