# https://github.com/mlflow/mlflow/blob/ed44d8e33cd087a3881a61d3e6107b2689d9da40/mlflow/transformers.py#L1
import ast
import contextlib
import json
import logging
...
from mlflow.models import Model, ModelInputExample, infer_pip_requirements
...
from mlflow.utils.docstring_utils import (  # docstring_version_compatibility_warning,
...
def load_model(model_uri: str, dst_path: str = None, return_type="pipeline", device=None, **kwargs):
    ...
    flavor_config = _get_flavor_configuration_from_uri(model_uri, FLAVOR_NAME)
...
def _load_model(path: str, flavor_config, return_type: str, device=None, **kwargs):
    ...
    if device is None:
        ...
        if os.getenv("MLFLOW_DEFAULT_PREDICTION_DEVICE"):
            try:
                device = int(os.environ["MLFLOW_DEFAULT_PREDICTION_DEVICE"])
            except ValueError:
                device = _TRANSFORMERS_DEFAULT_CPU_DEVICE_ID
        elif is_gpu_available():
            device = _TRANSFORMERS_DEFAULT_GPU_DEVICE_ID
    ...
    if hasattr(transformers, flavor_config[_PIPELINE_MODEL_TYPE_KEY]):
