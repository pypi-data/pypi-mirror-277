...
# TODO (chiragjn): Fix signature warnings and issues from this module in AbstractStore and implementations
...
class TrackingServiceClient:
    ...
    def _get_artifact_repo(self, run_id):
        ...
        if cached_repo is not None:
            ...
            # TODO (chiragjn): These don't seem to be passing in storage integration credentials?. Make sure
            #   it is not a bug
    ...
    # TODO (chiragjn): Give this a different name because artifact now does not refer to files/folders
    ...
    def get_run_by_name(
        ...
        return self.store.get_run_by_name(
            experiment_id=experiment_id, run_name=run_name, experiment_name=experiment_name
        )
    ...
    def list_files_for_artifact_version(
        ...
        return self.store.list_files_for_artifact_version(
            version_id=version_id, path=path, max_results=max_results, page_token=page_token
        )
    ...
    def get_model_version(
        self, experiment_id: int, model_name: str, version: typing.Optional[int]
    ) -> ModelVersion:
        return self.store.get_model_version(
            experiment_id=experiment_id, model_name=model_name, version=version
        )
    ...
    # TODO (chiragjn): Give this a better name, `list_artifacts` is taken by older run level artifacts!
    def list_artifacts_mlf(
        self,
        ml_repo_id: str,
        name: str,
    ...
    def authorize_user_for_model(
        ...
        return self.store.authorize_user_for_model(model_id=model_id, role=role)
    ...
    def authorize_user_for_model_version(
        ...
        return self.store.authorize_user_for_model_version(version_id=version_id, role=role)
    ...
    def get_dataset(
        ...
        return self.store.get_dataset(id=id)

    def get_dataset_by_fqn(
        self,
        fqn: str,
    ) -> Dataset:
        return self.store.get_dataset_by_fqn(
            fqn=fqn,
        )
    ...
    def get_signed_urls_for_dataset_read(
        ...
        return self.store.get_signed_urls_for_dataset_read(
            fqn=fqn,
            paths=paths,
        )
    ...
    def get_signed_urls_for_dataset_write(
        ...
        return self.store.get_signed_urls_for_dataset_write(
            fqn=fqn,
            paths=paths,
        )
    ...
    def list_files_for_dataset(
        ...
        return self.store.list_files_for_dataset(
            fqn=fqn,
            path=path,
            max_results=max_results,
            page_token=page_token,
    ...
    def create_multipart_upload_for_dataset(
        ...
        return self.store.create_multipart_upload_for_dataset(
            fqn=fqn,
            path=path,
            num_parts=num_parts,
        )
    ...
    def update_dataset(
        ...
        return self.store.update_dataset(
            fqn=fqn, description=description, dataset_metadata=dataset_metadata
        )
