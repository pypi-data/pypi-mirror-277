...
def init_model_directory(args, repo_manager, aoa_client, **kwargs):
    if args.cwd:
        set_cwd(args.cwd)
...
def list_resources(args, repo_manager, aoa_client, **kwargs):
    ...
    if args.projects:
        list_and_select_projects(repo_manager, aoa_client, True, False)
        exit(0)
...
def import_stats(args, **kwargs):
    ...
    try:
        ...
        categorical_features = {
            x: stats[x] for x in stats if stats[x]["type"] == "categorical"
        }
        store.save_feature_stats(
            args.metadata_table, "categorical", categorical_features
        )
        continuous_features = {
            x: stats[x] for x in stats if stats[x]["type"] == "continuous"
        }
...
filename = re.findall('File\s".*\/(.*)"', traceback.format_stack()[0])[0]
