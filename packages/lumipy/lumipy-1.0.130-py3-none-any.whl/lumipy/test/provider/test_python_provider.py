...
class TestPythonProviderIntegration(BaseIntTestWithAtlas):
    ...
    def setUpClass(cls) -> None:
        ...
        providers = int_test()
        cls.manager = lp.ProviderManager(*providers, port=5002, user=user, domain=domain, _dev_dll_path=dll_path, whitelist_me=True)
        cls.manager.start()
        ...
        while iters <= max_iters:
            ...
            print(f'iteration {iters}/{max_iters}. Only found {count} providers, but expected {provider_cnt}.')
            sleep(15)
            iters += 1
        ...
        if iters > max_iters:
            cls.manager.stop()
            super().tearDownClass()
            raise ValueError(f'Failed: found {count} providers, but expected {provider_cnt}.')
