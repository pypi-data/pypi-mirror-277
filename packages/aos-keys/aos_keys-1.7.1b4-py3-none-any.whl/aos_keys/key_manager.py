...
def print_cert_info(cert_file_path: str) -> None:
    ...
    table.add_row('Local time: ', str(datetime.now().astimezone().replace(microsecond=0)))
    table.add_row('Valid not before: ', str(datetime_from_utc_to_local(certificate.not_valid_before_utc)))
    table.add_row(
        'Validity not before: ',
        str(datetime.utcnow().replace(tzinfo=pytz.UTC) >= certificate.not_valid_before_utc.replace(tzinfo=pytz.UTC)))
    table.add_row('Valid not after: ', str(datetime_from_utc_to_local(certificate.not_valid_after_utc)))
    table.add_row(
        'Validity not after: ',
        str(datetime.utcnow().replace(tzinfo=pytz.UTC) <= certificate.not_valid_after_utc.replace(tzinfo=pytz.UTC)))
    console.print(table)
