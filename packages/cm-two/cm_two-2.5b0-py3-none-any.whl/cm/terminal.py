...
class Terminal:
    ...
    def close_entry_gate_operation_start(self):
        threading.Thread(target=self.rotate_gate_arrow, args=(
            self.settings.entry_gate, 'close', 'OUT', -1)).start()

    def close_exit_gate_operation_start(self):
        threading.Thread(target=self.rotate_gate_arrow, args=(
            self.settings.exit_gate, 'close', 'OUT', -1)).start()
    ...
    def draw_set_arrow(self, arrow_attr):
        ...
        tag = str(uuid.uuid4())
        tags = ['maincanv'] + [tag]
        self.kpp_barrier_arrows_tag.append(tag)
        ...
        tkimage = ImageTk.PhotoImage(
            self.gate_arrow_img.rotate(arrow_info['pos'], expand=True,
                                       center=(start, end)))
        try:
            self.can.delete(self.kpp_barrier_arrows_tag[:-1])
        except IndexError:
            pass
