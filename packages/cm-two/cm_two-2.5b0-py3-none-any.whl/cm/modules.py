...
class MainPage(Terminal):
    ...
    def destroyBlockImg(self, mode='total'):
        super().destroyBlockImg(mode=mode)
        self.drawMainTree()
...
class KPP(Terminal):
    ...
    def create_abort_photocell_waiting_btn(self):
        obj = self.settings.kpp_abort_photocell_waiting
        self.abort_photocell_waiting_btn = self.get_create_btn(
            self.settings.kpp_abort_photocell_waiting)
        self.can.create_window(obj[1], obj[2],
                               window=self.abort_photocell_waiting_btn,
                               tag="kpp_abort_photocell_waiting_btn")

    def delete_abort_photocell_waiting_btn(self):
        self.can.delete("kpp_abort_photocell_waiting_btn")
    ...
    def abort_photocell_waiting(self):
        ...
        if len(operator_comment) == 0:
            self.photocell_waiting_abort_note["bg"] = "red"
            return
        self.operator.ar_qdk.execute_method(
