...
from anyscale.sdk.anyscale_client.models import (
    ...
    TracingConfig as ExternalAPITracingConfig,
)
...
@pytest.fixture()
def service_sdk_with_fakes(
    sdk_with_fakes: Tuple[Anyscale, FakeAnyscaleClient, TestLogger, FakeTimer]
) -> Tuple[ServiceSDK, FakeAnyscaleClient, TestLogger, FakeTimer]:
    sdk, client, logger, timer = sdk_with_fakes
    return sdk.service, client, logger, timer


class TestDeploy:
    def test_validation(
        self,
        service_sdk_with_fakes: Tuple[
            ServiceSDK, FakeAnyscaleClient, TestLogger, FakeTimer
        ],
    ):
        sdk, fake_client, _, _ = service_sdk_with_fakes
    ...
    def test_custom_image_from_image_uri(
        ...
        assert fake_client.rolled_out_model.build_id
        assert (
            fake_client.rolled_out_model.build_id != build_id
    ...
    def test_disable_query_auth_token(
        ...
        sdk, fake_client, logger, _ = service_sdk_with_fakes
    ...
    def test_tracing_config(
        ...
        config = ServiceConfig(
            ...
            tracing_config=TracingConfig(
                enabled=True, exporter_import_path="path:exporter"
            ),
        )
        sdk.deploy(config)
        assert fake_client.rolled_out_model == ApplyServiceModel(
            ...
            tracing_config=ExternalAPITracingConfig(
                enabled=True, exporter_import_path="path:exporter", sampling_ratio=1,
            ),
        )
...
def _service_version_model(
    ...
    *,
    ...
    return ProductionServiceV2VersionModel(
        ...
        tracing_config=ExternalAPITracingConfig(**tracing_config)
        if tracing_config
        else None,
        local_vars_configuration=OPENAPI_NO_VALIDATION,
    )
...
SERVICE_STATUS_TEST_CASES = [
    ...
    # Test that tracing_config is mapped correctly
    ServiceStatusTestCase(
        state=ServiceEventCurrentState.RUNNING,
        primary_version=_service_version_model(
            ...
            100,
            tracing_config={
                "exporter_import_path": "path:exporter",
                "sampling_ratio": 1.0,
                "enabled": True,
            },
        ),
...
class TestStatus:
    ...
    def test_build_status_from_model(
        ...
        sdk, fake_client, _, _ = service_sdk_with_fakes
        fake_client.add_build(
            ClusterEnvironmentBuild(
                id=TEST_CLUSTER_ENV_BUILD_ID,
                docker_image_name=str(TEST_IMAGE_URI),
...
class TestWait:
    ...
    def test_start_sequence(
        ...
        sdk, fake_client, logger, timer = service_sdk_with_fakes
