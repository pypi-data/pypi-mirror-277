...
class AnyscaleClient(AnyscaleClientInterface):
    ...
    def _get_project_id_by_name(
        ...
        existing_projects = self._internal_api_client.find_project_by_project_name_api_v2_projects_find_by_name_get(
            name
        ).results
    ...
    def get_cluster_env_build_id_from_image_uri(
        self, image_uri: ImageURI, registry_login_secret: Optional[str] = None
    ) -> str:
    ...
    def logs_for_job_run(self, job_run_id: str) -> str:
        """Retrieves logs from the streaming job logs folder in S3/GCS"""
        logs = ""
        next_page_token = None
        cluster_journal_events_start_line = 0

        (logs, next_page_token, cluster_journal_events_start_line, _,) = asyncio.run(
