...
class ImageSDK(BaseSDK):
    ...
    def get(self, name: str) -> ImageBuild:
        ...
        if cluster_env:
            ...
            return ImageBuild(
                status=self._get_image_build_status(build), uri=str(image_uri),
            )
        else:
    ...
    def registery_image(
        self, image_uri: str, registry_login_secret: Optional[str] = None
    ) -> str:
        image_uri_checked = ImageURI.from_str(image_uri_str=image_uri)
        return self.client.get_cluster_env_build_id_from_image_uri(
            image_uri=image_uri_checked, registry_login_secret=registry_login_secret,
        )
