...
from blastp_2_dagchainer import main as blastp_2_dagchainer
from extract_longest_protein import main as extract_longest_protein
from merge import main as merge
from polish import main as polish
from zzq import main as zzq
...
class Cli(object):
    ...
    def polish(self, config: str, pan_matrix: str = 'SG_test', outpng: bool = True, outsvg: bool = False,
               outpdf: bool = False, verbose: str = 'INFO'):
        """
        :param config: <sample_name>    <fasta_path>    <gff3_path>
        :param pan_matrix: your syntelog-based pan-genome matrix pathway
        :param outpng: whether to save the pan-genome matrix in png format
        :param outsvg: whether to save the pan-genome matrix in svg format
        :param outpdf: whether to save the pan-genome matrix in pdf format
        :param verbose: CRITICAL, ERROR, WARNING, INFO, DEBUG, NOTSET
        :return:
        """
        return polish(config=config, pan_matrix=pan_matrix, outpng=outpng, outsvg=outsvg, outpdf=outpdf,
                      verbose=verbose)

    def zzq(self, config: str, pan_matrix: str = 'SG_test.pan', verbose: str = 'INFO'):
        """
        ...
        :return:
        """
        return zzq(config=config, pan_matrix=pan_matrix, verbose=verbose)
