...
class Vault:
    ...
    def get_dynamic_db_credentials(self, mount_point, database):
        try:
        ...
        return output
    def get_dynamic_azure_credentials(self, mount_point, role):
        try:
            ...
            while i < 10:
                ...
                try:
                    ...
                    time.sleep(10)
                    token_bytes = spnCreds.get_token("https://database.windows.net/.default").token.encode("UTF-16-LE")
                    break
                except Exception as e:
                    print(e)
...
class dbConnection:
    ...
    def __init__(self, server, port, db_name, driver='ODBC Driver 18 for SQL Server', encrypt='yes',
                 trustservercertificate='no', timeout=30, auth_method='token', token=None, username=None, password=None):
        if username is not None and token is not None:
            print("Token and basic auth are mutually exclusive, please use either username & password or Token")
        ...
        if auth_method == 'token':
            ...
            self.connection = pyodbc.connect(con_string, attrs_before={SQL_COPT_SS_ACCESS_TOKEN: token})
    ...
    def query(self, sql, params: None, outputResults: str = None, commit: bool = False):
