...
def build(port: Annotated[int, typer.Option(help="Port of the server, default is 8000")] = 8000):
    ...
    Build the image
    """
    if not os.path.exists(Path.cwd() / 'requirements_temp'):
        os.makedirs(Path.cwd() / 'requirements_temp')
        print(f"Folder '{Path.cwd() / 'requirements_temp'}' created.")
    else:
        print(f"Folder '{Path.cwd() / 'requirements_temp'}' already exists.")
    ...
    try:
        # Copy the content of file_1 to file_2
            shutil.copyfile(Path.cwd() / config_map['build']['python_requirements'], Path.cwd() / 'requirements_temp' / 'requirements.txt')
            config_map['build']['python_requirements']='requirements_temp/requirements.txt'
