...
class VampireInputApp:
    ...
    def create_main_page(self, parent):
       ...
       # Create a canvas to hold the background image
        ...
        btn_material_file = tk.Button(canvas, text="Create the Material File", width=20, command=lambda: self.notebook.select(self.tab1))
        canvas.create_window(250, 250, window=btn_material_file)  # Position the button

        btn_input_file = tk.Button(canvas,    text="Create the main Input File", width=20, command=lambda: self.notebook.select(self.tab2))
        canvas.create_window(250, 300, window=btn_input_file)  # Position the button
        btn_input_file = tk.Button(canvas,    text="Run vampire", width=20, command=lambda: self.notebook.select(self.tab3))
        canvas.create_window(250, 350, window=btn_input_file)  # Position the button
        btn_input_file = tk.Button(canvas,    text="Plot output", width=20, command=lambda: self.notebook.select(self.tab4))
        canvas.create_window(250, 400, window=btn_input_file)  # Position the button
        btn_input_file = tk.Button(canvas,    text="Visualization", width=20, command=lambda: self.notebook.select(self.tab5))
