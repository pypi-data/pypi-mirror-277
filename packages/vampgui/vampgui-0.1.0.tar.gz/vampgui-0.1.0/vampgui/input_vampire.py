...
from vampgui.main_run_vampire import RunVampire
from vampgui.main_plot import Plotmain
...
class VampireInputApp:
    def __init__(self, tab):
        ...
        self.notebook.add(self.tab1, text='Create the Material File')
        MainInputTab(self.tab1)
        
        self.notebook.add(self.tab2, text='Create the Input File')
        InputTab(self.tab2)
        
        self.notebook.add(self.tab3, text='Run vampire')
        RunVampire(self.tab3)
        
        self.notebook.add(self.tab4, text='plot setup')
        Plotmain(self.tab4)
        self.notebook.add(self.tab5, text='Visualization')
        VisuaVDC(self.tab5)
        ...
        file_menu.add_command(label="Material File", command=lambda: self.notebook.select(self.tab1))
        file_menu.add_command(label="Input File", command=lambda: self.notebook.select(self.tab2))
        file_menu.add_command(label="Run vampire", command=lambda: self.notebook.select(self.tab3))
        file_menu.add_command(label="Plots", command=lambda: self.notebook.select(self.tab4))
        file_menu.add_command(label="Visualization", command=lambda: self.notebook.select(self.tab5))
    ...
    def create_main_page(self, parent):
        ...
        btn_material_file = tk.Button(canvas, text="_Create the Material File_.", command=lambda: self.notebook.select(self.tab1))
        canvas.create_window(250, 250, window=btn_material_file)  # Position the button

        btn_input_file = tk.Button(canvas,    text="Create the main Input File", command=lambda: self.notebook.select(self.tab2))
        canvas.create_window(250, 300, window=btn_input_file)  # Position the button
        btn_input_file = tk.Button(canvas,    text="______Run vampire________.", command=lambda: self.notebook.select(self.tab3))
        canvas.create_window(250, 350, window=btn_input_file)  # Position the button
        btn_input_file = tk.Button(canvas,    text="_______Plot output________.", command=lambda: self.notebook.select(self.tab4))
        canvas.create_window(250, 400, window=btn_input_file)  # Position the button
        btn_input_file = tk.Button(canvas,    text="_______Visualization______.", command=lambda: self.notebook.select(self.tab5))
        canvas.create_window(250, 450, window=btn_input_file)  # Position the button
        
        description_text = """ Author: G. Benabdellah
            Departement of physic
            University of Tiaret , Algeria
            E-mail ghlam.benabdellah@gmail.com
            this program is part of VAMgui 
            ...
            You should have received a copy of the GNU General Public License
            along with this program. If not, see <http://www.gnu.org/licenses/>."""
        canvas.create_text(250,650, text=description_text, justify=tk.LEFT, font="calibri 9 bold" , fill="white") 
