...
class VisuaVDC:
    def __init__(self, tab):
        ...
        #self.png_mode = tk.LabelFrame(frame, text="view ouput povray: ", font=("Helvetica", 14, "bold"))
        #self.png_mode.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=8, pady=(8, 8))
    ...
    def view_png(self):
        try:
            ...
            logo_image_path ="spins.png"
            # Add widgets on top of the canvas
            logo = tk.PhotoImage(file=logo_image_path)  # Replace with actual path
            logo_label = tk.Label(self.Canvas, image=logo, bg="#FFFFFF")
            logo_label.image = logo  # Keep a reference to the image
            self.Canvas.create_window(150, 900, window=logo_label, anchor=tk.NW)  # Position the l
