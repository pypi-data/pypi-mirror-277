...
class RunVampire:
    def __init__(self, tab):
        self.command_running = False  # Attribute to track command execution
...
# Bind a function to adjust the canvas scroll region when the frame size changes
        ...
        self.file_entry.grid(row=1, column=1, padx=20, pady=20)
         
        
        #self.file_button = ttk.Button(self.mode, text="Browse", command=self.browse_input_serial)
        #self.file_button.grid(row=1, column=3, padx=10, pady=10)

        self.run_serial_button = tk.Button(self.mode, text="Run vampire-serial", command=self.run_vampire_serial)
        self.run_serial_button.grid(row=3, column=0, columnspan=3, pady=20)
        ...
        self.path_para.grid(row=5, column=1, padx=10, pady=20,columnspan=2, sticky="w")
        
        
        self.path_para = tk.Label(self.mode, text="Vampire Program Path:", font=("Helvetica", 12, "bold"))
        self.path_para.grid(row=6, column=0, padx=10, pady=20, sticky="ew")

        self.path_para_entry = ttk.Entry(self.mode, width=70)
        self.path_para_entry.grid(row=6, column=1,columnspan=3, padx=10, pady=20)
        
        self.browse_button_para = tk.Button(self.mode, text="Browse", command=self.browse_vampire_para)
        self.browse_button_para.grid(row=6, column=4, padx=10, pady=10)

        self.n_label =  tk.Label(self.mode, text="Number of Processes (n):", font=("Helvetica", 12, "bold"))
        self.n_label.grid(row=7, column=0, padx=10, pady=10, sticky="w")

        self.n_entry = ttk.Entry(self.mode, width=20)
        self.n_entry.grid(row=7, column=1, padx=10, pady=10 )

        self.file_label = tk.Label(self.mode, text="Input file name:", font=("Helvetica", 12, "bold"))
        self.file_label.grid(row=7, column=2, padx=10, pady=10, sticky="w")
        
        self.file_para_entry = ttk.Entry(self.mode, width=30)
        self.file_para_entry.grid(row=7, column=3, padx=20, pady=20)
         

        #self.file_button = ttk.Button(self.mode, text="Browse", command=self.browse_input_para)
        #self.file_button.grid(row=1, column=3, padx=10, pady=10)

        self.run_para_button = tk.Button(self.mode, text="Run vampire_parallel", command=self.run_vampire_para)
        self.run_para_button.grid(row=9, column=0, columnspan=4, pady=20)

        ## Load configuration
        self.load_config_serial()
        self.load_config_para()
        
        self.mode = tk.LabelFrame(frame, text="Log run: ", font=("Helvetica", 14, "bold"))
        self.mode.pack(side=tk.TOP, fill=tk.BOTH, expand=True ,padx=8, pady=(8, 8))
        ...
        self.text_output.grid(row=11, column=0, columnspan=4, padx=10, pady=10, sticky="nsew")
    ...
    def run_vampire_serial(self):
        if self.command_running:  # Check if a command is already running
            messagebox.showinfo("Already Running", "A command is already running.")
            return
        
        # Set command_running to True to indicate command execution
    ...
    def load_config_serial(self):
        ...
        try:
            with open(f"{base_path}/config_serial.txt", "r") as file:
    ...
    def save_config_serial(self):
        base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
        with open(f"{base_path}/config_serial.txt", "w") as file:
...
# para=============================================
    ...
    def run_vampire_para(self):
        ...
        try:
            ...
            # If the file was removed, enable the "Run" button again
            if  not file_state:
                self.run_par_button.config(state=tk.NORMAL)
                self.command_running = False
                messagebox.showinfo("File Removed", "The input file was removed at the terminal end.")
    ...
    def load_config_para(self):
        base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
        try:
            with open(f"{base_path}/config.txt", "r") as file:
                lines = file.readlines()
                if len(lines) >= 3:
    ...
    def save_config_para(self):
        base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
        with open(f"{base_path}/config.txt", "w") as file:
...
#class Application(tk.Tk):
    #def __init__(self):
        ...
        #self.notebook.add(self.tab3, text='Run Vampire')

        #RunVampire(self.tab3)
