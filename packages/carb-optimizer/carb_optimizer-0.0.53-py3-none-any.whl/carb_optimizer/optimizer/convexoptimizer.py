...
from .arboptimizerbase import ArbOptimizerBase
...
@dataclass
class ConvexOptimizerResult(OptimizerResult):

    problem: InitVar

    def __post_init__(self, O=None, problem=None, *args, **kwargs):
        super().__post_init__(*args, O=O, **kwargs)
...
class NofeesOptimizerResult(ConvexOptimizerResult):
    ...
    def __post_init__(
        self, O=None, problem=None, dx=None, dy=None, *args, **kwargs
    ):
        super().__post_init__(*args, O=O, problem=problem, **kwargs)
    ...
    def trade_instructions(self, ti_format=None):
        ...
        assert ti_format != ArbOptimizerBase.TIF.DFAGGR, "TIF.DFAGGR not implemented for convex optimization"
        assert ti_format != ArbOptimizerBase.TIF.DFPG, "TIF.DFPG not implemented for convex optimization"
        return ArbOptimizerBase.TradeInstruction.to_format(result, ti_format=ti_format)
    ...
    def dxdydf(self, *, as_dict=False, pretty=True, inclk=False):
...
class ConvexOptimizer(ArbOptimizerBase):
    """
    implements the convex optimization method
    """
    ...
    ConvexOptimizerResult = ConvexOptimizerResult
    NofeesOptimizerResult = NofeesOptimizerResult
    
    class SOLVER(Enum):
        """convex optimization solver (``enum``)"""
        ECOS = "ECOS"
        SCS = "SCS"
        OSQP = "OSQP"
        CVXOPT = "CVXOPT"
    ...
    def optimize(self, sfc, **params):
        ...
        curves_t = self.CC.curves
        c0 = curves_t[0]
        tt = self.curves.tokentable()
