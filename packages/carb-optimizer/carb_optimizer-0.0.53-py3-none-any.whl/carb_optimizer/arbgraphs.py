...
Licensed under MIT
"""
__VERSION__ = "3.0-rc3"  # TODO-RELEASE
...
@dataclass
class TrackedStateFloat(DCBase):
    """
...
class Edge(DCBase):
    ...
    def edgetype(self):
        """
        the type of edge (``EDGE.CONNECTION`` = connection only, ``EDGE.AMOUNT`` = plus amount)
        """
        if self.amount_in < 0:
            return self.EDGE.CONNECTION
        return self.EDGE.AMOUNT
    ...
    def is_amounttype(self):
        ...
        """
        return self.edgetype == self.EDGE.AMOUNT

    def assert_edgetype(self, edgetype=EDGE.AMOUNT, msg=""):
        """
    ...
    def __mul__(self, other):
        ...
        # self.assert_edgetype(self.EDGE.AMOUNT, "arithmetic operations only allowed on amount edges")
...
class ArbGraph(DCBase):
    ...
    def pricetable(self, include=None, *, exclude=None, as_df=True):
        """
    ...
    def cycles(self, *, as_generator=False):
        """
    ...
    def degree(self, inout=None, as_matrix=False):
        """
        get the degree of the nodes in the graph, possibly as a matrix

        :inout:     ``DEGREE.INOUT`` (default), ``DEGREE.IN``, ``DEGREE.OUT``
        """
        inout = inout or self.DEGREE.INOUT
        if inout is self.DEGREE.INOUT:
            # degree = nx.degree(self.as_graph(directed=False))
            degree = self.as_graph(directed=False).degree()
        elif inout is self.DEGREE.IN:
            degree = self.as_graph(directed=True).in_degree()
        elif inout is self.DEGREE.OUT:
    ...
    @dataclass
    class RunArbCycleResult(DCBase):
        cycle: Cycle = None
        start_ix: int = None
        token: str = None
        profit: float = None
        ...
        def as_dict(self, *, include_cycle=True, exclude=None, include=None):
            dct = {
                **asdict(self),
                "tokens": self.tokens(),
                "length": self.length,
            ...
            if not include_cycle:
                del dct["cycle"]
            return super().as_dict(dct=dct, exclude=exclude, include=include)

        def as_tuple(self, include_cycle=True):
            return tuple(self.as_dict(include_cycle).values())
    ...
    class ACFMT(Enum):
        """Desited result format for ``run_arbitrage_cycles`` (``enum`)"""
        GEN = "GEN"
        TUPLE = "TUPLE"
        DICTS = "DICTS"
        DF = "DF"
    ...
    def run_arbitrage_cycles(self, cycles, token=None, format=None, **params):
        ...
        :returns:   depends on the ``format`` parameter which is one of ``ACFMT.GEN``, 
                    ``ACFMT.TUPLE``, ``ACFMT.DICTS``, ``ACFMT.DF`` or ``ACFMT.PRETTYDF``
        """
        format = format or self.ACFMT.TUPLE
        arbcycles = (
            self.run_arbitrage_cycle(cycle, token=token, **params) for cycle in cycles
        )
        if format == self.ACFMT.GEN:
            return arbcycles
        if format == self.ACFMT.TUPLE:
    ...
    def run_arbitrage_cyclesf(self, rawresults, *, format=None):
        ...
        :rawresults:    the ACFMT.RAW result returned by run_arbitrage_cycles
        ...
        if format == self.ACFMT.GEN or format == self.ACFMT.TUPLE:
            return rawresults
        arbcycles_dcts = tuple(r.as_dict(False) for r in arbcycles)
        if format == self.ACFMT.DICTS:
            return arbcycles_dcts
        df0 = pd.DataFrame.from_dict(arbcycles_dcts)
        if format == self.ACFMT.DF:
        ...
        if format == self.ACFMT.AGGRDF:
            return df2
        if format == self.ACFMT.PRETTYADF:
            ...
            df = pd.DataFrame.from_dict(
                [
                    dict(
                        ...
                        tkn_out=e.node_out.tkn,
                        n=-e.amount_in,
                        is_reverse=not e.pairo.is_primary,
