"""
adaptor class inverting the pair of a ConstantProductCurve [DEPRECATED]
...
class CPCInverter:
    """
    adaptor class the allows for reverse-pair functions to be used as if they were of the same pair [DEPRECATED]
    """

    curve: ConstantProductCurve

    @classmethod
    def wrap(cls, curves, *, as_generator=False):
        """
        wraps an iterable of curves in CPCInverters if needed and returns a tuple (or generator)

        NOTE: only curves with ``c.pairo.is_primary == False`` are wrapped, the other ones are included
        ...
        """
        result = (cls(c) if not c.pairo.is_primary else c for c in curves)
        if as_generator:
            return result
        return tuple(result)

    @classmethod
    def unwrap(cls, wrapped_curves, *, as_generator=False):
        """
