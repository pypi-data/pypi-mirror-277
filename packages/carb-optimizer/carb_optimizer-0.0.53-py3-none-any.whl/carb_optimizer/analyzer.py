...
from .curves import ConstantProductCurve as CPC, CurveContainer, Pair
from .optimizer import ArbOptimizerBase
...
class CPCAnalyzer(DCBase):
    ...
    def pairsc(self):
        ...
        return {c.pairo.primary for c in self.CC if c.P("exchange_type")==ET.CARBON_V1.value}
    ...
    def curvesc(self, *, ascc=False):
        """all carbon curves"""
        result = [c for c in self.CC if c.P("exchange_type")==ET.CARBON_V1.value]
        if not ascc:
            return result
    ...
    def count_by_tokens(self, *, byexchange=True, as_dict=False):
        """
        ...
        :as_dict:        if True returns dict, otherwise DataFrame
        
        NOTE: the exchanges are current hardcoded, and should be made dynamic
        """
        if not byexchange:
            return self.CC.token_count(as_dict=as_dict)
        
        CCu3    = self.CC.by_params(exchange="uniswap_v3")
        CCu2    = self.CC.by_params(exchange="uniswap_v2")
        CCs2    = self.CC.by_params(exchange="sushiswap_v2")
        CCc1    = self.CC.by_params(exchange="carbon_v1")
        tc_u3 = CCu3.token_count(as_dict=True)
        tc_u2 = CCu2.token_count(as_dict=True)
        tc_s2 = CCs2.token_count(as_dict=True)
        tc_c1 = CCc1.token_count(as_dict=True)
        rows = [
    ...
    def count_by_pairs(self, *, minn=None, as_df=True, p2s=None):
        """
        ...
        :as_df:   if ``True`` (default), returns DataFrame, otherwise ``list`` of ``tuples``
        """
        ...
        if not p2s is None:
            curves_by_pair = [(p2s(p), c) for p, c in curves_by_pair]
        if not as_df:
    ...
    class CurveData(DCBase):
        ...
        def __post_init__(self, curve, analyzer=None, CC=None):
            ...
            if CC is None:
                CC = self.analyzer.CC.by_pairs(curve.pair)
            self.CC = CC
    ...
    def curve_data(self, curves=None, *, as_df=False):
    ...
    class PairData(DCBase):
        ...
        def __post_init__(self, pair, analyzer=None, CC=None):
            ...
            self.analyzer = analyzer
            if CC is None:
                CC = self.analyzer.CC.by_pairs(pair)
            self.CC = CC
        ...
        def curve_data(self, curves=None, *, as_df=False):
            ...
            if curves is None:
                curves = self.CC
            return self.analyzer.curve_data(curves, as_df=as_df)
    ...
    def pair_analysis(self, pair, **params):
        ...
        curvedf = paird.curve_data(as_df=True)
        ...
        CC_crb  = self.curvesc(ascc=True).by_pairs(pair)
    ...
    class RESULT(Enum):
        """Desired result for ``pool_arbitrage_statistics`` (``enum``)"""
        DICT = "dict"
        LIST = "list"
        DF = "df"
        TUPLE = "tuple"
    ...
    def pool_arbitrage_statistics(self, result = None, *, sort_price=True, only_pairs_with_carbon=True):
        ...
        :result:                    ``RESULT.DICT``, ``RESULT.LIST``, ``RESULT.DF`` (default)
        ...
        if only_pairs_with_carbon:
            curves_by_carbon_pair = {pair: self.CC.by_pairs([pair]) for pair in self.pairsc()}
        else:
            curves_by_carbon_pair = {pair: self.CC.by_pairs([pair]) for pair in self.pairs()}
        ...
        result = result or self.RESULT.DF
        assert isinstance(result, self.RESULT), f"invalid result type {result}"
            
        if result == self.RESULT.DICT:
            #print("returning dict")
            return prices_d
        
        prices_l = tuple(it.chain(*prices_d.values()))
        if result == self.RESULT.LIST:
    ...
    def price_ranges(self, result=None, *, short=True):
            ...
            :result:    ``RESULT.TUPLE``, ``RESULT.DICT``, ``RESULT.DF`` (default)
