...
class Params:
    ...
    @classmethod
    def new(cls, dct=None, defaults=None):
        """
        alternative constructor from ``dict`` ``dct``

        :dct:       typically a ``dict`` object; can also be a ``Params`` object which will be 
                    replicated (defaults can either be on the original object or here, but not 
                    on both; if you want to merge defaults, use ``set_default`` on the created 
                    object); a value of ``None`` creates an empty object
    ...
    def new_with(self, **kwargs):
        """
        creates a new object based on ``self`` with additional or changed parameters

        :kwargs:    the parameters to be added to or changed in the current object
        :returns:    a new ``Params`` object with defaults being all (!) values of the current object
        """
        return self.new(dct={**self._params, **kwargs}, defaults={**self.defaults})
    ...
    def __getattr__(self, item):
        """
        for all ``item``s starting with ``_`` this refers to ``super().__getattr__``
        """
        if item[:1] == "_":
    ...
    def __repr__(self):
        ...
        # print("\n[defaults]", defaults)
        return f"{self.__class__.__name__}.new({self._params},{defaults})"
