...
class ConstantProductCurve(CurveBase):
    r"""
    represents a, potentially levered, constant product curve [API]

    :k:          pool invariant k (2) 
    ...
    :alpha:      weight factor :math:`\alpha` of token x; default = 0.5 (3)
    :eta:        portfolio weight factor :math:`\eta`; default = 1 (3)
    :pair:       token pair in slash notation (``TKNB/TKNQ``); ``TKNB`` is on the x-axis, 
                ``TKNQ`` on the y-axis
    :cid:        unique id (optional); typically provided by the system
    :subcid:    sub id (optional); typically provided by the constructor when creating multiple curves 
                for one CID; ``subcid == ''`` asserts that there is only one curve for the CID
    ...
    NOTE 1: ALWAYS use the alternative constructors ``from_xx`` rather then the 
    ...
    MUST use keyword arguments. Changing the order of the parameters in the
    default constructor is not considered a breaking change.
    
    NOTE 2: This class implements two distinct types of constant product curves:
    (1) the standard constant product curve :math:`xy=k`, and
    (2) the weighted constant product curve :math:`x^\alpha y^{1-\alpha} = k^\alpha`
    Note that the case :math:`\alpha=0.5` is equivalent to the standard constant product curve
    :math:`xy=k`, including the value of k
    
    NOTE 3: There are two different ways of specifying the weights of the tokens
    
    - alpha: the weight of the x token such that :math:`x^\alpha y^{1-\alpha} = k^\alpha`; the equal weight case corresponds to :math:`\alpha=0.5`
    
    - eta: the relative weight of the x token, with :math:`\eta = \alpha / (1-\alpha)`; in this case for equal weight :math:`\eta=1`, and for x overweight  :math:`\eta>1`
    """
    ...
    def mul(self, factor):
        """
        'multiplies' the curve by a factor (returns a new curve)
        
        :factor:    factor to multiply the curve by
        :returns:   new curve object at same price point but with scaled liquidity
        
        NOTE. Whilst the ``mul`` method can be used, in practice it is probably more
        convenient to the use the overloaded multiplication operator. The following
        are all equivalent:
        ...
        return self.__class__(
            k=self.k * factor**2,
            x=self.x * factor,
            x_act=self.x_act * factor,
            y_act=self.y_act * factor,
            alpha=self.alpha,
            pair=self.pair,
            cid=str(self.cid)+f"*{factor}",
            subcid=self.subcid,
            fee=self.fee,
            descr=str(self.descr)+f" [*{factor}]",
            constr=self.constr+"*",
        )
    ...
    def __truediv__(self, factor):
        return self.mul(1/factor)

    def P(self, pstr, defaultval=None, *, raiseonerror=False):
        """
        convenience function to access parameters [API]

        :pstr:          parameter name as colon separated string, eg ``exchange`` or ``raw:yint`` (1)
        :defaultval:    default value if parameter not found
        :raiseonerror:  it ``True`` raise on error except returning ``defaultval``
        :returns:       parameter value or ``defaultval``

        NOTE 1: ``CC.pstr("exchange")`` is equivalent to ``CC.params["exchange"]`` 
        if defined, else ``defaultval``;  ``CC.pstr("a:b")`` is equivalent 
        to ``CC.params["a"]["b"]`` if defined, else ``defaultval``
        """
    ...
    @property
    def eta(self):
        "portfolio weight factor ``eta = alpha / (1-alpha)`` [API]"
        return self.alpha / (1 - self.alpha)
    
    def is_constant_product(self):
        "``True`` iff ``alpha == 0.5`` (deprecated; use ``is_symmetric``) [API]"
        return self.is_symmetric
    
    @property
    def is_symmetric(self):
        "``True`` iff ``alpha == 0.5`` [API]"
    ...
    @property
    def is_levered(self):
        "``True`` iff ``x!=x_act or y!=y_act`` [API]"
        return not self.is_unlevered
    
    @property
    def is_unlevered(self):
        "``True`` iff ``x==x_act and y==y_act`` [API]"
        if self.x == 0 or self.y == 0: return False
        return abs(self.x_act/self.x-1)<1e-6 and abs(self.y_act/self.y-1)<1e-6
    ...
    def as_dict(self):
        "returns a ``dict`` representation of the curve [API]"
        return asdict(self)

    @classmethod
    def from_dict(cls, d):
        "returns a curve from a ``dict`` representation [API]"
        return cls(**d)
    
    def set_cid(self, cid):
        """sets the curve id ``cid`` [can only be done once]"""
    ...
    def from_kx(
        ...
        *,
        ...
        "constructor: from ``k,x`` (and ``x_act``, ``y_act``) [API]"
        return cls(
            k=k,
            x=x,
            x_act=x_act,
    ...
    def from_ky(
        ...
        *,
        ...
        "constructor: from ``k,y`` (and ``x_act``, ``y_act``) [API]"
        return cls(
            k=k,
            x=k / y,
            x_act=x_act,
    ...
    def from_xy(
        ...
        *,
        ...
        "constructor: from ``x,y`` (and ``x_act``, ``y_act``) [API]"
        return cls(
            k=x * y,
            x=x,
            x_act=x_act,
    ...
    def from_xyal(
        ...
        *,
        ...
        "constructor: from ``x,y,alpha/eta`` (and ``x_act``, ``y_act``) [API]"
    ...
    def from_pk(
        ...
        *,
        ...
        """
        constructor: from ``k,p`` (and ``x_act``, ``y_act``) [API]
        """
        return cls(
            k=k,
            x=sqrt(k / p),
    ...
    def from_px(
        ...
        """
        constructor: from ``x,p`` (and ``x_act``, ``y_act``) [API]
        """
        return cls(
            k=x * x * p,
            x=x,
    ...
    def from_py(
        ...
        """
        constructor: from ``y,p`` (and ``x_act``, ``y_act``) [API]
        """
        return cls(
            k=y * y / p,
            x=y / p,
    ...
    def from_pkpp(
        ...
        *,
        ...
        """
        constructor: from ``k, p, p_min, p_max`` (default for last two is ``p``) [API]
        """
        if p_min is None:
            p_min = p
        if p_max is None:
    ...
    def from_univ2(
        ...
        *,
        ...
        """
        constructor: from Uniswap V2 pool; see class docstring for other parameters [API]
        ...
        :p:             current pool price, in tknb/tknq (1)
        :exchange_name: exchange name (optional); defaults to "uni_v2"
        :returns:       a list, currently containing one curve

        NOTE 1: either p and k is given, or exactly two of (k, liq_tknb, liq_tknq); a reminder 
        that x is TKNB and y is TKNQ and the pair is "TKNB/TKNQ" where the price is quoted
        in TKNQ per TKNB
        """
        cid = cid or rnd.randint(100_000, 999_999)
        
        if exchange_name is None: exchange_name = "uni_v2"
        ...
        if p is None:
            ...
            assert x is None, "x must be None if p is given"
            assert y is None, "y must be None if p is given"
            y = sqrt(p*k)  # p = y/x, k = yx, pk = y**2
            x = k / y
            
        raw = AttrDict(liq_tknb=x, liq_tknq=y)
        exchange_name = exchange_name or "uni_v2"
        newparams = AttrDict(exchange_name=exchange_name, exchange_type=ET.UNI_V2.value, raw=raw)
        params = AttrDict({**(params or {}), **newparams})
    ...
    def from_univ3(
            ...
            *, 
            ...
            params=None):
        r"""
        constructor: from Uniswap V3 pool; see class docstring for other parameters [API]

        :Pmarg:         current pool marginal price
        :uniL:          uniswap liquidity parameter (``uniL**2 == L**2 == k``)
        :uniPa:         uniswap price range lower bound :math:`P_a,\ P_a < P < P_b`
        :uniPb:         uniswap price range upper bound :math:`P_b,\ P_a < P < P_b`
        :exchange_name: exchange name (optional); defaults to "carbon_v1"
        :returns:       a list, currently containing one curve
        """
        cid = cid or rnd.randint(100_000, 999_999)
        
        if exchange_name is None: exchange_name = "uni_v3"
        assert not pair is None, "pair must not be None"
        assert not cid is None, "cid must not be None"
        assert not fee is None, "fee must not be None"
        ...
        raw = AttrDict(uniL=uniL, uniPa=uniPa, uniPb=uniPb, Pmarg=Pmarg)
        exchange_name = exchange_name or "uni_v3"
        newparams = AttrDict(exchange_name=exchange_name, exchange_type=ET.UNI_V3.value, raw=raw)
        params = AttrDict({**(params or {}), **newparams})
    ...
    def from_solidly(
        ...
        *,
        ...
        """
        constructor: from a Solidly curve; see class docstring for other parameters [API]

        :k:             Solidly pool constant ``k``, :math:`x^3 y + x y^3 = k` (1)
        :x:             current pool liquidity in token ``x`` (1)
        :y:             current pool liquidity in token ``y`` (1)
        :price_spread:  price spread to use for converting constant price -> constant product
        :returns:       a list, currently containing one curve
        
        NOTE 1: exactly 2 out of ``x,y,k`` must be given; the third one is calculated
        
        NOTE 2: The Solidly curve is NOT a constant product curve, as it follows the equation
        ...
        large invariant ``k``, and we will set the ``x_act`` and ``y_act`` parameters 
        so that the curve only covers the above range.
        """
        #if exchange_name is None: exchange_name = "solidly"
        ...
        raw = AttrDict(s_x = solidly_x, s_y = solidly_y, s_k = solidly_k, s_kbar = solidly_kbar, s_cpck=cpc_k, s_cpcx0 = cpc_x0,
                    s_xmin = solidly_xmin, s_xmax = solidly_xmax, s_price_spread = price_spread)
        # TODO: update in the same style as Carbon etc
        if params is None:
            params = AttrDict(raw=raw)
        else:
            params = AttrDict({**params, "raw": raw})
    ...
    def from_carbonv1(
        ...
        *,
        ...
        """
        constructor: from a single Carbon order; see class docstring for other parameters [API]

        :yint:          current pool y-intercept (also known as z; defaults to y)
        :y:             current pool liquidity in token y
        :pa:            carbon price range left bound (higher price in dy/dx) (1)
        :pb:            carbon price range right bound (lower price in dy/dx) (1)
        :A:             alternative to pa, pb: A = sqrt(pa) - sqrt(pb) in dy/dy (1)
        :B:             alternative to pa, pb: B = sqrt(pb) in dy/dy (1)
        :tkny:          token y
        :exchange_name: exchange name (optional); defaults to "carbon_v1"
        ...
        The parameters ``pa``, ``pb`` and ``B`` cannot be zero, with the following exceptions:
        .. code-block:: python
        
            (pa == pb == 0) and (A == B == None)
            (A == B == 0) and (pa == pb == None)
            
        those conditions indicate inactive curves in the system and they return ``[]``
        ...
        assert not cid is None, "cid must not be None"
        if exchange_name is None: exchange_name = "carbon_v1"
        
        if pa is None and pb is None and A == 0 and B == 0:
            # in the system this indicates that this curve does not exist
            # this happens when a Carbon strategy only consists of one order
            # in this case we just return an empty list
            return []
        
        if A is None and B is None and pa == 0 and pb == 0:
            # ditto
            return []
        ...
        if A is None or B is None:
            ...
            # print("[from_carbonv1] A, B:", A, B, pa, pb)
        ...
        if pa is None or pb is None:
            ...
            # print("[from_carbonv1] pa, pb:", A, B, pa, pb)
        ...
        if A is None or B is None:
            ...
            # make sure pa >= pb
            assert pa >= pb, f"in dy/dx, pa must be >= pb (pa={pa}, pb={pb}, isdydx={isdydx})"
            assert not pa == 0 and not pb == 0, f"pa, pb must not be zero (pa={pa}, pb={pb})"
            ...
            # validation
            #print("[from_carbonv1] pa, pb:", pa, pb, minrw, pa/pb-1)
            ...
            assert not B == 0, f"B must not be zero (A={A}, B={B})"
        ...
        raw = AttrDict(y=y, yint=yint, A=A0, B=B, pa=pa0, pb=pb0, minrw=minrw)
        exchange_name = exchange_name or "carbon_v1"
        newparams = AttrDict(exchange_name=exchange_name, exchange_type=ET.CARBON_V1.value, raw=raw)
        params = AttrDict({**(params or {}), **newparams})
    ...
    def execute(self, dx=None, dy=None, *, ignorebounds=False, verbose=False):
        ...
        """
        assert self.is_symmetric, "only implemented for symmetric constant product curves"
    ...
    @property
    def tknb(self):
        "base token ``tknb`` or ``tknx`` [API]"
        return self.pair.split("/")[0]
    tknx = tknb

    @property
    def tknq(self):
        "quote token ``tknq`` or ``tkny`` [API]"
        return self.pair.split("/")[1]
    tkny = tknq
    
    @property
    def token_s(self):
        """token set ``{tknb, tknq}`` as ``set``"""
        return set(self.pair.split("/"))
    ...
    @property
    def y(self):
        "(virtual) pool state ``y`` (virtual number of base tokens for sale) [API]"
    ...
    @property
    def p(self):
        "pool price (in ``dy/dx``) [API]"
        if self.is_symmetric:
            return self.y / self.x 
        
        return self.eta * self.y / self.x
    
    def price(self, pair=None, *, incl_pair=False):
        """
        returns the current price of the pool [API]
        
        :pair:          pair to use; default: ``self.pair`` (1)
        :incl_pair:     if ``True``, returns ``tuple``  ``(price, pair)``
        :returns:       the price of the pool in the quote direction of the pair
        
        NOTE 1. The purpose of this function is to get the price in the quote direction desired.
        Therefore, if pair is provided it must either be the pair of the pool, or the reverse.
        If it is neither, an exception is raised. 
        """
        pair = pair or self.pair
        if pair == self.pair:
            return self.p if not incl_pair else (self.p, self.pair)
        else:
            if not pair == self.pairo.pair_r:
                raise ValueError(f"pair for this curve must be {self.pair} or {self.pairo.pair_r}; not: {pair}")
            return 1/self.p if not incl_pair else (1/self.p, self.pairo.pair_r)
        
    def buysell(self, *, verbose=False, withprice=False):
        """
        returns ``'b'`` (buy primary ``tknb``), ``'s'`` (sells primary ``tknb``) or ``'bs'`` (buys and sells) [API]
        """
        b,s = ("b", "s") if not verbose else ("buy-", "sell-")
        xa, ya = (self.x_act, self.y_act) if self.pairo.is_primary else (self.y_act, self.x_act)
    ...
    def buy(self):
        """returns ``'b'`` if the curve buys the primary token, ``''`` otherwise [API]"""
        return self.buysell(verbose=False, withprice=False).replace("s", "")
    
    def sell(self):
        """returns ``'s'`` if the curve sells the primary token, ``''`` otherwise [API]"""
    ...
    def p_convention(self):
        """price convention for ``p`` (``dy/dx``) [API]"""
        return f"{self.tknyp} per {self.tknxp}"
    
    @property
    def primary(self):
        "alias for ``self.pairo.primary`` [API]"
        return self.pairo.primary
    
    @property
    def is_primary(self):
        "alias for ``self.pairo.is_primary`` [API]"
        return self.pairo.is_primary
    
    def primaryp(self, *, withconvention=False):
        "pool price in the native quote of the curve ``Pair`` object [API]"
    ...
    def kbar(self):
        r"""
        ``kbar`` is the pool invariant that scales linearly with the pool size [API]
        
        :math:`\bar k = \sqrt k` for constant product, 
        :math:`\bar k = k^\alpha` for general curves
        """
        if self.is_symmetric:
            return sqrt(self.k)
    ...
    def invariant(self, xvec=None, *, include_target=False):
        """
        returns the actual invariant of the curve (eg :math:`x*y` for constant product)
    ...
    def at_boundary(self):
        """``True`` iff ``x`` is at either ``x_min`` or ``x_max`` [API]"""
        return self.at_xmin or self.at_xmax

    @property
    def y_min(self):
        "minimum (virtual) ``y`` value [API]"
        if self.is_unlevered:
            return 0
        assert self.is_symmetric, "only implemented for symmetric constant product curves"
    ...
    def tkn_act(self, tkn):
        """shortcut for actual token amounts ``x_act`` or ``y_act``, depending on ``tkn`` [API]"""
        if tkn == self.tknx:
            return self.x_act
        elif tkn == self.tkny:
            return self.y_act
        else:
            raise ValueError(f"tkn must be one of tknx, tkny [{tkn}; {self.tknx}, {self.tkny}]")
    
    def tkn_virt(self, tkn):
        """shortcut for virtual token amounts ``x`` or ``y``, depending on ``tkn`` [API]"""
        if tkn == self.tknx:
            return self.x
        elif tkn == self.tkny:
            return self.y
    ...
    def tkn_max(self, tkn):
        """shortcut for ``x_max`` or ``y_max``, depending on ``tkn`` [API]"""
        if tkn == self.tknx:
            return self.x_max
        elif tkn == self.tkny:
            return self.y_max
    ...
    def tkn_min(self, tkn):
        """shortcut for ``x_min`` or ``y_min``, depending on ``tkn`` [API]"""
        if tkn == self.tknx:
            return self.x_min
        elif tkn == self.tkny:
            return self.y_min
    ...
    @property
    def p_max(self):
        "maximum pool price (in ``dy/dx``; ``None`` if unlimited) =`` y_max/x_min`` [API]"
        if self.is_unlevered:
            return None
        assert self.is_symmetric, "only implemented for symmetric constant product curves"
    ...
    def p_max_primary(self, swap=True):
        "``p_max`` in the native quote of the curve Pair object (``swap=True``: ``p_min``) [API]"
        if self.is_unlevered:
            return None
        p = self.p_max if not (swap and not self.is_primary) else self.p_min
        if p is None: return None
        return p if self.is_primary else 1/p
    
    @property
    def p_min(self):
        "minimum pool price (in ``dy/dx``; ``None`` if unlimited) = ``y_min/x_max`` [API]"
        if self.is_unlevered:
            return 0
        assert self.is_symmetric, "only implemented for symmetric constant product curves"
    ...
    def p_min_primary(self, swap=True):
        "``p_min`` in the native quote of the curve ``Pair`` object (``swap=True``: ``p_max``) [API]"
        if self.is_unlevered:
            return 0
        p = self.p_min if not (swap and not self.is_primary) else self.p_max
        if p is None: return None
        return p if self.is_primary else 1/p
    ...
    def xyfromp_f(self, p=None, *, ignorebounds=False, withunits=False):
        r"""
        returns ``x,y,p`` for a given marginal price ``p`` (stuck at the boundaries if ``ignorebounds=False``) [API]
    ...
    def xvecfrompvec_f(self, pvec, *, ignorebounds=False):
        """
        alternative API to xyfromp_f [API]
        ...
        if isnan(p) or isinf(p):
            print(f"[xvecfrompvec_f] invalid result (p={p}, pvec={pvec}, tknx={self.tknx}, tkny={self.tkny})")
            raise ValueError("[xvecfrompvec_f] invalid division result", pvec, self)
    ...
    def dxdyfromp_f(self, p=None, *, ignorebounds=False, withunits=False):
        """like ``xyfromp_f``, but returns ``dx,dy,p`` instead of ``x,y,p`` [API]"""
    ...
    def dxvecfrompvec_f(self, pvec, *, ignorebounds=False):
        """
        alternative API to ``dxdyfromp_f`` [API]
    ...
    def yfromx_f(self, x, *, ignorebounds=False):
        "``y`` value for given ``x`` value (if in range; ``None`` otherwise) [API]"
        if self.is_symmetric:
            y = self.k / x
        else:
            y = (self.k / x) ** self.eta
        ...
        if ignorebounds:
            return y
        if not self.in_range(y, self.y_min, self.y_max):
            return None
        return y

    def xfromy_f(self, y, *, ignorebounds=False):
        "``x`` value for given ``y`` value (if in range; ``None`` otherwise) [API]"
        if self.is_symmetric:
            x = self.k / y
        else:
        ...
        if ignorebounds:
            return x
        if not self.in_range(x, self.x_min, self.x_max):
            return None
        return x
    
    def tknfromtkn_f(self, from_tkn, from_amt, *, ignorebounds=False):
        """
        shortcut for ``xfromy_f`` and ``yfromx_f``
        
        :from_tkn:      if ``tknx``, use ``yfromx_f``, if ``tkny`` use ``xfromy_f``
        :from_amt:      the in amount (``x`` for ``yfromx_f``, ``y`` for ``xfromy_f``)
        :ignorebounds:  passed through
        """
        if from_tkn == self.tknx:
            return self.yfromx_f(from_amt, ignorebounds=ignorebounds)
        elif from_tkn == self.tkny:
            return self.xfromy_f(from_amt, ignorebounds=ignorebounds)
        else:
            raise ValueError(f"from_tkn must be one of tknx, tkny [{from_tkn}; {self.tknx}, {self.tkny}]")

    def dtknfromdtkn_f(self, from_tkn, from_damt, *, ignorebounds=False):
        """
        shortcut for ``dxfromdy_f`` and ``dyfromdx_f``
        
        :from_tkn:      if ``tknx``, use ``dyfromdx_f``, if ``tkny`` use ``dxfromdy_f``
        :from_amt:      the in amount (``dx`` for ``dyfromdx_f``, ``dy`` for ``dxfromdy_f``)
        :ignorebounds:  passed through
        """
        if from_tkn == self.tknx:
            return self.dyfromdx_f(from_damt, ignorebounds=ignorebounds)
        elif from_tkn == self.tkny:
            return self.dxfromdy_f(from_damt, ignorebounds=ignorebounds)
        else:
            raise ValueError(f"from_tkn must be one of tknx, tkny [{from_tkn}; {self.tknx}, {self.tkny}]")

    def dyfromdx_f(self, dx, *, ignorebounds=False):
        "``dy`` value for given ``dx`` value (if in range; ``None`` otherwise) [API]"
        #print("dyfromdx_f: x, dx = ", self.x, dx)
        y = self.yfromx_f(self.x + dx, ignorebounds=ignorebounds)
        if y is None:
            return None
        #print("dyfromdx_f: y, dy = ", self.y, y - self.y)
        return y - self.y

    def dxfromdy_f(self, dy, *, ignorebounds=False):
        "``dx`` value for given ``dy`` value (if in range; ``None`` otherwise) [API]"
    ...
    @property
    def dy_min(self):
        """minimum (=max negative) possible ``dy`` value of this pool (=-``y_act``) [API]"""
        return -self.y_act

    @property
    def dx_min(self):
        """minimum (=max negative) possible ``dx`` value of this pool (=-``x_act``) [API]"""
        return -self.x_act

    @property
    def dy_max(self):
        """maximum ``dy`` value of this pool (=``dy(dx_min)``) [API]"""
    ...
    @property
    def dx_max(self):
        """maximum ``dx`` value of this pool (=``dx(dy_min)``) [API]"""
        if self.y_act < self.y:
            return self.dxfromdy_f(self.dy_min)
        else:
            return None

    def dtkn_max(self, tkn):
        """shortcut for ``dx_max`` or ``dy_max``, depending on ``tkn``"""
        if tkn == self.tknx:
            return self.dx_max
        elif tkn == self.tkny:
            return self.dy_max
    ...
    def dtkn_min(self, tkn):
        """shortcut for ``dx_min`` or ``dy_min``, depending on ``tkn``"""
        if tkn == self.tknx:
            return self.dx_min
        elif tkn == self.tkny:
            return self.dy_min
    ...
    @staticmethod
    def in_range(v, minv=None, maxv=None):
        "True if ``minv <= v <= maxv``; ``None`` means no boundary [API]"
        if not minv is None:
            if v < minv:
                return False
        if not maxv is None:
    ...
    def isequal(self, x, y):
        "returns ``True`` if ``x`` and ``y`` are equal within ``self.EPS``"
        if x == 0:
            return abs(y) < self.EPS
        return abs(y / x - 1) < self.EPS

    def isbigger(self, small, big):
        "returns ``True`` if ``small`` is bigger than ``big`` within ``self.EPS`` (``small``, ``big`` > 0)"
        if small == 0:
            return big > self.EPS
    ...
    def digest(datastr, len=4):
        """returns a hash digest of ``datastr`` of a certain length"""
