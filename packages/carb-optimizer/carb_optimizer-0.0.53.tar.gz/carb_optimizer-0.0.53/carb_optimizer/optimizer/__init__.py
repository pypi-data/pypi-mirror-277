...
-   **Graph Optimization**: an alternative approach to 
    optimization that looks at all possible circuits (=closed
    paths) on the graph of tokens, and that calculates the 
    arbitrage opportunities on a path by path basis; this 
    method is particularly useful for highly levered
    configurations where gradient-based approaches like the
    marginal method may not converge
...
``MargPFullOptimizer``, pair optimization in the class
``MargPPairOptimizer``, and convex optimization in the class
``ConvexOptimizer``. All those classes are subclasses of
``ArbOptimizerBase``, and ultimately of ``OptimizerBase``.
...
from .arboptimizerbase import ArbOptimizerBase, F
from .margppairoptimizer import MargPPairOptimizer
from .margpfulloptimizer import MargPFullOptimizer
