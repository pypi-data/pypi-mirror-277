...
# TODO-DOC: REWRITE IN PROPER RST
@dataclass(frozen=True)
class SolidlySwapFunction(Function):
    r"""
    represents the Solidly AMM swap function
    
    :method: ``METHOD.FLOAT``, ``METHOD.DEC`` (default), ``METHOD.TAYLOR``
    ...
    as :math:`y=y(x; k)`. This gives the following result
    ...
    We can introduce intermediary **variables L and M** (:math:`L(x;k), M(x;k)`) 
    ...
    For simplicity we introduce the **variable xi** :math:`\xi=\xi(x,k)` as
    ...
    Note the Taylor expansion for :math:`\sqrt{1 + \xi} - 1` is 
    ...
    and tests suggest that it is very good for at least :math:`|\xi| < 10^{-5}`
    """
