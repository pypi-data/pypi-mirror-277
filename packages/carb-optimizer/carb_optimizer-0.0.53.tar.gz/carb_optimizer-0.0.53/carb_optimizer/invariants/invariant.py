...
class Invariant(ABC):
    ...
    def y_func_from_k_func(self, x, k, *, x0=None, x_lo=None, x_hi=None, method=None):
        ...
        :method:        one of ``METHOD.GRADIENT`` (default) or ``METHOD.BISECT``
        """
        method = method or self.METHOD.GRADIENT
        if method == self.METHOD.GRADIENT:
            if x0 is None:
                x0 = 1
            return self.goalseek_gradient(lambda y: self.k_func(x, y), x0=x0, target=k)
        elif method == self.METHOD.BISECT:
            if x_lo is None:
                x_lo = 1e-10
            if x_hi is None:
                x_hi = 1e10
            ...
            raise ValueError(f"method={method} must be one of self.METHOD.GRADIENT, self.METHOD.BISECT")
