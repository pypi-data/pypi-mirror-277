...
class Kernel():
    ...
    :method:        integration method (currently only ``IM.TRAPEZOID``)
    ...
    DEFAULT_XMIN = 0
    DEFAULT_XMAX = 1
    DEFAULT_KERNEL = KTYPE.FLAT
    DEFAULT_METHOD = IM.TRAPEZOID
    DEFAULT_STEPS = 100
    
    x_min: float = DEFAULT_XMIN
    x_max: float = DEFAULT_XMAX
    kernel: callable = None
    kernel_type: KTYPE = DEFAULT_KERNEL
    method: str = DEFAULT_METHOD
    steps: int = DEFAULT_STEPS
    
    def __post_init__(self):
        assert self.x_max > self.x_min, "x_max must be greater than x_min"
        if isinstance(self.kernel, self.KTYPE):
            self.kernel_type = self.kernel
            self.kernel = None
    ...
    def integrate(self, func, *, steps=None, method=None):
        ...
        NOTE 1: currently the only method supported is `IM.TRAPEZOID`
