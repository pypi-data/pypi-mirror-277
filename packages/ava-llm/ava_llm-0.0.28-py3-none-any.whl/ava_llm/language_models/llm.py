...
class ConversationChain:
    ...
    def stream(self, message: str, verbose=True, language="en") -> Any:
        inputs = {"prompt": message}

        prompt = self.prompt.format(
            history=self.memory.buffer, input=message, language=language
        )
    ...
    async def astream(
        self, message: str, verbose=True, ws_consumer=None, language="en"
    ) -> None:
        inputs = {"prompt": message}
        ...
        async for chunk in self.llm.astream(prompt):
            ...
            if ws_consumer is not None:
                await ws_consumer.send(text_data=json.dumps(chunk))
            if verbose:
                print(chunk, end="", flush=True)
