...
class IamTokenService:
    ...
    def __init__(self):
        with open(os.getenv('IAM'), 'r') as private:
            authorized_key = json.loads(private.read())
    ...
    def get_token(self):
        if self._expires_at and self._expires_at - timedelta(hours=4) > datetime.now():
    ...
    def _request(self):
        ...
        expires_at = datetime.strptime(response['expiresAt'].split('.')[0], "%Y-%m-%dT%H:%M:%S")
        expires_at = datetime(expires_at.year, expires_at.month, expires_at.day, expires_at.hour, expires_at.minute, 0)
        return response['iamToken'], expires_at
...
class RuPartnerService:
    def create_lead(self, referral_id: int):
        url = 'https://api.expressmoney.com/ru_partners/leads/lead'
    ...
    def create_referral(self, referral_id, partner_code):
        url = 'https://api.expressmoney.com/ru_partners/referrals/referral'
    ...
    def send_order_postback(self, user_id: int, order_id: int, utm_source: str, transaction_id: str, amount):
        url = 'https://api.expressmoney.com/ru_partners/cpa/order_client_post_back'
    ...
    def send_loan_postback(self, user_id: int, loan_id: int, utm_source: str, transaction_id: str, amount):
        url = 'https://api.expressmoney.com/ru_partners/cpa/loan_client_post_back'
        requests.post(url, headers=self._get_header(user_id), data=
        {
            'loan_id': loan_id,
            'utm_source': utm_source,
    ...
    def get_cpa_client(self, user_id: int):
        url = f'https://api.expressmoney.com/ru_partners/cpa/cpa_client/{user_id}'
        return requests.get(url, headers=self._get_header(user_id))

    @staticmethod
    def _get_header(user_id):
        access_token = RefreshToken.for_user(User.objects.get(pk=user_id)).access_token
