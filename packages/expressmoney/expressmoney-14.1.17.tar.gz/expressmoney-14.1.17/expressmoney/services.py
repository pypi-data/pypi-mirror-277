...
class IamTokenService:
    ...
    def get_token(self):
        if self._expires_at and self._expires_at - datetime.timedelta(hours=4) > timezone.now():
    ...
    def _request(self):
        ...
        return response['iamToken'], timezone.datetime.strptime(
            response['expiresAt'].split('.')[0], "%Y-%m-%dT%H:%M:%S").replace(tzinfo=timezone.timezone.utc)
