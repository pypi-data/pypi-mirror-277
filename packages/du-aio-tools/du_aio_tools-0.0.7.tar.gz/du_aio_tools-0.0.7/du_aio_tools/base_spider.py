...
class BaseSpider(BaseDataSpider):
    ...
    @staticmethod
    def parse_range(file_size, chuck=1024):
...
class AioSpider(BaseSpider):
    def __init__(self, name=None, headers=None, sem: int = 15):
        """
        name: str 爬虫名称/域名/表名
        """
    ...
    async def aio_download(self, url, file: Path, method='get', **kwargs):
        ...
        if filesize > 1_000_000:  # 1M
            divisional_ranges = self.parse_range(file_size=filesize, chuck=1024)
