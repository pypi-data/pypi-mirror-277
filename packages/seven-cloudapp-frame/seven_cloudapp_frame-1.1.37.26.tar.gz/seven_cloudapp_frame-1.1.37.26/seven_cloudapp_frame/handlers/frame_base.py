    ...
    def prepare(self):
        ...
        try:
            ...
            if is_api_encrypt == True:
                ...
                if self.request_params.__contains__("timestamp") and (now_time - int(self.request_params["timestamp"]) > encrypt_limit_time) or (int(self.request_params["timestamp"]) - now_time > encrypt_limit_time):
            ...
            if safe_config.get("traffic_control_ways", 0) == 1: # 流量控制方式 0-关闭 1-全局控制 2-指定handler控制
                #每分钟流量UV计数,用于登录接口限制登录
                SafeHelper.add_current_limit_count(str(dict_param["app_id"]), str(dict_param["open_id"]))
            # # 获取设备信息
