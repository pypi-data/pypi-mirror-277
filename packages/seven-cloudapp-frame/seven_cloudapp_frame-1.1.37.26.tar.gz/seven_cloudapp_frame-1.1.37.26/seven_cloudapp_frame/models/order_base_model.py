...
@LastEditTime: 2024-06-05 18:29:05
...
class OrderBaseModel(FrameBaseModel):
    ...
    def get_prize_order_list(self, app_id, act_id, user_id, open_id, nick_name, order_no, real_name, telephone, address, order_status, create_date_start, create_date_end, page_size=20, page_index=0, order_by="create_date desc", field="*", is_search_roster=False, is_cache=True, condition="", params=[], prize_roster_sub_table=None, page_count_mode="total"):
        ...
        if is_cache:
            page_list = prize_order_model.get_cache_dict_page_list(field, page_index, page_size, condition_where.to_string(), "", order_by, params_list, dependency_key=DependencyKey.prize_order_list(act_id,user_id), page_count_mode=page_count_mode)
        else:
        ...
        # 敏感字段处理
        page_list, status = self.sensitive_decrypt(page_list, ["real_name","telephone","province","city","county","street","address"])
    ...
    def get_tao_pay_order_list(self,app_id,act_id,user_id,open_id,nick_name,pay_date_start,pay_date_end,page_size=20,page_index=0,field="*",order_by="pay_date desc",condition="",params=[]):
        ...
        page_list, total = TaoPayOrderModel(context=self.context).set_sub_table(app_id).get_dict_page_list(field, page_index, page_size, where, "", order_by, params_list)
    ...
    def sync_tao_pay_order(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,asset_type=3,goods_id="",sku_id="",ascription_type=1,app_key="",app_secret="",is_log=False,check_user_nick=True,continue_request_expire=1,support_presale_order=False,default_order_status=None):
        """
    ...
    def get_horseracelamp_list(self, app_id, act_id, module_id, is_search_module=False, condition="", params=[], page_size=30):
        ...
        if total < page_size:
            ...
            if len(false_act_prize_list) > 0 and add_num > 0:
                ...
                user_info_list_dict = UserInfoModel(context=self.context).set_sub_table(false_act_prize_list[0].app_id).get_dict_list("act_id<%s and create_date>%s", params=[act_id,now_datetime], limit=str(add_num))
    ...
    def sync_shake_shop_pay_order(self, app_id, act_id, module_id, user_id, login_token, handler_name, request_code, is_log=False, asset_type=3, while_count=1, ascription_type=0, reward_order_status_list = [105, 2, 101, 3, 5]):
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                if len(order_data) > 0:
                    pay_order_no_list, redis_init, pay_order_cache_key = self._get_shake_shop_pay_order_no_list(app_id, user_id)
                    ...
                    for order in all_order_list:
                        if order["order_id"] not in all_order_id_list:
                            invoke_result_data_shake_shop = shake_shop_base_model.order_detail(order["order_id"], is_cache=is_order_cache)
                            if invoke_result_data_shake_shop.success == True:
                                ...
                                all_order_id_list.append(order["order_id"])
                        else:
                            shop_order_detail = query(all_order_detail_list).first_or_default(None, lambda x: str(x['order_id']) == str(order['order_id']))
                        shop_order_detail = query(all_order_detail_list).first_or_default(None, lambda x: str(x['order_id']) == str(order['order_id']))
                        if not shop_order_detail or (user_info_dict["union_id"] and user_info_dict["union_id"] != shop_order_detail["doudian_open_id"]):
                            continue
                        sku_order = query(shop_order_detail["sku_order_list"]).first_or_default(None, lambda x: str(x["product_id"]) == str(order["product_id"]) and str(x["sku_id"]) == str(order["sku_id"]))
                        if not sku_order:
                            continue
                        order["sub_order_id"] = sku_order["order_id"] # 子订单号
                    ...
                    for order in reward_order_list:
                        try:
                            ...
                            #判断是否已经加过奖励
                            if order["sub_order_id"] not in pay_order_no_list and int(order["order_status"]) in reward_order_status_list and order["after_sale_info"]["refund_status"] != '3': # 订单状态;1 待确认/待支付-订单创建完毕;105-已支付; 2-备货中； 101-部分发货; 3-已发货（全部发货）;4- 已取消;5 已完成（已收货）;
                                asset_object_name = ""
                                asset_object_id = ""
                                now_price_gear_dict = None
                                for price_gear_dict in price_gear_dict_list:
                                ...
                                third_pay_order.main_pay_order_no = order['order_id']
                                third_pay_order.sub_pay_order_no = order['sub_order_id']
                            ...
                            continue
                    if len(pay_order_list) > 0:
                        result = ThirdPayOrderModel(context=self.context).set_sub_table(app_id).add_list(pay_order_list)
                        if result == True and buy_num > 0:
                            asset_base_model = AssetBaseModel(context=self.context)
                            invoke_result_data.data = {}
                            invoke_result_data.data["asset_list"] = []
    ...
    def _get_shake_shop_pay_order_no_list(self, app_id, user_id):
        """
        :description: 获取抖店已存在的订单编号列表
        :param app_id:应用标识
        :param user_id:用户标识
        :return: 
        :last_editors: HuangJianYi
        ...
        pay_order_cache_key = f"shake_shop_pay_order_no_list:appid_{app_id}_userid_{user_id}"
        pay_order_no_list = redis_init.lrange(pay_order_cache_key,0,-1)
        pay_order_list = []
        is_add = False
        if not pay_order_no_list or len(pay_order_no_list)<=0:
