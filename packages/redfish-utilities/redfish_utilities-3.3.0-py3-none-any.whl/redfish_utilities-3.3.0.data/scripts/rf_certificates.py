...
info_argget.add_argument(
    ...
    "-details",
    action="store_true",
    help="Indicates if the full details of each certificate should be shown",
)
csrinfo_argget = subparsers.add_parser(
    "csrinfo", help="Displays information about options supported for generating certificate signing requests"
)
...
csr_argget.add_argument(
    "--keylen", "-len", type=int, help="The length of the key, in bits, if the key pair algorithm supports key size"
)
csr_argget.add_argument(
    "--keycurve", "-curve", type=str, help="The curve ID to use with the key if the key pair algorithm supports curves"
)
csr_argget.add_argument(
    "--out", "-out", type=str, help="The file, with optional path, to save the certificate signing request"
)
...
install_argget.add_argument(
    ...
    type=str,
    required=True,
    help="The installation URI of the certificate; either a certificate collection to insert, or an existing certificate to replace",
)
install_argget.add_argument(
    "--certificate", "-cert", type=str, required=True, help="The file, and optional path, of the certificate to install"
)
install_argget.add_argument(
    "--key", "-key", type=str, help="The file, and optional path, of the private key for the certificate to install"
)
delete_argget = subparsers.add_parser("delete", help="Deletes a certificate on the service")
delete_argget.add_argument(
    "--certificate", "-cert", type=str, required=True, help="The URI of the certificate to delete"
)
