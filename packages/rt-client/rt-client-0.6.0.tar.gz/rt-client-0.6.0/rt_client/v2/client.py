...
def _redirected_to_login_page(response):
    """Determine if the response was the result of a redirect from
...
def _missing_or_stale_session_cookie(response):
    ...
    if response.status_code == 401:
        ...
        return True
    elif _redirected_to_login_page(response):
...
class Client(object):
    def __init__(
        ...
        Args:
            ...
            endpoint (str): The base URL of the host RT system. e.g 'https://rt.host.com/'
    ...
    def request(self, method, url, retries=1, *args, **kwargs):
        ...
        # dump the raw request and response to the debug log to aid debugging
        # communication issues with RT.
        logger.debug(dump.dump_all(response).decode("utf-8"))

        if retries > 0 and _missing_or_stale_session_cookie(response):
            ...
            self.authenticate()
            logger.info(
                "Attempting request again: %s %s (retries remaining=%s)",
                method,
                _url,
                retries,
            )
