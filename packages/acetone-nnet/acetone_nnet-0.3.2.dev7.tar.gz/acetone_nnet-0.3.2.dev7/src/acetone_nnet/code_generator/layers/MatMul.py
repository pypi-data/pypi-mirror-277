...
class MatMul(Layer):
    ...
    def __init__(self, idx:int, size:int, input_shapes:list, weights:np.ndarray, side:int, activation_function:ActivationFunctions):
        ...
        if self.side == 0:
            ...
            self.output_channels = self.input_shapes[1]
            self.output_height = self.input_shapes[-1]
            self.output_width = self.weights.shape[-2]
        ...
        if self.side == 1:
            ...
            if self.size != self.weights.shape[-2] * self.input_shapes[-1]:
            ...
            if self.size != self.weights.shape[-1] * self.input_shapes[-2]:
            ...
            if self.input_shapes[1][-2] != self.input_shapes[0][-1]:
                raise ValueError("Error: non consistency between weight shape and input shape in MatMul ("+str(self.weights.shape[1][-2])+"!="+str(self.input_shapes[0][-1])+")")
            if self.size != self.weights.shape[1][-1] * self.input_shapes[0][-2]:
                raise ValueError("Error: size value in MatMul ("+str(self.size)+" !="+str(self.weights.shape[1][-1] * self.input_shapes[0][-2])+")")
        else: 
    ...
    def forward_path_layer(self, input_1:np.ndarray, input_2:np.ndarray|None = None):
        if self.side == 1:
            input_1 = input_1.reshape(self.input_shapes)
            ...
            input_1 = input_1.reshape(self.input_shapes)
            return self.activation_function.compute(np.matmul(input_1,self.weights))
        elif self.side == 2: 
            input_1 = input_1.reshape(self.input_shapes[0])
            input_2 = input_2.reshape(self.input_shapes[1])
