...
# TODO: use gold price as feature and silver price as target
...
def load_silver_data(fred_api_key, start=pd.Timestamp.today()-pd.tseries.offsets.BDay(300), end=pd.Timestamp.today()):
    if type(start) is int:
            start = end - pd.tseries.offsets.BDay(start)
    ...
    silver['ClosingPrice'] = silver['Close']
    silver = silver[['ClosingPrice']]
