...
from importlib.resources import read_binary, \
    files  # TODO: according to official doc, `read_binary` is not recommended. To be replaced with `files` in the future after some testing.
import io
...
class Analyzer:
    ...
    def time_space_diagram_traj(s, links=None, figsize=(12, 4), plot_signal=True, xlim=None, ylim=None):
        ...
        for lll in links:
            s.time_space_diagram_traj_links(linkslist=[lll], figsize=figsize, plot_signal=plot_signal, xlim=xlim,
                                            ylim=ylim)
    ...
    def time_space_diagram_density(s, links=None, figsize=(12, 4), plot_signal=True, xlim=None, ylim=None):
        ...
        for lll in tqdm(links, disable=(s.W.print_mode == 0)):
            ...
            if plot_signal:
                signal_log = [i * s.W.DELTAT for i in lange(l.end_node.signal_log) if
        ...
        for links in linkslist:
            ...
            for ll in links:
                ...
                for i in range(len(l.xss)):
                    lane_shift = l.ls[
                                     i] / l.lanes * s.W.DELTAT / 2  # vehicle with the same lane is plotted slightly shifted
    ...
    def cumulative_curves(s, links=None, figsize=(6, 4)):
        ...
        for link in links:
            ...
            ax2.plot(range(0, s.W.TMAX, s.W.DELTAT), link.traveltime_instant, ".", c="gray", ms=0.5,
                     label="instantaneous")
    ...
    @catch_exceptions_and_warn()
    def network(s, t=None, detailed=1, minwidth=0.5, maxwidth=12, left_handed=1, tmp_anim=0, figsize=(6, 6),
    ...
    @catch_exceptions_and_warn()
    def network_pillow(s, t=None, detailed=1, minwidth=0.5, maxwidth=12, left_handed=1, tmp_anim=0, figsize=6,
    ...
    def show_simulation_progress(s):
        ...
        if s.W.print_mode:
            ...
            print(
                f"{s.W.TIME:>8.0f} s| {sum_vehs:>8.0f} vehs|  {avev:>4.1f} m/s| {time.time() - s.W.sim_start_time:8.2f} s",
                flush=True)

    @catch_exceptions_and_warn()
    def network_anim(s, animation_speed_inverse=10, detailed=0, minwidth=0.5, maxwidth=12, left_handed=1,
        ...
        for t in tqdm(range(0, s.W.TMAX, s.W.DELTAT * timestep_skip), disable=(s.W.print_mode == 0)):
            if int(t / s.W.LINKS[0].edie_dt) < s.W.LINKS[0].k_mat.shape[0]:
                if detailed:
                    ...
                    s.network(int(t), detailed=detailed, minwidth=minwidth, maxwidth=maxwidth, left_handed=left_handed,
                              tmp_anim=1, figsize=figsize, node_size=node_size, network_font_size=network_font_size)
                else:
                    s.network_pillow(int(t), detailed=detailed, minwidth=minwidth, maxwidth=maxwidth,
    ...
    @catch_exceptions_and_warn()
    def network_fancy(s, animation_speed_inverse=10, figsize=6, sample_ratio=0.3, interval=5, network_font_size=0,
    ...
    def plot_vehicle_log(s, vehname):
        ...
        ax2.plot([veh.log_t[i] for i in lange(veh.log_t) if veh.log_state[i] != "home"],
                 [vehlinks[i] for i in lange(vehlinks) if veh.log_state[i] != "home"], 'k-', zorder=20)
        ax2.grid()
    ...
    def plot_vehicles_log(s, vehnamelist):
        ...
        for veh in vehs:
            vehlinks = [str(l.name) if l != -1 else "not in network" for l in veh.log_link]
            plt.plot([veh.log_t[i] for i in lange(veh.log_t) if veh.log_state[i] != "home"],
                     [vehlinks[i] for i in lange(vehlinks) if veh.log_state[i] != "home"], c=veh.color, label=veh.name)
        plt.grid()
        plt.ylabel('link')
        plt.legend()
        plt.ylim([0, None])
    ...
    def vehicle_trip_to_pandas(s):
        ...
        for veh in s.W.VEHICLES.values():
            ...
            out.append(
                [veh.name, veh.orig.name, veh_dest_name, veh.departure_time * s.W.DELTAT, veh_state, veh.travel_time,
                 veh_ave_speed])
    ...
    def basic_to_pandas(s):
        ...
        out = [["total_trips", "completed_trips", "total_travel_time", "average_travel_time", "total_delay",
                "average_delay"],
               [s.trip_all, s.trip_completed, s.total_travel_time, s.average_travel_time, s.total_delay,
                s.average_delay]]
    ...
    def od_to_pandas(s):
        ...
        for o, d in s.od_trips.keys():
            out.append([o.name, d.name, s.od_trips[o, d], s.od_trips_comp[o, d], s.od_tt_free[o, d], s.od_tt_ave[o, d],
                        s.od_tt_std[o, d]])
    ...
    def link_to_pandas(s):
        ...
        out = [["link", "traffic_volume", "vehicles_remain", "free_travel_time", "average_travel_time",
                "stddiv_travel_time"]]
        for l in s.W.LINKS:
            out.append([l.name, s.linkc_volume[l], s.linkc_remain[l], s.linkc_tt_free[l], s.linkc_tt_ave[l],
                        s.linkc_tt_std[l]])
    ...
    def link_cumulative_to_pandas(s):
        ...
        for link in s.W.LINKS:
            for i in range(s.W.TSIZE):
                out.append(
                    [link.name, i * s.W.DELTAT, link.cum_arrival[i], link.cum_departure[i], link.traveltime_actual[i],
