...
class Node:
    ...
    def __init__(self, W, name, x, y, signal=[0], signal_offset=0, flow_capacity=None, auto_rename=False,
        ...
        # interLinkTransitionRequestDemand
        self.incoming_vehicles = []

        # incomingVehicleQueue（vertical queue）
        self.generation_queue = deque()

        # signalizedIntersection
        # signal=[0]ifSoThereIsNoSignal
        # whenThereIsASignal signal=[group0 blueHour，group1 blueHour，...]toBe
        self.signal = signal
        self.signal_phase = 0
        self.signal_t = 0
    ...
    def signal_control(self):
        ...
        """
        if self.signal_t > self.signal[self.signal_phase]:
            self.signal_phase += 1
            self.signal_t = 0
            if self.signal_phase >= len(self.signal):
                self.signal_phase = 0
        self.signal_t += self.W.DELTAT

        self.signal_log.append(self.signal_phase)

    def flow_capacity_update(self):
        """
        flow capacity updates.
        """
        if self.flow_capacity != None:
            if self.flow_capacity_remain < self.W.DELTAN * self.lanes:
                self.flow_capacity_remain += self.flow_capacity * self.W.DELTAT
        else:
            self.flow_capacity_remain = 10e10
    ...
    def generate(self):
        ...
        """
        outlinks0 = list(self.outlinks.values())
        if len(outlinks0):
            for i in range(sum([l.lanes for l in outlinks0])):
                if len(self.generation_queue) > 0:
                    ...
                    outlinks = list(self.outlinks.values())
    ...
    def transfer(self):
        ...
        """
        outlinks = []
        for outlink in {veh.route_next_link for veh in self.incoming_vehicles if veh.route_next_link != None}:
        ...
        for outlink in outlinks:
            if (len(outlink.vehicles) < outlink.lanes or outlink.vehicles[
                ...
                vehs = [
                    veh for veh in self.incoming_vehicles
...
class Link:
    ...
    def __init__(s, W, name, start_node, end_node, length, free_flow_speed=20, jam_density=0.2,
        ...
        s._speed = -1  # averageSpeedAcrossLinks
        s._density = -1
        s._flow = -1
        s._num_vehicles = -1  # NumberOfVehicles
        s._num_vehicles_queue = -1  # NumberOfVehiclesBelowFreeStreamSpeed
...
class Vehicle:
    ...
    def __init__(s, W, orig, dest, departure_time, name=None, route_pref=None, route_choice_principle=None,
        ...
        s.log_link = []  # link
        s.log_x = []  # position
        s.log_s = []  # headingDistance
        s.log_v = []  # currentSpeed
        s.log_lane = []  # lane
        s.color = (random.random(), random.random(), random.random())

        s.log_t_link = [[int(s.departure_time * s.W.DELTAT), "home"]]  # When entering a new link, only the time and link are saved. For route analysis
        ...
        if s.name in [veh.name for veh in s.W.VEHICLES.values()]:
            if auto_rename:
                ...
                raise ValueError(
                    f"Vehicle name {s.name} already used by another vehicle. Please specify a unique name.")
    ...
    def end_trip(s):
        ...
        s.link.traveltime_actual[int(s.link_arrival_time / s.W.DELTAT):] = (
                                                                                       s.W.T + 1) * s.W.DELTAT - s.link_arrival_time  # improvedEdgeBehavior todo: carefulExamination
        ...
        s.link = None
        s.x = 0
        s.arrival_time = s.W.T  # TODO: arrival_time IsAlsoExpressedAsATimeStepNeedsCorrection
    ...
    def route_search_all(s, infty=np.inf, noise=0):
        ...
        for link in s.W.LINKS:
            ...
            if s.W.ADJ_MAT[i, j]:
                ...
                s.adj_mat_time[i, j] = s.adj_mat_time[i, j] * n / (n + 1) + new_link_tt / (
...
class World:
    ...
    def __init__(W, name="", deltan=5, reaction_time=1, duo_update_time=600, duo_update_weight=0.5, duo_noise=0.01,
        ...
        W.TMAX = tmax  # SimulationTime（s）

        W.DELTAN = deltan  # vehicleFleetSize（veh）
        W.REACTION_TIME = reaction_time  # ReactionTime（s）

        W.DUO_UPDATE_TIME = duo_update_time  # RouteSelectionTimeInterval（s）
        W.DUO_UPDATE_WEIGHT = duo_update_weight  # RouteSelectionUpdateWeight
        W.DUO_NOISE = duo_noise  # NoiseDuringRouteSelection（Preventing extreme routing choices in a fully symmetric network）
        W.EULAR_DT = eular_dt  # Eular DefaultTimeDiscretizationWidthForTypeData
    ...
    def adddemand_area2area(W, x_orig, y_orig, radious_orig, x_dest, y_dest, radious_dest, t_start, t_end, flow=-1,
    ...
    def finalize_scenario(W, tmax=None):
        ...
        W.ADJ_MAT_LINKS = {}  # Adjacency matrix (dictionary) containing link objects
        for link in W.LINKS:
