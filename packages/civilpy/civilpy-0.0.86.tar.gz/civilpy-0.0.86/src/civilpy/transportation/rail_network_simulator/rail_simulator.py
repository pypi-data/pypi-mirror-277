        ...
        if flow_capacity != None:
            ...
            if number_of_lanes != None:
                ...
                self.lanes = math.ceil(
                    flow_capacity / 0.8)  # TODO: Adjustment required. Currently, the number of lanes is determined based on the assumption that one lane is 0.8 veh/s.
...
class Link:
    ...
    def __init__(self, W, name, start_node, end_node, length, free_flow_speed=20, jam_density=0.2,
        ...
        if self.lanes != number_of_lanes:
            raise ValueError(f"number_of_lanes must be an integer. Got {number_of_lanes} at {self}.")
        ...
        if jam_density != 0.2 and jam_density_per_lane != None:
            self.kappa = jam_density_per_lane * number_of_lanes
            warnings.warn(f"{self}: jam_density is ignored because jam_density_per_lane is set.", UserWarning)
...
class Vehicle:
    ...
    def add_dest(self, dest, order=-1):
        ...
        if self.mode == "taxi":
            ...
            raise ValueError(f"Vehicle {self.name} is not in taxi mode. Cannot add destination.")
...
class World:
    ...
    def exec_simulation(self, until_t=None, duration_t=None):
        ...
        for self.T in range(start_ts, end_ts):
            ...
            if self.print_mode and self.show_progress and self.T % self.show_progress_deltat_timestep == 0 and self.T > 0:
                self.analyzer.show_simulation_progress()
    ...
    def show_network(self, width=1, left_handed=1, figsize=(6, 6), network_font_size=10, node_size=6):
        ...
        for n in self.NODES:
            # //TODO - Update nodes to turn red if their route is closed
            plt.plot(n.x, n.y, "o", color=n.color, ms=node_size, zorder=10)
            if network_font_size > 0:
                plt.text(n.x, n.y, n.name, c=n.color, horizontalalignment="center", verticalalignment="top", zorder=20,
        ...
        for l in self.LINKS:
            ...
            # //TODO - Update nodes to turn red if their route is closed
            plt.plot([x1, x2], [y1, y2], color=l.color, lw=width, zorder=6, solid_capstyle="butt")
            if network_font_size > 0:
                plt.text(xmid1, ymid1, l.name, c=l.color, zorder=20, fontsize=network_font_size)
