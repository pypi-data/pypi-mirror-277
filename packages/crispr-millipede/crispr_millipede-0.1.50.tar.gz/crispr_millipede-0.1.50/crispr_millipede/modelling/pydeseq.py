...
def run_pydeseq2(encoding_reps: List[pd.DataFrame], enriched_pop_df_reads_colname: str, baseline_pop_df_reads_colname: str, start: Optional[int] = None, end: Optional[int] = None) -> pd.DataFrame:
    ...
    nucleotide_ids = [nt_id for nt_id in nucleotide_ids if int(nt_id[:nt_id.find(">")-1]) in range(start, end)] if ((start is not None) and (end is not None)) else nucleotide_ids
    ...
    if (start is None) and (end is None):
        subsequence_df = merged_encoding_reps.reset_index()
        subseq = [x[start:end] for x in list(subsequence_df["allele"])]

        subsequence_df["allele"] = subseq
        subsequence_df = subsequence_df.set_index("allele")
        subsequence_df = subsequence_df.groupby("allele").sum()
