    ...
    async def send_document(
        ...
        if os.path.isfile(document):
            async with aiofiles.open(document, 'rb') as f:
                ...
                form.add_field('document', await f.read(), filename=os.path.basename(document))
            return await self.api.execute('sendDocument', form=form)
        else:
            data = {
                'chat_id': str(chat_id),
                'document': document,
            ...
            return await self.api.execute('sendDocument', data=data)