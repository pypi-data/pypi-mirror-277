import os
import sys
sys.path.append(f"{os.environ["DAC_PATH"]}")
...
from utils.stack_utils import StackUtils as utils

class Bluedac_Lambda(AWS_Lambda.Function):

    def __init__(self, lambda_name, file_name, handler_name, stack, environment):
        super().__init__(
            stack,
            lambda_name,
    ...
    def apply_deployment_strategy(self, fun_name: str, stack, environment: str,
        ...
        alias = AWS_Lambda.Alias(
            self,
            f"{release_strategy["name"]}-{fun_name}",
            alias_name=f"{fun_name}-{new_version.version}-alias",
            version=new_version
        )
        ...
        codedeploy.LambdaDeploymentGroup(
            ...
            alarms=[failure_alarm] if failure_alarm else None,
        )