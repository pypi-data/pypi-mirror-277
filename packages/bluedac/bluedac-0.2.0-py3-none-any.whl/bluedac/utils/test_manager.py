...
import subprocess
import json
from utils.generate_unit_int_tests import generate_unit_int_tests
from utils.generate_load_tests import generate_load_tests

def test_manager(argument: str = ""):
    try:
    ...
    if argument == '--generate':
        if os.path.exists(f'{os.getcwd()}/cdk.json'):
            ...
            generate_unit_int_tests(os.getcwd(), stack)
            generate_load_tests(os.getcwd(), stack)
        else:
    ...
    # There's at least a parameter (user specified a type of test)
        ...
        subprocess.run(["python", "-m", "pytest",
                        f"--cov-fail-under={coverage}" if argument == "unit" else "", 
                        "--cov=./resources", 
                        f"tests/{argument}"])
    else:
        subprocess.run(["python", "-m", "pytest",
