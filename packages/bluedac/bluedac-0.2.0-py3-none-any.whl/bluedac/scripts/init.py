...
def init():
    ...
    # # -------------- Adding utils -------------- #
    try:
        os.mkdir(f"{os.getcwd()}/utils")
    except FileExistsError as file_error:
        print(file_error)

    # Copying stack_utils file from BlueDAC to project.
    shutil.copy(f"{os.environ["DAC_PATH"]}/utils/stack_utils.py", f"{os.getcwd()}/utils/stack_utils.py")
