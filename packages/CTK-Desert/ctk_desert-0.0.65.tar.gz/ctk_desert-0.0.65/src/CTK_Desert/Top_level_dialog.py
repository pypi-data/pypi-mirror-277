...
class Dialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        windll.dwmapi.DwmSetWindowAttribute(windll.user32.GetParent(self.winfo_id()), 35, byref(c_int(TITLE_BAR_HEX_COLORS[f"{ctk.get_appearance_mode().lower()}"])), sizeof(c_int))
    ...
    def new(self, tag: str = None, text: str = "Are you sure?", font: tuple = (FONT, 20), button_text: str = "Confirm", button_color: str | tuple = (LIGHT_MODE["accent"], DARK_MODE["accent"]), button_function: Callable = lambda: None):

        frame = ctk.CTkFrame(self, fg_color=(LIGHT_MODE["primary"], DARK_MODE["primary"]), width = 600, height = 400, corner_radius=15, border_width=1, border_color=button_color)
        #^ Label
        ...
        cancel_button = ctk.CTkButton(buttons_frame, text="Cancel", command=self._hide, font=font)
