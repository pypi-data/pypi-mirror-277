...
const plugin = {
    ...
    activate: (app, palette, notebookTracker, settingRegistry) => {
        ...
        // unconditionally for now, will soon add a setting to allow people to on a temporary basis
        settingRegistry.load('@jupyterlab/notebook-extension:tracker').then((nbSettings) => {
            nbSettings.set('windowingMode', 'none'),
                console.log('jupyterlab-gridwidth: turned off windowing mode');
        }, (err) => console.error(`jupyterlab-gridwidth: Could not turn off windowing mode: ${err}`));
        let command;
...
//# sourceMappingURL=lib_index_js.c0c8163c08730b2cc16d.js.map