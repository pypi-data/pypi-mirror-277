...
# from lino_xl.lib.trading.mixins import SalesDocument, ProductDocItem
from lino_xl.lib.excerpts.mixins import Certifiable
...
class Order(Certifiable, LedgerRegistrable, Voucher, RecurrenceSet,
            EventGenerator, Duplicable, ProjectRelated, ContactRelated,
            InvoiceGenerator):

    # Voucher.voucher_date : date of signature / offer
    # Voucher.entry_date : when agreement starts
    ...
    # subscription_type = dd.ForeignKey('invoicing.SubscriptionType')
    ...
    #     try:
    #         return Area.objects.get(journal=self.journal)
    #     except Area.DoesNotExist:
    #         raise Warning(_("No invoicing area configured for {}").format(self.journal))
    ...
    #     if self.project_id:
    #         return self.project.get_invoiceable_partner()
    #
    ...
    def full_clean(self, *args, **kwargs):
        ...
        if self.partner is None:
            # e.g. in Presto the project is a presto.Client (subclass of
            # contacts.Person), in Noi it is a tickets.Site
            p = self.project
            if isinstance(p, ContactRelated):
                self.company = p.company
                self.contact_person = p.contact_person
                self.contact_role = p.contact_role
            elif isinstance(p, rt.models.contacts.Company):
                self.company = p
            elif isinstance(p, rt.models.contacts.Person):
                self.contact_person = p
        if self.partner is None:
    ...
    def get_simple_parameters(cls):
        for f in super().get_simple_parameters():
            yield f
        yield 'journal__room'
    ...
    def get_generators_for_plan(cls, plan, partner=None):
        ...
        # qs = qs.filter(journal__invoicing_area=plan.invoicing_area)

        if plan.order is None:
            qs = qs.exclude(state=OrderStates.cancelled)
            qs = qs.exclude(state=OrderStates.draft)
        ...
        # qs = cls.objects.all()
        # dd.logger.info("20210727 c %s", qs)
        ...
        if found:
            for oi in self.items.all():
                kwargs = dict(
                    ...
                    qty=oi.qty * info.invoiceable_qty)
                i = invoice.add_voucher_item(**kwargs)
                i.product_changed(ar)
                # i.reset_totals(ar)
                if i.total_incl is None:
                    print("20210731 invoice item without amount:", i.__class__,
                          i)
                yield i
...
class Enrolment(dd.Model):
    ...
    # def create_worker_choice(self, text, ar=None):
    #     """
...
dd.update_field(Enrolment, 'overview', verbose_name=Order._meta.verbose_name)
...
dd.inject_field('accounting.Journal', 'room',
