...
class Models(dd.VirtualTable):
    ...
    def row_as_summary(cls, ar, obj, **kwargs):
        return escape(obj._meta.verbose_name_plural)
    ...
    def detail_action_column(self, obj, ar):
        ...
        if t is None or t.detail_action is None:
            return ''
        return t.detail_action.full_name()
    ...
    def rows(self, obj, ar):
        return obj.get_default_table().request(user=ar.get_user(),
...
class Inspector(dd.VirtualTable):
    ...
    parameters = dict(inspected=models.CharField(_("Inspected object"),
                                                 max_length=100,
                                                 blank=True),
                      show_callables=models.BooleanField(_("show callables"),
                                                         default=False))
        ...
        if isinstance(o, (list, tuple)):
            ...
            for k in dir(o):
                if not k.startswith('__'):
                    if not ar.quick_search or (ar.quick_search.lower()
                                               in k.lower()):
                        v = getattr(o, k)
                        if flt(v):
    ...
    def i_name(self, obj, ar):
        ...
        if ar.param_values.inspected:
            pv.update(inspected=ar.param_values.inspected + obj.prefix +
                      obj.name)
        else:
