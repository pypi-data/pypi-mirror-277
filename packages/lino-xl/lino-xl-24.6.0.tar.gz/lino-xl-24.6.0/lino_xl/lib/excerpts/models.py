...
class ExcerptType(mixins.BabelNamed, PrintableType, MailableType, Duplicable):
    ...
    primary = models.BooleanField(
        _("Primary"),
        default=False,
        help_text=_("Whether this is the default type to use for this model. "
    ...
    def after_ui_save(self, ar, cw):
        ...
        settings.SITE.build_site_cache(later=True)
...
class ExcerptTypes(dd.Table):
    ...
    # 20240504 Removed this because I don't see why it was here
    # @classmethod
    # def get_choices_text(self, obj, ar, field):
    #     if field.name == 'content_type':
    #         m = obj.model_class()
    #         if m is not None:
    #             return "%s (%s)" % (dd.full_model_name(m), obj)
...
class Excerpt(TypedPrintable, UserAuthored, Controllable,
    ...
    def preview(self, ar):
        if ar is None or self.owner is None:
            ...
            return ''
        lang = self.get_print_language() or translation.get_language()
    ...
    def before_printable_build(self, bm):
        super().before_printable_build(bm)
    ...
    def get_printable_context(self, ar=None, **kw):
        ...
        kw = super().get_printable_context(**kw)
        ...
        if ct is not None:
            ...
            for m in rt.models_by_base(bm):
                ...
                if not hasattr(m, an):
                    ...
                    # dd.logger.info("20230702 Added print action to %s", m)
