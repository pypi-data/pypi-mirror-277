...
from .choicelists import PlannerColumns, EventEvents, DisplayColors
...
class Event(Component, Ended, Assignable, TypedPrintable, Mailable, Postable,
    ...
    def on_create(self, ar):
        if self.event_type is None:
            ...
            self.event_type = ar.get_user().event_type or \
                settings.SITE.get_config_value('default_event_type')
    ...
    def get_generators_for_plan(cls, plan, partner=None):
        ...
        # an invoice.
        qs = super(Event, cls).get_generators_for_plan(plan, partner)
...
if dd.is_installed('notify'):
        ...
        def get_duration():
            ...
            if oracle:
                raise NotImplementedError(
                    ...
                    "See: https://docs.djangoproject.com/en/5.0/ref/models/fields/#durationfield for help."
                )
    ...
    @dd.schedule_often(every=scheduler_interval * 60)
...
class EntryChecker(Checker):
    ...
    def get_responsible_user(self, obj):
        return obj.user or super(EntryChecker, self).get_responsible_user(obj)
...
class Guest(Printable):  # TODO: rename the model to "Presence"
    ...
    def after_ui_save(self, ar, cw):
        kw = super(Guest, self).after_ui_save(ar, cw)
        evt = self.event
    ...
    def get_generators_for_plan(cls, plan, partner=None):
        ...
        # an invoice.
        qs = super(Guest, cls).get_generators_for_plan(plan, partner)
        max_date = plan.get_max_date()
        if max_date:
...
dd.inject_field('system.SiteConfig', 'default_color',
                DisplayColors.field("Color", default='Blue'))
