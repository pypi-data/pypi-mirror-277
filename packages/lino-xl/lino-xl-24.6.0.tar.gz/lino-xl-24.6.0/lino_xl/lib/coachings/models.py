...
class Coaching(UserAuthored, mixins.DateRange, dd.ImportedFields,
               ChangeNotifier):
    ...
    ending = dd.ForeignKey('coachings.CoachingEnding',
    ...
    def __str__(self):
        ...
        cl = self.client
        if self.user_id is None:
            return "{} {}".format(self.__class__._meta.verbose_name, cl)
        if cl.first_name:
    ...
    def as_summary_row(self, ar, **kw):
        return format_html("{} ({})", self.client.as_summary_row(ar, **kw), self.state.text)
        # return ar.obj2htmls(self.client) + " ({})".format(self.state.text)
...
dd.inject_field('users.User', 'coaching_type',
                dd.ForeignKey('coachings.CoachingType', blank=True, null=True))

dd.inject_field('users.User', 'coaching_supervisor',
