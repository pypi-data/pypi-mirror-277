...
from lino.modlib.users.mixins import UserPlan, My, UserAuthored
...
class Task(Runnable, UserAuthored):
    ...
    def run_task(self, ar):
        plan = rt.models.invoicing.Plan.create_user_plan(self.user)
    ...
    try:
        for it in rt.models.invoicing.Task.objects.filter(disabled=False):
            ...
            if not dd.plugins.invoicing.use_default_record_id:
                rt.models.system.SiteConfig.define_action(**{an: StartInvoicing(it)})
            rt.models.contacts.Partner.define_action(**{an: StartInvoicingForPartner(it)})
...
@dd.schedule_daily(class_name='invoicing.Task')
