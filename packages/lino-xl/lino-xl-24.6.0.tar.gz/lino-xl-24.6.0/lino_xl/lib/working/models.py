...
class Session(UserAuthored, Started, Ended, Workable, InvoiceGenerator):
    ...
    def __str__(self):
        ...
        date_fmt = settings.SITE.date_format_strftime
        time_fmt = settings.SITE.time_format_strftime
        if self.start_time and self.end_time:
            # tpl = _("{user} worked {when} on {what}")
            if self.end_date is None or self.start_date == self.end_date:
                when = "%s %s-%s" % (self.start_date.strftime(date_fmt),
                                     self.start_time.strftime(time_fmt),
                                     self.end_time.strftime(time_fmt))
            else:
                when = "%s %s-%s %s" % (
                    self.start_date.strftime(date_fmt),
                    self.start_time.strftime(time_fmt),
                    self.end_date.strftime(date_fmt),
                    self.end_time.strftime(time_fmt))
            ...
            when = self.start_date.strftime(date_fmt)
        else:
    ...
    def as_paragraph(self, ar):
        ...
        if not ar.is_obvious_field('ticket'):
            txt = format_html(_("{} working on {}"), txt, ar.obj2htmls(self.get_ticket()))
        return txt
