...
class Partner(Duplicable, ContactDetailsOwner, Polymorphic, AddressOwner,
              UploadController, Feasible, Printable, DateRangeObservable,
              VatSubjectable, Commentable):
    ...
    # def as_copyright_owner(self, ar=None):
    #     html = _("Copyright (c) {}".format(datetime.date.today().year))
    #     html += "".join([tostring(e) for e in self.get_name_elems(ar)])
    #     html += ", "
    #     html += ", ".join([tostring(e) for e in self.address_location_lines()])
    #     if self.email:
    #         html += """, <a href="{0}">{0}</a>""".format(self.email)
    #     elif self.url:
    #         html += """, <a href="{0}">{0}</a>""".format(self.url)
    #     html += "."
    #     # elems += AddressOwner.get_overview_elems(self, ar)
    #     # elems += ContactDetailsOwner.get_overview_elems(self, ar)
    #     # return "".join([tostring(e) for e in elems])
    #     yield html

    # def as_paragraph(self, ar):
    #     elems = []
    #     elems += self.get_name_elems(ar, sep=" ")
    #     elems.append(E.br())
    #     elems += join_elems(list(self.address_location_lines()), sep=E.br)
    #     # elems += AddressOwner.get_overview_elems(self, ar)
    #     # elems += ContactDetailsOwner.get_overview_elems(self, ar)
...
class Company(Partner):
    ...
    if dd.is_installed('products') and dd.plugins.products.barcode_driver is not None:
        barcode_identity = dd.PositiveIntegerField(null=True, blank=True, unique=True)
