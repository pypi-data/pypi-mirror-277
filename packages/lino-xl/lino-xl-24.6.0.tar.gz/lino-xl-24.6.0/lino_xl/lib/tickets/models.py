...
from .choicelists import TicketStates, Priorities, SiteStates
...
class QuickAssignTo(dd.Action):
    ...
    parameters = dict(assign_to=dd.ForeignKey("users.User"),
...
class Site(Referrable, Starrable, DateRange, Commentable):
    # ContactRelated for backwards compatibility
    class Meta:
        app_label = 'tickets'
    ...
    private = models.BooleanField(_("Confidential"), default=False)
    ...
    def unused_get_row_permission(self, ar, state, ba):
        """
        ...
        if ((self.group and
             not self.group.members.filter(user__id=ar.get_user().id).exists())
...
class SpawnTicket(dd.Action):
    ...
    parameters = dict(ticket_summary=models.CharField(
    ...
    def run_from_ui(self, ar, **kw):
        ...
        user = ar.get_user()
        child = rt.models.tickets.Ticket(user=user,
...
class Ticket(UserAuthored, mixins.CreatedModified, TimeInvestment, Assignable,
             Hierarchical, Votable, Starrable, Workable, Feasible,
             UploadController, Referrable, MemoReferrable):
    ...
    order = dd.ForeignKey(order_model, null=True, blank=True)
    # order = dd.ForeignKey(dd.plugins.invoicing.order_model, null=True, blank=True)
    ...
    def quick_assign_to(self, ar):
        ...
        if self.group:
            ...
            qs = settings.SITE.user_model.filter_active_users(qs,
                                                              dd.today(),
                                                              prefix="user__")
            for m in qs:
    ...
    def get_request_queryset(cls, ar, **filter):
        ...
        if not user.user_type.has_required_roles([TicketsReader]):
            return qs.none()
        q1 = Q(private=False, site__private=False)
        if user.is_anonymous:
            return qs.filter(q1)
    ...
    def memo2html(self, ar, text, **kwargs):
        ...
        e = self.obj2href(ar, "#{}".format(self.pk), **kwargs)
        return tostring(e)
    ...
    def obj2href(self, ar, text=None, **kwargs):
    ...
    def as_paragraph(self, ar, **kwargs):
        ...
        # if self.order:
        #     num += "@" + str(self.order)
        # if self.group and self.group.ref:
        #     num += "@" + self.group.ref
        more = []
...
# class TicketGroupChecker(Checker):
#     model = Ticket
#     verbose_name = _("Fill the new group field of tickets")
#
#     def get_checkdata_problems(self, obj, fix=False):
#         if obj.group is None and obj.site and obj.site.group:
#             yield (True, _("Fill group from site"))
#             if fix:
#                 obj.group = obj.site.group
#                 obj.full_clean()
#                 obj.save()
#
# TicketGroupChecker.activate()
#
...
def setup_memo_commands(sender=None, **kwargs):
    ...
    mp.add_suggester("#", Ticket.objects.order_by('id'), 'id', value=lambda obj, _: str(obj)[1:])
