...
MALE = Path(__file__).parent / 'luc.jpg'
FEMALE = Path(__file__).parent / 'ly.jpg'
...
class BeIdCardHolder(SSIN):
    ...
    nationality_text = models.CharField(_("Nationality (text)"),
                                        max_length=200,
                                        blank=True)
    ...
    def get_beid_diffs(self, attrs):
        ...
        for fldname, new in attrs.items():
            ...
            if old != new:
                diffs.append("{} : {} -> {}".format(str(fld.verbose_name),
                                                    dd.obj2str(old),
                                                    dd.obj2str(new)))
    ...
    def card_number_to_image_path(cls, card_number):
        ...
        if card_number:
            ...
            return Path(settings.MEDIA_ROOT, *parts)
        return settings.STATIC_ROOT / "contacts.Person.jpg"
    ...
    def make_demo_picture(self):
        ...
        # dst = settings.SITE.site_dir.child(
        ...
        if needs_update(src, dst):
            ...
            # shutil.copy() can cause PermissionError if dst exists and is owned by another user
