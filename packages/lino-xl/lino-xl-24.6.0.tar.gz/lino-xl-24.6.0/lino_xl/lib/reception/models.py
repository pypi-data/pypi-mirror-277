...
from lino.utils.format_date import naturaltime
...
GuestStates.present_states = set([
    GuestStates.present, GuestStates.waiting, GuestStates.busy,
    GuestStates.gone
])
...
dd.inject_field(
    ...
    models.DateTimeField(_("Present until"),
                         ...
                         null=True))
dd.inject_field(
    'cal.Guest', 'waiting_number',
    models.CharField(_("Waiting number"), max_length=20, blank=True))
...
def create_prompt_event(project,
                        partner,
                        user,
                        summary,
                        guest_role,
    ...
    ekw.update(event_type=settings.SITE.get_config_value('prompt_calendar'))
    ...
    rt.models.cal.Guest(
        ...
        waiting_since=now,
        waiting_number=waiting_number).save()
    #~ event.full_clean()
...
class CheckinVisitor(NotifyingAction):
    ...
    def get_notify_subject(self, ar, obj):
        return _("%(partner)s has started waiting for %(user)s") % dict(
            user=obj.event.user, partner=obj.partner)

    def get_notify_body(self, ar, obj):
        return self.get_notify_subject(ar, obj)
    ...
    def run_from_ui(self, ar, **kw):
        ...
        def ok(ar):
            ...
            if not obj.event.start_time:
                ar.debug("event.start_time has been set")
...
class ExpectedGuests(cal.Guests):
    ...
    def get_request_queryset(cls, ar):
        ...
        return qs.filter(waiting_since__isnull=True,
                         state__in=(GuestStates.invited,
...
class WaitingVisitors(Visitors):
    ...
    column_names = ('since:15 partner event__user:10 waiting_number:5 '
    ...
    def position(self, obj, ar):
        # This might become useless after 20221022.
...
class MyWaitingVisitors(My, WaitingVisitors):
    ...
    column_names = ('since partner waiting_number '
                    'event__summary workflow_buttons')
