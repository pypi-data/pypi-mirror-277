...
# from lino_xl.lib.accounting.models import DebitOrCreditField
from lino_xl.lib.accounting.utils import DC, ZERO, Balance, myround
from lino_xl.lib.accounting.choicelists import DC, TradeTypes
#from lino_xl.lib.accounting.models import Voucher
from lino_xl.lib.accounting.mixins import PeriodRange
...
class Item(StructuredReferrable, BabelDesignated):
    ...
    mirror_ref = models.CharField(_("Mirror"),
                                  max_length=ref_max_length,
                                  blank=True,
                                  null=True)


Item.set_widget_options('dc', preferred_width=20)
...
class Collector(object):
    ...
    def addann(self, kw, name, dc, flt):
        mvts = rt.models.accounting.Movement.objects.filter(**flt)
        if dc == DC.debit:
        ...
        mvts = mvts.annotate(
            total=Sum('amount', output_field=dd.PriceField(decimal_places=14)))
        ...
        kw[name] = Subquery(mvts,
                            output_field=dd.PriceField(decimal_places=14))
...
class PartnerEntry(ReportEntry):
    ...
    def get_collectors(cls):
        ...
        for tt in TradeTypes.get_list_items():
            ...
            if isinstance(a,
                          rt.models.accounting.Account):  # might be MissingAccount
                yield TradeTypeCollector(tt,
                                         cls,
                                         'partner',
                                         'partner',
                                         account=a)
    ...
    def setup_parameters(cls, fields):
        fields.setdefault('trade_type', TradeTypes.field(blank=True))
...
class Report(UserPlan, PeriodRange, Certifiable, Story):
    ...
    def __str__(self):
        return "Report #{} ({}..{})".format(self.pk, self.start_period,
                                            self.end_period)
    ...
    def get_outer_queryset(self, coll):
        ...
        Outer objects can be accounting or analytical accounts, partners or sheet
        items.
    ...
    def get_story(self, ar, header_level=None):
        ...
        for tt in TradeTypes.get_list_items():
            yield PartnerEntriesByReport.request(
                self, parent=ar, param_values=dict(trade_type=tt))
...
class Reports(dd.Table):
    model = 'sheets.Report'
    # detail_layout = 'accounting.ReportDetail'
    detail_layout = """
    start_period end_period printed
    body
    """
...
from lino_xl.lib.accounting.ui import Movements
