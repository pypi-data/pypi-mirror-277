...
class ArbitrageFinderTriangleB3(ArbitrageFinderBaseTriangle):
    def get_combos(self) -> List[Any]:
        combos = []
        CC = self.CCm.by_params(exchange="bancor_v3")

        if self.context.LIMIT_BANCOR3_FLASHLOAN_TOKENS:
            flashloan_tokens = list(set(CC.tknys()) & set(self.flashloan_tokens))
        else:
            flashloan_tokens = CC.tknys()

        bancor_curves_dict = {tkn: CC.by_pairs(f"{self.context.BNT_ADDRESS}/{tkn}").curves for tkn in flashloan_tokens}
        ...
        for tkn0, tkn1 in [(tkn0, tkn1) for tkn0, tkn1 in product(flashloan_tokens, flashloan_tokens) if tkn0 != tkn1]:
            ...
            all_curves = list(set(self.CCm.by_pairs(f"{tkn0}/{tkn1}")) | set(self.CCm.by_pairs(f"{tkn1}/{tkn0}")))
            carbon_curves = [curve for curve in all_curves if ExchangeType(curve.params.exchange_type) == ExchangeType.CARBON_V1]
            other_curves = [curve for curve in all_curves if ExchangeType(curve.params.exchange) != ExchangeType.CARBON_V1]
            ...
            combos += [(tkn1, miniverse) for miniverse in miniverses]
