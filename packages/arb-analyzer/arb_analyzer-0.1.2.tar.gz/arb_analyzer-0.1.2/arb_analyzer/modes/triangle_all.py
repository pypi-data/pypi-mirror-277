"""
Defines the triangle-all arbitrage finder class
...
from .base_triangle import ArbitrageFinderBaseTriangle


logger = logging.getLogger(__name__)


class ArbitrageFinderTriangleAll(ArbitrageFinderBaseTriangle):
    def get_combos(self) -> List[Any]:
        combos = []

        for flt in self.flashloan_tokens:
            # Get the Carbon pairs
            carbon_pairs = sort_pairs(set([curve.pair for curve in self.CCm.curves if ExchangeType(curve.params.exchange_type) == ExchangeType.CARBON_V1]))
            ...
            logger.debug(f"len(all_relevant_pairs) {len(all_relevant_pairs)}")

            # Generate triangle groups
            triangle_groups = get_triangle_groups(flt, x_y_pairs)
            logger.debug(f"len(triangle_groups) {len(triangle_groups)}")

            # Get pair info for the cohort
            all_relevant_pairs_info = get_all_relevant_pairs_info(self.CCm, all_relevant_pairs)
...
def get_all_relevant_pairs_info(CCm, all_relevant_pairs):
    ...
    for pair in all_relevant_pairs:
        pair_curves = CCm.by_pairs(pair).curves
        carbon_curves = [curve for curve in pair_curves if ExchangeType(curve.params.exchange_type) == ExchangeType.CARBON_V1]
        other_curves = [curve for curve in pair_curves if ExchangeType(curve.params.exchange_type) != ExchangeType.CARBON_V1]
