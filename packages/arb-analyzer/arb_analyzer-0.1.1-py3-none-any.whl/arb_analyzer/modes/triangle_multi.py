...
class ArbitrageFinderTriangleMulti(ArbitrageFinderBaseTriangle):
    def get_combos(self) -> List[Any]:
        combos = []
        all_carbon_curves = self.CCm.by_params(exchange="carbon_v1").curves
        ...
        for flt in self.flashloan_tokens:
            ...
            for non_flt_carbon_curve in non_flt_carbon_curves:
                ...
                carbon_curves = self.CCm.by_pairs(f"{tknx}/{tkny}").by_params(exchange="carbon_v1").curves
                if len(carbon_curves) == 0:
                    continue

                filtered_curves = {tkn : set(self.CCm.filter_pairs(onein=tkn)) for tkn in [tknx, tkny, flt]}
                y_match_curves = self.CCm.by_pairs(filtered_curves[tknx] & filtered_curves[flt]).curves
                x_match_curves = self.CCm.by_pairs(filtered_curves[tkny] & filtered_curves[flt]).curves
