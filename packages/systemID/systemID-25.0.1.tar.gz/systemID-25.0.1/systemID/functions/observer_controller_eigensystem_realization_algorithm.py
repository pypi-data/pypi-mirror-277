...
def observer_controller_eigensystem_realization_algorithm(markov_controller_parameters, state_dimension, **kwargs):
    """
        Purpose:
    ...
    p = kwargs.get('p', int(np.floor((len(markov_controller_parameters) - 1) / 2)))
    p = min(p, int(np.floor((len(markov_controller_parameters) - 1) / 2)))
    q = kwargs.get('q', p)
    q = min(q, int(np.floor((len(markov_controller_parameters) - 1) / 2)))

    # Dimensions
    (output_dimension, input_dimension) = markov_controller_parameters[0].shape

    # Hankel matrices H(0) and H(1)
    H0 = np.zeros([p * (output_dimension + input_dimension), q * (input_dimension + output_dimension)])
    H1 = np.zeros([p * (output_dimension + input_dimension), q * (input_dimension + output_dimension)])
    for i in range(p):
        for j in range(q):
    ...
    A_id = np.matmul(LA.pinv(Op), np.matmul(H1, LA.pinv(Rq)))
    B_id = Rq[:, 0:input_dimension]
    G_id = Rq[:, input_dimension:input_dimension + output_dimension]
    C_id = Op[0:output_dimension, :]
    F_id = Op[output_dimension:output_dimension + input_dimension, :]
    ...
    return A, B, C, D, F, G, H0, H1, R, Sigma, St, Rn, Sigman, Snt, Op, Rq, MAC, MSV
