...
from GeneratePolynomialIndex import generatePolynomialIndex
from GeneratePolynomialBasisFunctions import generatePolynomialBasisFunctions
from ClassSignal import Signal, subtract2Signals


def sparseApproximation2_TwoBodyProblemv2(signals, dx0s, input_signals, order, max_order, post_treatment, l1, l2, alpha, delta, max_iterations, TU, shift):
    ...
    index0 = generatePolynomialIndex(dimension + 1, order, post_treatment, max_order)
    index0_length, _ = index0.shape
    basis_functions_temp = generatePolynomialBasisFunctions(dimension + 1, index0)
    rep = []
    ...
    for k in range(dimension):
        ...
        for s in range(number_signals):
            ...
            def Dynamics(X, t):
                ...
                dXdt[1] = -l1 * zv + x[k]
                dXdt[2] = zl_dot
                dXdt[3] = - (l1 + l2) * zl_dot - l1 * l2 * zl + x[k]
                dXdt[4] = zu_dot
                dXdt[5] = - (l1 + l2) * zu_dot - l1 * l2 * zu + u[k]
                ...
                for i in range(index_length):
                    ...
                    dXdt[6 + i] = Psi_dot
                    dXdt[6 + index_length + i] = - (l1 + l2) * Psi_dot - l1 * l2 * Psi + basis_functions[i](np.concatenate((x, np.array([r]))))
                    #dXdt[6 + index_length + i] = - (l1 + l2) * Psi_dot - l1 * l2 * Psi + basis_functions[i](x)
        ...
        y1 = Y[:, k:k+1] + 1e-6 * np.random.randn(sum(numbers_steps), 1)
        # plt.figure(101)
        # plt.plot(Y[:, k:k + 1])
        # plt.plot(y1)
        # plt.show()

        # print(min(np.abs(Y[:, k:k + 1])))
        # print(max(np.abs(Y[:, k:k + 1])))
        #theta = LA.lstsq(PSI[:, :, k], y1)[0]
        theta = np.matmul(LA.pinv(PSI[:, :, k]), Y[:, k:k+1])
        print('Rank Psi = ', LA.matrix_rank(PSI[:, :, k]))
        THETA_LS[:, k:k+1] = theta

        # errLS_2norm = LA.norm(Y[:, k:k+1] - np.matmul(PSI[:, :, k], theta))
        # errLS_meanabs = np.mean(np.abs(Y[:, k:k+1] - np.matmul(PSI[:, :, k], theta)))
        # errLS_maxabs = np.max(np.abs(Y[:, k:k + 1] - np.matmul(PSI[:, :, k], theta)))
        # errLS_rmse = errLS_2norm / np.sqrt(sum(numbers_steps))
        # ctrue = np.zeros([index_length, 1])
        # if k == 0:
        #     ctrue[25, 0] = -1
        # if k == 1:
        #     ctrue[34, 0] = -1
        # if k == 2:
        #     ctrue[52, 0] = -1
        # errTrue_2norm = LA.norm(Y[:, k:k+1] - np.matmul(PSI[:, :, k], ctrue))
        # errTrue_meanabs = np.mean(np.abs(Y[:, k:k+1] - np.matmul(PSI[:, :, k], ctrue)))
        # errTrue_maxabs = np.max(np.abs(Y[:, k:k + 1] - np.matmul(PSI[:, :, k], ctrue)))
        # errTrue_rmse = errTrue_2norm / np.sqrt(sum(numbers_steps))
        # print('errLS_2norm', errLS_2norm)
        # print('errLS_meanabs', errLS_meanabs)
        # print('errLS_maxabs', errLS_maxabs)
        # print('errLS_rmse', errLS_rmse)
        # print('errTrue_2norm', errTrue_2norm)
        # print('errTrue_meanabs', errTrue_meanabs)
        # print('errTrue_maxabs', errTrue_maxabs)
        # print('errTrue_rmse', errTrue_rmse)
    ...
    return interp_data, interp_inputs, index, THETA_LS, THETA_SPARSE, ZX, ZV, ZL, ZL_dot, ZU, ZU_dot, PSI, LS_signals, Sparse_signals, C
