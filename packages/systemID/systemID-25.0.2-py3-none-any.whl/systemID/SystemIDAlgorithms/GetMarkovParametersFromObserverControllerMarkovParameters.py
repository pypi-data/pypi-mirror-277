...
def getMarkovParametersFromObserverControllerMarkovParameters(observer_controller_markov_parameters, **kwargs):
    """
    Purpose:
    ...
    # Dimensions
    output_dimension, input_dimension = observer_controller_markov_parameters[0].shape

    # Get D
    markov_parameters = [observer_controller_markov_parameters[0]]

    # Number of Observer Markov parameters
    number_observer_markov_parameters = len(observer_controller_markov_parameters)
    ...
    for i in range(1, min(number_observer_markov_parameters, number_of_parameters)):
        hk1.append(observer_controller_markov_parameters[i][:, 0:input_dimension])
        hk2.append(-observer_controller_markov_parameters[i][:, input_dimension:output_dimension + input_dimension])
