...
class Artifact:
    ...
    def create(
        ...
        def composition_sync():
            ...
            if not artifact:
                return
            return Client.upload_sync(local_path=path, policy=artifact.get("upload_policy"))

        async def composition_async():
            artifact = Client.post("/app/artifacts", wait_response=True, json=json)
            if not artifact:
                return
            return await Client.upload_async(local_path=path, policy=artifact.get("upload_policy"))

        if is_running_event_loop():
            Client.execute(composition_sync, wait_response=wait_response, is_async_method=False)
        else:
            Client.execute(composition_async, wait_response=wait_response, is_async_method=True)
        return path
