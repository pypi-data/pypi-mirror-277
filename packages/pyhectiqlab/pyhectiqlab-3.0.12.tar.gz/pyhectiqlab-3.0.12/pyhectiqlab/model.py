...
class Model:
    ...
    def exists(
        ...
        Args:
            ...
            project (str, optional): Project of the model. If None, the project of the current run is used. Default: None.
        """
        res = Model.retrieve(name=name, version=version, project=project, fields=["id"])
    ...
    def download(
        ...
        logging.info(f"⏳ Downloading model {name}-{version} to {path}...")
        if is_running_event_loop():
            Model._client.execute(composition_sync, wait_response=wait_response, is_async_method=False)
        else:
            Model._client.execute(composition_async, wait_response=wait_response, is_async_method=True)
