...
def load_credentials(name: Optional[str] = None) -> Union[None, str]:
    path = getenv("HECTIQLAB_CREDENTIALS")
...
class Auth(httpx.Auth):
    ...
    @property
    def api_key(self):
        return getenv("HECTIQLAB_API_KEY") or self.data.get("value")
    ...
    def refresh_token(self):
        response = httpx.post(f"{getenv('HECTIQLAB_API_URL')}/app/auth/auth-api-key", headers=self.auth_header)
    ...
    def build_refresh_request(self):
        ...
        return httpx.Request("POST", f"{getenv('HECTIQLAB_API_URL')}/app/auth/auth-api-key", headers=self.auth_header)
