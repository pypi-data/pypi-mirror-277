...
class Dataset:
    ...
    @clisync.include()
    def exists(name: str, version: str, project: str):
        ...
        Args:
            ...
            version (str): Version of the dataset
        """
        body = Dataset.retrieve(name=name, project=project, version=version, fields=["id"])
    ...
    def list(
        ...
        keep_latest_version: Optional[bool] = None,
        ...
        params = {
            ...
            "search": search,
            "author": author,
            "keep_latest_version": keep_latest_version if keep_latest_version is not None else True,
            "fields": fields,
            "page": page,
            "limit": limit,
            "order_by": order_by,
    ...
    def upload(id: str, source: str):
        ...
        def upload_files(paths, policies):
            if is_running_event_loop():
                return Dataset._client.upload_many_sync(paths=paths, policies=policies)
            return asyncio.run(Dataset._client.upload_many_async(paths=paths, policies=policies))
        ...
        for batch in batched(all_files, batch_size):
            ...
            upload_files(paths=batch, policies=sorted_files)
    ...
    def download(
        ...
        if not Dataset.exists(name=name, version=version, project=project):
            logging.error(f"Dataset {name}-{version} not found.")
            return
        ...
        logging.info(f"⏳ Downloading dataset {name}-{version} to {path}...")
        if is_running_event_loop():
            res = Dataset._client.execute(composition_sync, wait_response=wait_response, is_async_method=False)
        else:
            res = Dataset._client.execute(composition_async, wait_response=wait_response, is_async_method=True)
        print(res)
        logging.info(f"✅ Done.")
        return path
