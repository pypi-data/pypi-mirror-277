...
class Client:
    ...
    def upload_fragment_sync(local_path: str, policy: dict, chunk_size: int = 1024 * 1024 * 32) -> None:
        ...
        with tqdm.tqdm(
            ...
            while upload.finished == False:
                ...
                pbar.update(upload.bytes_uploaded - bytes_uploaded)
    ...
    @staticmethod
    def download_many_sync(urls: List[str], local_paths: List[str], num_bytes: List[int]) -> None:
    ...
    @staticmethod
    async def download_many_async(urls: List[str], local_paths: List[str], num_bytes: List[int]) -> None:
    ...
    def request(call: str, url: str, wait_response: bool = False, **kwargs) -> ResponseType:
        ...
        if not Client.online():
            logging.error(f"`{call}` not executed. Dry (offline) mode.")
            return
