...
from .config import PANDOCPATH, WKHTMLTOXPATH
...
class PandocOperations(object):
    ...
    def md_pdf(self, md_path, pdf_path):
        system = f'{PANDOCPATH}  {md_path} -o {pdf_path}'
        print(system)
        os.system(system)
...
# os.system(f'{PANDOC}  {md_path} -pdf {pdf_path}')
import subprocess


def convert_docx_to_pdf(docx_file, pdf_file):
    command = f"{PANDOCPATH} {docx_file} -o {pdf_file} --pdf-engine=xelatex"
...
def convert_docx_to_pdf_2(docx_file, pdf_file):
    doc = docx.Document(docx_file)
    pdf = canvas.Canvas(pdf_file)
    # 加载字体
    pdfmetrics.registerFont(TTFont('Arial', r'F:\ContentPro\PythonPro\automaticoffice\字体\youyuan.TTF'))
