...
def run_test(number, title, XHTML=None, RST=None, HTML=None):
    ...
    tpl = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                       'test_template.odt')
    context = dict(locals())
    context.update(
...
#~ # 2 :
run_test(
    2, "List items are not rendered (Appy 0.6.6)", '''
<div class="document">
...
# 3 :
run_test(3,
         "Same as 2, but using `restify` to make the XHTML",
         RST=u'''
...
run_test(5,
         "SAX parser error on Python 2.6",
         RST=u"""
Some **bold** and some *italic* text.
...
#~ # 9 :
run_test(9,
         "Same as 2, but without wrapping <DIV>",
         HTML='''
<p>Some <strong>bold</strong> and some <em>italic</em> text.</p>
<p>A new paragraph.</p>
<p>A list with three items:</p>
