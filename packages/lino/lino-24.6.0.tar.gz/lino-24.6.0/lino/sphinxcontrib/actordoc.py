...
import rstgen
from rstgen.sphinxconf.insert_input import Py2rstDirective
...
def fields_table(fields):
    ...
    def rowfmt(f):
        cells = [f.name, fieldtype(f), f.verbose_name, f.help_text]
...
IGNORED_ACTIONS = (actions.ShowTable, actions.SubmitDetail, actions.ShowDetail,
                   actions.DeleteSelected, actions.ShowInsert,
...
class ddrefRole(XRefRole):
    ...
    def __call__(self,
        ...
        self._lineno = lineno
        return XRefRole.__call__(self, typ, rawtext, text, lineno, inliner,
                                 options, content)
...
class TempDataDirective(Directive):
    ...
    def get_current_value(cls, env):
        return env.temp_data.get(cls.temp_data_key, cls.get_default_value(env))
        ...
        with translation.override(self.language):
            ...
            for an in actor_names:
                ...
                desc = "**{0}** (:class:`{1} <{2}>`)".format(
                    force_str(cls.label), cls.__name__,
    ...
    def get_rst(self):
        with translation.override(self.language):
            ...
            if len(names) > 1:
                ...
                field_names = None
            return analyzer.show_fields(names[0], field_names=field_names)
    ...
    def get_rst(self):
        ...
        with translation.override(self.language):
            ...
            if issubclass(cls, models.Model):
                ...
                model_reports += [
                    r for r in kernel.slave_tables if r.model is cls
                ]
                s += rstgen.boldheader(
...
def menupath_role(typ, rawtext, text, *args, **kwargs):
    ...
    return MenuSelection('menuselection', rawtext, text, *args, **kwargs)
