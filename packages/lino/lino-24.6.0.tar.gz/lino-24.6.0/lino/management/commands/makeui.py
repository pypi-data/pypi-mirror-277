...
from lino.management.commands.makehelp import GeneratingCommand
...
class Command(GeneratingCommand):
    ...
    def handle(self, *args, **options):
        ...
        QXAPP_PATH = os.path.join(settings.QOOXDOO_PATH, 'lino_apps',
                                  settings.SITE.project_name)
        args = [QXAPP_PATH]
    ...
    def generate_files(self):
        ...
        context = dict(py2js=jsgen.py2js,
                       jsgen=jsgen,
                       a2class=a2class,
                       models=models,
                       settings=settings)
        for fn in ('config.json', 'generate.py', 'Manifest.json'):
            ...
            self.generate(fn + '.tmpl', join(self.output_dir, fn), **context)
            #~ join(self.output_dir,'qxapp',fn),**context)

        self.generate_class_file('lino.Application', 'Application.js.tmpl',
                                 **context)
        #~ self.generate('Application',application_lines(self))
        for rpt in (kernel.master_tables + kernel.slave_tables +
            ...
            for a in rpt.get_actions():
                ...
                if isinstance(a, actions.ShowDetail):
                    ...
                    self.generate_class_file(a2class(a),
                                             ...
                                             **context)
        for d in (join(self.output_dir, 'source',
                       'translation'), join(self.output_dir, 'source',
                                            'script'),
