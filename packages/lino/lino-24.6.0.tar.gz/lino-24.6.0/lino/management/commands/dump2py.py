...
''')
        ...
        #appnames = {
        #    p.app_name.split('.')[0]
        #    for p in settings.SITE.installed_plugins if (_ := lambda x: [
        #        1 for p in settings.SITE.migratable_plugin_prefixes + ['lino']
        #        if x.app_name.startswith(p)
        #    ])(p)
        #}
        #os_dict = {
        #    appname: import_string(f"{appname}.__version__")
        #    for appname in appnames
        #}
        # double-dump test would fail if OTHER_SOURCES is in default repr() with
        # arbitrary ordering of keys.
        #os_dict = repr(dict(sorted(os_dict.items())))
        #self.stream.write(f"OTHER_SOURCES = {os_dict}\n")
        ...
        while len(unsorted) and hope:
            ...
            for model in unsorted:
                deps = set([
                    ...
                    if f.remote_field is not None and f.remote_field.model
                    is not model and f.remote_field.model in unsorted
                ])
    ...
    def handle(self, *args, **options):
        ...
        if os.path.exists(self.output_dir):
            if options['overwrite']:
                ...
                raise CommandError("Specified output_dir %s already exists. "
                                   "Delete it yourself if you dare!" %
                                   self.output_dir)
        else:
        ...
        if self.database_errors:
            raise CommandError(
                ...
                "The dump in %s is not complete.", self.database_errors,
                self.output_dir)
