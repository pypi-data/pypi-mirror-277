...
def check_json_result(response, expected_keys=None, msg=''):
    ...
    if response.status_code != 200:
        raise Exception("Response status ({0}) was {1} instead of 200".format(
...
def show_workflow(actions, all=False, language=None):
    ...
    def doit():
        cells = []
        cols = [
            "Action name", "Verbose name", "Help text", "Target state",
            "Required states"
        ]  # , "Required roles"]
        for a in actions:
            ht = a.help_text or ''
    ...
    def doit(ar):
        ...
        for mi in mnu.walk_items():
            if mi.bound_action:
                ...
                # item += " ({})".format(mi)
                item += " : "
                if isinstance(mi.bound_action.action,
                              ShowTable) and not mi.params:
                    ...
                    try:
                        ...
                        if mt.model:
                            kwargs[constants.URL_PARAM_DISPLAY_MODE] = constants.DISPLAY_MODE_LIST
                            url = settings.SITE.buildurl(baseurl, **kwargs)
                            response = test_client.get(url)
                            result = check_json_result(
                                response, None,
...
def show_change_watchers():
    ...
    for m in get_models():
        ...
        if ws:
            rows.append([full_model_name(m), ws.master_key,
                ' '.join(sorted(ws.ignored_fields))])
