            ...
            if True:  # model._meta.managed:
                ...
                for base in model.__mro__:
                    if not issubclass(base,
                                      (models.Model, dd.Model)) or base in [
                                          models.Model, dd.Model
                    ...
                    if len(subbases) > 1 and len(base._meta.concrete_fields):
                        trees = sorted(list({
                            item
                            for item in trees if item.endswith(model.__name__)
                        }),
                                       key=lambda v: len(v))
...
def layout_fields(ba):
    ...
    elems = [
        str(f.name) for f in lh._data_elems if not isinstance(f, DummyField)
    ]
    return ', '.join(elems)
