...
class DjangoManageTestCase(DjangoTestCase, CommonTestCase):
    ...
    @unittest.skip(
        "Broken. I guess this has come because callbacks are now called again from the beginning...."
    )
    def check_callback_dialog(self, meth, username, url, dialog, *data,
                              **extra):
...
class RestoreTestCase(TransactionTestCase):
    ...
    def test_restore(self):
        ...
        for v in self.tested_versions:
            run_args = ["tests/dumps/{}/restore.py".format(v), "--noinput"]
