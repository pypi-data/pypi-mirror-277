...
Example values in :doc:`/topics/utils`.
...
class LookupConverter(Converter):
    ...
    def lookup(self, value, **kw):
        ...
        if isinstance(value, model):
            return value
        # raise Exception("20240220 {} is not a {}".format(value, model))
...
class ManyToManyConverter(LookupConverter):
    ...
    def convert(self, **kw):
        ...
        if values is not None:
            ...
            l = [self.lookup(value) for value in values.split(self.splitsep)]
...
class Chooser(FieldChooser):
    ...
    def __init__(self, model, field, meth):
        ...
        if hasattr(model, "create_%s_choice" % field.name):
            # There is a create_FOO_choice method
    ...
    def __str__(self):
        return "Chooser(%s.%s,%s)" % (self.model.__name__, self.field.name,
                                      self.context_params)

    def create_choice(self, obj, text, ar=None):
        # Call the create_FOO_choice method
    ...
    def get_request_choices(self, ar, tbl):
        ...
        if tbl.master_field is not None:
            ...
            master = tbl.master
            if (mt := rqdata.get(constants.URL_PARAM_MASTER_TYPE)) is not None:
                try:
                    master = ContentType.objects.get(pk=mt).model_class()
                except ContentType.DoesNotExist:
                    # master = None
                    raise BadRequest("Invalid content type {}={}".format(
                        constants.URL_PARAM_MASTER_TYPE, mt))

            pk = rqdata.get(constants.URL_PARAM_MASTER_PK, None)
            if pk and master:
                if 'get_master_instance' in tbl.__dict__:
                    kw[tbl.master_field.name] = tbl.get_master_instance(
                        ar, master, pk)
                else:
                    try:
                        kw[tbl.master_field.name] = master.objects.get(pk=pk)
                    except ValueError:
                        raise Exception("Invalid primary key %r for %s", pk,
                                        master.__name__)
...
def uses_simple_values(holder, fld):
    ...
    if not hasattr(fld, 'name'):
        raise Exception("DEBUG", holder, fld, fld.display_mode)
