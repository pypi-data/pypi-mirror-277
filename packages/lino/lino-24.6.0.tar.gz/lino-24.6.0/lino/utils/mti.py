...
See also :class:`lino.mixins.polymorphic`.
    ...
    def collect(self,
        ...
        if collect_related:
            ...
            related_objects = [
                ...
                and not f.concrete and f.model is model  ## local_only
            ]
...
def delete_child(obj, child_model, ar=None, using=None):
    ...
    msg = child._lino_ddh.disable_delete_on_object(obj, ignore_models)
    if msg:
...
def insert_child(obj, child_model, full_clean=False, **attrs):
    ...
    if parent_link_field is None:
        raise ValidationError(
            str("A %s cannot be parent for a %s" %
