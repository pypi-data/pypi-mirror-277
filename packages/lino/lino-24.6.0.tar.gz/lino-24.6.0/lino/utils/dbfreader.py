...
def unpack_long(number):
    return ord(number[0]) + 256 * (ord(number[1]) + 256 *
                                   (ord(number[2]) + 256 * ord(number[3])))


def unpack_long_rev(number):
    return ord(number[3]) + 256 * (ord(number[2]) + 256 *
                                   (ord(number[1]) + 256 * ord(number[0])))
...
def display_info(f):
    ...
    for field in f.get_fields():
        print("%s: %s (%d)" %
              (field.get_name(), field.get_type_name(), field.get_len()))
