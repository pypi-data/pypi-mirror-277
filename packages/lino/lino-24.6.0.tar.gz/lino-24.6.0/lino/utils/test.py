...
class CommonTestCase(unittest.TestCase):
    ...
    def assertEquivalent(self, a, b, report_plain=False):
        ...
        if a == b:
            return
        if False:  # this can help to find the differing line
            ta = a.strip().splitlines()
            tb = b.strip().splitlines()
...
class DemoTestCase(PythonTestCase, CommonTestCase):
    ...
    project that has been initialized with :cmd:`pm prep`.
    ...
    def login(self, username, pwd):
        ...
        from django.conf import settings
        url = settings.SITE.kernel.editing_front_end.build_plain_url(
            "api/about/About/-99998")
    ...
    def demo_get(self, username, url_base, json_fields, expected_rows,
                 **kwargs):
        from django.conf import settings
        case = HttpQuery(username, url_base, json_fields, expected_rows,
                         kwargs)
        ...
        user = settings.SITE.user_model.objects.get(username=case.username)
        result = self.check_json_result(response, case.json_fields,
        ...
        if num is not None:
            ...
            if result['count'] not in num:
                msg = "%s got %s rows instead of %s" % (url, result['count'],
                                                        num)
