        ...
        # except (ValidationError,ObjectDoesNotExist,IntegrityError), e:
            ...
            if True:
                if not settings.SITE.loading_from_dump:
                    ...
                    # order
                    logger.warning("Failed to save %s from manual fixture:" %
                                   obj2str(obj))
                    raise
            deps = [
        ...
        while self.saved and self.save_later:
            ...
            for msg_objlist in self.save_later.values():
                for objlist in msg_objlist.values():
                    try_again += objlist
            logger.info("Trying to save %d deferred objects.", len(try_again))
            self.save_later = {}
            self.saved = 0
...
def install_migrations(self, loader):
    ...
    if self.migration_class is not None:
        ...
        migrator = Migrator(self, loader)

    # if (oss := globals_dict.get('OTHER_SOURCES', None)) is not None:
    #     for p in settings.SITE.installed_plugins:
    #         for step in p.get_migration_steps(oss):
    #             getattr(p, "migrate_from_" + step)(migrator, globals_dict)
    ...
    while True:
        ...
        if m is not None:
            ...
            msg = "Migrating from version %s to %s" % (from_version,
                                                       to_version)
            if m.__doc__:
                msg += ":\n" + m.__doc__
            logger.info(msg)
