...
# from lino.utils import DelayedValue
from lino.core.permissions import Permittable
from lino.core.permissions import make_static_permission_handler
...
class Variable(Value):
    ...
    def __init__(self, name, value):
        ...
        if name is None:
            ...
            name = str(name)
            self.ext_name = "%s%s%d" % (id2js(name), self.ext_suffix,
                                        VARIABLE_COUNTER)
...
class VisibleComponent(Component, Permittable):
    ...
    def install_permission_handler(self):
        ...
        self.allow_read = curry(
            make_static_permission_handler(self, True, self.debug_permissions,
    ...
    def setup(
        ...
        if required_roles is not NOT_PROVIDED:
            if not isinstance(required_roles, set):
                raise Exception("20150628 %s has required_roles %s" %
                                (self, required_roles))
    ...
    def debug_lines(self):
        ...
        for e in self.walk():
            yield '<tr><td>' + sep.join([py2html(e, n)
                                         for n in cols]) + '</td></tr>'
...
def py2js(v, compact=True):
    ...
    """
    # if isinstance(v, DelayedValue):
    #     # v = dict(field=v.field.name, pk=v.obj.pk)
    #     v = dict(delayed_value_url=v.get_url())
    #     # print("20210619", v)
    ...
    if isinstance(v, (list, tuple)):  # (types.ListType, types.TupleType):
        elems = [
            py2js(x, compact=compact) for x in v
    ...
    if isinstance(v, dict):
        items = [
            ...
            i for i in v.items() if (not isinstance(v, VisibleComponent))
        ...
        sep = ", " if compact else ", \n"
        return "{ %s }" % sep.join([
            "%s: %s" % (py2js(k, compact=compact), py2js(i, compact=compact))
            for k, i in items
        ])
