...
def rangetext(self, fmt=fdl, empty_text=''):
    """
    Return a textual representation of the given date range.

    The date range is to be specified as an object that has two attributes
    `start_date` and `end_date`.

    See tested examples in :ref:`welfare.specs.aids`.
    Also used by :mod:`lino_xl.lib.vat`.
    """
    if self.start_date and self.end_date:
        if self.start_date == self.end_date:
            # s = tostring(E.b(fdl(self.start_date)))
            s = fmt(self.start_date)
            return pgettext("date", "on %s") % s
        else:
            kw = dict()
            kw.update(a=fmt(self.start_date))
            kw.update(b=fmt(self.end_date))
...
class CombinedDateTime(dd.Model):
    """
    Mixin for models having at least one couple of date and time fields that
    form a kind of editable timestamp field.
    """
...
class DateRangeObservable(Model):
    ...
    def setup_parameters(cls, fields):
        fields.update(start_date=models.DateField(
            _("Period from"),
            blank=True,
            null=True,
            default=cls.get_default_start_date,
            help_text=_("Start date of observed period")))
        fields.update(end_date=models.DateField(
            _("until"),
            blank=True,
            null=True,
...
class DateRange(DateRangeObservable):
    """
    Mixin for models that represent a period defined by a start date and an end
    date.
...
class Today(ParameterPanel):
    ...
    def __init__(self, verbose_name=_("Situation on"), **kw):
        kw.update(today=models.DateField(verbose_name,
                                         blank=True,
                                         null=True,
                                         default=dd.today,
