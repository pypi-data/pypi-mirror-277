...
name_prefixes1 = set(("HET", "'T", 'VAN', 'DER', 'TER', 'DEN', 'VOM', 'VON',
                      'OF', "DE", "DU", "DA", "DI", "DO", "DOS", "EL", "AL"))
name_prefixes2 = set(
    ("VAN DEN", "VAN DER", "VAN DE", "IN HET", "VON DER", "DE LA"))
    ...
    if len(a) == 0:
        ...
        if last_name_first:
            ...
            if a01.upper() in name_prefixes2:
                return dict(last_name=a01 + ' ' + a[2],
                            first_name=' '.join(a[3:]))
            elif a[0].upper() in name_prefixes1:
                return dict(last_name=a[0] + ' ' + a[1],
                            first_name=' '.join(a[2:]))
            else:
                return dict(last_name=a[0], first_name=' '.join(a[1:]))
        else:
            if len(a) >= 4:
                pc = a[-3] + ' ' + a[-2]  # prefix2 candidate
                if pc.upper() in name_prefixes2:
                    return dict(last_name=pc + ' ' + a[-1],
                                first_name=' '.join(a[:-3]))
            pc = a[-2]  # prefix candidate
            if pc.upper() in name_prefixes1:
                return dict(last_name=pc + ' ' + a[-1],
                            first_name=' '.join(a[:-2]))
        return dict(last_name=a[-1], first_name=' '.join(a[:-1]))
...
class Human(model.Model):
    ...
    title = models.CharField(pgettext("(of a human)", "Title"),
                             max_length=200,
                             blank=True)
...
Human.set_widget_options('first_name', detail_pointer=True)
Human.set_widget_options('last_name', detail_pointer=True)

# TODO: the following two calls should generate an error message
Human.set_widget_options('last_name', invalid_option=True)
Human.set_widget_options('invalid_field_name', detail_pointer=True)
...
class Born(model.Model):
    ...
    def get_age(self, today=None):
        ...
        :meth:`lino.utils.IncompleteDate.get_age`.
        """
        if self.birth_date:
