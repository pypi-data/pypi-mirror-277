...
class QuickLinksList:

    def __init__(self):
        self.items = []

    def add_action(self, ba, **kwargs):
        ba = resolve_action(ba)
        a = WrappedAction(ba, **kwargs)
        self.items.append(a)
...
class MenuItem(object):
    ...
    def __init__(
        ...
        if label is None and action is not None:
            # if instance is not None:
            #     label = str(instance)
...
class Menu(MenuItem):
    ...
    def add_instance_action(self, obj, **kw):
        ...
        Add a menu item that will run the specified action on the given
        :term:`database row`.

        Default for ``action`` is to open the detail view on the database row.

        Used for the :menuselection:`Configure --> System --> Site configuration`
        command.
    ...
    def add_separator(self, **kw):
        if len(self.items) > 0 and not self.items[-1].label == '-':
            return self.add_item_instance(MenuItem(None, '-', **kw))

    def add_menu(self, name, label, **kw):
        return self.add_item_instance(
            Menu(self.user_type, name, label, self, **kw))
    ...
    def add_item_instance(self, mi):
        ...
        if mi.bound_action is not None:
            if mi.bound_action.actor.is_abstract():
                return
            if mi.bound_action.actor.is_hidden():
                return
