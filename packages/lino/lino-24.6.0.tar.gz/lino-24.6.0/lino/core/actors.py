...
class Actor(actions.Parametrizable, Permittable, metaclass=ActorMetaClass):
    ...
    def row_as_summary(cls, ar, obj, **kwargs):
        return obj.as_summary_row(ar, **kwargs)
    ...
    def init_layouts(cls):
        ...
        for name, main in cls.get_extra_layouts():
            layout_instance = actions.resolve_layout(cls, 'extra_layout', main,
        ...
        if len(cls.simple_parameters) > 0:
            ...
            for name in cls.simple_parameters:
                if name not in cls.parameters:
                    ...
                    if db_field is None:
                        raise Exception(
                            "{}.get_simple_parameters() returned invalid name '{}'"
                            .format(cls, name))
    ...
    def get_request_detail_action(cls, ar):
        ...
        if (ar.actor.detail_action is not None and ar.actor
    ...
    def _collect_actions(cls):
        ...
        if cls.detail_layout:
            ...
            if cls.editable:
                cls.submit_detail = cls._bind_action('submit_detail',
                                                     SUBMIT_DETAIL, True)
        ...
        if cls.editable:
            ...
            if cls.allow_delete:
                cls.delete_action = cls._bind_action('delete_action',
                                                     DELETE_ACTION, True)
            cls.update_action = cls._bind_action('update_action',
                                                 UPDATE_ACTION, True)
            if cls.detail_layout:
                cls.validate_form = cls._bind_action('validate_form',
                                                     VALIDATE_FORM, True)
        ...
        if da is not None:
            if isinstance(da, actions.Action):
                cls.default_action = cls._bind_action('default_action', da,
                                                      True)
            else:
    ...
    def has_db_model(cls):
        ...
        if not isinstance(cls.model, type):
            raise Exception("{}.model is {!r} (must be a class)".format(
                cls, cls.model))
    ...
    def set_detail_layout(self, *args, **kw):
        ...
        """
        return self.set_form_layout('detail_layout', layouts.DetailLayout,
                                    *args, **kw)
    ...
    def set_insert_layout(self, *args, **kw):
        ...
        """
        return self.set_form_layout('insert_layout', layouts.InsertLayout,
                                    *args, **kw)
    ...
    def get_button_actions(self, parent):
        ...
        if not parent.opens_a_window:
            ...
            raise Exception("20180518 {} is not a windows action".format(
                parent.__class__))
        return [
            ba for ba in self._actions_list
    ...
    def get_data_elem(self, name):
        ...
        if len(s) == 1:
            ...
            if m is None:
                raise Exception("No plugin {} ({}.{})".format(
    ...
    def slave_as_html(cls, master, ar):
        ...
        """
        ar = cls.request(master,
                         request=ar.request,
                         param_values=ar.param_values if getattr(
                             cls, "use_detail_params_value", False) else {},
...
def resolve_action(spec, action=None):
    ...
    if isinstance(spec, str):
        ...
        if len(parts) < 2 or len(parts) > 3:
            raise Exception(
                "Invalid action specifier '{}' (must be of form `plugin_name.ClassName[.action_name]`)."
                .format(spec))
        ...
        if len(parts) == 2:
            ...
            if action is not None:
                raise Exception(
                    "Invalid action specifier '{}' when action keyword is specified."
                    .format(spec))
    ...
    if isinstance(spec, type) and issubclass(spec, Actor):
        if action:
            ...
            #~ print 20121210, a
            if a is None:
                raise Exception("{} has no action named '{}'".format(
                    spec, action))
        else:
