    ...
    if isinstance(spec, str):
        if '\n' in spec or not '.' in spec:
            ...
            if layout_class is None:
                raise Exception("Unresolved {} {!r} for {}".format(
                    k, spec, cls))
        ...
        if not isinstance(spec, layout_class):
            ...
            raise Exception(
                msg.format(cls.__module__, cls.__name__, k,
                           layout_class.__name__, spec))
        if spec._datasource is None:
            spec.set_datasource(cls)
            return spec
        elif not issubclass(cls, spec._datasource):
            raise Exception("Cannot reuse %s instance (%s of %r) for %r" %
...
class Action(Parametrizable, Permittable):
    ...
    def __init__(self, label=None, **kwargs):
        ...
        if self.icon_name:
            if self.icon_name not in constants.ICON_NAMES:
                raise Exception("Unkonwn icon_name '{0}'".format(
                    self.icon_name))
    ...
    def decorate(cls, *args, **kw):
        ...
        def decorator(fn):
            ...
            def wrapped(ar):
                obj = ar.selected_rows[
                    0] if ar.selected_rows else ar.actor.model
                return fn(obj, ar)
    ...
    def is_window_action(self):
        ...
        """
        return self.opens_a_window or (self.parameters
                                       and not self.no_params_window)
    ...
    def run_from_ui(self, ar, **kwargs):
        ...
        """
        raise NotImplementedError("%s has no run_from_ui() method" %
                                  self.__class__)
...
class ShowDetail(Action):
    ...
    def attach_to_actor(self, actor, name):
        self.help_text = _("Open a detail window on records of " +
                           actor.app_label + ".")
...
class ShowInsert(TableAction):
    ...
    def attach_to_actor(self, owner, name):
        if owner.model is not None:
            self.help_text = format_lazy(_("Insert a new {}."),
                                         owner.model._meta.verbose_name)
    ...
    def get_action_title(self, ar):
        ...
        if ar.actor.model is None:
            return _("Insert into %s") % force_str(ar.get_title())
        return format_lazy(_("Insert a new {}"),
                           ar.actor.model._meta.verbose_name)
...
class SubmitInsert(CreateRow):
    ...
    def run_from_ui(self, ar, **kwargs):
        ...
        if ar.actor.handle_uploaded_files is not None and len(
                ar.request.FILES.getlist("file")) > 1:
...
class ShowSlaveTable(Action):
    ...
    """
    TABLE2ACTION_ATTRS = ('icon_name', 'react_icon_name', '_label',
...
class WrappedAction(Action):
    ...
    *BoundAction.action* useful when binding to another :class:`Actor
    ...
    WRAPPED_ATTRS = ('help_text', 'icon_name', 'react_icon_name', 'label',
                     'sort_index', 'required_roles', 'button_text',
                     'window_type', 'opens_a_window', 'parameters',
                     'no_params_window')
    ...
    def __init__(self, bound_action, **kwargs):
        ...
        if 'action_name' not in kwargs and self.action_name is None:
            kwargs.update(
                action_name=(str(bound_action.actor).replace('.', '_') + '_' +
...
class DeleteSelected(MultipleRowAction):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        if len(cascaded_objects):
            lst = [
                "{} {}".format(
                    n, m._meta.verbose_name
                    if n == 1 else m._meta.verbose_name_plural)
                for m, n in cascaded_objects.items()
            ]
            msg += "\n" + gettext(
                "as well as all related volatile records ({})").format(
                    ", ".join(lst))
