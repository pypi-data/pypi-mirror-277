...
class BaseRequest(object):
    ...
    def alogger(self):
        if self._alogger is None:
            self._alogger = AttrDict({
                "info":
                sync_to_async(self.logger.info),
                "debug":
                sync_to_async(self.logger.debug),
                "warning":
                sync_to_async(self.logger.warning),
                "warn":
                sync_to_async(self.logger.warn),
                "error":
                sync_to_async(self.logger.error),
                "exception":
                sync_to_async(self.logger.exception)
            })
        ...
        if self.master is not None and settings.SITE.strict_master_check:
            if self.master_instance is None:
                raise exceptions.BadRequest(
                    "Request on slave table {} has no master instance ({})".
                    format(self.actor, locals()))
    ...
    def parse_req(self, request, rqdata, **kw):
        ...
        kw.update(
            current_project=rqdata.get(constants.URL_PARAM_PROJECT, None))
        ...
        kw.update(
            xcallback_answers={
                id: rqdata[id]
                for id in [
                    callbackID for callbackID in rqdata.keys()
    ...
    def get_printable_context(self, **kw):
        ...
        def parse(s):
            ...
            # template:
            kw.pop('self', None)
            return dd.plugins.jinja.renderer.jinja_env.from_string(s).render(
                **kw)
    ...
    def get_master_instance(self, master, mk, mt):
        ...
        if master is not None:
            ...
            # if pk is not None:
            if True:
                master_instance = self.actor.get_master_instance(
                    self, master, pk)
                # if master_instance is None:
    ...
    warning = lambda self, *args, **kwargs: self.logger.warning(
        *args, **kwargs)
...
signature as `django.core.mail.EmailMessage`.
        ...
        if '@example.com' in sender:
            ...
            self.logger.debug(
                PRINT_EMAIL.format(subject=subject,
                                   sender=sender,
                                   body=body,
        ...
        self.logger.info("Send email '%s' from %s to %s", subject, sender,
                         recipients)
    ...
    def show(self,
        ...
        def doit():
            ...
            if issubclass(ar.actor, Report):
                ...
                return ar.renderer.show_story(self,
                                              story,
                ...
                return ar.renderer.show_table(ar,
    ...
    def row_as_summary(self, obj, **kwargs):
        return obj.as_summary_row(self, **kwargs)
    ...
    def instance_action_button(self, ia, *args, **kwargs):
        ...
        return self.renderer.row_action_button(ia.instance, self,
                                               ia.bound_action, *args,
                                               **kwargs)
    ...
    def as_button(self, *args, **kw):
        ...
        """
        return self.renderer.action_button(None, self, self.bound_action,
                                           *args, **kw)
...
class ActionRequest(BaseRequest):
    ...
    def create_instances_from_request(self, **kwargs):
        ...
        if self.request is not None:
            for e in elems:
                self.ah.store.form2obj(self, self.request.POST or self.rqdata,
                                       e, True)
        for e in elems:
            e.full_clean()
        return elems
    ...
    def row_as_summary(self, obj, **kwargs):
        return self.actor.row_as_summary(self, obj, **kwargs)
                ...
                if self.actor.params_layout is None:
                    ...
                    if ps is not None:
                        ...
                        raise Exception(
                            ...
                            "in {1!r}".format(self.actor.params_layout,
                                              self.actor))
            else:
    ...
    def set_action_param_values(self, **action_param_values):
        ...
        for k in action_param_values.keys():
            if k not in apv:
                raise Exception("Invalid key '%s' in action_param_values "
...
class InstanceAction(object):
    ...
    def as_button_elem(self, ar, label=None, **kwargs):
        return settings.SITE.kernel.row_action_button(self.instance, ar,
                                                      self.bound_action, label,
                                                      **kwargs)
