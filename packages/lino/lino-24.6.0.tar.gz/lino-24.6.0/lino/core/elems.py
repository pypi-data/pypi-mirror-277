...
from lino.core.layouts import (FormLayout, ParamsLayout, ColumnsLayout,
                               ActionParamsLayout, DummyPanel)
        ...
        if isinstance(editor, FieldElement):
            if settings.SITE.use_quicktips:
                ...
                layout_handle.ui.renderer.add_help_text(
                    ...
                    self.editor.field.help_text if self.editor.field.help_text
                    and "<" not in self.editor.field.help_text else "",
                    "",  # Title
            ...
            if has_fk_renderer(editor.field):
                ...
                if has_fk_renderer(editor.field.return_type):
                    rend = fk_renderer(editor.field.return_type,
                                       editor.field.name)
            if rend:
...
class LayoutElement(VisibleComponent):
    ...
    def __repr__(self):
        return "<%s %s in %s>" % (self.__class__.__name__, self.name,
    ...
    def ext_options(self, **kw):
        ...
        if isinstance(self.parent, TabPanel):
            label = self.get_label()
            if not label:
                raise Exception("Item %s of tabbed %s has no label!" %
...
class FieldElement(LayoutElement):
    ...
    def get_column_options(self, **kw):
        ...
        if self.field.help_text is not None:
            ...
            help_text = "(%s.%s) " % (self.layout_handle.layout._datasource,
                                      self.field.name)
        else:
            help_text = ''
    ...
    def value2html(self, ar, v, **cellattrs):
        ...
        if self.field.primary_key:
            ...
            # url = ar.pk2url(v)
            if url is not None:
                elem = ar.renderer.clickable_link(None,
                                                  self.format_value(ar, v),
                                                  href=url)
                return E.td(elem)
...
class TextFieldElement(FieldElement):
    ...
    def __init__(self, layout_handle, field, **kw):
        self.format = getattr(field, 'format', None) \
        ...
        if self.format == 'html':
            if settings.SITE.is_installed('tinymce'):
                ...
                return LayoutElement.__init__(self, layout_handle, field.name,
                                              **kw)
            else:
                if layout_handle.ui.renderer.extjs_version == 3:
            ...
            raise Exception("Invalid textfield format %r for field %s.%s" %
    ...
    def as_plain_html(self, ar, obj):
        ...
        yield E.div(
            ...
            E.textarea(text,
                       rows=str(self.preferred_height),
    ...
    def get_field_options(self, **kw):
        ...
        if isinstance(self.field,
                      fields.VirtualField) and self.field.simple_elem:
...
class RemoteComboFieldElement(ComboFieldElement):
    ...
    def get_field_options(self, **kw):
        ...
        if self.editable:
            ...
            if self.layout_handle.ui.renderer.extjs_version == 3:
                kw.update(
                    store=js_code("new Lino.ComplexRemoteComboStore(%s)" %
                                  py2js(sto)))
            else:
...
class ForeignKeyElement(ComplexRemoteComboFieldElement):
    ...
    def get_field_options(self, **kw):
        ...
        if settings.SITE.kernel.web_front_ends[0].app_label == "react":
            options = dict(related_actor_id=actor.actor_id,
                           allowBlank=kw.get('allowBlank', False))
            da = actor.default_action
            options.update(view_permission=bool(da)
                           and da.get_view_permission(get_user_profile()))
        ...
        if not isinstance(self.layout_handle.layout, ColumnsLayout):
            ...
            if self.layout_handle.ui.renderer.extjs_version is not None:
                if actor is None:
                    raise Exception("20181229 {!r} {}".format(
...
class DateFieldElement(FieldElement):
    ...
    preferred_width = 13
    filter_type = 'date'
    gridfilters_settings = dict(type='date',
...
class NumberFieldElement(FieldElement):
    ...
    def sum2html(self, ar, sums, i, **cellattrs):
        ...
        return super(NumberFieldElement,
                     self).sum2html(ar, sums, i, **cellattrs)
...
class DisplayElement(FieldElement):
    ...
    def __init__(self, *args, **kw):
        ...
        if hasattr(fld, 'max_length'
                   ) and fld.max_length is not None:  # might be explicit 0
            # if fld.max_length == 0:
            ...
            if isinstance(self.field, mti.EnableChild):
                ...
                if rpt.detail_action is not None:
                    js = "Lino.show_mti_child('%s',Lino.%s)" % (
                        self.field.name, rpt.detail_action.full_name())
...
class HtmlBoxElement(DisplayElement):
    ...
    def as_plain_html(self, ar, obj):
        ...
        if etree.iselement(value):
            panel = E.div(E.div(value, **{'class': "panel-body"}),
...
class LightWeightContainer(HtmlBoxElement):
    ...
    def __init__(self, lh, slave, name, return_type, **kwargs):
        box = fields.HtmlBox(slave.label,
                             name=name,
                             help_text=slave.help_text,
                             delayed_value=True)
        ...
        if w == 0:
            raise Exception(
                "{} has preferred_width 0 ({} children: {})".format(
                    self, "vertical" if self.vertical else "horizontal",
                    self.elements))
...
class GridElement(Container):
    ...
    def __init__(self, layout_handle, name, rpt, *columns, **kw):
        ...
        layout_handle.ui.renderer.add_help_text(kw, rpt.help_text, rpt.title
                                                or rpt.label, rpt.app_label,
                                                rpt.actor_id)
...
class SlaveContainer(GridElement):
    ...
    def __init__(self, layout_handle, name, rpt, *columns, **kw):
        ...
        if constants.DISPLAY_MODE_HTML in dms:
            slaves['html'] = get_htmlbox_element(layout_handle, rpt, name,
                                                 **kw)
        if constants.DISPLAY_MODE_SUMMARY in dms:
            slaves['summary'] = get_summary_element(layout_handle, rpt, name,
                                                    **kw)
...
class TabPanel(Panel):
    ...
    def as_plain_html(self, ar, obj):
        ...
        for e in self.elements:
            main.append(
                E.div(*tuple(e.as_plain_html(ar, obj)),
                      id=e.ext_name,
                      **{'class': "tab-pane fade"}))
...
def field2elem(layout_handle, field, **kw):
    ...
    if ch:
        ...
        if ch.simple_values:
            ...
            if isinstance(field, models.ForeignKey):
                ...
                return ComplexRemoteComboFieldElement(layout_handle, field,
                                                      **kw)
    if field.choices:
    ...
    if isinstance(selector_field, fields.CustomField):
        e = selector_field.create_layout_elem(rnd, CharFieldElement,
...
def create_layout_element(lh, name, **kw):
    ...
    if de is None:
        ...
        raise Exception("Invalid data element '{1}' in {0}".format(
            lh.layout, name))
        ...
        if isinstance(lh.layout, FormLayout):
            ...
            if lh.ui.renderer.extjs_version is not None:
                ...
                if False:
                    ...
                    kw.update(
                        label='{} {}'.format(de.get_label(), tostring(btn)))

            if len(de.display_mode
                   ) > 1 and lh.ui.renderer.front_end.media_name == 'react':
                return SlaveContainer(lh, name, de, **kw)

            dm = de.get_display_mode()
            if dm in [
                    constants.DISPLAY_MODE_TABLE, constants.DISPLAY_MODE_CARDS,
