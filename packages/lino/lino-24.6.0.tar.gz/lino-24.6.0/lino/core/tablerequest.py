...
class TableRequest(ActionRequest):
    ...
    def gen_insert_button(self,
                          target,
                          button_attrs=dict(style="float: right;"),
                          **values):
        """
    ...
    def execute(self):
        ...
        try:
            ...
            # exception only once.
            self.no_data_text = "{} (set catch_layout_exceptions to see details)".format(
                e)
        ...
        if isinstance(self._data_iterator, (SearchQuery, SearchQuerySet)):
            self._sliced_data_iterator = tuple(
                self.actor.get_rows_from_search_query(self._data_iterator,
                                                      self))
        else:
    ...
    def parse_req(self, request, rqdata, **kw):
        ...
        if "master_instance" not in kw:
            kw.update(master_type=rqdata.get(constants.URL_PARAM_MASTER_TYPE,
                                             None),
                      master_key=rqdata.get(constants.URL_PARAM_MASTER_PK,
                                            None))
    ...
    def get_master_instance_unused(self, master, mk, mt):
    ...
    def dump2html(
        ...
        if not self.ah.actor.hide_headers:
            headers = [
                x for x in
                grid.headers2html(self, columns, headers, header_links, **
            ...
            if cellwidths:
                ...
                widths = [
                    str(int(int(w) * 100 / totwidth)) + "%" for w in cellwidths
                ]
    ...
    def sums2html(self, columns, sums, **cellattrs):
        ...
        return [
            fld.sum2html(self, sums, i, **cellattrs)
