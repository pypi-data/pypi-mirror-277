...
def check_role(rr, actor):
    if not issubclass(rr, UserRole):
        raise Exception("{0} (required on {1}) is not a UserRole".format(
            rr, actor))
    ...
    for cl in args:
        for b in cl.__mro__:
            if b is UserRole:
                break
            if b in bases:
                print("{} inherits {} is already in {}".format(
                    cl, b, bases[b]))
            else:
                bases[b] = cl
