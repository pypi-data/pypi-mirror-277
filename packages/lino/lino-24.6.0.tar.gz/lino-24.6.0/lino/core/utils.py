...
from lino.utils.html import E, assert_safe
...
def qs2summary(ar,
    ...
    for i in objects:
        ...
        #     tpl = "20240506 {}.row_as_summary({}) returned non-safe html"
        #     raise Exception(tpl.format(ar.actor, i.__class__))
        elems += s
        if n >= max_items:
...
def obj2str(i, force_detailed=False):
    ...
    if not force_detailed:
        ...
        try:
            return u"%s #%s (%s)" % (i.__class__.__name__, str(
                i.pk), repr(str(i)))
        except Exception as e:
            #~ except TypeError,e:
            return "Unprintable %s(pk=%r,error=%r" % (i.__class__.__name__,
                                                      i.pk, e)
            #~ return unicode(e)
...
class UnresolvedModel(object):
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(%s,%s)' % (self.model_spec,
                                                      self.app_label)
...
def navinfo(qs, elem, limit=None):
    ...
    if num > 0:
        ...
        try:
            ...
            message = _("Row %(rowid)d of %(rowcount)d") % dict(rowid=recno,
                                                                rowcount=num)
    if message is None:
...
def resolve_fields_list(model, k, collection_type=tuple, default=None):
    ...
    if qsf is None:
        ...
        raise ChangedAPI("{0}.{1} must be None or a string "
...
def DelayedValue(ar, fieldname, obj):
    ...
    return dict(delayed_value_url="values/{}/{}/{}".format(
