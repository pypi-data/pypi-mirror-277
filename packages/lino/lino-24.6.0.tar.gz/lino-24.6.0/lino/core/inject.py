    ...
    def todo(model):
        for k, v in actions.items():
            if not hasattr(model, k):
                raise Exception("%s has no attribute %s to update." %
                                (model, k))
            setattr(model, k, v)
    ...
    def todo(model):
        ...
        if True:  # 20181023
            try:
                ...
                raise Exception("Duplicate field {} on {}".format(name, model))
...
def update_field(model_spec, name, **kw):
    ...
    mixin, as done in :mod:`lino_xl.lib.outbox`::

      dd.update_field(Mail, 'user', verbose_name=_("Sender"))

    Note that you can update only certain attributes: ``verbose_name``,
    ``blank``, ``null``, the precision and length of character or decimal
    fields, ...

    You cannot, for example, update the ``related_model`` of a `ForeignKey`
    field because this property causes a myriad of other things to happen during
...
def inject_quick_add_buttons(model, name, target):
    ...
    inject_field(
        model, name,
        fields.VirtualField(fields.DisplayField(tm._meta.verbose_name_plural),
                            fn))
