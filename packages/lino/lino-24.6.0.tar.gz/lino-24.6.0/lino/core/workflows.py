...
class State(choicelists.Choice):
    ...
    def add_transition(
        ...
        if self.transition is not None:
            raise Exception(
                "Tried to add another transition to {}".format(self))
...
class ChangeStateAction(actions.Action):
    ...
    def get_confirmation_msg_context(self, ar, obj, **kwargs):
        kwargs.update(user=ar.get_user(), state=self.target_state)
        return kwargs
    ...
    def execute(self, ar, obj):
        ...
        def doit(ar):
            ...
            obj.set_workflow_state(ar, ar.actor.workflow_state_field,
                                   self.target_state)
