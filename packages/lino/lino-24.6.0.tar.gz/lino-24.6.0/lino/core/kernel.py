...
from lino.core.signals import (pre_ui_build, post_ui_build, pre_startup,
                               post_startup, pre_analyze, post_analyze)
...
class Kernel(object):
    ...
    def kernel_startup(self, site):
        ...
        if site._history_aware_logging:
            ...
            site.register_shutdown_task(goodbye)

        atexit.register(site.shutdown)

        sig = getattr(signal, site.stopsignal)
                ...
                if isinstance(f, models.ForeignKey):
                    ...
                    if not hasattr(f.remote_field.model, '_lino_ddh'):
                        msg = "20150824 {1} (needed by {0}) "\
                              "has no _lino_ddh"
                        raise Exception(
                            msg.format(f.remote_field, f.remote_field.model))
                    f.remote_field.model._lino_ddh.add_fk(m or model, f)

            # if not site.quick_startup:
        ...
        # ManyToManyField.
        for model in models_list:
            p = site.plugins.get(model._meta.app_label)
            if p.is_hidden():
                # models of inactive plugins need no _lino_default_table
                continue
        ...
        for a in actors.actors_list:
            if issubclass(a, tables.AbstractTable) and not a.abstract:
                try:
                    ...
                    logger.debug("Ignoring DatabaseError in %s.setup_columns",
                                 a)
            if issubclass(a, dbtables.Table
                          ) and a.model is not None and a.live_panel_update:
                if a.model.updatable_panels is None:
                    a.model.updatable_panels = []
                if a.actor_id not in a.model.updatable_panels:
                    a.model.updatable_panels.append(a.actor_id)
        ...
        if False:
            logger.info("Languages: %s. %d apps, %d models, %s actors.",
                        ...
                        len(site.modules), len(models_list),
        ...
        site.developer_site_cache = settings.DEBUG or is_devserver()

        # web.site_setup(site)

        for a in actors.actors_list:
            for i, ea in enumerate(a.collect_extra_actions()):
                a._bind_action(ea.action_name, ea, False)
            a._actions_list = sorted(a._actions_list,
                                     key=lambda ba: ba.action.sort_index)

        # for p in site.installed_plugins:
        #     p.after_discover()

        self.reserved_names = [
            getattr(constants, n) for n in constants.URL_PARAMS
        ]
        ...
        if self.site.web_front_ends is None:
            # backwards compatibility
            for p in self.site.installed_plugins:
                if p.app_name == self.site.default_ui:
                    p.url_prefix = None
                    self.web_front_ends = [p]
                    break
        else:
            lst = []
            for prefix, modname in self.site.web_front_ends:
                found = False
                for p in self.site.installed_plugins:
                    if p.app_name == modname:
                        p.url_prefix = prefix
                        lst.append(p)
                        found = True
                        break
        ...
        self.default_renderer = editing_wf.renderer
        ...
        for a in actors.actors_list:
            ...
            if a.get_welcome_messages is not None:
                site.add_welcome_handler(a.get_welcome_messages, a,
            ...
            if a.welcome_message_when_count is not None:
                ...
                site.add_welcome_handler(handler(a), a,
    ...
    def protect_foreignkeys(self, models_list):
        ...
        TradeTypes.purchases in accounting.
    ...
    def get_generic_related(self, obj):
        ...
        for gfk in self.GFK_LIST:
            ...
            assert not gfk.model._meta.abstract
    ...
    def get_broken_generic_related(self, model):
        """Yield all database objects of this model that have some broken
        GFK field.
            ...
            for gfk in gfks:
                ...
                for obj in qs:
                    ...
                    try:
                        ...
                        obj._message = msg.format(gfk.fk_field, fk,
                                                  fmn(pointed_model))
    ...
    def run_action(self, ar):
        ...
        try:
            if isinstance((resp := a.run_from_ui(ar)), HttpResponse):
                return resp
            if ar.request.method != 'GET' and ar.master_instance is not None and len(
                    ar.selected_rows):
                ar.set_response(
                    master_data=ar.selected_rows[0].get_master_data(ar))
            if a.parameters and not a.no_params_window and ar.response.get(
                    'success', False):
    ...
    def setup_handle(self, h, ar):
        ...
        if isinstance(h, tables.TableHandle):
            ...
            ll = layouts.ColumnsLayout(h.actor.get_column_names(ar),
                                       h.actor,
                                       hidden_elements=he)
    ...
    def make_cache_file(self, fn, write, force=False, verbosity=1):
        ...
        """
        # cachedir = self.site.site_dir.child('static', 'cache', 'js')
        ...
        # print("20231005 make_cache_file() {}".format(verbosity))

        # The following message is important to see in a developer console
        # because the process takes some time and when developing you are
        # watching at such messages. It should *not* be shown when running unit
        # tests because its occurence is not (easily) predictable.   On a
        # production site we want it to be seen on the console and logged to the
        # lino.log.
    ...
    def check_virgin(self):
        ...
        fn = self.site.site_dir / "dbhash.json"
        if not fn.exists():
            raise Exception("No `dbhash.json` in {} "
                            "(did you run `django-admin prep`?)".format(
                                self.site.site_dir))
...
def site_startup(self):
    ...
    with startup_rlock:
        ...
        #     raise Exception("No site_dir is defined. "
        ...
        # doctest docs/specs/react/linoweb.rst is failing with "Oops,
        # get_atomizer(contacts.Persons, contacts.RolesByPerson,
        # contacts.RolesByPerson) returned None" because the store fields for
        # slave tables were not yet created. Activating the following code fixes
        # this issue but causes other problems:
        if False:
            for res in actors.actors_list:
                if res.detail_layout is not None:
                    res.detail_layout.get_layout_handle()

        # print("20161219 Site.startup() done")
        self._startup_done = True

        # # doctest docs/specs/react/linoweb.rst was failing with "Oops,
        # # get_atomizer(contacts.Persons, contacts.RolesByPerson,
        # # contacts.RolesByPerson) returned None" because the store fields for
        # # slave tables were not yet created:
        # for res in actors.actors_list:
        #     if res.detail_layout is not None:
        #         res.detail_layout.get_layout_handle()
...
def register_actors():
    ...
    for rpt in slave_tables:
        ...
        if not isinstance(rpt.master, type):
            raise Exception("20160712 invalid master {!r} in {}".format(
                rpt.master, rpt))
...
def register_choicelist(cl):
    ...
    if k in CHOICELISTS:
        raise Exception("Cannot register %r : actor name '%s' "
