...
def request2data(request, user_language=None):
    ...
    request.requesting_panel = rqdata.get(constants.URL_PARAM_REQUESTING_PANEL,
                                          None)
    # request.device_type = rqdata.get(constants.URL_PARAM_DEVICE_TYPE,
    #                                  settings.SITE.device_type)
...
class NoUserMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        request.user = settings.SITE.get_anonymous_user()
    ...
    def process_request(self, request):
        ...
        if not user.is_anonymous:
            ...
            if update:
                ...
                request.session['last_ip_addr'] = get_client_ip_address(
                    request)
