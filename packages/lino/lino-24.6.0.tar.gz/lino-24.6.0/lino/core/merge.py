...
class MergeAction(actions.Action):
    ...
    def run_from_ui(self, ar):
        ...
        def ok(ar2):
            ...
            # same after-run behaviour as DeleteSelected
...
class MergePlan(object):
    ...
    def logmsg(self):
        ...
        lst = []
        lst.append(
            _("Merge %(origin)s into %(target)s") %
            ...
            for f, qs in getattr(self, name):
                if qs.count() > 0:
                    lst.append('- %d %s %s rows using %s : %s' %
                               (qs.count(), name, full_model_name(f.model),
                                f.name, ' '.join([str(o.pk) for o in qs])))
