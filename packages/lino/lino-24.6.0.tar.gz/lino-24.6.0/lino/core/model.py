...
class Model(models.Model, fields.TableRow):
    ...
    def collect_virtual_fields(model):
        ...
        fieldnames = {
            f.name
            for f in model._meta.private_fields + model._meta.local_fields
        }
    ...
    def lookup_or_create(model, lookup_field, value, **known_values):
        ...
        if isinstance(lookup_field, BabelCharField):
            flt = settings.SITE.lookup_filter(lookup_field.name, value,
                                              **known_values)
        else:
        ...
        if bleach:
            for f in self._bleached_fields:
                ...
                try:
                    ...
                    logger.warning("Could not bleach %r : %s (%s)", old, e,
                                   self)
                    continue
                if old != new:
                    logger.debug("Bleaching %s from %r to %r", f.name, old,
                                 new)
                    yield f, old, new
    ...
    def edge_ngram_field(self):
        values = [
            ...
            for field in self.quick_search_fields +
            self.quick_search_fields_digit
        ]
        return ' '.join(values)
    ...
    def navigation_panel(self, ar):
        ...
        qs = sliced_data_iterator(ar.data_iterator, navinfo['offset'],
                                  ar.limit)
        for obj in qs:
            if obj.pk == self.pk:
    ...
    for gfk, fk_field, qs in kernel.get_generic_related(instance):
        if gfk.name in qs.model.allow_cascaded_delete:
            ...
            if fk_field.null:  # clear nullable GFKs
                ...
                raise Warning(
                    instance.delete_veto_message(qs.model, qs.count()))
    for qs in must_cascade:
        if qs.count():
            logger.info("Deleting %d %s before deleting %s", qs.count(),
                        qs.model._meta.verbose_name_plural, obj2str(instance))
            for obj in qs:
                obj.delete()
