...
class Callback(object):
    ...
    def run(self, choice):
        if choice not in self.choices_dict:
            raise Exception(
                "Can not run choice|%s| not in available choices %s" %
...
def set_callback(ar, cb):
    ...
    if answer is None:
        buttons = []
        xcallback = dict(id=cb_id, title=cb.title, buttons=buttons)

        rqdata = {
            k: v[0] if len(v) == 1 else v
            for k, v in (ar.rqdata.lists() if getattr(ar, "rqdata", None
                                                      ) is not None else {})
        }
        rqdata.pop("_dc", None)
