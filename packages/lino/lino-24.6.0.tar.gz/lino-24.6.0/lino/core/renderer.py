...
class HtmlRenderer(Renderer):
    ...
    def href_button_action(self,
                           ba,
                           url,
                           text=None,
                           title=None,
                           icon_name=NOT_PROVIDED,
                           **kw):
        """
    ...
    def window_action_button(self,
        ...
        return self.href_button_action(ba, url, str(label), title
    ...
    def menu_item_button(self, ar, mi, label=None, icon_name=None, **kwargs):
        ...
        return self.window_action_button(ar,
                                         mi.bound_action,
                                         label=label,
                                         icon_name=icon_name,
                                         **kwargs)
    ...
    def row_action_button(self,
        ...
        if ar is not None and ar.permalink_uris:
            ...
            uri = self.js2url(js)
        return self.href_button_action(ba, uri, label, title
                                       or ba.action.help_text, **button_attrs)

    def row_action_button_ar(self,
                             obj,
                             ar,
                             label=None,
                             title=None,
        ...
        try:
            for item in forcetext(story):
                ...
                if isinstance(item, str):
                    ...
                    elems += [
                        tostring(e) for e in self.show_story(
...
class TextRenderer(HtmlRenderer):
    ...
    def table2story(self,
        ...
        if display_mode == constants.DISPLAY_MODE_LIST:
            ...
            if cls.insert_action is not None and cls.editable:
                ...
                if ir.get_permission():
                    items.append("(+) {}".format(
                        cls.insert_action.action.help_text))
        ...
        for row in ar.sliced_data_iterator:
            ...
            if show_urls:
                ...
                # return a SafeString
                rows.append(
                    [text2rst(x) for x in ar.row2text(fields, row, sums)])
    ...
    #         text = str(obj)
    #     # return "**{0}**".format(text)
    #     return format_html(settings.SITE.obj2text_template, text)
...
class JsRenderer(HtmlRenderer):
    ...
    def ar2js(self, ar, obj, **status):
        ...
        if ba.action.is_window_action():
            ...
            return "Lino.%s.run(%s,%s)" % (ba.full_name(), py2js(rp),
                                           py2js(status))
        ...
        if ba.action.select_rows:
            ...
            return (
                f"Lino.{ba.full_name()}({py2js(rp)},{py2js(ar.is_on_main_actor)},{py2js(pk)},{py2js(params)})"
            )
        # assert obj is None
        # params = self.get_action_params(ar, ba, obj)
        # url = ar.get_request_url()

        url = self.front_end.build_plain_url(ar.actor.app_label,
                                             ar.actor.__name__)
...
class JsCacheRenderer(JsRenderer):
    ...
    def prepare_layouts(self):
        ...
        self.actors_list = [
            rpt for rpt in kernel.master_tables + kernel.slave_tables +
            list(kernel.generic_slaves.values()) + kernel.virtual_tables +
            kernel.frames_list + list(kernel.CHOICELISTS.values())
        ]
        ...
        for res in self.actors_list:
            ...
            for ba in res.get_actions():
                if ba.action.parameters:
                    add(res, self.action_param_panels, ba.action.params_layout,
