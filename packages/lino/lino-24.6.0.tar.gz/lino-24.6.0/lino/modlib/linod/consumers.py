...
match_master = lambda ContentType, MasterModel, Master: Master == MasterModel or ContentType == Master\
...
class LinodConsumer(AsyncConsumer):
    ...
    async def send_push(self, event):
        ...
        if user is not None:
            user = await database_sync_to_async(
                settings.SITE.models.users.User.objects.get)(pk=user)
        kwargs = dict(
    ...
    async def send_panel_update(self, event):
        ...
        data = dict([(a.actor_id, {
            "mk": None,
            "mt": None,
            "pk": msg["pk"]
        }) for a in ups if not is_slave(a)])
        if msg['mk'] is not None:
            MasterModel = rt.models.resolve(msg["master_model"])
            ContentType = rt.models.contenttypes.ContentType
            mt = (await
                  database_sync_to_async(ContentType.objects.get_for_model)
                  (MasterModel)).pk
            data.update(
                **dict([(a.actor_id, {
                    "mk": msg["mk"],
