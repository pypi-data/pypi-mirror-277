...
if has_socialauth and dd.get_plugin_setting('users',
...
def format_timestamp(dt):
    if dt is None:
        return ''
    return "{} {} ({})".format(dt.strftime(settings.SITE.date_format_strftime),
...
class Sessions(dd.Table):
    ...
    def get_data_rows(cls, ar):
        ...
        #     order_by
        if ar.order_by:
            order_by = ar.order_by[0]  # or '-last_activity'
            if order_by.startswith('-'):
                reverse = True
                order_by = order_by[1:]
            else:
                reverse = False
            sk = cls.get_data_elem(order_by)
