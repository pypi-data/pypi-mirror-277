...
SYNCHRONIZED_FIELDS = ('first_name', 'last_name', 'email', 'language',
                       'remarks')
...
class User(AbstractBaseUser, Contactable, CreatedModified, Publishable,
           DateRange):
    ...
    if multi_ledger:
        ledger_subscribe__user = SubscribeToLedger(params_layout="""
        company role
        ledger
        """)
    ...
    def is_verification_code_expired(self):
        if self.verification_code_sent_on is None:
            return False
        return (self.verification_code_sent_on +
    ...
    def status(self, ar):
        ...
        s += ". " + (_("Verified")
                     if self.is_verified() else _("Not verified"))
    ...
    def get_active_users(cls,
        ...
        if unwanted_roles is not None:
            user_types = [
                t for t in user_types
                if not t.has_required_roles(unwanted_roles)
            ]
    ...
    def usertext(self):
        return "{0} {1}, {3} ({2})".format(self.last_name, self.first_name,
    ...
    def get_authorities(self):
        ...
        if self.has_required_roles([Supervisor]):
            ...
            qs = rt.models.users.Authority.objects.filter(
                authorized=self).exclude(
                    user__user_type='').select_related("user")
...
def setup_memo_commands(sender=None, **kwargs):
    ...
    mp.add_suggester(
        ...
        sender.models.users.User.objects.filter(
            username__isnull=False).order_by('username'), 'username')
