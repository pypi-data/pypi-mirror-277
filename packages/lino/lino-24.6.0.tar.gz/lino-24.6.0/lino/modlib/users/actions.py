...
class CheckedSubmitInsert(SubmitInsert):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        def ok(ar2):
            ...
            ar2.success(
                _("Your request has been registered. "
                  "An email will shortly be sent to {0}"
                  "Please check your emails.").format(obj.email))
...
class CreateAccount(dd.Action):
    ...
    select_rows = False
    parameters = dict(first_name=dd.CharField(_("First name")),
...
class VerifyUser(dd.Action):
    ...
    parameters = dict(email=models.EmailField(_("e-mail address")),
    ...
    def html_response(self, ar, user):
        ...
        if user is not None:
            ...
            result.update(
                instance=
                f'<a href="/#{href}">Preferences ({user.get_full_name()})</a>')
        page_content = ""
        for key, value in result.items():
            page_content += f"<tr><td>{key.capitalize()}</td><td>{value}</td></tr>"

        return HttpResponse(dd.plugins.jinja.renderer.jinja_env.get_template(
            'users/verification_response.html').render(
                **{
                    "site": settings.SITE,
    ...
    def run_from_ui(self, ar, **kwargs):
        ...
        try:
            ...
            ar.set_response(goto_url=ar.get_permalink(
                rt.models.users.Me.detail_action, user))
...
class ResetPassword(dd.Action):
    ...
    parameters = dict(email=models.EmailField(_("e-mail address")),
                      username=dd.CharField(_("Username (optional)"),
                                            blank=True),
                      new1=dd.PasswordField(_("New password"), blank=True),
                      new2=dd.PasswordField(_("New password again"),
                                            blank=True))
...
class SignOut(dd.Action):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        ar.success(_("User {} logged out.").format(user),
...
def validate_sessions_limit(request):
    ...
    if qs.count() >= dd.plugins.users.active_sessions_limit:
        raise Warning(
            _("There are more than {} active user sessions. Please try again later."
...
def get_social_auth_links_func(content_header, flex_row, ar=None):
    ...
    def text_fn(*args):
        elems = [
            E.div(E.hr(style="width: -moz-available; margin-right: 1ch;"),
...
class SignIn(dd.Action):
    ...
    parameters = dict(username=dd.CharField(_("Username")),
                      ...
                      social_auth_links=dd.Constant(
                          get_social_auth_links_func(
                              gettext("Or sign in with"), flex_row=True)))
    ...
    def run_from_ui(self, ar, **kw):
        ...
        if user is None:
            ...
            ar.success(_("Now signed in as {}").format(user),
                       close_window=True,
                       goto_url='/')
...
class ConnectAccount(dd.Action):
    ...
    def run_from_ui(self, ar, **kw):
        cb = ar.add_callback(
            get_social_auth_links_func(gettext("Connect account"),
                                       flex_row=False,
                                       ar=ar)())
