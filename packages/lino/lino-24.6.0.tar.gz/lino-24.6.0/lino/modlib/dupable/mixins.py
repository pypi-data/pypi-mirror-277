...
class Dupable(dd.Model):
    ...
    dupable_words = GenericRelation('dupable.PhoneticWord',
    ...
    def update_dupable_words(self, really=True):
        ...
        wanted = list(
            self.get_dupable_words(getattr(self, self.dupable_words_field)))
        if existing == wanted:
            return
        if really:
            qs.delete()
    ...
    def get_dupable_words(self, s):
        for c in '-,/&+':
            s = s.replace(c, ' ')
        return filter(
            None, map(rt.models.dupable.PhoneticWord.reduce_word, s.split()))
