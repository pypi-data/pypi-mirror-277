...
class WriteXmlAction(DirectPrintAction):
    ...
    def validate_result_file(self, bm, filename):
        ...
        try:
            ...
            raise Warning(
                _("Oops, the generated XML file {} is invalid: {}").format(
                    filename, e))
...
class EditTemplate(BasePrintAction):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        if filename == local_file:
            ...
            def ok(ar2):
                logger.info("%s made local template copy %s", ar.user,
                            local_file)
            ...
            msg = _("Before you can edit this template we must create a "
...
class ClearCacheAction(Action):
    ...
    def run_from_ui(self, ar):
        ...
        def doit(ar):
            elem.clear_cache()
            ar.success(_("%s printable cache has been cleared.") % elem,
                       refresh=True)
        ...
        if t is not None:
            ...
            t = datetime.datetime(t.year, t.month, t.day, t.hour, t.minute,
                                  t.second, elem.build_time.microsecond)
            if settings.USE_TZ:
                t = make_aware(t)
