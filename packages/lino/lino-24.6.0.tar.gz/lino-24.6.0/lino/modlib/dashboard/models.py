...
class UpdateWidgets(dd.Action):
    ...
    def get_action_permission(self, ar, obj, state):
        ...
        if not me.user_type.has_required_roles([SiteAdmin]):
            if obj != me:
                return False
        return super(UpdateWidgets, self).get_action_permission(ar, obj, state)
...
class Widget(UserAuthored, Sequenced):
    ...
    def item_name_choices(cls, user):
        return [i.name for i in settings.SITE.get_dashboard_items(user)]
    ...
    def after_ui_save(self, ar, cw):
        ...
        ar.get_user().get_preferences().invalidate()

dd.update_field(Widget, 'user', verbose_name=_("User"))
