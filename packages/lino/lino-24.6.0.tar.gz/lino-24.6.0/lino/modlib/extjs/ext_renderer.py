...
from lino.core.actions import (ShowEmptyTable, ShowDetail, ShowInsert,
                               ShowTable, SubmitDetail, SubmitInsert,
                               WrappedAction)
...
class ExtRenderer(JsCacheRenderer):
    ...
    def window_action_button(self,
        ...
        return self.href_button_action(ba, uri, label, title
                                       or ba.action.help_text, **kw)
    ...
    def quick_manage_toolbar(self, ar, obj):
        ...
        ba = ar.bound_action
        manage_btn = ar.renderer.action_button(obj,
                                               ar,
                                               ba,
                                               _("Manage"),
    ...
    def action_call_on_instance(self,
                                obj,
                                ar,
                                ba,
                                request_kwargs={},
    ...
    def action_call(self, ar, bound_action, status):
        ...
        if a.opens_a_window or (a.parameters and not a.no_params_window):
            ...
            if status:
                return "Lino.%s.run(%s,%s)" % (bound_action.full_name(),
                                               py2js(rp), py2js(status))
    ...
    def get_panel_btn_handler(self, ba, status={}):
        if isinstance(ba.action, WrappedAction):
            ...
            if original_ba.action.parameters and not original_ba.action.no_params_window:
        ...
        if not ba.action.action_name:
            raise Exception("Cannot make handler for nameless action %r" %
                            ba.action)
        if ba.action.js_handler:
            ...
            if ba.action.select_rows:
                ...
                h = "Lino.list_action_handler('{}',".format(
                    ba.actor.actor_url())
    ...
    def get_actor_url(self, actor, *args, **kw):
        return self.front_end.build_plain_url("api", actor.app_label,
                                              actor.__name__, *args, **kw)
    ...
    def get_request_url(self, ar, *args, **kw):
        ...
        if ar.bound_action != ar.actor.default_action:
            kw[constants.
               URL_PARAM_ACTION_NAME] = ar.bound_action.action.action_name
        return self.front_end.build_plain_url('api', ar.actor.app_label,
    ...
    def html_page_user(self, request, site):
        ...
        if request.user.is_authenticated:
            if request.subst_user:
                yield "Lino.set_subst_user(%s,%s);" % (py2js(
                    request.subst_user.id), py2js(str(request.subst_user)))
            ...
            if len(authorities):
                act_as = [
                    dict(text=t,
                         handler=js_code(
                             "function(){Lino.set_subst_user(%s, %s)}" %
                             (v, py2js(t)))) for v, t in authorities
                ]
                ...
                act_as.insert(
                    ...
                    dict(text=_("Myself"),
                         handler=js_code(
                             "function(){Lino.set_subst_user(null)}")))
            ...
            login_menu = dict(text=user_text,
    ...
    def write_lino_js(self, f):
        ...
        actors_list = [
            a for a in actors_list if a.default_action
            and a.default_action.get_view_permission(user_type)
        ]
        ...
        for rpt in actors_list:
            ...
            window_actions = [
                ba.action for ba in rpt.get_actions()
                if ba.action.opens_a_window
            ]
            for ba in rpt.get_actions():
                if isinstance(ba.action, WrappedAction):
                ...
                # elif self.is_custom_action(ba.action):
                    ...
                    for parent in window_actions:
                        if parent.window_type is None:
                            raise Exception("20210514 {}.{}".format(
                                rpt, parent))
        ...
        if user_type != get_user_profile():
            logger.warning("Oops, user_type %s != get_user_profile() %s",
                           user_type, get_user_profile())
    ...
    def js_render_FormPanelSubclass(self, dh):
        ...
        if dh.layout._formpanel_name.endswith('.DetailFormPanel'):
            yield "cls: %s," % py2js("l-DetailFormPanel " +
                                     tbl.actor_id.replace(".", "-"))
        if dh.layout._formpanel_name.endswith('.InsertFormPanel'):
            yield "cls: %s," % py2js("l-InsertFormPanel " +
                                     tbl.actor_id.replace(".", "-"))
        if dh.layout._formpanel_name.endswith('.ParamsPanel'):
            yield "cls: %s," % py2js("l-ParamsPanel " +
                                     tbl.actor_id.replace(".", "-"))
        if dh.layout._formpanel_name.endswith('_ActionFormPanel'):
            yield "cls: %s," % py2js("l-ActionFormPanel " +
                                     tbl.actor_id.replace(".", "-"))
        ...
        if len(hotkeys):
            yield "  actions_hotkeys: %s," % py2js(
                self.actions_hotkeys2js(hotkeys))
        if dh.layout.window_size and dh.layout.window_size[1] == 'auto':
            yield "  autoHeight: true,"
    ...
    def js_render_detail_action_FormPanel(self, rh, ba):
        ...
        def doit(dtl):
            yield ""
            yield "Lino.%sPanel = Ext.extend(Lino.%s,{" % (ba.full_name(),
                                                           dtl._formpanel_name)
            ...
            if ba.action.save_action_name is not None:
                yield "  save_action_name: %s," % py2js(
                    ba.action.save_action_name)
            yield "  ls_bbar_actions: %s," % py2js(
                self.toolbar(
                    rpt.get_toolbar_actions(ba.action, get_user_profile())))
            ...
            if ba.action.default_record_id is not None:
                yield "  default_record_id: {!r},".format(
                    ba.action.default_record_id)
...
'</td>')""" % (rh.actor.row_height * 11)
    ...
    def js_render_wrapped_action(self, rh, ba):
        ...
        if isinstance(ba.action, ShowDetail):
            ...
            if ba.action.select_rows:
                ...
                h = "\tLino.list_action_handler('{}'".format(
                    ba.actor.actor_url()) + ", %s, %s, %s)(panel, btn, step);"
            yield h % (py2js(ba.action.action_name),
                       py2js(ba.action.http_method), ba.action.preprocessor)
        yield "};"
    ...
    def js_render_custom_action(self, rh, ba):
        ...
        yield "    Lino.run_row_action(rp, is_main, %s, %s, pk, %s, params, %s);" % (
            py2js(uri), py2js(ba.action.http_method),
            py2js(ba.action.action_name), ba.action.preprocessor)
        yield "  };"
    ...
    def js_render_window_action(self, rh, ba):
        ...
        def doit(wl, ws):
            ...
            if True:
                if ws:
                    windowConfig.update(maximized=False,
                                        draggable=True,
                                        maximizable=True,
                                        modal=True)
            ...
            if ba.action.extjs_main_panel:
                ...
                p.update(
                    is_main_window=True)  # workaround for problem 20111206
    ...
    def linolib_intro(self):
        ...
        def fn():
            yield "// lino.js --- generated %s by %s for %s." % (
                time.ctime(), escape(
                    settings.SITE.site_version()), get_user_profile())
            # lino.__version__)
            ...
            # else:
            yield "GEN_TIMESTAMP = %s;" % py2js(
                settings.SITE.kernel.code_mtime)
    ...
    def goto_instance(self, ar, obj, detail_action=None, **kw):
        ...
        if ar.actor is not None:
            ...
            if da.actor == ar.actor:
                ...
                return
        return super(ExtRenderer, self).goto_instance(ar, obj, detail_action,
                                                      **kw)
