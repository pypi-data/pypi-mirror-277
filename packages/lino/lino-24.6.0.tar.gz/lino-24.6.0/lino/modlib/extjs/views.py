...
def elem2rec_empty(ar, ah, elem, **rec):
    ...
    if ar.actor.parameters:
        rec.update(param_values=ar.actor.params_layout.params_store.pv2dict(
            ar, ar.param_values))
    return rec
...
def test_version_mismatch(request):
    ...
    return dict(
        ...
        message=
        _("Your browser is using a previous version of the site, press OK to reload the site"
          ),
...
class RunJasmine(View):
    ...
    def get(self, request, *args, **kw):
        return http.HttpResponse(
            settings.SITE.kernel.extjs_renderer.html_page(request,
                                                          run_jasmine=True))
...
class ActionParamChoices(View):
    ...
    def get(self,
        ...
        qs, row2dict = choices_for_field(ba.request(request=request),
                                         ba.action, field)
        if field.blank:
            emptyValue = '<br/>'
        else:
    ...
    def get(self, request, app_label=None, actor=None, pk=None):
        ...
        if pk and pk != '-99999' and pk != '-99998':
            ...
            if len(ar.selected_rows):
                ...
                raise http.Http404("No permission to see {} {}.".format(
                    rpt, action_name))
        else:
...
class ApiList(View):
    ...
    def post(self, request, app_label=None, actor=None):
        ...
        if request.POST.get('_document_domain',
                            None) and response['Content-Type'] == "text/html":
