...
class Changes(dd.Table):
    ...
    param_object_type = dd.ForeignKey('contenttypes.ContentType',
                                      verbose_name=_("Object type"),
                                      blank=True)
    parameters = {
    ...
    def get_request_queryset(cls, ar, **filter):
        ...
        if not isinstance(qs, list):
            ...
            if pv.date:
                qs = qs.filter(time__range=(pv.date,
                                            pv.date + datetime.timedelta(1)))
...
if remove_after:
    ...
    def delete_older_changes(ar):
        ...
        if qs.count() > 0:
            ar.logger.info("Removing %d changes older than %d days.",
...
def on_add_child(sender=None, request=None, child=None, **kw):
    ...
    if master is None:
        return
    log_change(ChangeTypes.add_child, request, master, sender,
