...
emotion_range = dd.get_plugin_setting('comments', 'emotion_range', '')
if emotion_range == "business":
    add('ok', _("Okay"), 'ok')
    # add('but', _("But"), 'but', button_text="⁈")
    add('agree', _("Agree"), 'agree', button_text="✅")
    add('disagree', _("Disagree"), 'disagree', button_text="❎")
elif emotion_range == "social":
    ...
    def add_filter(self, qs, pv):
        if pv.start_date:
            start_datetime = datetime.combine(
                pv.start_date, datetime.min.time()).replace(tzinfo=pytz.UTC)
            qs = qs.filter(**{
        ...
        if pv.end_date:
            end_datetime = datetime.combine(
                pv.end_date, datetime.max.time()).replace(tzinfo=pytz.UTC)
            qs = qs.filter(**{
