...
from .mixins import UploadController, UploadBase
...
class Upload(UploadBase, UserAuthored, Controllable, Publishable):
    ...
    library_file = models.CharField(_("Library file"),
                                    max_length=255,
                                    blank=True)
    ...
    # copyright = models.CharField(_("Copyright"), max_length=100, blank=True)
    ...
    def get_file_url(self):
        ...
        if self.library_file and self.volume_id and self.volume.base_url:
            return settings.SITE.build_static_url(self.volume.base_url,
                                                  self.library_file)
    ...
    def preview(self, ar):
        ...
        if url is None or url.endswith(".pdf"):
            ...
            return '<p style="text-align: center;padding: 2em;">({})</p>'.format(
                txt)
    ...
    def as_page(self, ar, **kwargs):
        ...
        url = self.get_file_url()
        yield '<img src="{}" style="width: 100%;">'.format(url)
        if self.description:
...
class UploadsFolderChecker(Checker):
    ...
    def get_checkdata_problems(self, obj, fix=False):
        ...
        for filename in Path(pth).rglob("*"):
            ...
            n = qs.count()
            if n == 0:
                msg = format_lazy(_("File {} has no upload entry."),
                                  rel_filename)
                # print(msg)
