...
PARAGRAPH_TAGS = {
    "p", "h1", "h2", "h3", "h4", "h5", "h6", "h7", "h8", "h9", "pre", "li",
    "div"
}
WHITESPACE_TAGS = PARAGRAPH_TAGS | {
    "[document]", "span", "ul", "html", "head", "body", "base"
}
...
MARKDOWNCFG = dict(
    ...
    extension_configs=dict(
        toc=dict(toc_depth=3, permalink=True)))
...
class BasePreviewable(dd.Model):
    ...
    def parse_previews(self, source, ar=None, mentions=None, **context):
        ...
        full = settings.SITE.plugins.memo.parser.parse(source,
                                                       ar=ar,
                                                       context=context,
                                                       mentions=mentions)
    ...
    def get_saved_mentions(self):
        ...
        return Mention.objects.filter(**flt).order_by('source_type',
                                                      'source_id')
...
class Previewable(BasePreviewable):
    ...
    body = PreviewTextField(_("Body"),
                            blank=True,
                            format='html',
                            bleached=True)
    body_short_preview = RichTextField(_("Preview"),
                                       blank=True,
                                       editable=False)
    body_full_preview = RichTextField(_("Preview (full)"),
                                      blank=True,
                                      editable=False)
    ...
    def as_paragraph(self, ar):
        ...
        # s = ar.obj2htmls(self)
        s = format_html("<b>{}</b> : {}", s, mark_safe(self.body_short_preview)
                        or _("(no preview)"))
        return s
...
class BabelPreviewable(BasePreviewable):
    ...
    body_short_preview = BabelTextField(_("Preview"),
                                        blank=True,
                                        editable=False)
    body_full_preview = BabelTextField(_("Preview (full)"),
                                       blank=True,
                                       editable=False)
...
class PreviewableChecker(Checker):
    ...
    def get_checkdata_problems(self, obj, fix=False):
        for x in self._get_checkdata_problems(settings.SITE.DEFAULT_LANGUAGE,
                                              obj, fix):
            yield x
