...
def waitServiceSocketResult(uuid):
    ...
    if activeSocketResult[5] != 0x01 or activeSocketResult[4] != 0x04:
        print('摄像头协议解析失败',activeSocketResult[5],activeSocketResult[4],activeSocketResult)
...
def __sendLockByClient(val):
    try:
        if not val or not clientServiceSocket:
            return
        with clientlock:
            activeMethod = clientServiceSocket.send if isinstance(val, str) else clientServiceSocket.send_bytes
            activeMethod(val)
            # time.sleep(0.05)
    except Exception as e:
