...
from soda_generator.connectors.connection_manager import ConnectionManager
import datetime
...
class Profiler(ConnectionManager):
    ...
    def compute_metrics(self, lazyframe, dt: str=None):
        ...
        df_describe =  pl.concat([
            ...
            lf.select(pl.col(numeric_columns+temporal_columns).min()).melt(variable_name='column',
                                                                           value_name='minimum'),
            lf.select(pl.col(numeric_columns+temporal_columns).max()).melt(variable_name='column',
                                                           value_name='maximum')
    ...
    def save_to_csv(self, lazyframe, dt: str=None):
        ...
        file_name = f'{lazyframe}_report.csv'
        ...
        print(f'Report {file_name} saved!')
