...
def parse_docx(filepath):
    ...
    MediLink_ConfigLoader.log("Extracting Date of Service from {}".format(filepath), level="INFO")
    
    date_of_service = extract_date_of_service(filepath)  # Extract date of service
    MediLink_ConfigLoader.log("Date of Service recorded as: {}".format(date_of_service), level="INFO")
...
def extract_date_of_service(docx_path):
    ...
    try:
        ...
        with zipfile.ZipFile(docx_path, 'r') as docx:
            ...
            MediLink_ConfigLoader.log("Extracted DOCX to: {}".format(extract_to))

        file_path = find_text_in_xml(extract_to, "Surgery Schedule")
        if file_path:
            return extract_date_from_file(file_path)
        else:
            MediLink_ConfigLoader.log("Target text 'Surgery Schedule' not found in any XML files.")
            return None
    finally:
        # Clean up the extracted files
        remove_directory(extract_to)
        MediLink_ConfigLoader.log("Cleaned up extracted files in: {}".format(extract_to))
                ...
                try:
                    ...
                    for elem in root.xpath('//w:t', namespaces=namespaces):
                        if elem.text and target_text in elem.text:
                            MediLink_ConfigLoader.log("Found target text in file: {}".format(file_path), level="INFO")
                            return file_path
                except Exception as e:
...
def extract_date_from_file(file_path):
    try:
        ...
        MediLink_ConfigLoader.log("Combined text: {}".format(combined_text), level="INFO")
        ...
        MediLink_ConfigLoader.log("Day of week found: {}".format(day_of_week.group() if day_of_week else 'None'), level="INFO")
        MediLink_ConfigLoader.log("Month and day found: {}".format(month_day.group() if month_day else 'None'), level="INFO")
        MediLink_ConfigLoader.log("Year found: {}".format(year_match.group() if year_match else 'None'), level="INFO")
