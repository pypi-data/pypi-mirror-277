...
def add_column(csv_data, column_header):
    # TODO column_header should be upgraded to a list. (put a check to make sure it is)
    for row in csv_data:
        row[column_header] = ''  # Initialize the column with empty values
...
def update_diagnosis_codes(csv_data):
    try:
        ...
        for row_num, row in enumerate(csv_data, start=1):
            ...
            surgery_date = row.get('Surgery Date', '').strip() 
            # This surgery date should match the docx with the CSV so we're getting the correct diagnosis code.
            # TODO current implementation is always going to pull the first diagnosis.
            if patient_id in all_patient_data and surgery_date in all_patient_data[patient_id]:
                diagnosis_code, left_or_right_eye, femto_yes_or_no = all_patient_data[patient_id][surgery_date]
                ...
                # Log if patient data not found for the row
                MediLink_ConfigLoader.log("Patient data not found for row {}: Patient ID: {}, Surgery Date: {}".format(row_num, patient_id, surgery_date), level="WARNING")
