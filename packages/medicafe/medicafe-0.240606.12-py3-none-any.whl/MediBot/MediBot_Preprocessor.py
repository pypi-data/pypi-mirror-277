...
def preprocess_csv_data(csv_data, crosswalk):
    try:
        ...
        # TODO This needs to be upgraded to be a list since we'll be adding a bunch of columns.
        MediLink_ConfigLoader.log("CSV Pre-processor: Adding 'Ins1 Insurance ID' column to the CSV data...", level="INFO")
        MediBot_Preprocessor_lib.add_column(csv_data, 'Ins1 Insurance ID')
        MediLink_ConfigLoader.log("CSV Pre-processor: Adding 'Default Diagnosis #1' column to the CSV data...", level="INFO")
        MediBot_Preprocessor_lib.add_column(csv_data, 'Default Diagnosis #1')
        MediLink_ConfigLoader.log("CSV Pre-processor: Adding 'Procedure Code' column to the CSV data...", level="INFO")
        MediBot_Preprocessor_lib.add_column(csv_data, 'Procedure Code')
        MediLink_ConfigLoader.log("CSV Pre-processor: Adding 'Minutes' column to the CSV data...", level="INFO")
        MediBot_Preprocessor_lib.add_column(csv_data, 'Minutes')
        MediLink_ConfigLoader.log("CSV Pre-processor: Adding 'Amount' column to the CSV data...", level="INFO")
        MediBot_Preprocessor_lib.add_column(csv_data, 'Amount')
