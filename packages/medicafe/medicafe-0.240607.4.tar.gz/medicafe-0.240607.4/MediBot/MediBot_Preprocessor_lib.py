...
def update_diagnosis_codes(csv_data):
    try:
        ...
        for row_num, row in enumerate(csv_data, start=1):
            ...
            MediLink_ConfigLoader.log("Patient ID: {}, Surgery Date: {}".format(patient_id, surgery_date), level="INFO")

            if patient_id in all_patient_data:
                if surgery_date in all_patient_data[patient_id]:
                    diagnosis_code, left_or_right_eye, femto_yes_or_no = all_patient_data[patient_id][surgery_date]
                    MediLink_ConfigLoader.log("Found diagnosis data for Patient ID: {}, Surgery Date: {}".format(patient_id, surgery_date), level="INFO")
