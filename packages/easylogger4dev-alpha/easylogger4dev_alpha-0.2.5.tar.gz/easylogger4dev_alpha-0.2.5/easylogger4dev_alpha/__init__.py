...
def logger_ini(is_log: bool = True):
    """初始化日志文件配置文件，使用日志模块前请先初始化，只需要初始化一次否则无法正常使用，可以自行开关，默认关闭
...
def log4csv(module, remarks=None, is_log: bool = True):
    """
             一个用于记录日志的装饰器，接受日志目录和日志编写模式的参数输入，在日志中记录运行的函数和运行的时间，仅适用于返回单个值的函数
...
def log4excel(module, remarks=None, is_log: bool = True):
    """
             一个用于记录日志的装饰器，接受日志目录和日志编写模式的参数输入，在日志中记录运行的函数和运行的时间，仅适用于返回单个值的函数
