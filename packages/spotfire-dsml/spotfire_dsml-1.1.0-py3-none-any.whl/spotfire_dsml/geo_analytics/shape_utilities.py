...
def _calculate_centroid(crs,gdf, 
    ...
    If representative_point is True, the centroid will be evaluated by geopandas as
    ...
    and ignore_projection is set to False, it will be re-projected to EPSG:6933, 
    ...
    gdf : geopandas GeoDataFrame
        The input geo dataframe.
    representative_point: bool, optional
        If True, then return a representative point that is guaranteed to lie
        within the perimeter. If False, calculate the centroid.
        The default is False.
    ignore_projection: bool, optional
        Ignored if representative_point is True.
        Otherwise, if True, and the CRS is geographic, the centroid is calculated 
        without projecting. If False, the 
        geometry is first projected to EPSG:6933 before calculating the centroid.
        Ignored if the CRS is already projected.
        The default is True.
    ...
    if representative_point:
        ...
        if not ignore_projection and not crs_utilities.crs_is_projected(crs):
            try:
...
def create_bounding_box(width,height,unit,a,b=None,box_id=None,source_crs='EPSG:4326'):
    ...
    bounding_box['geometry'] = bounding_box.apply(lambda x: 
                                                    _build_bounding_rectangle(x.lat, 
                                                                         x.lon, 
