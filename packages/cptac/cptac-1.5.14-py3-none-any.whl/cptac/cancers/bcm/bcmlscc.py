...
class BcmLscc(Source):
    def __init__(self, no_internet=False):
        ...
        self.data_files = {
            ...
            "phosphoproteomics" : ["LSCC_phospho_site_abundance_log2_reference_intensity_normalized_Tumor.txt","LSCC_phospho_site_abundance_log2_reference_intensity_normalized_Normal.txt.gz"],
            "CNV" : "LSCC_WES_CNV_gene_ratio_log2.txt.gz",
            "miRNA" : ["LSCC_miRNAseq_mature_miRNA_RPM_log2_Tumor.txt.gz","LSCC_miRNAseq_mature_miRNA_RPM_log2_Normal.txt.gz"]
        }
    ...
    def load_phosphoproteomics(self):
        ...
        if df_type not in self._data:
            ...
            for file_path in file_path_list:
                ...
                if file_name == "LSCC_phospho_site_abundance_log2_reference_intensity_normalized_Tumor.txt":
                ...
                if file_name == "LSCC_phospho_site_abundance_log2_reference_intensity_normalized_Normal.txt.gz":
                    # Load and process the file
                    df = pd.read_csv(file_path, sep='\t')
                    df.index.name = 'gene'
    ...
    def load_miRNA(self):
        ...
        if df_type not in self._data:
            ...
            for file_path in file_path_list:
                ...
                # Load and process the files
                if file_name == "LSCC_miRNAseq_mature_miRNA_RPM_log2_Tumor.txt.gz":
                    df = pd.read_csv(file_path, sep='\t')
                ...
                if file_name == "LSCC_miRNAseq_mature_miRNA_RPM_log2_Normal.txt.gz":
                    df = pd.read_csv(file_path, sep='\t')
