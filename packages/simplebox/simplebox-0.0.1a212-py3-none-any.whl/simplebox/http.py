...
class RestFast(object):
    ...
    def __init__(self, host, http2: bool = False, retry_times: int = 3, retry_backoff_factor: int = 5,
        ...
        self.__method: str = HttpMethod.OPTIONS.value
        self.__kw = kwargs
    ...
    def method(self,  method: Union[HttpMethod, str]) -> 'RestFast':
        ...
        if isinstance(method, str):
            self.__method = HttpMethod.get_by_value(method.upper()).value
        elif isinstance(method, HttpMethod):
            self.__method = method.value
        else:
            _LOGGER.warning(f"invalid http method: '{method}', use 'OPTIONS'")
    ...
    def __request(self, func: callable, kwargs: dict, api_name: str = None, server_name: str = None, host: str = None,
        ...
        try:
            ...
            try:
                ...
                _LOGGER.log(level=LogLevel.WARNING.value, msg=f"RestResponse content decode error: {str(e)}", stacklevel=2)
