...
def assert_game_result_data(data: dict = None):
    ...
    assert type(data) == dict
    assert_contains_keys(data, [K_FRAME_USED, K_STATUS, K_ATTACHMENT])
    assert data[K_STATUS] in GameResultState.__dict__.values()

    assert isinstance(data[K_ATTACHMENT], list)
    for obj in data[K_ATTACHMENT]:
        assert_contains_keys(obj,['rank','player_num'])
