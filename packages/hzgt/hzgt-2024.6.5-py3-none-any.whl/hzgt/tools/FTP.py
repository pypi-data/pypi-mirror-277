...
class Ftpserver:
    ...
    def add_user(self, homedir: str, username: str = "", password: str = "", perm: str = ""):
        """
        添加用户权限和路径 可以为不同的用户添加不同的目录和权限 如果 username 为空, 则添加匿名用户
...
class Ftpclient:
    def __init__(self, host: str, port: int = 2121, username: str = "anonymous", password: str = "", encoding: str = "utf-8"):
        ...
        print(f"`{restrop(username, f=4)}` 登陆FTP服务器 `{restrop(host)}`", end="    ")
    ...
    def getfile(self, server_filename: str, savepath: str = "FTP_Files", savename: str = "", blocksize: int = 8 * 1024):
        ...
        with (trange(sfsize[2], desc='下载中', unit="B/s", unit_divisor=1024, unit_scale=True, ncols=100) as tbar,
    ...
    def upload(self, local_file: str, server_savename: str = "", blocksize: int = 8 * 1024):
        ...
        time.sleep(0.17)
        with (trange(lfsize[2], desc='上传中', unit="B/s", unit_divisor=1024, unit_scale=True, ncols=100) as tbar,
