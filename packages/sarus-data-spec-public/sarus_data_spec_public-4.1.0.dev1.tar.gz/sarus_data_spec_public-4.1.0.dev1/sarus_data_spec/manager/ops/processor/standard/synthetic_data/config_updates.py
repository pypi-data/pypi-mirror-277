...
def update_table_config(
    ...
    if table_config.independent_generation is not None:
        independent_config = update_independent_config(
            table_stat, table_config.independent_generation
        )
        table_config.independent_generation = independent_config
...
def update_independent_config(
    table_stat: st.Statistics, independent_config: sd_config.IndependentConfig
) -> sd_config.IndependentConfig:
    ...
    independent_config.sampling_config.total_size = table_stat.size()
    stat_fields = table_stat.children()

    for column_name, col_config in independent_config.columns.items():
        curr_stat = stat_fields[column_name]
        if isinstance(col_config, sd_config.OptionalIndependentColumn):
...
def update_correlation_config(
    ...
    stat_fields = table_stat.children()
    # update columns distributions
    for column_name, col_config in correlation_config.columns.items():
        curr_stat = stat_fields[column_name]
    ...
    sub_builders = [
        builder_from_col_config(name, col_config, stat_fields[name])
...
def builder_from_col_config(
    ...
    if isinstance(col_config, sd_config.OptionalCorrelationColumn):
        return OptionalBuilder(
            sub_builder=builder_from_col_config(
                OPTIONAL_VALUE,
