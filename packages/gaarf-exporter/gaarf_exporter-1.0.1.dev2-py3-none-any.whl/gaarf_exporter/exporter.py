...
class GaarfExporter:
  ...
  def _init_service_metrics(self, namespace: str = 'gaarf_') -> None:
    ...
    """
    self.export_started = self._define_gauge(
      'export_started_seconds', suffix='Remove', namespace=namespace
    )
    self.export_completed = self._define_gauge(
      'export_completed_seconds', suffix='Remove', namespace=namespace
    )
