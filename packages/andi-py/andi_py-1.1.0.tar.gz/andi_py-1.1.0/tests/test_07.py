...
are averaged depending on the avg parameter
...
def plot_close(event):
    active[0] = False
def mouse_evt(event):
    if event.button == 'down' and avg[0]>1:
        avg[0]//=2
    if event.button == 'up':
        avg[0]*=2
    print(avg[0])
...
with ai.Andi() as test:
    ...
    # set the generators
    test.sine(channel=1, freq=500.1, amp=0.2)
    ...
    fig.canvas.mpl_connect('scroll_event', mouse_evt)
