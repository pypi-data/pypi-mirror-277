...
class EnrichTypes:
    ...
    def enrich_doc(self, csv_doc: dict, line_number: int) -> dict:
        """
        ...
        try:
            ...
            self._logger.error(f"At {line_number}:{self._filename}")
        ...
        # for i, (k, v) in enumerate(csv_doc.items(), 1):
        #     # print( "field: %s" % k )
        ...
        #     # try:
        #     try:
        #         new_doc[k] = self._converter.convert(self._field_file.type_value(k), v, self._field_file.format_value(k))
        #
        #     except ValueError:
        #         if self._onerror == ErrorResponse.Fail:
        #             if self._log:
        ...
        #         elif self._onerror == ErrorResponse.Warn:
        #             self._log.warning(f"Parse failure at line {line_number}:{self._filename} at field '{k}'\n"
        #                               f"type conversion error: Cannot convert '{v}' to type {type_field} using string type instead")
        #             new_doc[k] = str(v)
        #         elif self._onerror == ErrorResponse.Ignore:
        #             new_doc[k] = str(v)
        #         else:
