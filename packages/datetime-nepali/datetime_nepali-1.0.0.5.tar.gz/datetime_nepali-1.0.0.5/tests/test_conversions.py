...
from datetime_nepali.config import MINDATE, REFERENCE_DATE_AD
...
class TestConversion:
    ...
    def test_reference_dates(self):
        ...
        date_nepali = datetime_nepali.date.from_datetime_date(from_date=date_english)
    ...
    def test_random_conversions(self):
        for rd_maps in RANDOM_CONVERSION_MAPS:
            np_dt = datetime_nepali.date.from_datetime_date(datetime.date(**rd_maps['ad']))
            assert np_dt == datetime_nepali.date(**rd_maps['bs'])

            dt = datetime_nepali.date.to_datetime_date(datetime_nepali.date(**rd_maps['bs']))
