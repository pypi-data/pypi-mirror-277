...
from budget.utils import flatten, sum_amounts
...
def to_cells(s: Summary | None) -> tuple[str, str]:
    ...
    return f"{s.balance:.2f}", f"{perc}% of {s.budget}"
...
@click.pass_obj
def balance(
    args: Args, title, no_auto_excludes, unbudgeted, only_unbudgeted, empty, **kwargs
):
    ...
    total = sum_amounts(db.iter())

    t = Table(title=title.format(commodity=", ".join(db.commodities)))
    t.add_column(header="Account")

    footer: list[str] = []
    for comm in db.commodities:
        t.add_column(header=["Balance", comm], style=ColumnStyle(justify="right"))
        t.add_column(
            header=["Budget Execution", comm], style=ColumnStyle(justify="right")
        )
        footer.extend(to_cells(total.get(comm)))
    ...
    for account, commodities in collect_rows(
        db.iter(**matchers), db.commodities
    ).items():
        values = flatten([to_cells(c) for c in commodities.values()])
        t.add_row(account, *values)
