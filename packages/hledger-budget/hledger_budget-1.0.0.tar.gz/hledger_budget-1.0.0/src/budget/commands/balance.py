...
from budget.utils import flatten, rprint, sum_amounts
...
def balance(
    ...
    t = Table(
        title=title.format(commodity=", ".join(db.commodities)),
        show_header=True,
        show_footer=True,
        header_style="bold",
        box=box.MINIMAL,
    ...
    for commodity in db.commodities:
        cells = to_cells(total.get(commodity))
        t.add_column(f"Balance\n{commodity}", cells[0], justify="right")
        t.add_column(f"Budget Execution\n{commodity}", cells[1], justify="right")
