...
Examples:
    ...
    :attr:`ccu.fancyplots.gui.root.FancyPlotsGUI.cache` and dumped into
...
class FEDData(TypedDict):
    ...
    Keys:
        energy_data: Each entry contains energies for each step in the
            mechanism.
...
class FormattingParameters(TypedDict):
    ...
    font: Annotated[
        str,
        "Defines a non-default font type. Currently supported: "
...
class Annotation(NamedTuple):
    ...
    color: str = "k"
    fontsize: float = DEFAULT_PARAMETERS["fontsize"] * 0.8
    text: str = ""
    x: float = 0.0
    y: float = 0.0
