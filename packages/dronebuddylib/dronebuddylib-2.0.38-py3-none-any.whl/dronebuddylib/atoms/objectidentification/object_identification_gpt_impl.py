...
class ObjectIdentificationGPTImpl(IObjectIdentification):
    ...
    def __init__(self, engine_configurations: EngineConfigurations):
        ...
        model = configs.get(AtomicEngineConfigurations.OBJECT_IDENTIFICATION_GPT_MODEL.name,
                                     configs.get(AtomicEngineConfigurations.OBJECT_IDENTIFICATION_GPT_MODEL))
        self.model = model
    ...
    def get_agent(self, agent_name):
        """
        Gets the agent with the given name.

        Args:
            agent_name: The name of the agent to get.

        Returns:
            ILLMAgent: The LLM agent with the given name.
        """
        if agent_name == LLMAgentNames.OBJECT_IDENTIFIER:
            return self.object_identifier
        elif agent_name == LLMAgentNames.IMAGE_VALIDATOR:
            return self.image_validator
        elif agent_name == LLMAgentNames.IMAGE_DESCRIBER:
    ...
    def validate_reference_image(self, image, image_type) -> ImageValidatorResults:
        ...
        """
        self.image_validator.send_encoded_image_message_to_llm_queue("user",
                                                                                "VALIDATE( " + image_type + ")",
                                                                                image)


        validity = self.image_validator.get_result()
        return validity
