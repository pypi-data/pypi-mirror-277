...
class Bd:
    ...
    def mouse_drag(self,x,y,x2,y2,delay=0.4):
        '''
    ...
    def img_click(self,bind,img,wait=0.5,grayscale=True,Offset_x=0,Offset_y=0,confidence=0.9):
        ...
        try:
            print(f"嘗試點擊圖像: {img}")
            search_img = img_center_point2(img, bind, confidence=confidence, grayscale=grayscale, Offset_x=Offset_x, Offset_y=Offset_y)
            if search_img:
                x,y = search_img
                self.x = x; self.y = y
                ...
                time.sleep(wait)
                win32api.SendMessage(self.hwnd, win32con.WM_LBUTTONDOWN, 1, ((click_x) << 16 | (click_y)))
                win32api.SendMessage(self.hwnd, win32con.WM_LBUTTONUP, 0, ((click_x) << 16 | (click_y)))
