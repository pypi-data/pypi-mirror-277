...
def sampling_algorithm_structural_analysis(setup):
    ...
    Args:
        ...
        setup keys:
            'objective function' (Python function [def]): Objective function
            'number of samples' (Integer): Number of samples
            'dimension' (Integer): Number of dimensions
            'numerical model' (Dictionary): Numerical model settings
    ...
    try:
        ...
        if type_process.upper() == 'PARALLEL':
            ...
            with Pool() as p:
                result = p.map_async(func=parepyco.evaluation_model, iterable=information_model)
                result = result.get()
            for i in range(len(dataset_x)):
                capacity[i, :] = result[i][0].copy()
                demand[i, :] = result[i][1].copy()
                state_limit[i, :] = result[i][2].copy()
                indicator_function[i, :] = [0 if value <= 0 else 1 for value in result[i][2]]
