...
class RFEConnector(BaseConnector):
    """
    ...
    def get_blobs(self, file_prefixes, delete_after_downloads = True):
        """
        Downloads files from an Azure Blob Storage container using a SAS key, filtering by file prefixes.
        ...
        delete_after_downloads : bool, optional
            If True, deletes the files from the blob storage after downloading (default is True).
        ...
        Notes
        -----
        This method uses a SAS (Shared Access Signature) key to authenticate and authorize access 
        to the Azure Blob Storage container. It filters the blobs based on the provided file prefixes, 
        downloads the matching files, optionally deletes them from the storage, and logs each step of 
        the process.
        """
        try:
