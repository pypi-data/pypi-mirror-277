...
def create_fake_line(layer, empty_gdf):
    ...
    for name, dtype in dict(empty_gdf.dtypes).items():
        if str(dtype) == "object":
            fake_line[name] = "fake"
        elif str(dtype).lower().find("int") >= 0:
            fake_line[name] = 1
        elif str(dtype).lower().find("float") >= 0:
            fake_line[name] = 0.1
        elif str(dtype).lower().find("datetime") >= 0:
            fake_line[name] = datetime.datetime(2020, 1, 1, 0, 0, 0)
        elif str(dtype).lower().find("geometry") >= 0:
            fake_line[name] = fake_geometry
    
    return gpd.GeoDataFrame(fake_line, geometry = "SHAPE", crs=empty_gdf.crs)
