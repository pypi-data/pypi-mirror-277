...
class Section(xySection):
    ...
    def _make_path(self, path: Path, auto_create_path: bool = False) -> Path | None:
        if not is_kind_of_type(path, Path):
            return None
        path = self.absolute_path(path=path)
        if auto_create_path and is_kind_of_type(path, Path):
            if path.suffix:
    ...
    def absolute_path(self, path: Path) -> Path | None:
        if not path.is_absolute():
            ...
            if is_kind_of_type(settings_cfg_path, Path) and settings_cfg_path.exists():
                path = settings_cfg_path.parent.resolve().joinpath(path)
        if is_kind_of_type(path, Path):
            return path.resolve()
        return path
