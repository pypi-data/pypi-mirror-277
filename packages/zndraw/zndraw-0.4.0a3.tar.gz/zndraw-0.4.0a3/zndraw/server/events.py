...
from ..tasks import run_analysis, run_modifier, run_selection, run_geometry
...
def webclient_connect():
    ...
    emit("modifier:schema", Modifier.updated_schema())
    emit("scene:schema", Scene.updated_schema())
    emit("geometry:schema", Geometry.updated_schema())
    emit("analysis:schema", Analysis.updated_schema())
...
@io.on("geometry:run:finished")
def geometry_run_finished():
    """Forwarding finished message."""
    room = session.get("token")
    emit("geometry:run:finished", to=room)


@io.on("geometry:run:running")
def geometry_run_running():
    """Forwarding running method."""
    room = session.get("token")
    emit("geometry:run:running", to=room)
