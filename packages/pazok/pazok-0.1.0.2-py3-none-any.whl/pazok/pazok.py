...
def cook():
    class b_azok:
        ...
        @property
        def mid(self):
            return re.search(r'"mid":\{"value":"(.*?)"', self.text).group(1)
        
        @property
        def ig_did(self):
            return re.search(r'"_js_ig_did":\{"value":"(.*?)"', self.text).group(1)
        
        @property
        def datr(self):
            return re.search(r'"_js_datr":\{"value":"(.*?)"', self.text).group(1)
        
        @property
        def cid(self):
            return re.search(r'"client_id":"(.*?)"', self.text).group(1)
