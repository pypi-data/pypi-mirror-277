...
def cook():
    ...
    class b_azok:
        def init(self):
            ...
            self._mid = re.search(r'"mid":\{"value":"(.*?)"', response.text).group(1)
            self._ig_did = re.search(r'"_js_ig_did":\{"value":"(.*?)"', response.text).group(1)
            self._datr = re.search(r'"_js_datr":\{"value":"(.*?)"', response.text).group(1)
            self._client_id = re.search(r'"client_id":"(.*?)"', response.text).group(1)
...
def install(pkg):
    ...
    installed_version = _get_latest_version(pkg)
    if installed_version:
        print(f"Installed version of {pkg}: {installed_version}")
        latest_version = json.loads(url.urlopen(f"https://pypi.org/pypi/{pkg}/json").read())['info']['version']
        print(f"Latest version of {pkg}: {latest_version}")
        if latest_version != installed_version:
            print(f"Updating {pkg} to version {latest_version}")
            cmd = [sys.executable, '-m', 'pip', 'install', '--upgrade', pkg]
            subprocess.check_call(cmd)
        else:
            print(f"{pkg} is already up-to-date")
    else:
        print(f"{pkg} is not installed. Installing latest version")
