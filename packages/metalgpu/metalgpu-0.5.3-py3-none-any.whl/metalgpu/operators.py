...
def sqrt(buf):
    ...
    kernel void sqrt_func(device {buf.toMetalType(buf.contents.dtype)} *arr1 [[buffer(0)]], device {buf.toMetalType(buf.contents.dtype)} *arr2 [[buffer(1)]], uint id [[thread_position_in_grid]]) {{
...
def cos(buf):
    ...
    kernel void cos_func(device {buf.toMetalType(buf.contents.dtype)} *arr1 [[buffer(0)]], device {buf.toMetalType(buf.contents.dtype)} *arr2 [[buffer(1)]], uint id [[thread_position_in_grid]]) {{
        arr2[id] = cos(arr1[id]);
    }};
    """
    prevShader = buf.interface._loaded_shader
...
def sin(buf):
    ...
    #include <metal_stdlib>
    using namespace metal;
    kernel void sin_func(device {buf.toMetalType(buf.contents.dtype)} *arr1 [[buffer(0)]], device {buf.toMetalType(buf.contents.dtype)} *arr2 [[buffer(1)]], uint id [[thread_position_in_grid]]) {{
        arr2[id] = sin(arr1[id]);
    }};
    """
    prevShader = buf.interface._loaded_shader
