...
class Buffer:
    ...
    def __add__(self, other):
        ...
        outBuffer = self.interface.create_buffer(len(self.contents), self.contents.dtype)
        add_kernel = """
        #include <metal_stdlib>

        using namespace metal;

        kernel void add(const device *arr1 [[buffer(0)]], const device *arr2 [[buffer(1)]], device *arr3 [[buffer(2)]], uint id [[thread_position_in_grid]]) {
    ...
    def __sub__(self, other):
        ...
        #include <metal_stdlib>
        using namespace metal;
        kernel void sub(const device *arr1 [[buffer(0)]], const device *arr2 [[buffer(1)]], device *arr3 [[buffer(2)]], uint id [[thread_position_in_grid]]) {
            arr3[id] = arr1[id] - arr2[id];
        };
        """
        prevShader = self.interface._loaded_shader
