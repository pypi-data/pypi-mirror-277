...
class InvoiceSerializer:
    ...
    def _invoice_body(self) -> FatturaElettronicaBody:
        ...
        summary_by_category_and_rate = defaultdict(list)

        for tax in tax_summary:
            category = get_tax_category(tax)
            rate = tax.tax_rate

            summary_by_category_and_rate[category, rate].append(tax)

        dati_riepilogo = []

        for (category, tax_rate), taxes in summary_by_category_and_rate.items():
            imponibile_importo = sum([tax.gross_value - tax.tax_value for tax in taxes])
            imposta = sum([tax.tax_value for tax in taxes])
            aliquota_iva = tax_rate

            dati_riepilogo.append(
                DatiRiepilogo(
                    aliquota_iva=str(aliquota_iva),
                    imponibile_importo=str(imponibile_importo),
                    imposta=str(imposta),
                    natura=category,
                )
            )
        ...
        total_invoice = str(sum([line.gross_value for line in lines]))
        ...
        dati_pagamento = DatiPagamento(
            ...
            dettaglio_pagamento=DettaglioPagamento(
                modalita_pagamento=payment_method, importo_pagamento=total_invoice
            ),
        )
