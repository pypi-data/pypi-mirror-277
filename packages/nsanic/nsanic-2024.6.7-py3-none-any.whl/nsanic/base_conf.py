...
from nsanic.libs.consts import StaCode, G_TIME_ZONE
...
class BaseConf(Config):
    ...
    def __new__(cls, *args, **kwargs):
        with BaseConf.__ins_lock:
            if not hasattr(BaseConf, '_ins'):
                BaseConf._ins = Config.__new__(cls)
        return BaseConf._ins
    ...
    CONF_RDS = {
        'default': {
            'host': '122.122.122.12',
            'port': 6379,
            'password': 'local!redis',
            'db': 1,
            'max_connections': 5,  # 最大连接数
            # 'decode_responses': True,  # 默认解析字符串，不解析默认为bytes类型
            'socket_connect_timeout': 1,  # 连接超时时长 s
        }
    }
    ...
    def set_conf(cls):
        ...
        cls.log = MultLogger(cls.LOG_PATH, cls.SERVER_NAME)
        if cls.CONF_RDS:
            cls.rds = RdsClient.init(cls.CONF_RDS['default'], logs=cls.log)

    @classmethod
    def __db_conf(cls, model_list: list):
        return {
