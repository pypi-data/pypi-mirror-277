...
rds_val = None if use_redis != 'y' else '''{
        'default': {
            'host': '127.0.0.1', 
            'port': 6379, 
            'password': '', 
            'db': 1, 
            'max_connections': 5,  # 最大连接数 进程开多按情况降低连接数
            # 'decode_responses': True,  # 默认解析字符串，不解析默认为bytes类型
            'socket_connect_timeout': 2,  # 连接超时时长
        }, 
    }'''
...
ws_model = '''from nsanic.base_ws import RdsWS
from nsanic.libs.manager import WsConnector''' if use_mode in (2, 3) else ''
ws_base = f'''\n\n\nclass BaseWS(RdsWS):
