...
class RdsWS(ConfDI):
    ...
    async def listen_offline_queue(self):
        while 1:
            ukey = await self.off_queue.get()
            await self.ws_manager.call_offline(ukey)
            for k, v in self.CMD_MAP.items():
                try:
                    await v.off_line(ukey)
