"""
@generated by mypy-protobuf.  Do not edit manually!
isort:skip_file
"""

import abc
import collections.abc
import grpc
import grpc.aio
import typing
import yandex.cloud.priv.iam.v1.console.user_account_service_pb2
import yandex.cloud.priv.operation.operation_pb2

_T = typing.TypeVar("_T")

class _MaybeAsyncIterator(collections.abc.AsyncIterator[_T], collections.abc.Iterator[_T], metaclass=abc.ABCMeta): ...

class _ServicerContext(grpc.ServicerContext, grpc.aio.ServicerContext):  # type: ignore[misc, type-arg]
    ...

class UserAccountServiceStub:
    def __init__(self, channel: typing.Union[grpc.Channel, grpc.aio.Channel]) -> None: ...
    RevokeAllAccessKeys: grpc.UnaryUnaryMultiCallable[
        yandex.cloud.priv.iam.v1.console.user_account_service_pb2.RevokeAllAccessKeysRequest,
        yandex.cloud.priv.operation.operation_pb2.Operation,
    ]
    """Deletes all access keys for the current authenticated user."""

class UserAccountServiceAsyncStub:
    RevokeAllAccessKeys: grpc.aio.UnaryUnaryMultiCallable[
        yandex.cloud.priv.iam.v1.console.user_account_service_pb2.RevokeAllAccessKeysRequest,
        yandex.cloud.priv.operation.operation_pb2.Operation,
    ]
    """Deletes all access keys for the current authenticated user."""

class UserAccountServiceServicer(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def RevokeAllAccessKeys(
        self,
        request: yandex.cloud.priv.iam.v1.console.user_account_service_pb2.RevokeAllAccessKeysRequest,
        context: _ServicerContext,
    ) -> typing.Union[yandex.cloud.priv.operation.operation_pb2.Operation, collections.abc.Awaitable[yandex.cloud.priv.operation.operation_pb2.Operation]]:
        """Deletes all access keys for the current authenticated user."""

def add_UserAccountServiceServicer_to_server(servicer: UserAccountServiceServicer, server: typing.Union[grpc.Server, grpc.aio.Server]) -> None: ...
