    ...
    def __init__(self):
        ...
        if self.repo:
            if self.repo.endswith('.git'):
                ...
                elif self.repo.startswith('ssh://yl-gerrit'):  # like 'ssh://yl-gerrit.eng.t-head.cn:29518/yocopen/manifest.git'
                    self.group = self.repo.split('/')[-2]
                else:
                ...
                if self.repo.startswith('http'):    # like 'https://gitee.com/yocop'
                    ...
                    self.group = self.repo.split(':')[-1]
                elif self.repo.startswith('ssh://yl-gerrit'):  # like 'ssh://yl-gerrit.eng.t-head.cn:29518/yocopen'
                    self.group = self.repo.split('/')[-1]
                else:
                    put_string('The repo is wrong!', level='warning')
