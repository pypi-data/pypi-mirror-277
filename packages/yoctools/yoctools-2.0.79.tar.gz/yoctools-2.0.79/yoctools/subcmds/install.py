...
"""
    ...
    def Execute(self, opt, args):
        ...
        if yoc.conf.commit_msg_url:
            cmsg_tmp_file = os.path.join('/tmp', "commit-msg")
            if os.path.isfile(cmsg_tmp_file):
                try:
                    os.remove(cmsg_tmp_file)
                except Exception as e:
                    put_string("[%s]\nCan't update %s, maybe will fail to install commit-msg in component." % (str(e), cmsg_tmp_file), level='warning')
                    cmsg_tmp_file = 'not_permitted'

        def __post_function(comp_path):
            try:
                if yoc.conf.commit_msg_url and os.path.exists(comp_path):
                    if cmsg_tmp_file != 'not_permitted' and not os.path.isfile(cmsg_tmp_file):
                        cmd = "wget %s -q -O %s && chmod +x %s >/dev/null" % (yoc.conf.commit_msg_url, cmsg_tmp_file, cmsg_tmp_file)
                        os.system(cmd)
                    shutil.copy2(cmsg_tmp_file, os.path.join(comp_path, ".git/hooks"))
            except:
                pass
