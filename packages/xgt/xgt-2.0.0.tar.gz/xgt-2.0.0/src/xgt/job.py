...
class Job(object):
  ...
  """
  def __init__(self, conn, job_response = None, python_errors = None):
    """
  ...
  def start_time(self) -> str:
    ...
    if 'start_time' in self._data:
      if '.' in self._data['start_time']:
        return datetime.strptime(self._data['start_time'],
                                 '%Y-%m-%dT%H:%M:%S.%f')
  ...
  def total_rows(self):
    ...
    """
    _deprecated('total_rows', 'num_rows')
  ...
  def get_data(self, offset : int = 0, length : int = None,
               rows : Iter[int] = None, columns : Iter[Union[int, str]] = None,
               format : str = 'python') -> Union[List[List], pandas.DataFrame,
                                                 pyarrow.Table, None]:

    """
    Returns results data starting at a given offset and spanning a given
    length.
  ...
  def get_data_arrow(self, offset=0, length=None):
    """
    Returns an Apache Arrow Table containing results data starting at a given
    ...
    return self._load_arrow_table(offset, length)

  def get_data_pandas(self, offset=0, length=None):
    """
    Returns a Pandas DataFrame containing results data starting at a given
    offset and spanning a given length.
    ...
    return _get_data_pandas_from_table(self._load_arrow_table(offset, length),
                                       self._data['schema'], self._conn)
