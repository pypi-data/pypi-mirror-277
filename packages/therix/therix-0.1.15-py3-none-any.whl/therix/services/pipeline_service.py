...
from ..db.session import SessionLocal
from therix.utils.rag import chat, create_embeddings, get_embedding_model, get_vectorstore, get_loader, async_chat
...
class PipelineService:
    ...
    async def async_invoke_pipeline(self, pipeline_id, question, session_id, trace_details=None, system_prompt=None, pipeline_ids=None):
        embedding_model = self.get_pipeline_configuraitons_by_type(
            pipeline_id, "EMBEDDING_MODEL"
        )
    ...
    def invoke_pipeline(self, pipeline_id, question, session_id, trace_details=None, system_prompt=None, pipeline_ids=None):
        embedding_model = self.get_pipeline_configuraitons_by_type(
            pipeline_id, "EMBEDDING_MODEL"
        )
    ...
    def async_search_keywords(self, keyword_search_params):
        if(not keyword_search_params.get("prompt") or not keyword_search_params.get("keywords") or not keyword_search_params.get("output_parser")):
        ...
        if("pipeline_ids" in keyword_search_params and len(keyword_search_params.get('pipeline_ids'))):
            combined_retrievers = []
            inference_model = self.get_pipeline_configuraitons_by_type(keyword_search_params.get("pipeline_ids")[len(keyword_search_params.get("pipeline_ids"))-1], ConfigType.INFERENCE_MODEL)
            for pipeline_id in keyword_search_params.get('pipeline_ids'):
                embedding_model = self.get_pipeline_configuraitons_by_type(pipeline_id, ConfigType.EMBEDDING_MODEL)
                store = get_vectorstore(embedding_model[0], str(pipeline_id))
                combined_retrievers.append(store.as_retriever())
            retriever = MergerRetriever(retrievers=combined_retrievers)
        else:
            inference_model = self.get_pipeline_configuraitons_by_type(keyword_search_params.get("pipeline_id"), ConfigType.INFERENCE_MODEL)
            embedding_model = self.get_pipeline_configuraitons_by_type(keyword_search_params.get("pipeline_id"), ConfigType.EMBEDDING_MODEL)
            store = get_vectorstore(embedding_model[0], str(keyword_search_params.get("pipeline_id")))
