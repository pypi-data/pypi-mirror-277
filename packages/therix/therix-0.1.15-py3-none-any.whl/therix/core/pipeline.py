...
class Pipeline:
    ...
    async def async_invoke(self, question=None, session_id=None , keyword_search_params=None, dynamic_system_prompt=None):
        ...
        if pipeline_type == PipelineTypeMaster.RAG.value:
            ...
            if keyword_search_params and question is None:
                if(isinstance(self.pipeline_ids,list) and len(self.pipeline_ids) > 0 and isinstance(self.pipeline_data.id , uuid.UUID)):
                ...
                answer = self.pipeline_service.async_search_keywords(keyword_search_params)
            else:
                if(isinstance(self.pipeline_ids,list) and len(self.pipeline_ids) > 0 and isinstance(self.pipeline_data.id , uuid.UUID)):
                    self.pipeline_ids.append(str(self.pipeline_data.id))
                    answer = self.pipeline_service.async_invoke_pipeline(
                        self.pipeline_data.id, question, session_id, trace_details, pipeline_system_prompt, pipeline_ids=self.pipeline_ids
                    ) if pipeline_system_prompt else self.pipeline_service.async_invoke_pipeline(
                        self.pipeline_data.id, question, session_id, trace_details, pipeline_ids=self.pipeline_ids
                    )
                elif(isinstance(self.pipeline_ids,list) and len(self.pipeline_ids) > 0):
                    pipeline_data_id = self.pipeline_ids[len(self.pipeline_ids)-1]
                    answer = self.pipeline_service.async_invoke_pipeline(
                        pipeline_data_id, question, session_id, trace_details, pipeline_system_prompt, pipeline_ids=self.pipeline_ids
                    ) if pipeline_system_prompt else self.pipeline_service.async_invoke_pipeline(
                        pipeline_data_id, question, session_id, trace_details, pipeline_ids=self.pipeline_ids
                    )
                else:
                    answer = self.pipeline_service.async_invoke_pipeline(
        ...
        # Handle Summarizer pipeline type
            ...
            answer = self.pipeline_service.async_invoke_summarizer_pipeline(
                self.pipeline_data.id, text=question, trace_details=trace_details, system_prompt=pipeline_system_prompt
            ) if pipeline_system_prompt else self.pipeline_service.async_invoke_summarizer_pipeline(
