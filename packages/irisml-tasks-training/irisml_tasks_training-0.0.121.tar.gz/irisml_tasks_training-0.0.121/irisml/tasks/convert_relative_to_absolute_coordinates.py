...
class Task(irisml.core.TaskBase):
    ...
    def execute(self, inputs):
        ...
        for idx, (image, _) in enumerate(inputs.dataset):
            ...
            inputs.predictions[idx][:, 2:] = self._convert_coordinates(inputs.predictions[idx][:, 2:], height, width)
            inputs.targets[idx][:, 1:] = self._convert_coordinates(inputs.targets[idx][:, 1:], height, width)

        return self.Outputs(inputs.predictions, inputs.targets)
