...
class TestConvertRelativetoAbsoluteCoordinates(unittest.TestCase):
    def test_convert_relative_to_absolute_coordinates(self):
        ...
        self.assertEquals(outputs.predictions[0][0].tolist(), [0, 0, 0, 0, 100, 100])
        self.assertEquals(outputs.predictions[0][1].tolist(), [0, 0, 0, 0, 50, 50])
        self.assertEquals(outputs.targets[0][0].tolist(), [0, 0, 0, 100, 100])
