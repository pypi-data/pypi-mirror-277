...
class WOLightSource(LightSource, LightSourceDecorator):
    ...
    def to_python_code(self, do_plot=True):
        ...
        if self._kind_of_wave == 0: #plane
            ...
            if self._dimension == 1:
                txt += "\noutput_wavefront.set_gaussian(sigma_x=%f, amplitude=%f,shift=%f)"%\
                       (self._sigma_h, self._amplitude, self._gaussian_shift)
            elif self._dimension == 2:
                txt += "\noutput_wavefront.set_gaussian(sigma_x=%f, sigma_y=%f, amplitude=%f)"%\
                       (self._sigma_h, self._sigma_v, self._amplitude)

        elif self._kind_of_wave == 3: # g.s.m.
            if self._dimension == 1:
                txt += "\noutput_wavefront.set_gaussian_hermite_mode(sigma_x=%g,amplitude=%g,mode_x=%d,shift=%f,beta=%g)"%\
