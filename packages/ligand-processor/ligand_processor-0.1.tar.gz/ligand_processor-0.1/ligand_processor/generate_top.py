...
# Define standard masses for common atom types
standard_masses = {
    'H': 1.008,
    'C': 12.01,
    'N': 14.01,
...
def generate_topology():
    ...
    # Manually create the PRM file (Lennard-Jones format)
    lj_params = {
        'L_C': {'sigma': 0.34, 'epsilon': 0.457},
        'L_C.3': {'sigma': 0.34, 'epsilon': 0.457},
        'L_C.ar': {'sigma': 0.34, 'epsilon': 0.457},
        'L_O': {'sigma': 0.28, 'epsilon': 0.78},
        'L_O.3': {'sigma': 0.28, 'epsilon': 0.78},
        'L_H': {'sigma': 0.1, 'epsilon': 0.067},
        # Add other atom types if necessary
    }
    ...
    with open(prm_file, 'w') as f:
        ...
        for atom in ligand.atoms:
            ...
            if atom_type not in unique_atom_types and atom_type in lj_params:
                sigma = lj_params[atom_type]['sigma']
                epsilon = lj_params[atom_type]['epsilon']
                element = get_element(atom.type)  # Determine the element
                mass = standard_masses.get(element, 0.0) if element else 0.0
                f.write(f'{atom_type} {atom.atomic_number} {mass} {atom.charge} A {sigma} {epsilon}\n')
                unique_atom_types.add(atom_type)
            else:
                print(f'Warning: No LJ parameters found for atom type {atom_type}')
