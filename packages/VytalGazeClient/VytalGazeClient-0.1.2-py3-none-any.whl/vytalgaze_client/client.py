...
class Client:
    ...
    def predict_from_video(self, video_path, calib_mat=None):    
        ...
        with open(video_path, 'rb') as f:
            ...
            response = requests.post(
                # f'http://vytalgazeapilb-1065850898.us-east-1.elb.amazonaws.com/{function_endpoint}',
                f'http://127.0.0.1:8000/{function_endpoint}',
                params={"api_key": self.api_key} if calib_mat is None else {"api_key": self.api_key, "calib_mat": calib_mat_bytes},
                files = {"file": f},
                timeout=1000,
            )
