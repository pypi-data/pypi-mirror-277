...
def tangent_space_approximation(
    ...
    # # For smaller datasets we can use less iterartions
    # if data.shape[0] <= 5000:
    #     s_time_average = round(np.sqrt(data.shape[0]/100))
    # else:
    #     s_time_average = round(np.sqrt(data.shape[0]/10))
   
    # print(f'Average over {s_time_average} times')
    ...
    T2 = time.time()
    if featuremap_kwds['verbose']:
        print(ts() + f' Average time is {T2-T1}')
