...
class Instance(CompositeModel):
    ...
    def __validate_port_(cls, values: dict[str, Any]) -> dict[str, Any]:
        ...
        >>> i = Instance(name="i", version="12")
        >>> i.port, "port" in i.settings
        (5432, False)
        >>> i = Instance(name="i", version="13", settings={"port": 5423})
        >>> i.port, i.settings["port"]
        (5423, 5423)
        >>> i = Instance(name="i", version="14", port=5454)
        >>> i.port, "port" in i.settings
        (5454, False)
        ...
        >>> i = Instance(name="i", version="16", settings={"port": 5455})
        >>> i.port, i.settings["port"]
        (5455, 5455)
        >>> i = Instance(name="i", version="16", port=123, settings={"port": "123"})
        >>> i.port, i.settings["port"]
        (123, 123)
        >>> Instance(name="i", version="16", port=321, settings={"port": 123})
        Traceback (most recent call last):
          ...
        pydantic_core._pydantic_core.ValidationError: 1 validation error for Instance
          Value error, 'port' field and settings['port'] mismatch [type=value_error, input_value={'name': 'i', 'version': ...ettings': {'port': 123}}, input_type=dict]
            ...
        >>> Instance(name="i", version="16", settings={"port": "abc"})
        Traceback (most recent call last):
          ...
        pydantic_core._pydantic_core.ValidationError: 1 validation error for Instance
          Value error, invalid literal for int() with base 10: 'abc' [type=value_error, input_value={'name': 'i', 'version': ...tings': {'port': 'abc'}}, input_type=dict]
            ...
        """
        config_port = None
        try:
