...
def render_dropdown_section(data: HTMLBase, id="variables-dropdown")-> list:
    return [
        HTMLDropdown(
        dropdown_items= list(data),
...
def render_report(data: TableDescription, minimal:bool,report_name: str=None) -> Any:
    ...
    if not minimal:
        ...
        type = "box",
        name = "Variables",
        container_items = render_dropdown_section(render_variables_section(data))
    )
    ...
    missing_section = HTMLContainer(
        ...
        name="Missing",
        container_items=[
            HTMLPlot(plot=plot_to_base64(create_missing_bar_plot(data.df)),
            type="large",
            id="missingplot",
