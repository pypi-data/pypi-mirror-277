...
class _RpcClient:
    ...
    def setTimeToZeroAtSystemTime(self, systemTime: int) -> None:
        """Sets the FPGA time to zero when the USRP's system time reaches systemTime (ms).
        This happens as accurate as possible, but jitter is expected."""
        self.__rpcClient.setTimeToZeroAtSystemTime(systemTime)
    ...
    def getCurrentSystemTime(self) -> int:
        """Queries current system time from RPC server (ms)."""
    ...
    def setClockSource(self, clockSource: str) -> None:
        """Set clock source. See
        https://files.ettus.com/manual/classuhd_1_1rfnoc_1_1mb__controller.html#ae283d5e177e9a7d853144406e7f574fc
        for details.

        Args:
            clockSource (str): Use "internal" or "external".

        """
        self.__rpcClient.setClockSource(clockSource)

    def setTimeSource(self, timeSource: str) -> None:
        """Set time source. See
        https://files.ettus.com/manual/classuhd_1_1rfnoc_1_1mb__controller.html#a4e21edd153b205dc592cc17ae276ac32
        for details.

        Args:
            timeSource (str): Use "internal" or "external".

        """
        self.__rpcClient.setTimeSource(timeSource)
