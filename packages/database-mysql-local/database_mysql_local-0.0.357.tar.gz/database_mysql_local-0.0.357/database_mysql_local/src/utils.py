...
def generate_where_clause_for_ignore_duplicate(data_dict: dict, constraint_columns: list[list[str]]) -> (str, tuple):
    ...
    for constraint_column in constraint_columns:
        ...
        if len(temp_values) == len(constraint_column) * (2 if 'point' in constraint_column or 'coordinate' in constraint_column else 1):
...
def generate_where_clause_for_ignore_duplicate(data_dict: dict, constraint_columns: list[list[str]]) -> (str, tuple):
    ...
    for constraint_column in constraint_columns:
        ...
        if len(formatted_columns) == len(constraint_column) * (2 if 'point' in constraint_column or 'coordinate' in constraint_column else 1):
            where_clauses.append(f"({' AND '.join(formatted_columns)})")
