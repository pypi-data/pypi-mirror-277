...
def _NBI(N: int, M:int):
    ...
    W = np.array(list(combinations(range(1, H1 + M), M - 1))) - np.tile(np.arange(M - 1), (comb(H1 + M - 1, M - 1).astype(int), 1)) - 1
    ...
    if H1 < M:
        ...
        if H2 > 0:
            W2 = np.array(list(combinations(range(1, H2 + M), M - 1))) - np.tile(np.arange(M - 1), (comb(H2 + M - 1, M - 1).astype(int), 1)) - 1
