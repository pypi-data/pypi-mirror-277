...
class CondaEagerUpdateProvider(EagerUpdateProvider, CondaReusePinProvider):
    pass


@register_provider("reuse-installed")
class CondaReuseInstalledProvider(ReuseInstalledProvider, CondaReusePinProvider):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.installed_candidates: dict[str, CondaCandidate] = {}

    def get_reuse_candidate(self, identifier: str, requirement: Requirement | None) -> Candidate | None:
        candidate = super().get_reuse_candidate(identifier, requirement)
        if self.is_conda_initialized and candidate is not None:
            if candidate.name == "python":
                return None
            from pdm_conda.models.setup import CondaSetupDistribution

            if (dist := self.installed.get(candidate.name, None)) is not None and isinstance(
                dist,
                CondaSetupDistribution,
            ):
                if not self.installed_candidates:
                    self.installed_candidates = self.repository.environment.env_candidates
                return self.installed_candidates[candidate.name]
