...
class CondaSynchronizer(Synchronizer):
    ...
    def _should_update(self, dist: Distribution, can: Candidate) -> bool:
        # Always update if one of the two is conda and the other is not
        # or as line is different
        return (
            super()._should_update(dist, can)
            or (
                isinstance(can, CondaCandidate)
                and (
                    not isinstance(dist, CondaSetupDistribution)
                    or dist.as_line()
                    != can.req.as_line(conda_compatible=True, with_channel=True, with_build_string=True)
                )
            )
            or (isinstance(dist, CondaSetupDistribution) and not isinstance(can, CondaCandidate))
        )
    ...
    def compare_with_working_set(self) -> tuple[list[str], list[str], list[str]]:
        ...
        if to_remove:
            to_remove = [p for p in to_remove if p not in self.environment.env_candidates]
