...
def enum_child_windows(biggest, frame_aspect_ratio):
    ...
    def child_callback(hwnd, _):
        ...
        logger.debug(f'find_hwnd child_callback {visible} {biggest[0]} {parent} {rect}')
...
#
#     def callback(hwnd, _):
#         if win32gui.IsWindowVisible(hwnd) and win32gui.GetParent(hwnd) == parent_handle:
#             rect = win32gui.GetWindowRect(hwnd)
#             width = rect[2] - rect[0]
#             height = rect[3] - rect[1]
#             print(f"Child HWND: {hwnd} - Window Text: {win32gui.GetWindowText(hwnd)} {rect} {width} {height}")
#         return True
#
