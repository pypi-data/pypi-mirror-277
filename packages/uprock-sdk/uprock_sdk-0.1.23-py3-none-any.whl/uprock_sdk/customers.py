...
class BaseCustomer(BaseModel):
    ...
    telegram_bots: Optional[Set[int]] = None
...
async def list_(filter_: Optional[CustomerFilter], pagination: Optional[PageParams] = None) -> Page[CustomerRead]:
    params = {}
...
async def list_all(filter_: Optional[CustomerFilter]) -> List[CustomerRead]:
    customers = []

    customer_page = await list_(filter_=filter_)
    customers = customers + list(customer_page.items)

    for page in range(1, customer_page.pages):
        customer_page = await list_(filter_=filter_, pagination=PageParams(page=page))
        customers = customers + list(customer_page.items)
