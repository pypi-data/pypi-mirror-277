...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def run(
        ...
        for df in dfs:
            ...
            # If we don't have a path to show, we don't need to populate
            # these columns in the csv.
            if (
                hasattr(self.datasource, "cache_images")
                and self.datasource.cache_images is False
            ):
                df = df[df.columns.drop(list(df.filter(regex="Pixel Data")))]
            ...
            append_dataframe_fn: Callable[[pd.DataFrame, Path], None] = (
                lambda dataframe, path: dataframe.round(decimals=2).to_csv(
                    path, mode="a", header=not csv_path.exists(), index=False
                )
            )
