...
class _InferenceAndCSVReportModelIncompatibleWorkerAlgorithm(
    ...
    def run(self, *, return_data_keys: bool = False) -> Any:
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        if not isinstance(model_predictions, pd.DataFrame):
            raise TypeError(
                f"Model prediction must return a Dataframe to enable CSV report output;"
                f" got {type(model_predictions)} instead."
            )
        ...
        # algorithm so we cna enable saving to the project database
