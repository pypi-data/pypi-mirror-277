...
if __name__ == '__main__':
    ...
    if parsed.output is None:
        ...
        created_file_for_work, OUTPUT_FOLDER_NAME = write_folder(OUTPUT_DIRECTORY, top=parsed.topology,
                                                                 res="_".join(parsed.perturbed_residues),
                                                                 ff=parsed.protein_ff + '_' + parsed.water_ff,
                                                                 just_min_or_md=parsed.just_minimize)
    ...
    api_logger.info("Switching Distance Condition: \t%s" % parsed.use_switching_distance)
    ...
    api_logger.info("Just Minimize Condition:\t%s" % parsed.just_minimize)
    ...
    Classic_MD_Engine(pdb_path=parsed.topology, protein_ff=parsed.protein_ff, water_ff=parsed.water_ff,
                      ...
                      use_switching_distance=parsed.use_switching_distance, switching_distance=parsed.switch_distance,
                      ...
                      temp=parsed.temperature, platform_name=parsed.platform, precision='mixed',
                      friction_cofficient=parsed.friction_coff, minimize=parsed.minimize,
                      minimize_steps=parsed.minimize_step, just_minimize=parsed.just_minimize,
                      CPU_Threads=parsed.cpu_thread,
                      ...
                      user_selected_res=parsed.perturbed_residues)
    ...
    for i in range(len(parsed.velocity_speed_factor)):

        name_of_changed_state_xml = change_velocity(state_file_path, int(parsed.velocity_speed_factor[i]),
                                                    modify_atoms)
        new_dissipated_trajectory_name = dissipated_trajectory_name + str(parsed.velocity_speed_factor[i])
        # new_undissipated_trajectory_name = undissipated_trajectory_name + str(parsed.velocity_speed_factor[i])
        ...
        if i == 0:
            Reference_MD_Engine(pdb_path=last_pdb_file_path, state_file=state_file_path, protein_ff=parsed.protein_ff,
                                ...
                                nonbondedCutoff=parsed.nonbonded_cutoff,
                                use_switching_distance=parsed.use_switching_distance,
                                switching_distance=parsed.switch_distance,
        ...
        if i == 0:
            residue_based_decomposition(topol=unwrap_pdb, trj_pos_list=position_list, start_res=0, stop_res=250,
                                        ...
                                        modif_energy_name='modified_energy_file_%s.csv' % int(
                                            parsed.velocity_speed_factor[i]), origin_last_pdb=last_pdb_file_path,
                                        ff=parsed.protein_ff)
        if i != 0:
            # --> RESIDUE BASED DECOMPOSITION
            residue_based_decomposition(topol=unwrap_pdb, trj_pos_list=position_list, start_res=0, stop_res=250,
                                        output_directory=created_file_for_work,
        ...
        getResidueResponseTimes(os.path.join(created_file_for_work, 'reference_energy_file.csv'),
                                os.path.join(created_file_for_work, 'modified_energy_file_%s.csv'
                                             % int(parsed.velocity_speed_factor[i])),
