...
class Telemetry:
    ...
    _posthog: Posthog
    ANON_ID_PATH = str(Path.home() / ".cache" / "agiflow" / "telemetry_anon_id")
    UNKNOWN_ANON_ID = "UNKNOWN"

    def __new__(cls) -> "Telemetry":
        if not hasattr(cls, "instance"):
            obj = cls.instance = super(Telemetry, cls).__new__(cls)
            obj._telemetry_enabled = EnvironmentVars.AGIFLOW_TELEMETRY.lower() == "true" and "pytest" not in sys.modules

            if obj._telemetry_enabled:
                obj._posthog = Posthog(
                    project_api_key="phc_nsDxxaVGqYFOmxWsUpTY2hOuxSlk6gDLe4LPYEF8oEw",
