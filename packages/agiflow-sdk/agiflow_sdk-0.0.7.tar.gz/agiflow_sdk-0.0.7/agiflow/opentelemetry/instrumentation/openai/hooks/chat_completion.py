...
from agiflow.opentelemetry.instrumentation.openai.hooks.base_llm import OpenAILLMSpanCapture
    ...
    def capture_output(self, result):
        ...
        if hasattr(result, "usage") and result.usage is not None:
            ...
            if usage is not None:
                ...
                self.set_span_attribute(SpanAttributes.LLM_TOKEN_COUNTS, json.dumps(usage_dict))
    ...
    def get_prompt_tokens(self, result):
        ...
        if self.functions is not None:
            for function in self.functions:
                prompt_tokens += calculate_prompt_tokens(
                    json.dumps(function), self.model
                )
