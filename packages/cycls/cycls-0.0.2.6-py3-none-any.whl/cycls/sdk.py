...
class Cycls:
    ...
    def publish(self):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            if self.debug:
                executor.submit(
                    uvicorn.run(self.server, host="127.0.0.1", port=self.port)
                )  # perhaps keep traces?
            else:
                executor.submit(
                    uvicorn.run(
        ...
        try:
            with httpx.Client() as client:
                response = client.post(
                    f"{self.network}/register",
                    json={"handle": f"@{self.handle}", "url": self.url, "mode": mode},
                )
    ...
    def tunnel(self):
        ...
        ssh_command = [
            ...
            "-R",
            f"{self.handle}-cycls:80:localhost:{self.port}",
            "serveo.net",
        ]
        try:
            if self.debug:
                subprocess.run(
                    ssh_command, stdin=subprocess.DEVNULL
                )  # very tricky! STDIN is what was messing with me
            else:
                subprocess.run(
                    ssh_command,
...
def Text(x):
    return (
        StreamingResponse(x, media_type="text/event-stream")
        if type(x) == AsyncGeneratorType
        else PlainTextResponse(str(x))
    )
