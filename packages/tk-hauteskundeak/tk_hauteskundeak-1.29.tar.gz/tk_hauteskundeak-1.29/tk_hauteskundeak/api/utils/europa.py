...
LURRALDE_KODEAK = ["01", "16", "20", "31", "48"]
REMOVE_COLUMN_QTY = 3


def import_europa(data, hauteskundea, total):
    f = StringIO(data)
    ...
    for row in reader:
        ...
        if not total:
            ...
            if row[1] not in ["PR", "TO"] or (
                row[1] == "PR" and row[3] not in LURRALDE_KODEAK
            ):
                continue
            TOWN_INFO_COLUMN_QTY = 22
        ...
        iterations = int((total_rows - TOWN_INFO_COLUMN_QTY) / PARTY_INFO_COLUMN_BLOCK)
        ...
        for iter in range(iterations):
            ...
            hauteskundeEmaitzakTokian.botoak = int(row[index + alderdi_botoak_index])
