...
class NodeFileProcessor:
    ...
    def process_node_file(self):
        ...
        while this_row < last_row:
            ...
            for primary_property in self.contents.get_property_values_list(this_row, node_columns, False, self.primary_property):
                time_now = amu.get_formatted_time_now()

                # Build a simple MERGE query using queries
                query_text = (QueryBuilder()
                              .merge()
                              .node(labels=merge_labels, ref_name=property_ref, properties=Properties(primary_property))
                              )
                ...
                query = Neo4jQuery('node', query_text, time_now, merge_labels, new_labels, primary_property, properties)
    ...
    def process_node_file_batch(self):
        ...
        while this_row < last_row:
            validated_csv_labels = lm.validate_labels(self.contents.get_column_values_list(this_row, label_columns))
            merge_labels, new_labels, remove_labels = lm.sort_node_labels(self.required_labels, validated_csv_labels)
            row_properties = self.contents.get_property_values_dict(this_row, property_columns, allow_blanks=self.allow_blanks)

            for primary_property in self.contents.get_property_values_list(this_row, node_columns, False, self.primary_property):
                if len(self.required_properties) > 0:
                    properties = {**self.required_properties, **row_properties}
                else:
                    properties = row_properties.copy()
                # Remove any properties we don't want
                for property in self.unrequired_properties:
                    try:
                        properties.pop(property)
                        ...
                        pass
                properties = {**properties, **primary_property}
                ...
                dictionary_key = [new_labels, merge_labels, remove_labels, list(primary_property)[0].split(':')[0]]
                try:
        ...
        for new_labels, updates in query_dictionary.items():
            ...
            while batch_num <= num_batches:
                ...
                if batch_keys[new_labels][0]:
                    merge_labels += batch_keys[new_labels][0]
                primary_property = batch_keys[new_labels][3]
                query_text = (QueryBuilder()
