...
class RelationshipFileProcessor:
    ...
    def process_relationship_file(self):
        ...
        while this_row < last_row:
            ...
            properties = self.contents.get_property_values_dict(this_row, property_columns, self.allow_blanks, prefix=relation_ref_dot)
            # If there are multiple From or To nodes, create a relationship query for each combination
            for from_node in self.contents.get_property_values_list(this_row, from_columns, False, self.primary_property, self.primary_property):
                ...
                for to_node in self.contents.get_property_values_list(this_row, to_columns, False, self.primary_property, self.primary_property):
...
##                                print(f'FN: {from_node}')
##                                print(f'FN: {Properties(from_node).format()}')
##                                print(f'FN: {Properties.format_value(from_node)}')
        ...
        for from_to_labels, updates in query_dictionary.items():
            ...
            from_key, from_type = rm.split_node_key(batch_keys[from_to_labels][3])
            to_key, to_type = rm.split_node_key(batch_keys[from_to_labels][4])
