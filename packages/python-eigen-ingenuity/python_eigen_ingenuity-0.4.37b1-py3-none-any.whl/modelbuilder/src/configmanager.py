...
class ConfigManager:
    ...
    def __init__(self):
        ...
        parser.add_argument('-ab', '--allowblanks', action='store_true', help='Update a node_key even if it is blank')
        ...
        parser.add_argument('-rp', '--requiredproperties', default='', help='Required properties - appended to contents of config; use !node_key to remove')
        ...
        self.allow_blanks = args.allowblanks
...
##            settings.set_azure_client_id('74efa022-e6d6-42ad-bc15-edc76c525bdd')
##            settings.set_azure_tenant_id('74efa022-e6d6-42ad-bc15-edc76c525bdf')
        else:
    ...
    def read_db_config(self):
        database_index = None
        database_name_list = self.get_config_value_list("Database", "DatabaseNames")
        ...
        if database_index is not None:
            ...
            self.db_connection_dict['url'] = self.get_config_value_list("Database", "urls")[database_index]
            self.db_connection_dict['user'] = self.get_config_value_list("Database", "users")[database_index]
            self.db_connection_dict['password'] = self.get_config_value_list("Database", "passwords")[database_index]
            print(f'Connecting to {database_name_list[database_index]}')

    def read_version_config(self):
        version_mode = self.get_config_value("Versions", "Versions", "on")
        self.version_labels_list = lm.validate_labels(self.get_config_value_list("Versions", "VersionLabels", "!Current,Version"))
        if version_mode.lower() == 'on' and not self.no_version:
            self.frozen_labels_list = lm.validate_labels(self.get_config_value_list("Versions", "FrozenLabels", ""))
            self.version_labels_list = lm.validate_labels(self.get_config_value_list("Versions", "VersionLabels", "!Current,Version"))
            self.version_prefix = self.get_config_value("Versions", "VersionLabelPrefix", "")
            self.version_counter = pm.validate_properties(self.get_config_value("Versions", "VersionCounterProperty", "versionCount"))
            self.version_number = pm.validate_properties(self.get_config_value("Versions", "VersionNumberProperty", "versionNumber"))
            self.first_version_property = pm.validate_properties(self.get_config_value("Versions", "FirstVersionProperty", "firstVersion"))
            self.last_version_property = pm.validate_properties(self.get_config_value("Versions", "LastVersionProperty", "lastVersion"))
            self.version_relationship = rm.validate_relationships(self.get_config_value("Versions", "VersionRelationship", "has_version"))
            self.next_version_relationship = rm.validate_relationships(self.get_config_value("Versions", "NextVersionRelationship", "next_version"))
            self.version_valid_from_property = pm.validate_properties(self.get_config_value("Versions", "VersionValidFromProperty", "validFrom"))
            self.version_valid_to_property = pm.validate_properties(self.get_config_value("Versions", "VersionValidToProperty", "validTo"))
        else:
            self.no_version = True

    def read_csv_config(self):
        self.primary_property = pm.validate_properties(self.primary_property_override or self.get_config_value("Model", "PrimaryProperty", "node"))
        self.required_labels_list = lm.combine_required_labels(self.get_config_value_list("Model", "RequiredLabels", ""), self.required_labels_override)
        self.required_properties_dict, self.unrequired_properties_list = pm.combine_required_properties(self.get_config_value_list("Model", "RequiredProperties", ""), [self.required_properties_override])

        no_system_properties = self.get_config_value("Model", "SystemProperties", "on")
        if no_system_properties.lower() != 'on':
            self.no_version = True
            self.no_system_properties = True
        self.unique_id_property = pm.validate_properties(self.get_config_value("Model", "UniqueIDProperty", "uuid"))
        self.creation_time_property = pm.validate_properties(self.get_config_value("Model", "CreationTime", "creationTime"))
        self.update_time_property = pm.validate_properties(self.get_config_value("Model", "UpdateTime", "updateTime"))
        header_mappings = self.get_config_value_list("Model", "Mappings", "")
        self.default_data_type = self.get_config_value("Model", "DataType", "")

        self.label_alias_list = lm.validate_labels(self.get_config_value_list("Aliases", "Labels", "label"))
        self.node_alias_list = pm.validate_properties(self.get_config_value_list("Aliases", "Nodes", "node") + [self.primary_property])
        self.from_alias_list = pm.validate_properties(self.get_config_value_list("Aliases", "FromNodes", "from,start"))
        self.from_label_alias_list = lm.validate_labels(self.get_config_value_list("Aliases", "FromLabels", "fromLabel"))
        self.to_alias_list = pm.validate_properties(self.get_config_value_list("Aliases", "ToNodes", "to,end"))
        self.to_label_alias_list = lm.validate_labels(self.get_config_value_list("Aliases", "ToLabels", "toLabel"))
        self.relationship_alias_list = rm.validate_relationships(self.get_config_value_list("Aliases", "Relationships", "relationship"))

        try:
            self.header_mappings_dict = {
                i.replace('=', '->').split('->')[0].rstrip(): i.replace('=', '->').split('->')[1].lstrip() for i in
                header_mappings}
        except:
            self.header_mappings_dict = {}
        alias_header_mappings = [i + '->' + self.primary_property for i in self.node_alias_list if
                                 i != self.primary_property]
        try:
            self.alias_header_mappings_dict = {
                i.replace('=', '->').split('->')[0].rstrip(): i.replace('=', '->').split('->')[1].lstrip() for i in
                alias_header_mappings}
        except:
            self.alias_header_mappings_dict = {}
    ...
    def get_config_value_list(self, section, option, default=None):
    ...
    def get_config_value(self, section, option, default=None):
        # Get a node_value from the settings file, reporting an error if it's missing
        value_list = self.get_config_value_list(section, option, default)
        if value_list:
            value = value_list[0]
            if len(value_list) > 1:
                warning('Warning! ', f'Multiple values for [{section}]{option} found - using {value}')
        else:
            value = default
