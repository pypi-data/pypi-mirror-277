                    ...
                    .map(|x| match x {
                        ListInfo::Info(info) => {
                            let location = bucket_path
                                .join(info.location.to_string())
                                .to_str()
                                .unwrap()
                                .to_string();
                            let obj_type = match self.rt.block_on(store.is_dir(location.as_str())) {
                                Ok(true) => "directory",
                                Ok(false) => "file",
                                Err(_) => return Err(Error::msg("Error getting type")),
                            };
                            let key_vals = vec![
                                ("Key", location.to_object(py)),
                                // ("LastModified", info.last_modified),
                                ("size", info.size.to_object(py)),
                                ("ETag", info.e_tag.as_ref().to_object(py)),
                                ("name", location.to_object(py)),
                                ("type", obj_type.to_object(py)),
                                // ("Version", info.version.as_ref().unwrap().to_object(py)),
                            ];
