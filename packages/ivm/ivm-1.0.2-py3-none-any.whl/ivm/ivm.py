"""
Project generated by builder 0.10.21 protocol 1.0.2
"""
import logging
from sys import version_info
import struct
from ctypes import CDLL, Structure, Array, CFUNCTYPE, byref, create_string_buffer, cast
from ctypes import c_ushort, c_uint, c_float, c_ubyte, c_short
...
urpc_builder_version_bugfix = 21
urpc_builder_version_suffix = ""
urpc_builder_version = "0.10.21"
_Ok = 0
_Error = -1
_NotImplemented = -2
_ValueError = -3
...
def _load_lib():
    ...
    if os_kind == "windows":
        if 8 * struct.calcsize("P") == 32:
            paths = (_near_script_path("ivm-win32\\ivm.dll"),
                     _near_script_path("ivm.dll"),
                     "ivm.dll")
        else:
            paths = (_near_script_path("ivm-win64\\ivm.dll"),
                     _near_script_path("ivm.dll"),
                     "ivm.dll")
    elif os_kind == "darwin":
        paths = (_near_script_path("ivm-darwin/libivm.dylib"),
                 _near_script_path("libivm.dylib"),
                 "libivm.dylib")
    elif os_kind == "freebsd" or "linux" in os_kind:
        paths = (_near_script_path("ivm-debian/libivm.so"),
...
class IvmDeviceHandle:
    ...
    class GetDeviceRankResponse(_IterableStructure):
        _fields_ = (
            ("_rank", c_uint),
            ("_reserved0", c_ubyte*128),
        )

        class Rank(int):
            RANKING_NOT_SUPPORTED = 0
            RANK_PRIMARY = 1

            @property
            def ranking_not_supported(self) -> int:
                return self & self.RANKING_NOT_SUPPORTED

            @property
            def rank_primary(self) -> int:
                return self & self.RANK_PRIMARY
        ...
        @rank.setter
        def rank(self, value: Union[int, c_uint]) -> None:
            self._rank = _normalize_arg(value, c_uint)

        @property
        def reserved0(self) -> c_ubyte*128:
            return self._reserved0

        @reserved0.setter
        def reserved0(self, value: Union[Sequence[int], c_ubyte*128]) -> None:
            self._reserved0 = _normalize_arg(value, c_ubyte*128)

    def get_device_rank(self, **kwargs) -> "GetDeviceRankResponse":
        dst_buffer = kwargs.get("dst_buffer", self.GetDeviceRankResponse())
        _validate_call(_lib.ivm_get_device_rank(self._handle, byref(dst_buffer)))
        return dst_buffer
