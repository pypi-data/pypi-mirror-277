...
########################################################################################################################
#
# The code in this section contains a default model interface that each model API has to provide. After that we get into
# data types that are more abstract and are used to for more complex tasks like tree based operations.
#
########################################################################################################################
...
class ThreadsTree:
    ...
    def rollout(
        ...
        parent_thread = self.pick(to=from_)
        if parent_thread.chats[-1].role != Message.HUMAN:
            # this step is the human prompt
            pass
...
class ThreadsList(list):
    ...
    def append(self, __object: Thread, expected: str = None) -> None:
        if not self.items:
        ...
        self.expected[__object.value_hash] = expected
    ...
    def from_dict(cls, data):
        ...
        bench_dataset.expected = data.get("expected", {})
