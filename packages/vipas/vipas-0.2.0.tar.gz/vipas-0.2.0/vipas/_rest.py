...
class RESTClientObject:
    ...
    async def request(self, method, url, headers=None, body=None):
        ...
        for attempt in range(attempts):
            ...
            try:
                predict_response = await self.client.request(method, url, headers=headers, content=body)
            ...
            if payload_type == "url":
                try:
                    output_data_response = await self.client.request("GET", predict_data.get("presigned_download_url"))
                    ...
                    # Define the function and execute the logic from the schema string
                        local_vars = {'deployment_system': predict_data.get("deployment_system"), 'output_data': output_data}
