...
from typing import Tuple, PushSubscription
import nats
...
class IAPListener():
    ...
    def __init__(self,
        ...
        self.inputs = {}
        for item in self._dag.input_def:
            if item["type"] != "query":
    ...
    async def _run_async(self):
        ...
        for name in self.inputs_name:    
            ...
            sub_in = await js.subscribe(queue_in+".>",
                                        queue=self.queue+"-"+self.inputs_name,
                                        stream=self.queue)
            self._subs.append(sub_in)
            
        queue_out = self.queue + "." + self.outputs_name           
        ...
        async def handle_msg(msgs, is_batch: bool):
            if is_batch:
                ...
                batch = list(batch)
                reply, err = self._process_data(batch, params_lst, is_batch)
                if not err:
            ...
            for msg in msgs:
                ...
                reply, err = self._process_data([data], [params], is_batch)
                if err:
        ...
        async def wait_msg(sub_in: PushSubscription):
        ...
        thread_lst = []
        for sub_in in self._subs:
            th = threading.Thread(target=asyncio.run, args=(wait_msg(sub_in),))
    ...
    def _process_data(self, requests: list, reqs_parameters: list, is_batch: bool = False) -> Tuple[list, str | None]:
        ...
        try:
            ...
            # THis part is outside of the scope of this snippet
            # _found = False
            # if is_input:
            #     _def = self.dag["input_def"]
            # else:
            #     _def = self.dag["output_def"]
            # for item in _def:
            #     if item["name"] == self.inputs_name:
            #         _found = True
            #         _type = item["type"]
            #         _conf = item
            #         break
            # if not _found and len(_def) == 1:
            #     _conf = _def[0]
