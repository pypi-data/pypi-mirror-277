...
class AIPlatform(SyncAPIClient):
    ...
    def auth_headers(self) -> dict[str, str]:
        ...
        for key, value in self._access_key_header.items():
            auth_headers[key] = value
        for key, value in self._access_secret_key_header.items():
            auth_headers[key] = value
        return auth_headers

    @property
    def _access_key_header(self) -> dict[str, str]:
        access_key = self.access_key
        return {"KILM-ACCESS-KEY": access_key}

    @property
    def _access_secret_key_header(self) -> dict[str, str]:
        access_secret_key = self.access_secret_key
        return {"KILM-ACCESS-SECRET-KEY": access_secret_key}
