...
QuerySetID = NewType("QuerySetID", str)
...
DFExampleSet must have:
    ...
    | exampleSetId | exampleId | query | context (?) | fullyResolvedPrompt (?) | output (?)| groundTruth | ragQueryTraceId (?) |
...
DFMinimalExampleSet must have:
    ...
    | query | context (?) | fullyResolvedPrompt (?) | output (?)| groundTruth | ragQueryTraceId (?) |


DFQuerySet:

    | querySetId | queryId | query | groundTruth |
...
DFQuerySet = NewType("DFQuerySet", pd.DataFrame)
...
def df_as_df_example_set(df: pd.DataFrame) -> Res[DFExampleSet]:
    ...
    if not df.set_index("exampleId").index.is_unique:  # type: ignore
        return Err(ValueError("'exampleId' must be a key column"))

    return Ok(DFExampleSet(df))


def df_as_df_query_set(df: pd.DataFrame) -> Res[DFQuerySet]:
    if "querySetId" not in df.columns:
        return Err(ValueError("DataFrame must have a 'querySetId' column"))

    if "queryId" not in df.columns or (not df.set_index("queryId").index.is_unique):  # type: ignore
        return Err(ValueError("'queryId' must be a key column"))

    return Ok(DFQuerySet(df))


def df_as_df_minimal_example_set(df: pd.DataFrame) -> Res[DFMinimalExampleSet]:
    if "exampleId" in df.columns:
        return df_as_df_example_set(df).map(DFMinimalExampleSet)
    else:
        df = df.rename(columns={"input": "query"}, errors="ignore")
        required_to_is_nullable = {"query": False}
        for required_col, is_nullable in required_to_is_nullable.items():
            if required_col not in df.columns:
                return Err(
                    ValueError(
                        f"DataFrame must have a '{required_col}' column"
                    )
                )
