...
async def _load_launchflow_service(
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        services_client = ServicesAsyncClient(
            client, base_url=launch_url, api_key=launch_api_key
        )
        try:
...
async def _save_launchflow_service(
    ...
    async with httpx.AsyncClient() as client:
        services_client = ServicesAsyncClient(client, launch_url, launch_api_key)
    ...
    async def delete_service(self, lock_id: str):
        if isinstance(self.backend, LocalBackend):
            ...
            async with httpx.AsyncClient() as client:
                services_client = ServicesAsyncClient(
                    client,
                    base_url=self.backend.launchflow_url,
