...
from launchflow.cache import delete_run_cache_from_disk
...
from launchflow.utils import generate_run_id
...
async def destroy(
    ...
    _set_global_project_and_environment(None, environment)
    if local_mode:
        await destroy_resources(environment, local_only=True, prompt=not auto_approve)
        return

    try:
        await destroy_resources(environment, prompt=not auto_approve)
...
async def run(
    ...
    Sample commands:
        ...
        launchflow run local -- ./run.sh
            ...
            - The environment MUST be specified in the launchflow.yaml if non-local resources are
              created in infra.py.
    """
    config_environment = launchflow.config.environment
    ...
    if remote_resources:
        ...
        await create_resources(remote_environment, *remote_resources, prompt=True)
    ...
    # TODO: Determine if we should allow the user to set the run id themselves or if we
    # should always generate a new one. If they set it, should we clear the cache below?
    run_id = config.env.run_id or generate_run_id()
    current_env["LAUNCHFLOW_RUN_ID"] = run_id
    ...
    try:
        ...
        try:
            ...
            pass
        delete_run_cache_from_disk(run_id)
        rich.print("\n──────────────────────────")
