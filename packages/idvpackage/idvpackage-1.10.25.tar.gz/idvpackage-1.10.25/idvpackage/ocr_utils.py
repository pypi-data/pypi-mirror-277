...
def data_consistency_check(data, front_id_text, back_id_text, country):
    ...
    if country == 'IRQ':
        ...
        if front_id_number and back_id_number:
            print(f"\n\n----------------------------\nFRONT ID Number: {front_id_number}")
            print(f"BACK ID Number: {back_id_number}")
    ...
    try:
        ...
        if country == 'IRQ' or country == 'LBN' or country == 'QAT':
            if country == 'IRQ':
                main_prop_data = {}
                if data.get('doc_type') == 'passport':
                    keys_to_remove = ['name', 'first_name', 'issuing_place', 'occupation', 'employer', 'issuing_country', 'mrz_line3']
                    keys = ['full_name', 'id_number', 'mrz']
                else:
                    keys = ['name_en', 'issuing_authority', 'gender_ar', 'place_of_birth', 'issuing_authority_en', 'place_of_birth_en']

            if country == 'LBN':
                main_prop_data = {}
                keys_to_remove = ['name', 'issuing_place', 'occupation', 'employer', 'issuing_country', 'mrz_line3']
                if data.get('doc_type') == 'passport':
            ...
            if country == 'QAT':
                main_prop_data = {}
                keys_to_remove = ['gender', 'issuing_place', 'mrz_line1', 'mrz_line2', 'mrz_line3']
...
def form_final_facial_similarity_report(data, selfie, facial_similarity, liveness_result, face_match_threshold, country):
    ...
    print(f"POS MATCH: {front_photo_on_screen_result, data.get('photo_on_screen_result')}")
    ...
    print(f"\n\n---------------- IMAGE INTEGRITY FINAL RESULT: {image_integrity_final_result}")
