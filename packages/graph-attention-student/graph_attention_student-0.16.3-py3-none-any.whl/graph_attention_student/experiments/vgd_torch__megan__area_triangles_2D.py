...
VISUAL_GRAPH_DATASET: str = 'random_area_triangles_2D'
# :param DATASET_TYPE:
...
REGRESSION_REFERENCE: t.Optional[float] = 700.0
...
def target_from_metadata(e: Experiment,
    ...
    for e, (i, j) in enumerate(graph['edge_indices']):
        edge_attributes.append([
            graph['edge_attributes'][e][0],
            np.linalg.norm(graph['node_coordinates'][i] - graph['node_coordinates'][j]),
        ])
...
def after_dataset(e: Experiment,
    ...
    e.log(f'target statistics'
          f' - mean: {np.mean(target_values):.3f}'
          f' - median: {np.median(target_values):.3f}')
