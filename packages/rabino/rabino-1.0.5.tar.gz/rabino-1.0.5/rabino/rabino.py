...
class rubino:
	...
	def getPostByShareLink(self,link,profile_id=None):
		if link.startswith("https://rubika.ir/post/"):
			god = link.split("https://rubika.ir/post/")[1]
			print(god)
	...
	def addPostViewCount(self,post_id,post_target_id,prof=None):
		inData = {"post_id":post_id,"post_profile_id":post_target_id,"profile_id":prof}
	...
	def upload(self,file,Type,file_name=None,prof=None):
		if not "http" in file:
			...
			if len(bytef) <= 131072:
				...
				REQUEST = {
			"file_name": file_name if file_name else file.split("/")[-1],
