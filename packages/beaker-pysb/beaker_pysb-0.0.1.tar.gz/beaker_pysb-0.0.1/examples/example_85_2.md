# Description
Render reactions and visualize using Graphviz. Creates a dot file from the model and then generates a PDF from the dot file.
...
def run(model, include_rate_species=False):
    ...
    for i, cp in enumerate(model.species):
        ...
        if len([s for s in ic_species if s.is_equivalent_to(cp)]):
            color = '#aaffff'
        graph.add_node(species_node, label=slabel, shape='Mrecord', fillcolor=color, style='filled', color='transparent', fontsize='12', margin='0.06,0')
