...
class Episodes():
    ...
    def set_content(self, raw_html_file):
        ...
        with open(f'{os.path.dirname(abs_path)}/template.html') as f:
    ...
    def fetch_content(self):
        ...
        with open(f'{os.path.curdir()}\\template\\template.html') as f:
            template = env.from_string(f.read())
        file = template.render(title = self.title, content = '\n'.join(self.content))
        self.html_file = file
        
        if not os.path.exists(f'./cache'): os.mkdir('./cache')
        with open(f'./cache/{self.episode_id}.html','w', encoding='utf8') as f:
            f.write(file)
