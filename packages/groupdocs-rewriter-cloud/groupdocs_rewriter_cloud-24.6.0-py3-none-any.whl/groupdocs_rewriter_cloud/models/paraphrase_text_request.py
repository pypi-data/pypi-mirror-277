...
class ParaphraseTextRequest(BaseModel):
    ...
    suggestions: Optional[StrictStr] = Field(None, description="Number of variants for rewriting")
    diversity_degree: Optional[DegreeEnum] = Field(None, alias="diversityDegree")
    tokenize: Optional[StrictBool] = Field(None, description="Determines whether to return a tokens list")
    __properties = ["language", "text", "texts", "origin", "suggestions", "diversityDegree", "tokenize"]

    @validator('suggestions')
    def suggestions_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('One', 'Two', 'Three'):
            raise ValueError("must be one of enum values ('One', 'Two', 'Three')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ParaphraseTextRequest:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ParaphraseTextRequest:
        """Create an instance of ParaphraseTextRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ParaphraseTextRequest.parse_obj(obj)

        _obj = ParaphraseTextRequest.parse_obj({
