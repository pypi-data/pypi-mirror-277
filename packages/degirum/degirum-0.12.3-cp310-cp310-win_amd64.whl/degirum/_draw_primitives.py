...
class _PrimitivesDrawOpenCV:
    ...
    def image_overlay(self):
        """Return image overlay with proper blending"""
        if self._alpha < 1:
            alpha = Fraction(self._alpha).limit_denominator(255)
            alpha_complement = Fraction(1.0 - self._alpha).limit_denominator(255)
            mask = self._cv.cvtColor(self._image, self._cv.COLOR_BGR2GRAY) != 0
            ret = self._original_image.copy()
            ret[mask] = (
                (self._image[mask].astype(numpy.uint16) * alpha.numerator)
                // alpha.denominator
                + (
                    self._original_image[mask].astype(numpy.uint16)
                    * alpha_complement.numerator
                )
