...
class Model(ABC):
    ...
    def device_type(self, val: Union[str, List[str]]):
        ...
        # we do not allow changing device type for absent SupportedDeviceTypes
        if self._model_parameters.SupportedDeviceTypes == "":
            raise DegirumException(
                f"Model {self._model_name} does not support dynamic device type selection: "
                + "model property SupportedDeviceTypes is not defined."
            )
        ...
        if isinstance(val, list):
            ...
            agent_device = check_device_type(val)
            if agent_device is None:
                raise DegirumException(
                    f"Device type {val} is not supported by model {self._model_name}. "
                    + f"Supported device types are: {self._supported_device_types}."
                )
