...
class Burp:
    ...
    def check_and_close_issues(self) -> None:
        ...
        :rtype: None
        """
        existing_burp_issues = {issue.burpId: issue for issue in self.existing_issues}
        parsed_burp_issues = {issue.burpId: issue for issue in self.regscale_issues}
        closed_issues = []
        with create_progress_object() as close_issue_progress:
            ...
            for burp_id, issue in existing_burp_issues.items():
                if burp_id and burp_id not in parsed_burp_issues and issue.status != "Closed":
