...
class ContainerScan(ABC):
        ...
        with open(self.attributes.file_path, encoding="utf-8") as file:
            if file.name.endswith(".csv"):
                ...
                try:
                    ...
                    data = [dat for dat in json.load(file) if dat]
    ...
    def create_assets(self, func: Callable) -> None:
        ...
        :rtype: None
        """
        existing_assets: List[Asset] = Asset.get_all_by_parent(self.attributes.parent_id, self.attributes.parent_module)
        ...
        if insert_assets := [asset for asset in self.data["assets"] if asset not in existing_assets]:
            ...
            ReportGenerator(
                objects=insert_assets,
                to_file=True,
                report_name=f"{self.attributes.name}_insert_assets",
        ...
        if update_assets := [asset for asset in self.data["assets"] if asset in existing_assets]:
            ...
            ReportGenerator(
                objects=update_assets,
                to_file=True,
                report_name=f"{self.attributes.name}_update_assets",
                regscale_id=self.attributes.parent_id,
                regscale_module=self.attributes.parent_module,
            )
        # Refresh assets
    ...
    def check_and_close_issues(self, existing_issues: list[Issue]) -> None:
        ...
        parsed_cves = {issue.cve for issue in self.data["issues"]}
        closed_issues = []
        with create_progress_object() as close_issue_progress:
            closing_issues = close_issue_progress.add_task(
                "Comparing parsed issue(s) and existing issue(s)...", total=len(existing_cves)
            )
    ...
    def create_issue(self, **kwargs: dict) -> Optional[Issue]:
        ...
        iss = self.update_due_dt(iss=iss, kev_due_date=kev_due_date, scanner=self.name.lower(), severity=severity)
