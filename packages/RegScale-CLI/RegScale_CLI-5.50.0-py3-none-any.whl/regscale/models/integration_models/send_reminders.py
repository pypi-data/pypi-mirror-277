...
class SendReminders:
    ...
    def get_and_send_reminders(self) -> None:
        ...
        with self.job_progress:
            ...
            if len(self.tenant_pipeline) > 0:
                ...
                create_threads(
                    ...
                    args=(self.config, analyze_items, dict_users, self.api),
        ...
        send_email(api=self.api, domain=self.config["domain"], payload=email_payload)
    ...
    def get_upcoming_or_expired_items(self, user: dict) -> Optional[Pipeline]:
        ...
        if total_tasks := sum(len(pipeline["Pipeline"]) for pipeline in pipelines.values()) > 0:
    ...
    def format_and_email(self, args: Tuple, thread: int) -> None:
        ...
        for i in range(len(threads)):
            ...
            if not self.final_pipeline[threads[i]].emailed:
                ...
                emailed = send_email(api=api, domain=config["domain"], payload=email_payload)
