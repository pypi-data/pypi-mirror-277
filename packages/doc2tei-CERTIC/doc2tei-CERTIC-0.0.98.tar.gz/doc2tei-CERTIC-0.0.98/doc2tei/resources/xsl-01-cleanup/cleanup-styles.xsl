...
<!--                 <xsl:comment>*0* coll. doing nada</xsl:comment> -->
            ...
            <xsl:when test="not(preceding::text:p[@text:style-name='Title'])">
                <xsl:element name="text:h">
                    ...
                    <xsl:apply-templates select="//text:span[@text:style-name='TEI_5f_reviewed_5f_title-inline']/node()"/>
                </xsl:element>
                <xsl:element name="text:p">
                    <xsl:copy-of select="@*"/>
                    <xsl:apply-templates/>
            ...
            <xsl:attribute name="rendition">
                <xsl:choose>
                    <xsl:when test="//style:style[@style:name=$currentStyle]/style:paragraph-properties[@fo:text-align and @style:writing-mode='rl-tb']">
                        <xsl:value-of select="concat('#',//style:style[@style:name=$currentStyle]/style:paragraph-properties/@fo:text-align,' #rtl')"/>
                    </xsl:when>
