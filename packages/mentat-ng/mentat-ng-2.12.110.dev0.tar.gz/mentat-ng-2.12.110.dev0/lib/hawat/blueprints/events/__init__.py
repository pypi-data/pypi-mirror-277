...
_EVENTS_SECTION = chart_const.ChartSection(
...
_AGG_SECTIONS = [chart_const.CHART_SECTION_MAP[key] for key in (
...
DASHBOARD_CHART_SECTIONS = [
    _EVENTS_SECTION,
    chart_const.CHART_SECTION_MAP[mentat.stats.idea.ST_SKEY_ABUSES]
] + _AGG_SECTIONS
...
class AbstractDashboardView(SQLAlchemyMixin, BaseSearchView):  # pylint: disable=locally-disabled,abstract-method
    ...
    def _add_charts(self, stats, timeline_cfg, grp_key):
        for chsection in DASHBOARD_CHART_SECTIONS:
            response_chart_dict = self.response_context.setdefault(chart_const.KEY_CHARTS, {}).setdefault(grp_key, {})[chsection.key] = {}
            table_data = self.response_context.setdefault(chart_const.KEY_TABLE_DATA, {}).setdefault(grp_key, {})[chsection.key] = {}
    ...
    def do_after_search(self, items):
        ...
        if items:
            ...
            if self.always_include_charts:
                for grp_key in mentat.stats.idea.LIST_STAT_GROUPS:
                    self._add_charts(self.response_context['statistics'], timeline_cfg, grp_key)
                self.response_context.update(
                    chart_sections=DASHBOARD_CHART_SECTIONS,
                    chart_sections_external=DASHBOARD_CHART_SECTIONS_EXTERNAL
                )
