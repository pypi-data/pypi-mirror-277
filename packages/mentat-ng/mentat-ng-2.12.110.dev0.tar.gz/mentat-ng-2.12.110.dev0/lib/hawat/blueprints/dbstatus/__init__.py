...
import flask
import flask_login
from flask_babel import gettext, lazy_gettext, format_decimal
...
from hawat.utils import get_format_byte_size_function, fallback_formatter
...
CHART_SECTIONS = [
    ...
    chart_const.ChartSection(
        ...
        chart_const.DataComplexity.SINGLE,
        lazy_gettext('Table name'),
        value_formats=chart_const.ValueFormats(
            column_name=lazy_gettext('Total table size'),
            format_function=fallback_formatter(
                get_format_byte_size_function(format_decimal)
            )
        ),
        allow_table_aggregation=False
    ),
    chart_const.ChartSection(
        ...
        lazy_gettext('Table name'),
        value_formats=chart_const.ValueFormats(
            column_name=lazy_gettext('Table size'),
            format_function=fallback_formatter(
                get_format_byte_size_function(format_decimal)
            )
        ),
    ...
    chart_const.ChartSection(
        ...
        lazy_gettext('Table name'),
        value_formats=chart_const.ValueFormats(
            column_name=lazy_gettext('Index size'),
            format_function=fallback_formatter(
                get_format_byte_size_function(format_decimal)
            )
        ),
...
class ViewView(HTMLMixin, PsycopgMixin, SimpleView):
    ...
    def do_before_response(self, **kwargs):
        self.response_context.update(
            query_status_events=self._enrich_result_queries(self.get_db().queries_status(discard_parallel_workers=False)),
        ...
        self._add_charts(dbstatistics_events)
...
class DashboardView(HTMLMixin, SQLAlchemyMixin, SimpleView):  # pylint: disable=locally-disabled,abstract-method
    ...
    def do_before_response(self, **kwargs):
        ...
        self.response_context['users_disabled'] = self.dbquery(UserModel). \
            filter(UserModel.enabled.is_(False)). \
        ...
        self.response_context['groups_disabled'] = self.dbquery(GroupModel). \
            filter(GroupModel.enabled.is_(False)). \
        ...
        self.response_context['filters_disabled'] = self.dbquery(FilterModel). \
            filter(FilterModel.enabled.is_(False)). \
        ...
        self.response_context['settingsrep_redirected'] = self.dbquery(SettingsReportingModel). \
            filter(SettingsReportingModel.redirect is True). \
