...
def transform2bufr():
    ...
    for item in results:
        ...
        bufr4 = item['bufr4']
        transform_bufr2geojson(bufr4)
        subprocess.call(['./upload_to_minio.sh', 'localhost:9000', 'minio', 'minio123', 'wis2-public',
                         'topic/hierarchy/one', f'{bufr4}', identifier])
        

def transform_bufr2geojson(bufr_data: bytes):
    client = Elasticsearch('http://localhost:9200')

    def gendata(features):
        """
        Generator function to yield features
        param: features - dictionary of geojson records, each key is unique identifier combined with a bufr descriptor index
        """
        for key in features.keys():
            feature = features[key]['geojson']
            feature['properties']['id'] = feature['id']
            yield {
                '_index': 'fm14_transform',
                '_id': feature['id'],
                '_source': feature
            }

    geojson_results = bufr2geojson.transform(bufr_data, serialize=False)
    for item in geojson_results:
        helpers.bulk(client, gendata(item))
