...
class CardRequest(BaseModel):
    ...
    CardRequest
    """ # noqa: E501
    affiliations: List[Dict[str, Any]] = Field(description="The affiliations of the card request - the cardholders associations fetched from the University identity systems.")
    attributes: Optional[Dict[str, Any]] = Field(default=None, description="The attributes of the card - describing what is present on the physical version of the card. This is an untyped block of JSON and the structure may change between card types, as different card types will allow different data to be printed on them. The attributes of temporary cards will not be populated as data relating to what has been printed on temporary cards has not been recorded.")
    back_visualization_link: StrictStr = Field(alias="backVisualizationLink")
    can_update: StrictBool = Field(alias="canUpdate")
    cancel_reason: Optional[StrictStr] = Field(default=None, description="Reason the card request was CANCELLED", alias="cancelReason")
    card_request_version: Optional[Annotated[int, Field(le=2147483647, strict=True, ge=-2147483648)]] = Field(default=None, description="Auto-incremented version number of the card request", alias="cardRequestVersion")
    ...
    cardholder_status: Optional[StrictStr] = Field(default=None, description="An approximation of the status of the cardholder at the point this card request has been raised. Only used to allow filtering of card requests, rather than as a deterministic indication of a cardholder's status.", alias="cardholderStatus")
    college_institution_id: Optional[StrictStr] = Field(default=None, description="The college institution id used to set the card request scarf  * `NO_COLLEGE` - no-college * `CAIUS` - gonville * `CATH` - st-catherines * `CHRISTS` - christs * `CHURCH` - churchill * `CLARE` - clare * `CLAREH` - clare-hall * `CORPUS` - corpus-christi * `DARWIN` - darwin * `DOWN` - downing * `EDMUND` - st-edmunds * `EMM` - emmanuel * `FITZ` - fitzwilliam * `GIRTON` - girton * `HOM` - homerton * `HUGHES` - hughes-hall * `JESUS` - jesus * `JOHNS` - st-johns * `KINGS` - kings * `LCC` - lucy-cavendish * `MAGD` - magdalene * `NEWH` - murray-edwards * `NEWN` - newnham * `PEMB` - pembroke * `PET` - peterhouse * `QUEENS` - queens * `RIDLEY` - ridley-hall * `ROBIN` - robinson * `SEL` - selwyn * `SID` - sidney-sussex * `TRIN` - trinity * `TRINH` - trinity-hall * `WESLEY` - wesley * `WESTCOT` - westcott * `WESTMIN` - westminster * `WOLFC` - wolfson", alias="collegeInstitutionId")
    created_at: Optional[datetime] = Field(description="When this record was created", alias="createdAt")
    destination: Optional[StrictStr] = Field(default=None, description="The location the physical card will be delivered to once created")
    error_reason: Optional[StrictStr] = Field(default=None, description="ERROR raised by card request", alias="errorReason")
    front_visualization_link: StrictStr = Field(alias="frontVisualizationLink")
    hold_reason: Optional[StrictStr] = Field(default=None, description="Reason the card request was placed on HOLD", alias="holdReason")
    ...
    requestor: Optional[StrictStr] = Field(default=None, description="Origin of the card request")
    rfid_data: List[Dict[str, Any]] = Field(alias="rfidData")
    ...
    @field_validator('college_institution_id')
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # and model_fields_set contains the field
        if self.college_institution_id is None and "college_institution_id" in self.model_fields_set:
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "cardRequestVersion": obj.get("cardRequestVersion"),
            "cardType": obj.get("cardType"),
            "cardholderStatus": obj.get("cardholderStatus"),
            "collegeInstitutionId": obj.get("collegeInstitutionId"),
