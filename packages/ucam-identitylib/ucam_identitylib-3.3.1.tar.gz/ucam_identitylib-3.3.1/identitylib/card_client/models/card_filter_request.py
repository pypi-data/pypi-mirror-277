...
class CardFilterRequest(BaseModel):
    ...
    CardFilterRequest
    """ # noqa: E501
    identifiers: Annotated[List[Annotated[str, Field(min_length=1, strict=True)]], Field(min_length=1, max_length=50)] = Field(description="Identifiers to filter by, in the format {value}@{scheme}. {scheme} may be omitted in which case the scheme is assumed to be CRSid")
    institution: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, description="Filter by the institutions that cardholders belong to")
    status: Optional[StrictStr] = Field(default=None, description="Status to filter by, if omitted cards of all statuses are returned  * `ISSUED` - Issued * `REVOKED` - Revoked * `RETURNED` - Returned * `EXPIRED` - Expired * `UNACTIVATED` - Unactivated")
    updated_at__gte: Optional[datetime] = Field(default=None, description="Filter updatedAt by IsoDateTime greater than")
    updated_at__lte: Optional[datetime] = Field(default=None, description="Filter updatedAt by IsoDateTime less than")
    __properties: ClassVar[List[str]] = ["identifiers", "institution", "status", "updated_at__gte", "updated_at__lte"]
