...
class MetricsListResponseType(BaseModel):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "issuedAt": [IssuedAtType.from_dict(_item) for _item in obj["issuedAt"]] if obj.get("issuedAt") is not None else None,
            "revokedAt": [RevokedAtType.from_dict(_item) for _item in obj["revokedAt"]] if obj.get("revokedAt") is not None else None,
            "revokedAtNotes": [RevokedAtNotesType.from_dict(_item) for _item in obj["revokedAtNotes"]] if obj.get("revokedAtNotes") is not None else None
        })
        return _obj
