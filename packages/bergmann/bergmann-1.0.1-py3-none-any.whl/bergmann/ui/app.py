...
from bergmann.entities.load_items_result import LoadItemsResult
...
class Bergmann(App[None]):
    ...
    def __init__(self, debug: bool):
        super().__init__()
        self._interactor = di.interactor
        self._debug = debug
    ...
    async def _load_db(self, path: Path) -> LoadItemsResult:
        if not self._interactor.is_store_initialized(path):
