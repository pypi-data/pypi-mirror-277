...
class SQLResultChain(BaseDocugamiChain[ExplainedSQLResult]):
    ...
    def optimize(
        ...
        if self._optim_active.is_set():
            if completion_callback:
                completion_callback(
                    False, Exception("Optimization is already in progress.")
                )
            return

        def optimize_task() -> None:
            self._optim_active.set()
            try:
                optimized_db = convert_to_typed(
                    db=self.db,
                ...
                self._optim_active.clear()

        self._optim_executor.submit(optimize_task)
