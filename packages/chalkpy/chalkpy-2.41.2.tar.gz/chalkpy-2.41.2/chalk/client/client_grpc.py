...
class ChalkGRPCClient:
    ...
    def _parse_uri_for_engine(cls, query_server_uri: str) -> ParsedUri:
        """
        If the scheme is 'http' or it's pointed at localhost/within a VPC, then DON'T use tls
        :param query_server_uri:
        :return:
        """
        url_parsed = urlparse(query_server_uri)
        if url_parsed.scheme == "http" or any(
            url_parsed.netloc.startswith(pfx) for pfx in ["localhost", "127.0.0.1", "10."]
        ):
            use_tls = False
        else:
            use_tls = True
