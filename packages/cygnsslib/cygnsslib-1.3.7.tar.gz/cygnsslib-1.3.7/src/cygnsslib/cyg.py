    ...
    if parallel:
        ...
        with Pool() as pool:
            args = ((cygnss_l1_path, i_date, download_cygnss_data, thresh_ddm_snr, ddm_quality_filter, bbox, poly, tf_poly, lon_mode360,
                     ref_pos, radius, tf_print_screen, save_ddm_img, img_save_type, plt_tag, plt_thresh_noise, title_img_inc_ddm_time, out_folder,
                     fig_out_folder, plt_reflectivity, None, search_calib_rawif_sampling_rate) for i_date in date_list)
            if pbar_tf:
...
def _find_sp_in_day(cygnss_l1_path: str, i_date: datetime.date, download_cygnss_data: bool, thresh_ddm_snr: float, ddm_quality_filter: int,
                    ...
                    title_img_inc_ddm_time: str, out_folder: str, fig_out_folder: str, plt_reflectivity: bool = False, pbar: Optional[tqdm] = None, rawif_sampling_rate=None) -> tuple[list, list]:
                    ...
                    if sp_is_in_poly:
                        ...
                        if ddm_snr[i_samp, i_chan] >= thresh_ddm_snr and save_ddm_img:  # Save plot of BRCS
                            ...
                            plot_cyg_brcs(ddm_, ddm_snr[i_samp, i_chan], sp_inc_angle[i_samp, i_chan], lyr_name, plt_tag, f"{i_samp}",
                                          img_save_type, fig_out_folder, tf_print_screen, plt_reflectivity, plt_cmax, delay_scale, dopp_scale)
        nc_file.close()
...
def plot_cyg_brcs(brcs_sel: np.ndarray, ddm_snr: float, sp_inc_angle: str, lyr_name: str, plt_tag: str, pt_name: str,
                  ...
                  cbar_min_max: Optional[list[float]] = None, delay_scale=None, dopp_scale=None):
    ...
    fig = plot_single_ddm(brcs_db, img_title, img_save_name, fig_out_folder, fig_save_types=img_save_type, cbar_min_max=cbar_min_max,
                          cbar_title=cbar_title, delay_scale=delay_scale, dopp_scale=dopp_scale)
    if tf_print_screen:
        print(img_save_name)
    plt.close(fig)
