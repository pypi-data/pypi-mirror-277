...
import cygnsslib.istarmap  # fix for tqdm with multiprocessing
    ...
    if parallel:
        ...
        with Pool() as pool:
            args = ((cygnss_l1_path, i_date, download_cygnss_data, thresh_ddm_snr, ddm_quality_filter, bbox, poly, tf_poly, lon_mode360,
                     ...
                     plt_tag, plt_thresh_noise, title_img_inc_ddm_time, out_folder, plt_reflectivity, None, search_calib_rawif_sampling_rate) for i_date in date_list)
            if pbar_tf:
                for day_pnt_list_high, day_pnt_list_low in tqdm(pool.istarmap(_find_sp_in_day, args), total=len(date_list),
                                                                desc=f'Searching: {file_tag}', unit='day'):
                    if day_pnt_list_high:
                        pnt_list_high += day_pnt_list_high
                    if day_pnt_list_low:
                        pnt_list_low += day_pnt_list_low
            else:
                for day_pnt_list_high, day_pnt_list_low in pool.starmap(_find_sp_in_day, args):
                    if day_pnt_list_high:
                        pnt_list_high += day_pnt_list_high
                    if day_pnt_list_low:
                        pnt_list_low += day_pnt_list_low
        ...
        for i_date in date_list:
            day_pnt_list_high, day_pnt_list_low = _find_sp_in_day(cygnss_l1_path, i_date, download_cygnss_data, thresh_ddm_snr, ddm_quality_filter,
                                                                  ...
                                                                  out_folder, plt_reflectivity, pbar, search_calib_rawif_sampling_rate)
            if day_pnt_list_high:
                pnt_list_high += day_pnt_list_high
            if day_pnt_list_low:
                pnt_list_low += day_pnt_list_low
...
def _export_sp_points(pnt_list: list[dict], klm_out_file_path: str, lyr_options, sampling_rate: float, save_klm: bool, sp_color_hex: str, save_csv: bool) -> Optional[
    pd.DataFrame]:
    df_data = None
    if not pnt_list:
...
def _find_sp_in_day(cygnss_l1_path: str, i_date: datetime.date, download_cygnss_data: bool, thresh_ddm_snr: float, ddm_quality_filter: int,
                    ...
                    title_img_inc_ddm_time: str, out_folder: str, plt_reflectivity: bool = False, pbar: Optional[tqdm] = None, rawif_sampling_rate=None) -> tuple[list, list]:
                ...
                for i_samp in sample_sel:
                    ...
                    if sp_is_in_poly:
                        ...
                        if ddm_snr[i_samp, i_chan] >= thresh_ddm_snr and save_ddm_img:  # Save plot of BRCS
                            if plt_reflectivity:
                                sc_pos_x = float(nc_file.variables["sc_pos_x"][i_samp])
                                sc_pos_y = float(nc_file.variables["sc_pos_y"][i_samp])
                                sc_pos_z = float(nc_file.variables["sc_pos_z"][i_samp])
                                sc_pos = np.array([sc_pos_x, sc_pos_y, sc_pos_z])
                                tx_pos_x = float(nc_file.variables["tx_pos_x"][i_samp, i_chan])
                                tx_pos_y = float(nc_file.variables["tx_pos_y"][i_samp, i_chan])
                                tx_pos_z = float(nc_file.variables["tx_pos_z"][i_samp, i_chan])
                                tx_pos = np.array([tx_pos_x, tx_pos_y, tx_pos_z])
                                sp_pos_x = float(nc_file.variables["sp_pos_x"][i_samp, i_chan])
                                sp_pos_y = float(nc_file.variables["sp_pos_y"][i_samp, i_chan])
                                sp_pos_z = float(nc_file.variables["sp_pos_z"][i_samp, i_chan])
                                ref_pos = np.array([sp_pos_x, sp_pos_y, sp_pos_z])
                                ddm_ = brcs2reflectivity(brcs[i_samp, i_chan, :, :], tx_pos, sc_pos, ref_pos)
                            else:
                                ddm_ = brcs[i_samp, i_chan, :, :]
                            plot_cyg_brcs(ddm_, ddm_snr[i_samp, i_chan], sp_inc_angle[i_samp, i_chan], lyr_name, plt_tag, f"{i_samp}",
                                          img_save_type, out_folder, tf_print_screen, plt_reflectivity)
        nc_file.close()
    return day_pnt_list_high, day_pnt_list_low


def brcs2reflectivity(brcs, tx_pos, rx_pos, ref_pos):
    tx_sp_rng = np.sqrt(np.sum((ref_pos - tx_pos) ** 2, axis=0))
    sp_rx_rng = np.sqrt(np.sum((ref_pos - rx_pos) ** 2, axis=0))
    reflectivity = brcs / (4 * np.pi) / (tx_sp_rng * sp_rx_rng) ** 2 * (tx_sp_rng + sp_rx_rng) ** 2
    return reflectivity
...
def plot_cyg_brcs(brcs_sel: np.ndarray, ddm_snr: float, sp_inc_angle: str, lyr_name: str, plt_tag: str, pt_name: str,
                  ...
                  cbar_min_max: Optional[list[float]] = None):
    brcs_db = pwr2db_threshold(brcs_sel)
    img_save_name = plt_tag + ('_' if plt_tag else '') + lyr_name + "_samp" + pt_name
    img_title = 'Reflectivity [dB]' if reflectivity_tf else 'BRCS [dBsm]'
    img_title += f"\nddm_snr = {ddm_snr:.2f} dB, inc = {sp_inc_angle:.1f} deg"
    img_title += f', SP Reflectivity = {np.max(brcs_db):.2g} dB' if reflectivity_tf else f', SP BRCS = {np.max(brcs_db):.2g} dBsm'
