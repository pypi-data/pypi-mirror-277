...
class Dumper:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self._dataclasscache = {}  # type: Dict[Type[Any], Tuple[Set[str], Dict[str, Any], Dict[str, Any], Dict[str, str], Set[str]]]
...
def _dataclassdump(d: Dumper, value, t) -> Dict[str, Any]:
    t = type(value)
    try:
        fields, defaults, type_hints, renames, needs_dump  = d._dataclasscache[t]
    except KeyError:
        ...
        needs_dump = fields.difference({
            ...
            if (v.type in d.basictypes and d.handlers[0][1] == _identitydump) or is_literal(v.type)
        })

        d._dataclasscache[t] = (fields, defaults, type_hints, renames, needs_dump)

    r = value.__dict__.copy()
    for k in fields:
        if not d.hidedefault or k not in defaults or defaults[k] != r[k]:
            if k in needs_dump:
                r[k] = d.dump(r[k], type_hints[k])
        else:
            del r[k]
    ...
    if renames:
        ...
        for f in renames:
            if f in r:
                renamesqueue.append((renames[f], r.pop(f)))
        for k,v  in renamesqueue:
            r[k] = v
