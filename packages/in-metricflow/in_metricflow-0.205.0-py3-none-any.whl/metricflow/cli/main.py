...
from metricflow.cli.api_context import APIContext

logger = logging.getLogger(__name__)

pass_config = click.make_pass_decorator(CLIContext, ensure=True)
pass_config = click.make_pass_decorator(APIContext, ensure=True)
...
def query(
    ...
    spinner.start()
    print(metrics, saved_query, group_by, limit, start_time, end_time, where, order)
    # All the input params to the query
    ...
    if explain:
        explain_result = cfg.new_mf.explain(mf_request=mf_request)
    else:
        query_result = cfg.new_mf.query(mf_request=mf_request)
...
def metrics(cfg: CLIContext, show_all_dimensions: bool = False, search: Optional[str] = None) -> None:
    ...
    metrics = cfg.new_mf.list_metrics()
...
def dimensions(cfg: CLIContext, metrics: List[str]) -> None:
    ...
    dimensions = cfg.new_mf.simple_dimensions_for_metrics(metrics)
    if not dimensions:
...
def entities(cfg: CLIContext, metrics: List[str]) -> None:
    ...
    entities = cfg.new_mf.entities_for_metrics(metrics)
    if not entities:
...
def dimension_values(
    ...
    try:
        dim_vals = cfg.new_mf.get_dimension_values(
            metric_names=metrics,
