...
class Auth:
    ...
    def _login_with_signature_method(self, sign_method) -> bool:
        ...
        if nonce:
            ...
            if "payload" in auth_response:
                ...
                print("Login failed. No payload in auth response.")
        else:
            print("Login failed. No nonce received.")
    ...
    def unlink_rsa_key(self):
        ...
        """
        try:
            nonce_response = self.parent.post("/keys/requestUnlinkRsaKey", json={})
            nonce = nonce_response[
                "payload"
            ]  # This line may raise KeyError if 'payload' is not in nonce_response
            eth_signature = self.sign_eth_message(
                typed_data=self._construct_typed_data(nonce)
            )
            unlink_response = self.parent.post(
                "/keys/confirmUnlinkRsaKey",
                json={"nonce": nonce, "signature": eth_signature},
            )
            ...
            error_msg = (
                "Error: 'payload' key not found in the response. "
                "This may indicate that there is no RSA key currently"
