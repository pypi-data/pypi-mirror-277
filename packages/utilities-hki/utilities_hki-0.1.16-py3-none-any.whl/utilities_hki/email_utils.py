...
def download_attachments(response, folder_name):
    """
...
def readmails(username, password, tag, folder_name, sender, n_mails=np.inf, min_date=''):
    """
    ...
    for index in datetimeorder.values():
        ...
        for response in msg:
            filenames = download_attachments(response, folder_name)
            if filenames:
