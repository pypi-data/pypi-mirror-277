...
import fused._public_api
from fused._formatter.formatter_project import fused_project_repr
from fused._global_api import get_api
from fused._options import options as OPTIONS
from fused._str_utils import append_url_part, table_to_name
...
from fused.models.internal.dataset import (
    DatasetInputV2,
    DatasetInputV2Table,
    DatasetInputV2Type,
    DatasetOutputV2,
...
from fused.warnings import FusedIgnoredWarning, FusedPathWarning, FusedRefreshingWarning
...
class Project(BaseModel):
    ...
    def open_table(
        ...
        *,
        ...
        return fused._public_api.open_table(
            path=path,
    ...
    def ingest(
        ...
        *,
        ...
        return fused._public_api.ingest(
            input=input,
    ...
    def ingest_nongeospatial(
        ...
        *,
        ...
        return fused._public_api.ingest_nongeospatial(
            input=input,
            output=project_output,
            output_metadata=project_output_metadata,
    ...
    def delete(
        ...
        return fused._public_api.delete(
    ...
    def list(self, path: Optional[str] = None) -> List[str]:
        ...
        """
        project_path = self.path(path) if path is not None else self.base_path
        return fused._public_api.list(project_path)
    ...
    def get(self, path: str) -> bytes:
        ...
        return fused._public_api.get(project_path)
    ...
    def download(self, path: str, local_path: Union[str, Path]) -> None:
        ...
        """
        project_path = self.path(path)
        return fused._public_api.download(project_path, local_path=local_path)
    ...
    def sign_url(self, path: str) -> str:
        ...
        """
        project_path = self.path(path)
        return fused._public_api.sign_url(project_path)
    ...
    def sign_url_prefix(self, path: str) -> Dict[str, str]:
        ...
        """
        project_path = self.path(path)
        return fused._public_api.sign_url_prefix(project_path)
