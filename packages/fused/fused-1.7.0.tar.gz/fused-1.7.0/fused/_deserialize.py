...
from fused.models.api import JobStepConfig
from fused.models.udf import Chunk, ChunkMetadata, Chunks
...
def extract_chunk(zf: ZipFile, prefix: str, *, n_rows: Optional[int] = None) -> Chunk:
    ...
    metadata = ChunkMetadata.parse_raw(
        zf.read(f"{prefix}/row_group_metadata"), content_type="json"
    )
...
def zip_to_map_args(buffer: bytes, *, n_rows: Optional[int] = None) -> MapInput:
    with BytesIO(buffer) as bio, ZipFile(bio, mode="r") as zf:
        args_metadata = ArgsMetadata.parse_raw(
            zf.read("fused_args/metadata"), content_type="json"
        )
