...
class Header(BaseModel):
    ...
    _preceding_sys_modules: Optional[Sequence[str]] = PrivateAttr(None)
    ...
    def write_code(self):
        # TODO: select between write_code_to_disk and write_code_to_gist
    ...
    def _register_custom_finder(self, preceding_sys_modules):
        ...
        # Register custom finder if not in sys.modules
        if module_name not in preceding_sys_modules:
            ...
            # Existing module, don't override
            warnings.warn(
                f"Header `{module_name}` was ignored because its name conflicts with that of a reserved module. Consider renaming it.",
                FusedWarning,
            )
        return self
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        # Remove modules
        try:
            if self.module_name not in self._preceding_sys_modules:
