...
from typing import IO, Any, Dict, List, Literal, Optional, Type, Union
...
from fused.warnings import FusedDefaultWarning, FusedReservedWarning
...
def data_type_from_arrow(dtype: pa.DataType) -> DataType:
    ...
    warnings.warn(
        f"unimplemented type {dtype}; defaulting to String", FusedDefaultWarning
    )
...
TimestampType.update_forward_refs()
ListType.update_forward_refs()
LargeListType.update_forward_refs()
FixedSizeListType.update_forward_refs()
FixedSizeBinaryType.update_forward_refs()
...
class Schema(BaseModel):
    ...
    def from_string(cls, str: str) -> Schema:
        ...
        if str.lstrip().startswith("{"):
            return cls.parse_raw(str, content_type="json")
