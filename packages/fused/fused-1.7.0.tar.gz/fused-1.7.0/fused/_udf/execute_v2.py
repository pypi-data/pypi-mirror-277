...
from fused.models.udf import EMPTY_UDF, BaseUdf
...
def execute_against_sample(
    ...
    **kwargs,
    ...
    try:
        ...
        # Store names of modules prior to rendering Headers
        preceding_sys_modules = list(sys.modules.keys())
        ...
        with ExitStack() as stack:
            ...
            if udf.headers is not None:
                for header in udf.headers:
                    stack.enter_context(
                        header._register_custom_finder(preceding_sys_modules)
                    )
            ...
            _output = _fn(*args_to_pass, **kwargs_to_pass)
...
def validate_imports_whitelist(udf: BaseUdf, validate_imports: Optional[bool] = None):
    ...
    if not fused.api.CREDENTIALS.has_credentials():
        return
