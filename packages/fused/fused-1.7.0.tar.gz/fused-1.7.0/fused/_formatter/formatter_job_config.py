...
def get_dataset_input_settings(input: Union[DatasetInput, DatasetInputV2]) -> str:
    results = ""
...
def summarize_output(
    ...
    *,
    ...
    def get_dataset_output_settings(output: DatasetOutputBase) -> str:
        results = ""
...
def summarize_inputs(
    inputs: List[Union[str, DatasetInput, DatasetInputV2]],
...
def summarize_outputs(outputs: List[DatasetOutputBase]):
    li_items = []
...
def get_object_settings(obj: BaseModel, exclude: Sequence[str] = ()) -> str:
    ...
    return (
        f"<ul>{''.join([_format_kv(key, val) for key, val in obj.dict().items()])}</ul>"
    )
...
def fused_map_repr(map: MapJobStepConfig) -> str:
    ...
    obj_type = f"fused.{map.__class__.__name__}: {map.name or output_name}"
    header_components = [
        f"<div class='xr-obj-type'>{escape(obj_type)}</div>",
...
def fused_join_repr(join: JoinJobStepConfig) -> str:
    ...
    obj_type = f"fused.{join.__class__.__name__}: {join.name or output_name}"
    header_components = [
        f"<div class='xr-obj-type'>{escape(obj_type)}</div>",
        get_object_settings(
            join,
