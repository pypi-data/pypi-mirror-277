...
from fused._formatter.common import (
    copyable_text,
    format_number,
    icon,
    load_static_files,
...
if TYPE_CHECKING:
    from fused.models.api import Dataset
    from fused.models.api.dataset import Table
...
def summarize_table(
    ...
    *,
    ...
    def get_parent_copy(table: Table):
        if table.parent:
            copyable = copyable_text(
                f"fused.experimental.job({repr(table.parent.job.json())}, content_type='json')",
                show_text=False,
            )
    ...
    def parent_udf(table: Table):
        if table.parent and "udf" in table.parent.job.dict():
...
def fused_dataset_repr(dataset: Dataset) -> str:
    obj_type = f"fused.Dataset: {dataset.base_path}"
...
def fused_table_repr(table: Table) -> str:
    obj_type = f"fused.Table: {table.name}"
    header_components = [f"<div class='xr-obj-type'>{escape(obj_type)}</div>"]
