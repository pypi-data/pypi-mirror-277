...
from fused._str_utils import detect_passing_local_file_as_str, table_to_name
...
def job(
    ...
    if isinstance(input, str):
        ...
        if content_type == "json":
            return JobConfig(
                steps=[JobStepConfig.parse_raw(input, content_type="json")]
            )
        else:
            assert (
                False
...
def plot(
    ...
    source: Union[str, TileProvider] = cx.providers.CartoDB.DarkMatterNoLabels,
...
def load_job(path: Union[str, Path, BinaryIO]):
    ...
    with change_directory(path):
        ...
        for step in validated_job.steps:
            ...
            if step["type"] == "join":
                input_params = [
                    DatasetInput.parse_obj(step["input_left"]),
                    DatasetInput.parse_obj(step["input_right"]),
                ]
                buffer_distance = step.get("buffer_distance")
                join_how = step.get("how")
            elif step["type"] == "map":
                input_params = [DatasetInput.parse_obj(step["input"])]
            ...
            udf_registry = fused.experimental.load_udf(
        ...
        job = fused.experimental.job(jobs)
        job.metadata = (
