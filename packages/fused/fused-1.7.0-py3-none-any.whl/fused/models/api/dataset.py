...
from fused._formatter.formatter_dataset import fused_dataset_repr, fused_table_repr
from fused._options import options as OPTIONS
from fused.models._project_aware import FusedProjectAware
from fused.models.api.enums import JoinType
from fused.models.api.job import (
    JobStepConfig,
    JoinJobStepConfig,
...
class Table(FusedBaseModel, FusedProjectAware):
    ...
    def metadata_gdf(self) -> gpd.GeoDataFrame:
        ...
        if self.chunk_metadata:
            return gpd.GeoDataFrame(
                [chunk.dict() for chunk in self.chunk_metadata],
    ...
    def refresh(self, fetch_samples: Optional[bool] = None) -> Table:
        ...
        for key in new_table.dict().keys():
...
class Dataset(FusedBaseModel, FusedProjectAware):
    ...
    @staticmethod
    def _parse_udf(udf: Union[BaseUdf, None]) -> BaseUdf:
        if udf is None:
            return EMPTY_UDF
        elif isinstance(udf, BaseUdf):
            return udf
        else:
