...
class Output(BaseModel):
    ...
    data: Union[pa.Table, pd.DataFrame, gpd.GeoDataFrame, None] = None
    ...
    def validate_data(cls, v, values, **kwargs):
        if values.get("skip_fused_index_validation"):
            return v
...
class PyArrowOutput(Output):
    ...
    def validate_data_with_schema(self, *, strict: bool = False):
        super().validate_data_with_schema(strict=strict)
        assert (
            INDEX_COLUMN_NAME in self.data.column_names
        ), f"`{INDEX_COLUMN_NAME}` should be a column on `output.data`"

        index_type = self.data.field(INDEX_COLUMN_NAME)
        if (
            pa.int64() == index_type
            or pa.int32() == index_type
            or pa.int16() == index_type
            or pa.int8() == index_type
            or pa.uint64() == index_type
            or pa.uint32() == index_type
            or pa.uint16() == index_type
            or pa.uint8() == index_type
        ):
            # TODO: Check for negative indexes here
            pass
        else:
            warnings.warn(
                f"Expected `{INDEX_COLUMN_NAME}` to be an integer, but it was {index_type}",
                FusedUdfWarning,
            )
