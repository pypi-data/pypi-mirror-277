...
class Chunk(BaseModel):
    ...
    sidecar: Optional[Dict[StrictStr, bytes]]
...
class Chunks(BaseModel):
    ...
    def concat_data(self, **kwargs) -> ChunkData:
        ...
        first = self.__root__[0].data
        if isinstance(first, pd.DataFrame):
            assert all(isinstance(chunk.data, pd.DataFrame) for chunk in self.__root__)
            return pd.concat([chunk.data for chunk in self.__root__], **kwargs)
        else:
            return pa.concat_tables([chunk.data for chunk in self.__root__], **kwargs)
