...
def main():    
    ...
    parser.add_argument("--ConfigFileNames", type=str, nargs='+', default=[default_config_file])
    ...
    for config_path in config_paths:
        ...
        if not file_name:
            print("No filename specified, using example file from: {0} and place a copy in the current folder".format(example_file))
            destination = os.path.join(os.getcwd(), os.path.basename(example_file))
            shutil.copy(example_file, destination)
            file_name = example_file
            
        csv_service = CsvService()
        flow_metrics_service = FlowMetricsService(show_plots, charts_folder, logo_path)
