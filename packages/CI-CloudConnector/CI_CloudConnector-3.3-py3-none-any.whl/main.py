    ...
    def on_modified(self, event):
        ...
        try:
            if event.src_path.endswith(self.main_file):
                self.myService.logger.info("Main file has been modified. Signaling service to restart...")

                # Stop the service
                self.myService.ServiceUpdated()
                self.myService.logger.info("Service stopped. You may restart it if necessary.")

        except Exception as e:
            self.myService.logger.error(f"Error occurred during file modification event handling: {e}")
...
def monitor_main_file(main_file, service):
    try:
        ...
        observer.start()
        service.logger.info(f"Monitoring main file: {main_file}")

        return observer

    except Exception as e:
        service.logger.error(f"Error occurred while setting up file monitoring: {e}")
        return None
...
def set_service_startup_type(service_name, startup_type):
    ...
    try:
        ...
        print(f"Service '{service_name}' startup type set to '{startup_type}'.")
    except subprocess.CalledProcessError as e:
...
def init():
    ...
    set_service_startup_type("CloudConnectorService", "auto")
