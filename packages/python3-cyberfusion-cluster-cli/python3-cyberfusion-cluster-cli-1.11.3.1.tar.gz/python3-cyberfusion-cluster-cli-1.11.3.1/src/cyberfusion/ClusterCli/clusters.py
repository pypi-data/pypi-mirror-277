...
def create(
    ...
    new_relic_infrastructure_license_key: Optional[str] = typer.Option(
        default=None
    ),
...
@app.command(rich_help_panel=HELP_PANEL_UPDATE)
@catch_api_exception
def update_mariadb_cluster_name(name: str, mariadb_cluster_name: str) -> None:
...
@app.command(rich_help_panel=HELP_PANEL_SHOW)
@catch_api_exception
def get_properties(name: str) -> None:
    ...
    if ClusterGroup.DB in cluster.groups:
        ...
        panels["Group: Database"] = {
            "database_toolkit_enabled": "Database Toolkit"
        }
    ...
    if ClusterGroup.WEB in cluster.groups or ClusterGroup.DB in cluster.groups:
        panels["Groups: Web, Database"] = {
            "sync_toolkit_enabled": "Sync Toolkit"
        }
