...
class Application:
    ...
    _termination_signal_hooked = False

    def __init__(self) -> None:
        pass

    @staticmethod
    def hook_termination_signal():
        """Hook termination signals to Application"""

        def on_termination(signum, frame):
            Application._termination_event.set()

        signal.signal(signal.SIGINT, on_termination)
        signal.signal(signal.SIGTERM, on_termination)
        Application._termination_signal_hooked = True
    ...
    def is_terminating() -> bool:
        ...
        """
        assert Application._termination_signal_hooked, "termination signal is not hooked"
        return Application._termination_event.is_set()

    @staticmethod
    def wait_for_termination():
        """Block current thread until termination signal is received"""
        assert Application._termination_signal_hooked, "termination signal is not hooked"
        Application._termination_event.wait()
