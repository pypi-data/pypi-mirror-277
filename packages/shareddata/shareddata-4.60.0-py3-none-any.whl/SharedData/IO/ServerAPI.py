...
if not 'SHAREDDATA_API_TOKEN' in os.environ:
    raise Exception('SHAREDDATA_API_TOKEN environment variable not set')
...
def subcribe(database, period, source, tablename):
    try:        
        ...
        if clienttoken != os.environ['SHAREDDATA_API_TOKEN']:
