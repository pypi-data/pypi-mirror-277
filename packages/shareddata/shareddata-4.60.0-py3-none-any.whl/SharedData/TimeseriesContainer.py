...
class TimeseriesContainer:
    ...
    def get_path(self):
        ...
        if (self.shareddata.save_local) | (self.type == 'MEMORY'):
            ...
            if self.type == 'DISK':
                if not _tag in self.tags.keys():
                    self.tags[_tag] = TimeSeriesDisk(self.shareddata, self, self.database,
                                                    self.period, self.source, tag=_tag, value=df, user=self.user)
                else:
