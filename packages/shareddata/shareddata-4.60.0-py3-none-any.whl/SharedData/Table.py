...
from SharedData.IO.AWSS3 import S3Upload, S3Download, UpdateModTime, S3ListFolder, S3GetMtime
...
class Table:
    ...
    def __init__(self, shareddata, database, period, source, tablename,
        ...
        try:
            ...
            if (self.mutex['type']==0):
                self.mutex['type'] = self.type
            elif self.mutex['type'] != self.type:
                if self.mutex['type'] == 1:
                    raise Exception('Table %s is loaded as DISK!' % (self.relpath))
                elif self.mutex['type'] == 2:
                    raise Exception('Table %s is loaded as MEMORY!' % (self.relpath))
                else:
                    raise Exception('Table %s is loaded with unknown type!')

            self.exists_local = self.filepath.is_file()
            self.exists_remote = self.exists_local
            if not self.exists_local:
                keys = S3ListFolder(self.shm_name.replace('\\', '/'))
                self.exists_remote = np.any(
                    [self.relpath+'/' in s for s in keys])
            self.exists = (self.exists_local) | (self.exists_remote)

            if self.type == 2: # MEMORY
                self.exists = (self.exists) | (self.mutex['isloaded']!=0)
    ...
    def init_paths(self):
        ...
        self.database_folder = os.environ['DATABASE_FOLDER']
        self.path = Path(os.environ['DATABASE_FOLDER'])
        if self.shareddata.dbfolderdict:
            key = self.database+'/'+self.period
            if key in self.shareddata.dbfolderdict:
                self.path = Path(self.shareddata.dbfolderdict[key])
                self.database_folder = self.shareddata.dbfolderdict[key]
    ...
    def write(self):
        ...
        try:
            ...
            thread_s3 = threading.Thread(
                target=self.upload, args=(write_head, mtime))
            thread_s3.start()
    ...
    ############### UPLOAD ###############
    def upload(self, write_head, mtime):
        if self.shareddata.s3write:
            ...
            if (write_head) | (not remote_head_is_updated):
    ...
    def subscribe(self, host, port, lookbacklines=1000, 
                  lookbackdate=None, method='websocket', snapshot=False, bandwidth=1e6):
    ...
    def publish(self, host, port, lookbacklines=1000, 
                  lookbackdate=None, method='websocket', snapshot=False, bandwidth=1e6):
