...
class MainWindow(QMainWindow):
    ...
    def update_widget(self, widget, width=None, height=None):
        ...
        self.move(int(centerPoint.x() / 5), int(centerPoint.y() / 5))
        self.show()
        ...
        if self.token == 0:
            if self.isPractice:
                ...
                msgBox = QMessageBox.warning(
                    None,
                    "Warning",
                    "Wrong AccessCodes. Do you want to continue as practice mode?",
    ...
    def process_request(self, code1, code2):
        try:
            ...
            self.user_info = self.pd_client.get(
                "getUser", json={"userIdentity": self.u.identity}
            )
    ...
    def receive_db_user(self, username, password):
        ...
        auth_success = localdb_authenticator.authenticator(
            self.localDB, self.db_user, self.db_pass
        )
