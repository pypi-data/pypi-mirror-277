...
class InspectionWindow(QWidget):
    def __init__(self, parent=None):
        ...
        if "pictures_param" in self.parent.custom:
            self.img_brightness = self.parent.custom["pictures_param"].get(
                "Brightness", 0
            )
            self.img_contrast = self.parent.custom["pictures_param"].get(
                "Contrast", 1.0
            )
        else:
            self.brightness = 0
        ...
        if self.parent.overlay:
            self.view_over = self.setImg(self.path_target, 2, overlay=True)
        ...
        label_message2.setText(
            self.parent.msg_map.get(self.parent.parent.info_dict.get("currentStage"))
        )
    ...
    def split_img(self, _img, n_page):
        ...
        ncol, _mod = divmod(self.parent.tot_page, self.parent.nsplit)
        assert _mod == 0
        nrow = n_page / ncol
        ...
        # Change picture navigation order according to currentStage
        currentStage = self.parent.stage
        log.info(f"split_img(): current stage = {currentStage}")
    ...
    def make_tab_layout(self):
        ...
        for view, title in zip([self.view, self.view_gm], ["&Target", "&Reference"]):
            self.add_tab(
                self.make_tab(view),
                self.TabWidget,
                title,
