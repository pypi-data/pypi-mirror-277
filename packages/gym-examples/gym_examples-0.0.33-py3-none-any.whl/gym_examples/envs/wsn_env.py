...
inital_energy = 1  # initial energy of each sensor
...
class WSNRoutingEnv(gym.Env):
    def __init__(self, n_sensors=10, coverage_radius=50, max_hops=30):
        ...
        self.observation_space = Tuple(tuple([Box(np.array([[0], [lower_bound, lower_bound]]), np.array([[inital_energy], [upper_bound, upper_bound]]), dtype=np.float64)] * self.n_agents))
    ...
    def reset(self):
        ...
        self.remaining_energy = np.ones(self.n_sensors) * inital_energy
