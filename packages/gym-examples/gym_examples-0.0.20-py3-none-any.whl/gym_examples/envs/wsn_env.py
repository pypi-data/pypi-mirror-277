...
class WSNRoutingEnv(gym.Env):
    def __init__(self, n_sensors=20, coverage_radius=0.5, max_hops=30):
            ...
            for action in actions:
                ...
                if action == self.n_sensors:
                    if self.distance_to_base[i] > self.coverage_radius:
                        ...
                        energy_transmitted = info_amount * (Eelec + Eamp * distance_to_next_hop**2) # energy consumption for transmitting data
                        self.consumption_energy[i] += energy_transmitted 
                        self.remaining_energy[i] -= energy_transmitted
                else:
                    distance_to_action = np.linalg.norm(self.sensor_positions[i] - self.sensor_positions[action])
                    if (self.remaining_energy[action] <= 0) or (distance_to_action > self.coverage_radius):
                        continue
                    else:
                        # Calculate the energy consumption for transmitting data to the next hop
                        distance_to_next_hop = distance_to_action
                        energy_transmitted = info_amount * (Eelec + Eamp * distance_to_next_hop**2) # energy consumption for transmitting data
                        energy_received = info_amount * Eelec  # energy consumption for receiving data
                        self.consumption_energy[i] += energy_transmitted
                        self.consumption_energy[action] += energy_received 
                        self.remaining_energy[i] -= energy_transmitted
                        self.remaining_energy[action] -= energy_received
                        ...
                        is_within_coverage = (distances_to_sensors <= self.coverage_radius) & (distances_to_sensors > 0)
                        ...
                        partial_std_action = (self.remaining_energy[action] - remaining_energy_within_coverage.mean())**2
                        partial_std_action = np.sqrt(partial_std_action/remaining_energy_within_coverage.size)
                        ...
                        reward_remaining_energy = 1 - (partial_std_action / remaining_energy_within_coverage.std())
