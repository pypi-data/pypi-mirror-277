...
initial_energy = 1  # initial energy of each sensor
lower_bound = 0  # lower bound of the sensor positions
upper_bound = 100  # upper bound of the sensor positions
base_station_position = np.array([50, 50])  # position of the base station
...
class WSNRoutingEnv(gym.Env):
    def __init__(self, n_sensors=10, coverage_radius=50, max_hops=30):
        ...
        self.observation_space = Tuple(tuple([Box(np.array(np.stack([np.array([0], dtype=np.float64), np.array([lower_bound, lower_bound], dtype=np.float64)])), np.array(np.stack([np.array([initial_energy], dtype=np.float64), np.array([upper_bound, upper_bound], dtype=np.float64)])), dtype=np.float64)] * len(self.n_agents)))
    ...
    def step(self, actions):
        ...
        for i in range(self.n_sensors):
            ...
            for action in actions:
                ...
                # If a sensor tries to transmit data to itself, ignore this action and continue            
                if (action == i):
                    continue

                self.distance_to_base[i] = np.linalg.norm(self.sensor_positions[i] - base_station_position)
                transmission_energy_to_base = info_amount * (Eelec + Eamp * self.distance_to_base[i]**2) # energy consumption for transmitting data to the base station
                if action == self.n_sensors and (self.remaining_energy[i] < transmission_energy_to_base or self.distance_to_base[i] > self.coverage_radius):
                    ...
                    transmission_energy_to_action = info_amount * (Eelec + Eamp * distance_to_action**2) # energy consumption for transmitting data to the next hop
                    reception_energy = info_amount * Eelec  # energy consumption for receiving data
                    if (self.remaining_energy[action] < reception_energy) or (distance_to_action > self.coverage_radius) or (self.remaining_energy[i] < transmission_energy_to_action):
                        ...
                        variation_remaining_energy_within_coverage_after_action = self.remaining_energy[is_within_coverage].std()
                        total_consumption_energy_within_covergae_after_action = self.consumption_energy[is_within_coverage].sum()

                        # Calculate individual rewards
                        reward_angle = 1 - (np.abs(angle) / np.pi)
                        reward_distance = 1 - (distance_to_action / self.coverage_radius)
                        # reward_remaining_energy = 1 - contrib_action_balance_energy/(balance_energy + epsilon)
                        # reward_consumption_energy = 1 - self.consumption_energy[action]/consumption_energy_within_coverage.sum() 
                        ...
                        if total_consumption_energy_within_coverage_before_action < total_consumption_energy_within_covergae_after_action:
            ...
            if not(np.all(self.sensor_positions[i] >= 0) and np.all(self.sensor_positions[i] <= 1)):
                self.sensor_positions[i] -= displacement

        return self._get_observation(), rewards, dones, {}

    def _get_observation(self):
        return [(e, d) for e, d in zip(self.remaining_energy, self.sensor_positions)]