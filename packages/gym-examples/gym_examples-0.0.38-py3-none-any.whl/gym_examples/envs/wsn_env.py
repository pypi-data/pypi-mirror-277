...
class WSNRoutingEnv(gym.Env):
    def __init__(self, n_sensors=10, coverage_radius=50, max_hops=30):
        ...
        self.observation_space = Tuple([Box(low=np.array([0, lower_bound, lower_bound]), high=np.array([initial_energy, upper_bound, upper_bound]), dtype=np.float64) for _ in range(self.n_agents)])
