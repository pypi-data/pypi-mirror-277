...
class WSNRoutingEnv(gym.Env):
    ...
    def render(self, mode='human'):
        ...
        plt.plot(0.5, 0.5, 'go', markersize=20, label='Base Station')

        # Plot the sensors
        for i in range(self.n_sensors):
            if self.remaining_energy[i] <= 0:
                # Sensors with no remaining energy are marked with a red 'x'
                plt.plot(self.sensor_positions[i, 0], self.sensor_positions[i, 1], 'rx', markersize=10)
            elif i == self.current_sensor:
                # The current sensor is marked with a blue square
                plt.plot(self.sensor_positions[i, 0], self.sensor_positions[i, 1], 'bs', markersize=10)
            else:
                # Other sensors are marked with black dots
                plt.plot(self.sensor_positions[i, 0], self.sensor_positions[i, 1], 'ko', markersize=10)
