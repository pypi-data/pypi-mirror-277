...
def consolidate_linker(
    ...
    for model_r in ann_groups[""].ranges():
        ...
        for model_ann in model_anns:
            ...
            if kb_match:
                if perfect:
                    if labels_are_compatible(model_ann, kb_match, params):
    ...
    if params.resolve_lastnames and params.person_labels:
        for ann in sorted_annotations:
            if ann.labelName in params.person_labels:
...
def is_meta(a: Annotation, params: EscrimReconciliationParameters):
    return a.labelName in params.meta_labels


def is_person(a: Annotation, params: EscrimReconciliationParameters):
    return a.labelName in params.person_labels


def is_geo(a: Annotation, params: EscrimReconciliationParameters):
    return a.labelName in params.geo_labels


def is_equipment(a: Annotation, params: EscrimReconciliationParameters):
    return 'equipement' in a.labelName


def labels_are_compatible(a: Annotation, b: Annotation, params: EscrimReconciliationParameters):
    return a.labelName == b.labelName or a.labelName == params.white_label or (
                is_geo(a, params) and is_geo(b, params)) or (is_equipment(a, params) and is_equipment(b, params))
    ...
    for gname in gnames:
        if (
                ...
                and (a.start in ann_groups[gname]
                     or a.end - 1 in ann_groups[gname])
        ):
