...
from relari.core.utils import validate_metric_data
...
class MetricsClient:
    ...
    def compute(
        self, metric: Metric, args: Union[MetricsArgs, List[MetricsArgs]], **kwargs
    ):
    ...
    def _compute(self, metric: Metric, args: MetricsArgs, **kwargs):
        # Version 1: metric_name, dataset, and sample_id
        if "dataset" in kwargs and isinstance(args, DatasetDatum):
            validate_metric_data(metric, args.data, with_optional=False)
        ...
        # Version 2: metric_name and args
            ...
            validate_metric_data(metric, args, with_optional=True)
            endpoint = "metrics/"
            payload = {
                "metric": metric.value,
                "kwargs": args,
    ...
    def _batch_compute(
        ...
        **kwargs,
        ...
        if "dataset" in kwargs and isinstance(args[0], DatasetDatum):
            [validate_metric_data(metric, x.data, with_optional=False) for x in args]
            ...
            [validate_metric_data(metric, x, with_optional=True) for x in args]
