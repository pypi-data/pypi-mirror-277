...
from ._exceptions import SGLLMError, APIStatusError
...
class SGLLM(SyncAPIClient):
    ...
    with_raw_response: SGLLMWithRawResponse
    with_streaming_response: SGLLMWithStreamedResponse
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous sgllm client instance.

        This automatically infers the following arguments from their corresponding environment variables if they are not provided:
        - `api_key` from `SGLLM_API_KEY`
        - `organization` from `SGLLM_ORG_ID`
        - `project` from `SGLLM_PROJECT_ID`
        """
        if api_key is None:
            api_key = os.environ.get("SGLLM_API_KEY")
        if api_key is None:
            raise SGLLMError(
                "The api_key client option must be set either by passing api_key to the client or by setting the SGLLM_API_KEY environment variable"
            )
        self.api_key = api_key

        if organization is None:
            organization = os.environ.get("SGLLM_ORG_ID")
        self.organization = organization

        if project is None:
            project = os.environ.get("SGLLM_PROJECT_ID")
        self.project = project

        if base_url is None:
            base_url = os.environ.get("SGLLM_BASE_URL")
        if base_url is None:
            base_url = f"https://api.sgllm.com/v1"
        ...
        self.with_raw_response = SGLLMWithRawResponse(self)
        self.with_streaming_response = SGLLMWithStreamedResponse(self)
    ...
    def default_headers(self) -> dict[str, str | Omit]:
        return {
            **super().default_headers,
            ...
            "SGLLM-Organization": self.organization if self.organization is not None else Omit(),
            "SGLLM-Project": self.project if self.project is not None else Omit(),
...
class AsyncSGLLM(AsyncAPIClient):
    ...
    with_raw_response: AsyncSGLLMWithRawResponse
    with_streaming_response: AsyncSGLLMWithStreamedResponse
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async sgllm client instance.
        ...
        self.with_raw_response = AsyncSGLLMWithRawResponse(self)
        self.with_streaming_response = AsyncSGLLMWithStreamedResponse(self)
...
class SGLLMWithRawResponse:
    def __init__(self, client: SGLLM) -> None:
...
class AsyncSGLLMWithRawResponse:
    def __init__(self, client: AsyncSGLLM) -> None:
...
class SGLLMWithStreamedResponse:
    def __init__(self, client: SGLLM) -> None:
...
class AsyncSGLLMWithStreamedResponse:
    def __init__(self, client: AsyncSGLLM) -> None:
