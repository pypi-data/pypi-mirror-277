...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=None, prog="sgllm")
    parser.add_argument(
        "-v",
        "--verbose",
        action="count",
    ...
    parser.add_argument(
        ...
        type=str,
        choices=("sgllm", "azure"),
        help="The backend API to call, must be `sgllm` or `azure`",
    )
    parser.add_argument(
        "--api-version",
        help="The Azure API version, e.g. 'https://learn.microsoft.com/en-us/azure/ai-services/sgllm/reference#rest-api-versioning'",
    )

    # azure
    parser.add_argument(
        "--azure-endpoint",
        help="The Azure endpoint, e.g. 'https://endpoint.sgllm.azure.com'",
    )
    parser.add_argument(
        "--azure-ad-token",
...
def _main() -> None:
    ...
    sgllm.http_client = http_client

    if args.organization:
        sgllm.organization = args.organization

    if args.api_key:
        sgllm.api_key = args.api_key

    if args.api_base:
        sgllm.base_url = args.api_base

    # azure
    if args.api_type is not None:
        sgllm.api_type = args.api_type

    if args.azure_endpoint is not None:
        sgllm.azure_endpoint = args.azure_endpoint

    if args.api_version is not None:
        sgllm.api_version = args.api_version

    if args.azure_ad_token is not None:
        sgllm.azure_ad_token = args.azure_ad_token
