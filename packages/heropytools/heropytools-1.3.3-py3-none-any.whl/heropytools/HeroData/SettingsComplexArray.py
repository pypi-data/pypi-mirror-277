...
class SettingsComplexArray(SettingsValueArray):

    def __init__(self, value,  min_real = float('-inf'), max_real = float('inf'),
            min_imag = float('-inf'), max_imag = float('inf'), min_magnitude = float('-inf'), max_magnitude=float('inf'), allow_nan=True,  
            min_number_of_elements=0, max_number_of_elements=64, read_only=False, visible=True, description="", can_be_input=False, 
            is_input=False, full_input_name=True):
    ...
    def __repr__(self) -> str:
        return f'ComplexList: [value: {self.value}, min_real: {self.min_real}, max_real: {self.max_real}, min_imag: {self.min_imag}, max_imag: {self.max_imag}, min_magnitude: {self.min_magnitude}, max_imag: {self.max_magnitude}, allow_nan: {self.allow_nan}, ' + super().__repr__()


    @staticmethod
    def create_from_dict(data: dict):
        s = SettingsComplexArray(data["_value"], data["_min_real"], data["_max_real"], data["_min_imag"], data["_max_imag"], 
                data["_min_magnitude"], data["_max_magnitude"], data["_allow_nan"], data["_min_number_of_elements"], data["_max_number_of_elements"], 
                data["_read_only"], data["_visible"], data["_description"], data["_can_be_input"],  data["_is_input"], data["_full_input_name"])
        s._id = data["_id"]
        return s

    def __eq__(self, other: SettingsComplexArray):
        return super(SettingsComplexArray, self).__eq__(other) and self._compare_lists(self._value, other._value) and self._min_real == other._min_real and \
                self._max_real == other._max_real and self._min_imag == other._min_imag and self._max_imag == other._max_imag and \
                self._min_magnitude == other._min_magnitude and self._max_magnitude == other._max_magnitude and self._allow_nan == other._allow_nan 


    # --- Autogenerated --- #

    # Name of the type.
    _type_str = "SettingsComplexArray"

    # Datatype attributes.
    _attributes = Attributes("SettingsComplexArray", version=0, member_count=17)

    # Serialization.
    def serialize(self, writer_fun: Callable[[Any, str, str, Optional[CustomSerializer]], None]):
        super(SettingsComplexArray, self).serialize(writer_fun)
    ...
    def deserialize(reader_fun: Callable[[str, Optional[CustomSerializer]], Any]):
        data = super(SettingsComplexArray, SettingsComplexArray).deserialize(reader_fun)
