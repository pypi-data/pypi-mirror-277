...
class SettingsFloat(SettingsValue):

    def __init__(self, value=0, min_value=float('-inf'), max_value = float('inf'), allow_nan = True, read_only=False, visible=True, description="", can_be_input=False, is_input=False, full_input_name=True):
    ...
    def create_from_dict(data: dict):
        s = SettingsFloat(data["_value"], data["_min_value"], data["_max_value"], data["_allow_nan"],data["_read_only"], data["_visible"], data["_description"], data["_can_be_input"], data["_is_input"], data["_full_input_name"])
        s._id = data["_id"]
        return s

    def __eq__(self, other):
        return super(SettingsFloat, self).__eq__(other) and self._value == other._value and self._min_value == other._min_value and self._max_value == other._max_value and self._allow_nan == other._allow_nan

    def __repr__(self) -> str:
        return f'Real: [value: {self.value}, min_value: {self.min_value}, max_value: {self.max_value}, allow_nan: {self.allow_nan}, ' + super().__repr__()
    
    # --- Autogenerated --- #

    # Name of the type.
    _type_str = "SettingsFloat"

    # Datatype attributes.
    _attributes = Attributes("SettingsFloat", version=0, member_count=11)

    # Serialization.
    def serialize(self, writer_fun: Callable[[Any, str, str, Optional[CustomSerializer]], None]):
        super(SettingsFloat, self).serialize(writer_fun)
    ...
    def deserialize(reader_fun: Callable[[str, Optional[CustomSerializer]], Any]):
        data = super(SettingsFloat, SettingsFloat).deserialize(reader_fun)
