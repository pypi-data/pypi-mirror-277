...
class PluginViteConfig(PluginVue, PluginUniapp):
    ...
    def get_item_list(self, key, dedup=True):
        ...
        for data_dek in self.dek_info_list:
            data = data_dek.get(self.vite_config_standard_name)
            if data:
                items = data.get(key)
                if items:
                    result.extend(items)
        return list_dedup(result) if dedup else result
    ...
    all_items = [
        ['list', 'plugins', 'addPlugin', True],
        ['list', 'scssHeader', 'addScssHeader', False],
        ['dict', 'alias', 'addAlias', True],
        ['dict', 'buildInputs', 'addBuildInputs', True],
        ['dict', 'define', 'addDefine', True],
    ]
    ...
    def run(self):
        ...
        for (typed, name, method, raw) in self.all_items:
            if typed == 'list':
        ...
        ss = ''.join([
            s[:index], '\n',
            '\n'.join([x for x in self.get_item_list('imports') if x not in s]), '\n',
            '\n'.join(str_list), '\n',
            s[index:]
        ])
