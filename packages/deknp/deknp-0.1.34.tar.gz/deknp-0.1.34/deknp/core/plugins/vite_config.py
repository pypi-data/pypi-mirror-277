...
class PluginViteConfig(PluginVue, PluginUniapp):
    ...
    @property
    def all_vite_buildinputs(self):
        result = {}

        for data_dek in self.dek_info_list:
            items = (data_dek.get(self.vite_config_standard_name) or {}).get('buildInputs') or {}
            result.update(items)

        return result

    @property
    def all_vite_define(self):
        result = {}

        for data_dek in self.dek_info_list:
            items = (data_dek.get(self.vite_config_standard_name) or {}).get('define') or {}
            result.update(items)
    ...
    def run(self):
        ...
        for key in sorted(self.all_vite_buildinputs):
            value = self.all_vite_buildinputs[key]
            mk = f'marker--addBuildInputs--{self.get_data_uid([key, value])}'
            if mk not in s:
                str_list.append(f'dekConfig.addBuildInputs({json.dumps(mk)}, {json.dumps(key)}, {value})')

        for key in sorted(self.all_vite_define):
            value = self.all_vite_define[key]
            mk = f'marker--addDefine--{self.get_data_uid([key, value])}'
            if mk not in s:
                str_list.append(f'dekConfig.addDefine({json.dumps(mk)}, {json.dumps(key)}, {value})')
