...
from titanq._model.expression_parser import extract_bias_vector_and_weight_matrix_from_expression
...
from .variable import BinaryVariableVector, BipolarVariableVector, ContinuousVariableVector, IntegerVariableVector, VariableVector, Vtype, Expression
...
class Model:
    ...
    def get_weight_matrix_from_objective(self) -> np.ndarray:
        """
        Retrieve the weight matrix from the model's objective.

        Return
        ------
        weights
            The quadratic objective matrix as a NumPy 2-D dense ndarray.
        """
        return self._objective._weights
    
    def get_bias_vector_from_objective(self) -> np.ndarray:
        """
        Retrieve the bias vector from the model's objective.

        Return
        ------
        bias
            The linear objective vector as a NumPy 1-D ndarray.
        """
        return self._objective._bias
    ...
    def set_objective_expression(self, expr: Expression, target=Target.MINIMIZE):
        ...
        **This feature is experimental**
        ...
        target
            The target for the optimization, either `Target.MINIMIZE` or `Target.MAXIMIZE`. 
            The default is `Target.MINIMIZE`.

        Examples
        --------
        >>> from titanq import Model, Vtype
        >>> x_variables = model.add_variable_vector('x', 2, Vtype.BINARY)
        >>> y = model.add_variable_vector('y', 1, Vtype.BINARY)
        >>> expr = np.array([3, 4])*x_variables + np.array([[2], [0]])*(x_variables*y) - 5*y
        >>> model.set_objective_expression(expr) 
        """
        weight_matrix, bias_vector = extract_bias_vector_and_weight_matrix_from_expression(expr, self._variables)
        self.set_objective_matrices(weight_matrix, bias_vector, target)
