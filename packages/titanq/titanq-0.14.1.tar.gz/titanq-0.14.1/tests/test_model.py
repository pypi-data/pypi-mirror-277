...
def test_set_objective_expression(model_s3_client):
        # Create a sample expression
        x_variables = model_s3_client.add_variable_vector('x', 2, Vtype.BINARY)
        y = model_s3_client.add_variable_vector('y', 1, Vtype.BINARY)
        expr = np.array([3, 4])*x_variables + np.array([[2], [0]])*(x_variables*y) - 5*y + 7*x_variables + 8*y + 10

        # Expected bias vector and weight matrix
        expected_bias_vector = np.array([10.0, 11.0, 3.0], dtype=np.float32)
        expected_weight_matrix = np.zeros((3, 3), dtype=np.float32)
        expected_weight_matrix[0, 2] = 2
        expected_weight_matrix[2, 0] = 2

        # Call the method
        model_s3_client.set_objective_expression(expr)

        # Check the internal state
        np.testing.assert_array_almost_equal(model_s3_client.get_weight_matrix_from_objective(), expected_weight_matrix)
        np.testing.assert_array_almost_equal(model_s3_client.get_bias_vector_from_objective(), expected_bias_vector)
