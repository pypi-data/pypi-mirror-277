...
class BaseDataConnection(ABC):
    ...
    def _download_json_file(self, path) -> dict:
        ...
        if self.type == DataConnectionTypes.S3:
            warn(
                message="S3 DataConnection is deprecated! Please use data_asset_id instead."
            )
    ...
    def _check_if_connection_asset_is_s3(self) -> bool:
        try:
            ...
            if (
                ...
                and getattr(self.connection, "cos_type", False)
            ):
                return True
                ...
                if self._api_client is not None:
                    ...
                    if "connection_id" not in attachment_details:
                        if (
                            ...
                            connection_details = (
                                self._create_conn_details_for_container()
                            )
                            attachment_details["connection_path"] = (
                                "/"
                                + attachment_details["handle"]["bucket"]
                                + "/"
                                + attachment_details["handle"]["key"]
                            )
                        elif (
                            "url" in attachment_details
                            and "name" in attachment_details
                            and "bucket" in attachment_details
                        ):
                            connection_details = (
                                self._create_conn_details_for_container()
                            )
                            attachment_details["connection_path"] = (
                                "/"
                                + attachment_details["bucket"]["bucket_name"]
                                + "/"
                    ...
                    try:
                        ...
                        try:
                            ...
                            RuntimeError
                        ):  # when data asset is normal not s3 or database
                            return False
            ...
            if self._api_client is not None:
                ...
                if (
                    datasource_type == datasource_type_id_ibm_cos
                    or datasource_type == datasource_type_id_aws_cos
                    or datasource_type == "bluemixcloudobjectstorage"
                    or datasource_type == "cloudobjectstorage"
                ):
                    cos_type = True
            ...
            if cos_type:
                if "cos_hmac_keys" in str(connection_details["entity"]["properties"]):
                    creds = json.loads(
                        connection_details["entity"]["properties"]["credentials"]
                    )
                    connection_details["entity"]["properties"]["access_key"] = creds[
                        "cos_hmac_keys"
                    ]["access_key_id"]
                    connection_details["entity"]["properties"]["secret_key"] = creds[
                        "cos_hmac_keys"
                    ]["secret_access_key"]
                ...
                if self.connection is None:
                    ...
                    if "api_key" in str(
                        connection_details["entity"]["properties"]
                    ) and "iam_url" in str(connection_details["entity"]["properties"]):
                        self.connection = S3Connection(
                            api_key=connection_details["entity"]["properties"].get(
                                "api_key"
                            ),
                            auth_endpoint=connection_details["entity"][
                                "properties"
                            ].get("iam_url"),
                            endpoint_url=connection_details["entity"]["properties"].get(
                                "url"
                            ),
                            resource_instance_id=connection_details["entity"][
                                "properties"
                        ...
                        self.connection = S3Connection(
                            ...
                            access_key_id=connection_details["entity"][
                                "properties"
                            ].get("access_key"),
                            secret_access_key=connection_details["entity"][
                                "properties"
                            ].get("secret_key"),
                            _internal_use=True,
                        )
                    ...
                    if "api_key" in str(
                        ...
                        self.connection.api_key = connection_details["entity"][
                            "properties"
                        ].get("api_key")
                        self.connection.auth_endpoint = connection_details["entity"][
                            "properties"
                        ].get("iam_url")
                        self.connection.endpoint_url = connection_details["entity"][
                            "properties"
                        ].get("url")
                        self.connection.resource_instance_id = connection_details[
                            "entity"
                        ]["properties"].get("resource_instance_id")

                    else:
                        self.connection.secret_access_key = connection_details[
                            "entity"
                        ]["properties"].get("secret_key")
                        self.connection.access_key_id = connection_details["entity"][
                            "properties"
                        ].get("access_key")
                        self.connection.endpoint_url = connection_details["entity"][
                            "properties"
                ...
                if self.type == "container":
                    self.location.bucket = connection_details["entity"]["properties"][
                        "bucket_name"
                    ]
                ...
                if self.type == "data_asset":
                    try:
                        ...
                        self.location.bucket = attachment_details[
                            "connection_path"
                        ].split("/")[1]

                        # Remove excel sheet name from connection_path if data asset is excel file:
                        if (
                            ".xlsx/" in attachment_details["connection_path"]
                            or ".xls/" in attachment_details["connection_path"]
                        ):
                            excel_sheet = attachment_details["connection_path"].split(
                                "/"
                            )[-1]
                            ...
                            self.location.path = "/".join(
                                attachment_details["connection_path"].split("/")[2:-1]
                            )
                        else:
                            self.location.path = "/".join(
                                attachment_details["connection_path"].split("/")[2:]
                            )

                    except IndexError:
                        self.location.bucket = connection_details["entity"][
                            "properties"
                        ]["bucket"]
            ...
            if self.type == "data_asset":
                ...
                if self._api_client is not None:
                    ...
                    try:
                        ...
                        #       [Prevents unnecessary logging].
                        return any(
                            file["asset_id"] == _id for file in project.get_files()
                        )
                        # --- end note
        ...
        if self._api_client is not None:
            ...
            if "handle" in asset_details["attachments"][0]:
                ...
                return self._download_data_from_nfs_connection_using_id_and_path(
                    connection_id, connection_path, binary
                )
    ...
    def _download_data_from_nfs_connection_using_id_and_path(
        self, connection_id, connection_path, binary: bool = False
    ) -> DataFrame | bytes:
        """Download training data for this connection. NFS."""
    ...
    def _create_conn_details_for_container(self) -> dict:
        if self._api_client is not None:
            ...
            creds = (
                creds
                if creds
                else details["entity"]["storage"]["properties"]["credentials"].get(
                    "editor"
                )
            )
            ...
            try:
                ...
                _project = Project(
                    project_id=os.environ.get("PROJECT_ID"), project_access_token=token
                )
