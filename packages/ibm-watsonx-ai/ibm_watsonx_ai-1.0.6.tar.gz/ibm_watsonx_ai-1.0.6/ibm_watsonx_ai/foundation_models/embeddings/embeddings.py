...
class Embeddings(BaseEmbeddings, WMLResource):
    ...
    **Example**
        ...
        if isinstance(inputs, list) and len(inputs) > _MAX_INPUTS_LENGTH:
            ...
            if len(inputs_splited) <= concurrency_limit:
                with ThreadPoolExecutor() as executor:
                    response_batch = list(
                        executor.map(
                            self.generate, inputs_splited, [params] * len(inputs)
                        )
                    )
                generated_responses.extend(response_batch)
            else:
                for i in range(0, len(inputs_splited), concurrency_limit):
                    inputs_batch = inputs_splited[i : i + concurrency_limit]
                    with ThreadPoolExecutor() as executor:
                        response_batch = list(
                            executor.map(
                                self.generate,
    ...
    def _generate(
        self, generate_url: str, inputs: list[str], params: ParamsType | None = None
    ) -> dict:
