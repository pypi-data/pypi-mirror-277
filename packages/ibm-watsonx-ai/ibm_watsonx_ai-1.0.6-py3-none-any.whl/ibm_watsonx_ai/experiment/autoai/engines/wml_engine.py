...
class WMLEngine(BaseEngine):
    ...
    def __init__(self, workspace: "WorkSpace") -> None:
        ...
        warnings.warn(
            (
                "`WMLEngine` is deprecated and will be removed in future. "
                "Instead, please use `ServiceEngine`."
            ),
            category=DeprecationWarning,
        )
        # --- end note
        self.workspace = workspace
        self._wml_client = workspace.api_client
    ...
    def _initialize_wml_pipeline_metadata(self, **kwargs) -> None:
        ...
        self._wml_pipeline_metadata = {
            self._wml_client.pipelines.ConfigurationMetaNames.NAME: self._auto_pipelines_parameters.get(
                "name", "Default name."
            ),
            self._wml_client.pipelines.ConfigurationMetaNames.DESCRIPTION: self._auto_pipelines_parameters.get(
                        ...
                        "nodes": [
                            {
                                ...
                                "parameters": {
                                    "stage_flag": True,
                                    "output_logs": True,
                                    "input_file_separator": self._auto_pipelines_parameters.get(
                                        "csv_separator"
                                    ),
                                    "encoding": self._auto_pipelines_parameters.get(
                                        "encoding"
                                    ),
                                    "drop_duplicates": self._auto_pipelines_parameters.get(
                                        "drop_duplicates"
                                    ),
                                    "outliers_columns": self._auto_pipelines_parameters.get(
                                        "outliers_columns"
                                    ),
                                    "incremental_learning": self._auto_pipelines_parameters.get(
                                        "incremental_learning"
                                    ),
                                    "enable_early_stop": self._auto_pipelines_parameters.get(
                                        "early_stop_enabled"
                                    ),
                                    "early_stop_window_size": self._auto_pipelines_parameters.get(
                                        "early_stop_window_size"
                                    ),
                                    "optimization": {
                                        "learning_type": self._get_learning_type(),
                                        "cognito_transform_names": self._auto_pipelines_parameters.get(
                ...
                "runtimes": [
                    {
                        ...
                        "app_data": {"wml_data": {"hardware_spec": {}}},
                    }
                ],
        ...
        if kwargs.get("use_flight") is not None:
            self._wml_pipeline_metadata[
                self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT
            ]["pipelines"][0]["nodes"][0]["parameters"]["use_flight"] = kwargs[
                "use_flight"
            ]
        ...
        if (
            (self._wml_client.CLOUD_PLATFORM_SPACES or self._wml_client.CPD_version)
            and pred_type != PredictionType.FORECASTING
            and pred_type != PredictionType.TIMESERIES_ANOMALY_PREDICTION
        ):
            ...
            if (
                ...
                or self._wml_client.CPD_version >= 4.5
            ):
        ...
        if (
            ...
            if self._auto_pipelines_parameters["pipeline_types"]:
                ...
                self._wml_pipeline_metadata[
                    ...
                    pipeline.value
                    for pipeline in self._auto_pipelines_parameters["pipeline_types"]
                ]
            if (
        ...
        if self._20_class_limit_removal_test:
            self._wml_pipeline_metadata[
                self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT
            ]["pipelines"][0]["nodes"][0]["parameters"].update(
        ...
        if 0 < len(t_size) <= 2:
            self._wml_pipeline_metadata[
                self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT
            ]["runtimes"][0]["app_data"]["wml_data"]["hardware_spec"][
                "name"
            ] = t_size.upper()
            ...
            self._wml_pipeline_metadata[
                self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT
            ]["runtimes"][0]["app_data"]["wml_data"]["hardware_spec"]["id"] = t_size
        # --- end note
        ...
        if self._auto_pipelines_parameters.get("include_only_estimators") is not None:
            ...
            try:
                self._wml_pipeline_metadata[
                    ...
                    algorithm.value
                    for algorithm in self._auto_pipelines_parameters.get(
        ...
        if (
            self._auto_pipelines_parameters.get("include_batched_ensemble_estimators")
            is not None
        ):
            # note: transform values from Enum
            try:
        ...
        if (
            self._auto_pipelines_parameters.get("positive_label") is not None
            and self._auto_pipelines_parameters.get("prediction_type")
        ...
        if (
            ...
            and self._auto_pipelines_parameters.get("excel_sheet") != 0
        ):
            self._wml_pipeline_metadata[
                self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT
            ]["pipelines"][0]["nodes"][0]["parameters"][
        ...
        if (
            ...
            and self._auto_pipelines_parameters.get("test_data_excel_sheet") != 0
        ):
            self._wml_pipeline_metadata[
                self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT
            ]["pipelines"][0]["nodes"][0]["parameters"][
        ...
        if (
            ...
            if self._auto_pipelines_parameters.get("numerical_imputation_strategy"):
                self._wml_pipeline_metadata[
                    self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT
                ]["pipelines"][0]["nodes"][0]["parameters"]["optimization"].update(
                    get_values_for_imputation_strategy(
                        self._auto_pipelines_parameters.get(
                ...
                if (
                    ...
                    if self._auto_pipelines_parameters.get(
                        ...
                        is list
                        and ImputationStrategy.VALUE
                        in self._auto_pipelines_parameters.get(
                            "numerical_imputation_strategy"
                        )
                    ):
                        self._wml_pipeline_metadata[
            ...
            if self._auto_pipelines_parameters.get(
                "numerical_imputation_value"
            ) is not None or self._auto_pipelines_parameters.get(
                "imputation_threshold"
            ):
        ...
        if (
            not self._wml_client.ICP_PLATFORM_SPACES
            and self._auto_pipelines_parameters["autoai_pod_version"] is not None
        ):
            self._wml_pipeline_metadata[
                self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT
            ]["runtimes"][0]["version"] = self._auto_pipelines_parameters[
                "autoai_pod_version"
            ]
        # --- end note

    def _initialize_wml_training_metadata(
        self,
        ...
        self._wml_training_metadata = {
            self._wml_client.training.ConfigurationMetaNames.TAGS: ["autoai"],
            ...
            self._wml_client.training.ConfigurationMetaNames.TRAINING_RESULTS_REFERENCE: training_results_reference._to_dict(),
            self._wml_client.training.ConfigurationMetaNames.PIPELINE: {
                "id": self._wml_stored_pipeline_details["metadata"]["id"]
            },
            self._wml_client.training.ConfigurationMetaNames.NAME: f"{self._auto_pipelines_parameters.get('name', 'Default name.')[:40]} - "
            f"wml pipeline: {self._wml_stored_pipeline_details['metadata']['id']}",
            self._wml_client.training.ConfigurationMetaNames.DESCRIPTION: f"{self._auto_pipelines_parameters.get('desc', 'Default description')[:40]} - "
            f"wml pipeline: {self._wml_stored_pipeline_details['metadata']['id']}",
        }
        ...
        for connection in self._wml_training_metadata[
            self._wml_client.training.ConfigurationMetaNames.TRAINING_DATA_REFERENCES
        ] + [
            self._wml_training_metadata[
    ...
    def initiate_remote_resources(self, params: dict, **kwargs) -> None:
        ...
        self._wml_stored_pipeline_details = self._wml_client.pipelines.store(
            meta_props=self._wml_pipeline_metadata, **kwargs
        )
    ...
    def fit(
        ...
        """Run a training process on WML of autoai on top of the training data referenced by training_data_reference.
        ...
        :rtype: dict
        """
        self._initialize_wml_training_metadata(
            training_data_reference=training_data_reference,
            training_results_reference=training_results_reference,
            test_data_references=test_data_references,
        )

        run_params = self._wml_client.training.run(
            meta_props=self._wml_training_metadata, asynchronous=True
        )

        self._current_run_id = run_params["metadata"].get(
            "id", run_params["metadata"].get("guid")
        )

        if background_mode:
            return self._wml_client.training.get_details(
            ...
            while not end:
                ...
                websocket.add_header(
                    ...
                    bytes(
                        "Bearer " + self._wml_client.service_instance._get_token(),
                        "utf-8",
                    ),
                )
                    ...
                    for event in websocket.connect():
                        ...
                        if event.name == "text":
                            ...
                            if status.get("state") == RunStateTypes.FAILED:
                                ...
                                if "Not Found" in str(message) or "HeadObject" in str(
                                    message
                                ):
                                    ...
                                    raise AutoAIComputeError(
                                        self._current_run_id, reason=f"Error: {message}"
                                    )
                                ...
                                if self._current_run_id in text and "completed" in text:
                                    ...
                                    if len(prepared_text) > max_len:
                                        progress_bar.set_description(
                                            desc=prepared_text[: (max_len - 3)] + "..."
                                        )
                                    else:
                                        progress_bar.set_description(
                                            desc=prepared_text
                                            + " " * (max_len - len(prepared_text))
                                        )

                                progress_bar.increment_counter(
                                    gen.get_progress(process)
                                )
                                progress_bar.update()
                            elif process:
                                progress_bar.increment_counter(
    ...
    def get_run_status(self) -> str:
        ...
        if self._current_run_id is None:
            raise FitNeeded(
                reason="Firstly schedule a fit job by using the fit() method."
            )

        return self._wml_client.training.get_status(
            training_id=self._current_run_id
        ).get("state")
    ...
    def get_run_details(self, include_metrics: bool = False) -> dict:
        ...
        details = self._wml_client.training.get_details(
            training_id=self._current_run_id, _internal=True
        )
    ...
    def cancel_run(self) -> None:
        ...
        if self._current_run_id is None:
            raise FitNeeded(
                reason="To cancel an AutoAI run, first schedule a fit job by using the fit() method."
            )
    ...
    def summary(
        ...
        if self._current_run_id is None:
            raise FitNeeded(
                reason="To list computed pipelines, first schedule a fit job by using the fit() method."
            )
        ...
        summary = create_summary(
            details=details,
            scoring=scoring,
            sort_by_holdout_score=sort_by_holdout_score,
        )
        ...
        return summary.rename(
            {
                f"training_{optimized_scoring}": f"training_{optimized_scoring}_(optimized)"
            },
            axis="columns",
        )
        ...
        for pipeline in run_params["entity"]["status"].get("metrics", [])[::-1]:
            if (
                ...
                if "fairness" in pipeline["context"]:
                    pipeline_parameters["fairness_details"] = pipeline["context"].get(
                        "fairness"
                    )
                ...
                if "timeseries" not in pipeline["context"]:
                    ...
                    if (
                        self._auto_pipelines_parameters["prediction_type"]
                        == PredictionType.TIMESERIES_ANOMALY_PREDICTION
                    ):
                        for pipeline_metrics in run_params["entity"]["status"].get(
                            "metrics", []
                        )[::-1]:
                            pipeline_phase = pipeline_metrics["context"].get("phase")
                            if (
                                pipeline_metrics["context"]["intermediate_model"][
                                    "name"
                                ].split("P")[-1]
                                == pipeline_name.split("_")[-1]
                                and pipeline_phase == "after_holdout_execution"
                            ):
                                pipeline_parameters["tsad_metrics"] = self._get_metrics(
                                    pipeline_metrics
                                )
                                break
                    else:
                ...
                if "incremental_training" in pipeline["context"]:
                    pipeline_parameters["learning_curve"] = self._get_learning_curve(
                        pipeline, run_params
                    )

                if self._auto_pipelines_parameters["prediction_type"] == "binary" or (
                    self._auto_pipelines_parameters["prediction_type"]
                    == "classification"
                    ...
                    pipeline_parameters["confusion_matrix"] = (
                        self._get_confusion_matrix(pipeline, run_params)
                    )
                    pipeline_parameters["roc_curve"] = self._get_roc_curve(
                        pipeline, run_params
                    )

                elif self._auto_pipelines_parameters[
                    "prediction_type"
                ] == "multiclass" or (
                    self._auto_pipelines_parameters["prediction_type"]
                    == "classification"
                    ...
                    one_vs_all_data = self._get_data_from_property_or_file(
                        ...
                        pipeline["context"].get(
                            "multi_class_classification", {"one_vs_all": []}
                        ),
                    ...
                    pipeline_parameters["roc_curve"] = self._get_one_vs_all(
                        one_vs_all_data, run_params, "roc_curve"
                    )
                    pipeline_parameters["confusion_matrix"] = self._get_one_vs_all(
                        one_vs_all_data, run_params, "confusion_matrix"
                    )
                    ...
                    self._auto_pipelines_parameters["prediction_type"] == "forecasting"
                    or self._auto_pipelines_parameters["prediction_type"]
                    == PredictionType.TIMESERIES_ANOMALY_PREDICTION
                ):
                    phases_to_combine_visualization = [
                        "after_backtest_execution",
                        "after_holdout_execution",
                    ]
                    for pipeline in run_params["entity"]["status"].get("metrics", [])[
                        ...
                        # end note
                        if (
                            pipeline["context"]["intermediate_model"]["name"].split(
                                "P"
                            )[-1]
                            == pipeline_name.split("_")[-1]
                            and pipeline_phase in phases_to_combine_visualization
                        ):
                            phases_to_combine_visualization.remove(pipeline_phase)
                            data = self._get_data_from_property_or_file(
                                "daub_status",
                                pipeline["context"]["timeseries"],
                                run_params["entity"]["results_reference"],
                                [{}, {}],
                            )
                            if "visualization" in data:
                                ...
                                pipeline_parameters["visualization"].update(
                                    data["visualization"]
                                )

                else:
                    raise InvalidPredictionType(
                        self._auto_pipelines_parameters["prediction_type"],
    ...
    def _get_data_from_property_or_file(
        self,
        property_name: str,
        details: Dict,
        ...
        if f"{property_name}_location" in details:
            ...
            conn._api_client = self._wml_client
    ...
    def _get_learning_curve(
        self, pipeline_details: Dict, run_params: Dict
    ) -> "DataFrame":
    ...
    def _get_features_importance(pipeline_details: Dict) -> "DataFrame":
        ...
        features_importance_data = DataFrame(
            data=values, columns=columns, index=["features_importance"]
        )
        features_importance_data = features_importance_data.transpose()
        features_importance_data.sort_values(
            by="features_importance", ascending=False, inplace=True
        )
    ...
    def _get_roc_curve(self, pipeline_details: Dict, run_params: Dict) -> "DataFrame":
        ...
        index = MultiIndex.from_product(
            [["fpr", "thresholds", "tpr"], true_classes],
    ...
    def _get_one_vs_all(
        self, one_vs_all_data: List, run_params: Dict, option: str
    ) -> "DataFrame":
        ...
        if option == "confusion_matrix":
            confusion_matrix_data = [
                self._get_data_from_property_or_file(
                    "confusion_matrix",
                    item,
                    run_params["entity"]["results_reference"],
                    [{}, {}],
                )
                for item in one_vs_all_data
            ]

            columns = ["fn", "fp", "tn", "tp", "true_class"]
            confusion_matrix_values = [
                [item[c] for c in columns] for item in confusion_matrix_data
            ]
            confusion_matrix_data = DataFrame(
                data=confusion_matrix_values, columns=columns
            )
            confusion_matrix_data.index = confusion_matrix_data["true_class"]
            confusion_matrix_data.drop(["true_class"], axis=1, inplace=True)
            ...
            roc_curve_data = [
                self._get_data_from_property_or_file(
                    "roc_curve", item, run_params["entity"]["results_reference"], []
                )
                for item in one_vs_all_data
            ]
            ...
            values = (
                [item["fpr"] for item in roc_curve_data]
                + [item["thresholds"] for item in roc_curve_data]
                + [item["tpr"] for item in roc_curve_data]
            )
            index = MultiIndex.from_product(
                [["fpr", "thresholds", "tpr"], true_classes],
                names=["measurement types", "true_class"],
            ...
            raise ValueError(
                f'Wrong option for "one_vs_all": {option}. '
