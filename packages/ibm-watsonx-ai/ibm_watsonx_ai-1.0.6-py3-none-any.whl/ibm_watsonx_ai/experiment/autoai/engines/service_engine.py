...
class ServiceEngine(BaseEngine):
    ...
    def _initialize_pipeline_metadata(self, **kwargs) -> None:
        ...
        self._pipeline_metadata = {
            self._api_client.pipelines.ConfigurationMetaNames.NAME: self._auto_pipelines_parameters.get(
                "name", "Default name."
            ),
            self._api_client.pipelines.ConfigurationMetaNames.DESCRIPTION: self._auto_pipelines_parameters.get(
        ...
        if (
            (self._api_client.CLOUD_PLATFORM_SPACES or self._api_client.CPD_version)
            ...
            if (
                self._api_client.CLOUD_PLATFORM_SPACES
                or self._api_client.CPD_version >= 4.5
            ):
            ...
            if (
                self._api_client.CLOUD_PLATFORM_SPACES
                or self._api_client.CPD_version >= 4.8
            ):
        ...
        if (
            not self._api_client.ICP_PLATFORM_SPACES
    ...
    def _initialize_training_metadata(
        ...
        self._training_metadata = {
            self._api_client.training.ConfigurationMetaNames.TAGS: ["autoai"],
            ...
            self._api_client.training.ConfigurationMetaNames.TRAINING_RESULTS_REFERENCE: training_results_reference._to_dict(),
            self._api_client.training.ConfigurationMetaNames.PIPELINE: {
                "id": self._stored_pipeline_details["metadata"]["id"]
            },
            self._api_client.training.ConfigurationMetaNames.NAME: f"{self._auto_pipelines_parameters.get('name', 'Default name.')[:40]} - "
            f"pipeline: {self._stored_pipeline_details['metadata']['id']}",
            self._api_client.training.ConfigurationMetaNames.DESCRIPTION: f"{self._auto_pipelines_parameters.get('desc', 'Default description')[:40]} - "
            f"pipeline: {self._stored_pipeline_details['metadata']['id']}",
        }
        ...
        for connection in self._training_metadata[
            self._api_client.training.ConfigurationMetaNames.TRAINING_DATA_REFERENCES
        ] + [
            self._training_metadata[
    ...
    def initiate_remote_resources(self, params: dict, **kwargs) -> None:
        ...
        self._stored_pipeline_details = self._api_client.pipelines.store(
            meta_props=self._pipeline_metadata, **kwargs
        )
    ...
    def fit(
        ...
        run_params = self._api_client.training.run(
            meta_props=self._training_metadata, asynchronous=True
        )
        ...
        if background_mode:
            return self._api_client.training.get_details(
            ...
            while not end:
                ...
                websocket.add_header(
                    ...
                    bytes(
                        "Bearer " + self._api_client.service_instance._get_token(),
                        "utf-8",
                    ),
                )
    ...
    def get_run_status(self) -> str:
        ...
        return self._api_client.training.get_status(
    ...
    def get_run_details(self, include_metrics: bool = False) -> dict:
        ...
        details = self._api_client.training.get_details(
            training_id=self._current_run_id, _internal=True
        )
    ...
    def get_pipeline_details(self, pipeline_name: str = None) -> dict:
        ...
        run_params = self._api_client.training.get_details(
            training_id=self._current_run_id, _internal=True
        )
