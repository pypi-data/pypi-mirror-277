        ...
        if self.credentials.instance_id is None:
            ...
            if (
                ...
                response_get_instance = requests.get(
                    "{}/ml/v4/instances/{}".format(
                        self.credentials.url, self.credentials.instance_id
                    ),
                ...
                if response_get_instance.status_code == 200:
                    if "plan" in response_get_instance.json()["entity"]:
                        if (
                            response_get_instance.json()["entity"]["plan"]["version"]
                            == 2
                            and response_get_instance.json()["entity"]["plan"]["name"]
                            != "lite"
                        ):
                            self.CLOUD_PLATFORM_SPACES = True
                            ...
                            if not self._is_IAM():
                                raise WMLClientError(
                                    Messages.get_message(
                                        message_id="apikey_not_provided"
                                    )
                                )
                            ...
                            if self._internal:
                                ...
                                if (
                                    ...
                                    in self.PLATFORM_URLS_MAP.keys()
                                ):
                                    self.PLATFORM_URL = self.PLATFORM_URLS_MAP[
                                        self.credentials.url
                                    ]
                                else:
                                    raise WMLClientError(
                                        Messages.get_message(
                                            message_id="url_not_provided"
                                        )
                                    )
                            ...
                            if self.CLOUD_PLATFORM_SPACES:
                                print(
                                    Messages.get_message(
                                        message_id="instance_id_is_not_required_for_v2_plans"
                                    )
                                )

                elif response_get_instance.status_code == 404:
                    raise WMLClientError(
                        Messages.get_message(message_id="instance_id_not_found")
                    )
                elif response_get_instance.status_code == 401:
                    raise WMLClientError(
                        Messages.get_message(
                            message_id="not_authorized_to_access_the_instance_id"
                        )
                    )
        ...
        if zen:
            ...
            if self.proceed is True:
                token_to_use = (
                    ...
                    if self.credentials.projects_token is not None
