...
p2_delegated_puzzle_or_hidden_puzzle is essentially the "standard coin" in chik.
...
- The user's wallet contains a primary sk/pk pair which are used to derive to one
  ...
  https://chiklisp.com/docs/standard_transaction. Therefore, when a wallet needs
...
- Python code previously appeared which was written like:
  ...
  In context, delegated_puzzle_solution here is any *chiklisp program*, here one
  ...
  The second and third arguments are a chiklisp program and its corresponding
...
MOD = load_klvm_maybe_recompile("p2_delegated_puzzle_or_hidden_puzzle.clsp")
