...
from chik.protocols.wallet_protocol import CoinState, RequestBlockHeader, RespondBlockHeader
...
from chik.wallet.cat_wallet.cat_utils import CAT_MOD, SpendableCAT, construct_cat_puzzle
from chik.wallet.cat_wallet.cat_utils import get_innerpuzzle_from_puzzle as get_innerpuzzle_from_cat_puzzle
from chik.wallet.cat_wallet.cat_utils import unsigned_spend_bundle_for_spendable_cats
...
from chik.wallet.conditions import AssertCoinAnnouncement, Condition, parse_timelock_info
from chik.wallet.dao_wallet.dao_info import DAOInfo, DAORules, ProposalInfo, ProposalType
...
from chik.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, TXConfig
...
class DAOWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSChikConnection, coin_data: Optional[Any]) -> None:
        """
    ...
    async def resync_treasury_state(self) -> None:
        ...
        wallet_node: Any = self.wallet_state_manager.wallet_node
        peer: WSChikConnection = wallet_node.get_full_node_peer()
        if peer is None:  # pragma: no cover
            raise ValueError("Could not find any peers to request puzzle and solution from")
    ...
    async def generate_proposal_vote_spend(
        ...
        if fee > 0:
            ...
            spend_bundle = unsigned_spend_bundle.aggregate([unsigned_spend_bundle, dao_cat_spend, chik_tx.spend_bundle])
        else:
            ...
            if proposal_type == ProposalType.SPEND:
                ...
                if sum > 0:
                    ...
                    for xck_coin in xck_coins:
                        ...
                        coin_spends.append(make_spend(xck_coin, p2_singleton_puzzle, solution))
