...
from chik.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle
from chik.wallet.dao_wallet.dao_info import DAORules, ProposalType
...
DAO_CAT_LAUNCHER: Program = load_klvm("dao_cat_launcher.clsp")
P2_SINGLETON_MOD: Program = load_klvm("p2_singleton_via_delegated_puzzle.clsp")
P2_SINGLETON_MOD_HASH: bytes32 = P2_SINGLETON_MOD.get_tree_hash()
DAO_UPDATE_PROPOSAL_MOD: Program = load_klvm("dao_update_proposal.clsp")
DAO_UPDATE_PROPOSAL_MOD_HASH: bytes32 = DAO_UPDATE_PROPOSAL_MOD.get_tree_hash()
DAO_CAT_EVE: Program = load_klvm("dao_cat_eve.clsp")
P2_SINGLETON_AGGREGATOR_MOD: Program = load_klvm("p2_singleton_aggregator.clsp")
...
def get_spend_p2_singleton_puzzle(treasury_id: bytes32, xck_conditions: Program, asset_conditions: Program) -> Program:
    # TODO: typecheck get_spend_p2_singleton_puzzle arguments
    # TODO: add tests for get_spend_p2_singleton_puzzle: pass xck_conditions as Puzzle, List and ConditionWithArgs
    #
...
def generate_simple_proposal_innerpuz(
    ...
    for recipient_puzhash, amount, asset_type in zip(recipient_puzhashes, amounts, asset_types):
        if asset_type:
            ...
            xck_conds.append([51, recipient_puzhash, amount])
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(xck_conds), Program.to(cat_conds))
    return puzzle
...
async def generate_mint_proposal_innerpuz(
    ...
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(xck_conditions), Program.to([]))
    return puzzle
