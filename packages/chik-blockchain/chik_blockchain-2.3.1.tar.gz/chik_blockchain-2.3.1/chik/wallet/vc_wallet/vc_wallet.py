...
if TYPE_CHECKING:
    from chik.wallet.wallet_state_manager import WalletStateManager  # pragma: no cover
...
class VCWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSChikConnection, coin_data: Optional[Streamable]
    ) -> None:
        """
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if fee > 0:
            ...
            chik_tx = await self.wallet_state_manager.main_wallet.create_tandem_xck_tx(
                fee,
                tx_config,
        ...
        if chik_tx is not None and chik_tx.spend_bundle is not None:
            spend_bundles.append(chik_tx.spend_bundle)
    ...
    async def revoke_vc(
        ...
        if fee > 0:
            chik_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.create_tandem_xck_tx(
                fee, tx_config, extra_conditions=(vc_announcement,)
            )
            assert did_tx.spend_bundle is not None
            assert chik_tx.spend_bundle is not None
            new_bundle = SpendBundle.aggregate([chik_tx.spend_bundle, did_tx.spend_bundle])
