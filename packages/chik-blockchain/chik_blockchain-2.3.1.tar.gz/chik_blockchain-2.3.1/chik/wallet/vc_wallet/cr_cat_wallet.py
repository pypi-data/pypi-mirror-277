...
from chik.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from chik.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from chik.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSChikConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
        ...
        for coin in coin_records:
            ...
            if first:
                ...
                if need_chik_transaction:
                    ...
                    elif regular_chik_to_claim > fee:
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [chik_tx.spend_bundle]
            ...
            [*vc_txs, *([chik_tx] if chik_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(chik_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if chik_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, chik_tx.spend_bundle])
        else:
            chik_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(chik_tx, spend_bundle=None),) if chik_tx is not None else []),
        ]
