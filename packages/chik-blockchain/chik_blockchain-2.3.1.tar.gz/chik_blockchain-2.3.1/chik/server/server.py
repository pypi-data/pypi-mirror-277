...
class ChikServer:
    ...
    all_connections: Dict[bytes32, WSChikConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        chik_ca_crt_key: Tuple[Path, Path],
        name: str = __name__,
    ) -> ChikServer:
        ...
        chik_ca_crt_path, chik_ca_key_path = chik_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=chik_ca_crt_path,
                ca_key=chik_ca_key_path,
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSChikConnection] = []
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("ChikServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSChikConnection] = None
        try:
            connection = WSChikConnection.create(
    ...
    async def connection_added(
        self, connection: WSChikConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    async def connection_closed(
        self, connection: WSChikConnection, ban_time: int, closed_connection: bool = False
    ) -> None:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use chik's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.chiknetwork.com/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSChikConnection, trusted_peers: Dict[str, Any]) -> bool:
