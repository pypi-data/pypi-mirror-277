...
from chik.cmds.init_funcs import check_keys, chik_init
...
from chik.util.ws_message import WsRpcMessage, create_payload, format_response
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        if plotter == "bladebit":
            ...
            # 'chik plotters bladebit ramplot' or 'chik plotters bladebit diskplot'
            plot_type = request["plot_type"]
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"chik-blockchain version: {chik_short_version()}")
