...
from chik.full_node.tx_processing_queue import TransactionQueueFull
...
class FullNodeAPI:
    ...
    async def request_peers(
        self, _request: full_node_protocol.RequestPeers, peer: WSChikConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: WSChikConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSChikConnection
    ) -> Optional[Message]:
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: WSChikConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: WSChikConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: WSChikConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: WSChikConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: WSChikConnection
    ) -> Optional[Message]:
        """
            ...
            async with self.full_node.blockchain.priority_mutex.acquire(priority=BlockchainMutexPriority.high):
                ...
                if peak is not None:
                    ...
                    if mempool_bundle is not None:
                        ...
                        # allowed to be serialized with the improved KLVM
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: WSChikConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: WSChikConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: WSChikConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: WSChikConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def new_compact_vdf(
        self, request: full_node_protocol.NewCompactVDF, peer: WSChikConnection, request_bytes: bytes = b""
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @api_request(peer_required=True)
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSChikConnection) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def register_interest_in_puzzle_hash(
        self, request: wallet_protocol.RegisterForPhUpdates, peer: WSChikConnection
    ) -> Message:
    ...
    async def register_interest_in_coin(
        self, request: wallet_protocol.RegisterForCoinUpdates, peer: WSChikConnection
    ) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
    ...
    async def request_remove_puzzle_subscriptions(
        self, request: wallet_protocol.RequestRemovePuzzleSubscriptions, peer: WSChikConnection
    ) -> Message:
    ...
    async def request_remove_coin_subscriptions(
        self, request: wallet_protocol.RequestRemoveCoinSubscriptions, peer: WSChikConnection
    ) -> Message:
        peer_id = peer.peer_node_id
        subs = self.full_node.subscriptions
    ...
    async def request_puzzle_state(
        self, request: wallet_protocol.RequestPuzzleState, peer: WSChikConnection
    ) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
        subs = self.full_node.subscriptions
    ...
    async def request_coin_state(self, request: wallet_protocol.RequestCoinState, peer: WSChikConnection) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
        subs = self.full_node.subscriptions
    ...
    def max_subscribe_response_items(self, peer: WSChikConnection) -> int:
        if self.is_trusted(peer):
