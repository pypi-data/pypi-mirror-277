...
from chik_rs import AugSchemeMPL, Coin, G2Element
...
from chik.types.eligible_coin_spends import EligibleCoinSpends, UnspentLineageInfo
...
class Mempool:
    ...
    def total_mempool_cost(self) -> KLVMCost:
        return KLVMCost(uint64(self._total_cost))
    ...
    async def create_bundle_from_mempool_items(
        ...
        for row in cursor:
            ...
            try:
                ...
                if new_cost_sum > self.mempool_info.max_block_klvm_cost:
                    # Let's skip this item
                    log.info(
                        "Skipping mempool item. Cumulative cost %d exceeds maximum block cost %d",
                        new_cost_sum,
                        self.mempool_info.max_block_klvm_cost,
                    )
                    skipped_items += 1
                ...
                if self.mempool_info.max_block_klvm_cost - cost_sum < MIN_COST_THRESHOLD:
                    break
