...
import pytest
from chik_rs import ELIGIBLE_FOR_DEDUP, ELIGIBLE_FOR_FF, G1Element, G2Element
...
from chik.types.eligible_coin_spends import (
    DedupCoinSpend,
...
async def instantiate_mempool_manager(
    ...
    *,
    ...
    mempool_manager = MempoolManager(get_coin_records, constants, max_tx_klvm_cost=max_tx_klvm_cost)
...
async def setup_mempool_with_coins(
    *,
    ...
    max_block_klvm_cost: Optional[int] = None,
    max_tx_klvm_cost: Optional[uint64] = None,
    ...
    if max_block_klvm_cost is not None:
        constants = dataclasses.replace(constants, MAX_BLOCK_COST_KLVM=max_block_klvm_cost)
    ...
    mempool_manager = await instantiate_mempool_manager(
        get_coin_records, constants=constants, max_tx_klvm_cost=max_tx_klvm_cost
    )
...
async def test_create_bundle_from_mempool_on_max_cost(num_skipped_items: int, caplog: pytest.LogCaptureFixture) -> None:
    ...
    mempool_manager, coins = await setup_mempool_with_coins(
        ...
        max_block_klvm_cost=550_000_000,
        max_tx_klvm_cost=uint64(550_000_000 * 0.6),
    ...
    # Combined with a big cost spend bundle, we'd exceed the maximum block klvm cost
