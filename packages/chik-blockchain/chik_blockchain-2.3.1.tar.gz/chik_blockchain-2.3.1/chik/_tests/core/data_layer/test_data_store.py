...
from chik._tests.util.misc import BenchmarkRunner, Marks, datacases
...
async def test_subscribe_unsubscribe(data_store: DataStore, tree_id: bytes32) -> None:
    await data_store.subscribe(Subscription(tree_id, [ServerInfo("http://127:0:0:1/9234", 1, 1)]))
    ...
    assert urls == ["http://127:0:0:1/9234"]

    await data_store.subscribe(Subscription(tree_id, [ServerInfo("http://127:0:0:1/9235", 2, 2)]))
    subscriptions = await data_store.get_subscriptions()
    urls = [server_info.url for subscription in subscriptions for server_info in subscription.servers_info]
    assert urls == ["http://127:0:0:1/9234", "http://127:0:0:1/9235"]

    await data_store.subscribe(
        Subscription(
            tree_id, [ServerInfo("http://127:0:0:1/9234", 100, 100), ServerInfo("http://127:0:0:1/9235", 200, 200)]
        )
    )
    subscriptions = await data_store.get_subscriptions()
    assert subscriptions == [
        Subscription(tree_id, [ServerInfo("http://127:0:0:1/9234", 1, 1), ServerInfo("http://127:0:0:1/9235", 2, 2)]),
    ]
    ...
    await data_store.subscribe(
        Subscription(
            tree_id2, [ServerInfo("http://127:0:0:1/9234", 300, 300), ServerInfo("http://127:0:0:1/9235", 400, 400)]
        )
    )
    subscriptions = await data_store.get_subscriptions()
    assert subscriptions == [
...
async def test_server_selection(data_store: DataStore, tree_id: bytes32) -> None:
    ...
    await data_store.subscribe(
        Subscription(tree_id, [ServerInfo(f"http://127.0.0.1/{port}", 0, 0) for port in range(9234, 9244)])
    )

    free_servers = {f"http://127.0.0.1/{port}" for port in range(9234, 9244)}
    tried_servers = 0
    random = Random()
    random.seed(100, version=2)
    ...
    assert selected_servers == {f"http://127.0.0.1/{port}" for port in range(9234, 9244)}

    for _ in range(100):
        servers_info = await data_store.get_available_servers_for_store(tree_id=tree_id, timestamp=current_timestamp)
        random.shuffle(servers_info)
        assert servers_info != []
        if servers_info[0].url != "http://127.0.0.1/9234":
    ...
    assert servers_info[0].url == "http://127.0.0.1/9234"
...
async def test_server_http_ban(
    ...
    sinfo = ServerInfo("http://127.0.0.1/9237", 0, 0)
    ...
    with monkeypatch.context() as m:
        m.setattr("chik.data_layer.download_data.http_download", mock_http_download)
