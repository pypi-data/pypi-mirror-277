...
from chik.cmds.data_funcs import (
    clear_pending_roots,
    get_keys_cmd,
    get_keys_values_cmd,
    get_kv_diff_cmd,
...
from chik.data_layer.data_layer_errors import KeyNotFoundError, OfferIntegrityError
...
from chik.data_layer.download_data import get_delta_filename, get_full_tree_filename
...
async def run_cli_cmd(*args: str, root_path: Path) -> asyncio.subprocess.Process:
    process = await asyncio.create_subprocess_exec(
        ...
        "chik",
        *args,
        env={**os.environ, "CHIK_ROOT": os.fspath(root_path)},
...
async def test_subscriptions(
    ...
    async with init_data_layer(wallet_rpc_port=wallet_rpc_port, bt=bt, db_path=tmp_path) as data_layer:
        ...
        response = await data_rpc_api.subscribe(request={"id": store_id.hex(), "urls": ["http://127.0.0.1/9234"]})
...
async def offer_setup_fixture(
    ...
    async with contextlib.AsyncExitStack() as exit_stack:
        ...
        await maker.api.subscribe(request={"id": taker.id.hex(), "urls": ["http://127.0.0.1/9234"]})
        await taker.api.subscribe(request={"id": maker.id.hex(), "urls": ["http://127.0.0.1/9234"]})
...
async def test_clear_pending_roots(
    ...
    async with init_data_layer_service(wallet_rpc_port=wallet_rpc_port, bt=bt, db_path=tmp_path) as data_layer_service:
        ...
        if layer == InterfaceLayer.direct:
            ...
            process = await asyncio.create_subprocess_exec(
                *args,
                env={**os.environ, "CHIK_ROOT": str(bt.root_path)},
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
            await process.wait()
...
async def test_mirrors(
    ...
    async with init_data_layer(wallet_rpc_port=wallet_rpc_port, bt=bt, db_path=tmp_path) as data_layer:
        ...
        urls = ["http://127.0.0.1/9234", "http://127.0.0.1/9235"]
        ...
        assert mirror["urls"] == ["http://127.0.0.1/9234", "http://127.0.0.1/9235"]
...
def populate_reference(count: int, keys_to_prove: int) -> ProofReference:
    ret = ProofReference(
        ...
        verify_proof_response={
            "current_root": True,
            "success": True,
            "verified_klvm_hashes": {
                "store_id": "",
                "inclusions": [
                    {
                        "key_klvm_hash": "0x" + std_hash(b"\1" + value.to_bytes(length=1, byteorder="big")).hex(),
                        "value_klvm_hash": "0x"
...
async def test_dl_proof(offer_setup: OfferSetup, reference: ProofReference) -> None:
    ...
    reference.verify_proof_response["verified_klvm_hashes"]["store_id"] = f"0x{offer_setup.maker.id.hex()}"
    ...
    fakeproof = HashOnlyProof(
        key_klvm_hash=bytes32([1] * 32), value_klvm_hash=bytes32([1] * 32), node_hash=bytes32([3] * 32), layers=[]
    )
    assert fakeproof.root() == fakeproof.node_hash

    fakeproof = HashOnlyProof(
        key_klvm_hash=bytes32([1] * 32),
        value_klvm_hash=bytes32([1] * 32),
...
async def test_dl_proof_verify_errors(offer_setup: OfferSetup, seeded_random: random.Random) -> None:
    ...
    two_key_proof.verify_proof_response["verified_klvm_hashes"]["store_id"] = f"0x{offer_setup.maker.id.hex()}"
    ...
    # test bad key
    badproof = deepcopy(proof["proof"])
    badproof["store_proofs"]["proofs"][0]["key_klvm_hash"] = bytes32.random(seeded_random).hex()
    with pytest.raises(ValueError, match="Invalid Proof: node hash does not match key and value"):
        await offer_setup.taker.api.verify_proof(request=badproof)

    # test bad value
    badproof = deepcopy(proof["proof"])
    badproof["store_proofs"]["proofs"][0]["value_klvm_hash"] = bytes32.random(seeded_random).hex()
    with pytest.raises(ValueError, match="Invalid Proof: node hash does not match key and value"):
        await offer_setup.taker.api.verify_proof(request=badproof)
...
async def test_unsubmitted_batch_db_migration(
    ...
    with monkeypatch.context() as m:
        ...
        m.setattr("chik.data_layer.data_layer_util.Status", ModifiedStatus)
        m.setattr("chik.data_layer.data_store.Status", ModifiedStatus)
        m.setattr("chik.data_layer.data_layer.Status", ModifiedStatus)
        ...
        async with init_data_layer_service(
            ...
            m.setattr("chik.data_layer.data_layer_util.Status", OldStatus)
            m.setattr("chik.data_layer.data_store.Status", OldStatus)
            m.setattr("chik.data_layer.data_layer.Status", OldStatus)
