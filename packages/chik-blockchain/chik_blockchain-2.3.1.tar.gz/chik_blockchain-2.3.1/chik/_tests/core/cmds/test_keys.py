...
from chik.util.default_root import DEFAULT_KEYS_ROOT_PATH
...
class TestKeysCommands:
    def test_generate_with_new_config(self, tmp_path, empty_keyring):
        ...
        the correct xck_target_address entries.
        """
        ...
        # Verify that the config has the correct xck_target_address entries
        address_matches = re.findall(r"xck1[^\n]+", result.output)
        assert len(address_matches) > 1
        address = address_matches[0]

        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["xck_target_address"] == address
        assert config["pool"]["xck_target_address"] == address
    ...
    def test_generate_with_existing_config(self, tmp_path, empty_keyring):
        ...
        the original xck_target_address entries.
        """
        ...
        assert existing_config["farmer"]["xck_target_address"] == address
        assert existing_config["pool"]["xck_target_address"] == address
        ...
        # Verify that the config's xck_target_address entries have not changed
        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["xck_target_address"] == existing_config["farmer"]["xck_target_address"]
        assert config["pool"]["xck_target_address"] == existing_config["pool"]["xck_target_address"]
    ...
    def test_derive_search_wallet_address(self, tmp_path, keyring_with_one_key):
        ...
        result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "xck1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs0kyx8x",
            ],
            catch_exceptions=False,
        )
