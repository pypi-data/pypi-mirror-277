...
from chik.daemon.server import WebSocketServer, plotter_log_path, service_plotter
...
from chik.util.keychain import Keychain, KeyData, supports_os_passphrase_storage
from chik.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE, KeyringWrapper
from chik.util.ws_message import create_payload, create_payload_dict
from chik.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk

chikpos_version = pkg_resources.get_distribution("chikpos").version
...
@dataclass
class ChikPlottersBladebitArgsCase:
    case_id: str
    plot_type: str
    count: int = 1
    threads: int = 0
    ...
    async with aiohttp.ClientSession() as client:
        async with client.ws_connect(
            ...
            async with setup_full_node(
                ...
                await time_out_assert_not_none(30, ws_server.connections.get, "chik_full_node")

                payload = create_payload("get_blockchain_state", {}, service_name, "chik_full_node")
                await ws.send_str(payload)

                response = await ws.receive()
                message = assert_response_success_only(response)
                assert message["command"] == "get_blockchain_state"
                assert message["origin"] == "chik_full_node"
        ...
        response={
            ...
            "wallet_addresses": {
                test_key_data.fingerprint: [
                    {
                        "address": "xck1zze67l3jgxuvyaxhjhu7326sezxxve7lgzvq0497ddggzhff7c9szu5hqu",
                        "hd_path": "m/12381/8444/2/0",
                    },
                ],
                test_key_data_2.fingerprint: [
                    {
                        "address": "xck1fra5h0qnsezrxenjyslyxx7y4l268gq52m0rgenh58vn8f577uzsxl4em8",
                        "hd_path": "m/12381/8444/2/0",
                    }
                ],
            },
    ...
    WalletAddressCase(
        ...
        response={
            ...
            "wallet_addresses": {
                test_key_data.fingerprint: [
                    {
                        "address": "xck16jqcaguq27z8xvpu89j7eaqfzn6k89hdrrlm0rffku85n8n7m7sqgxzjeu",
                        "hd_path": "m/12381/8444/2/1",
                    },
                    {
                        "address": "xck1955vj0gx5tqe7v5tceajn2p4z4pup8d4g2exs0cz4xjqses8ru6q56mwl0",
    ...
    WalletAddressCase(
        ...
        response={
            ...
            "wallet_addresses": {
                test_key_data.fingerprint: [
                    {
                        "address": "xck1k996a7h3agygjhqtrf0ycpa7wfd6k5ye2plkf54ukcmdj44gkqkq06kssc",
...
@datacases(
    ...
    RouteCase(
        route="register_service",
        description="chik_plotter",
        request={
            "service": "chik_plotter",
        },
        response={"success": True, "service": "chik_plotter", "queue": []},
    ),
    RouteCase(
    ...
    RouteCase(
        ...
        response={
            ...
            "plotters": {
                ...
                "chikpos": {"display_name": "Chik Proof of Space", "installed": True, "version": chikpos_version},
...
async def test_plotter_options(
    ...
    # register for chik_plotter events
    service_name = "chik_plotter"
    data = {"service": service_name}
    payload = create_payload("register_service", data, "chik_plotter", "daemon")
    await ws.send_str(payload)
    response = await ws.receive()
    assert_response_success_only(response)
...
async def test_plotter_roundtrip(
    ...
    # Write chikpos magic words to the log file to signal finished
...
@datacases(
    ChikPlottersBladebitArgsCase(case_id="1", plot_type="cudaplot"),
...
def test_run_plotter_bladebit(
    ...
    mock_daemon_with_config_and_keys,
    bt: BlockTools,
    case: ChikPlottersBladebitArgsCase,
) -> None:
    ...
    mocker.patch("chik.plotters.bladebit.get_bladebit_version", side_effect=get_bladebit_version)
    mock_run_plotter = mocker.patch("chik.plotters.bladebit.run_plotter")
