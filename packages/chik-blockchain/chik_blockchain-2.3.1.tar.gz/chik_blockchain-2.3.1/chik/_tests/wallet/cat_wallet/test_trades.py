...
from chik._tests.wallet.vc_wallet.test_vc_wallet import mint_cr_cat
...
async def test_cat_trades(
    ...
    wallet_taker = env_taker.xck_wallet
    ...
    chik_for_multiple_cat: OfferSummary = {
        wallet_maker.id(): -7,
        cat_wallet_maker.id(): 8,
        new_cat_wallet_maker.id(): 9,
    }
    multiple_cat_for_chik: OfferSummary = {
        wallet_maker.id(): 10,
        cat_wallet_maker.id(): -11,
        new_cat_wallet_maker.id(): -12,
    }
    chik_and_cat_for_cat: OfferSummary = {
        wallet_maker.id(): -13,
    ...
    success, trade_make, _, error = await trade_manager_maker.create_offer_for_ids(
        chik_for_cat, wallet_environments.tx_config, fee=uint64(1)
    )
        ...
        [
            WalletStateTransition(
                ...
                post_block_balance_updates={
                    "xck": {
                        "pending_coin_removal_count": -1,
                        "confirmed_wallet_balance": -2,  # One for offered XCK, one for fee
                        "unconfirmed_wallet_balance": -2,  # One for offered XCK, one for fee
                        ">#spendable_balance": 0,
                        ">#max_send_amount": 0,
                    },
                    "new cat": (
            ...
            WalletStateTransition(
                pre_block_balance_updates={
                    "xck": {
                        ...
                        "<=#spendable_balance": -2,
                        "<=#max_send_amount": -2,
                        # Unconfirmed balance doesn't change because receiveing 1 XCK and spending 1 in fee
                        "unconfirmed_wallet_balance": 0,
                ...
                post_block_balance_updates={
                    "xck": {
                        ...
                        ">#spendable_balance": 0,
                        ">#max_send_amount": 0,
                        # Confirmed balance doesn't change because receiveing 1 XCK and spending 1 in fee
                        "confirmed_wallet_balance": 0,
    ...
    # cat_for_chik
    success, trade_make, _, error = await trade_manager_maker.create_offer_for_ids(
        cat_for_chik, wallet_environments.tx_config
    )
    assert error is None
    assert success is True
    assert trade_make is not None
    ...
    xck_tx: TransactionRecord = next(tx for tx in tx_records if tx.wallet_id == 1)
    assert xck_tx.amount == 3
    assert xck_tx.fee_amount == 1
...
async def test_trade_cancellation(wallets_prefarm):
    ...
    await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, xck_to_cat_amount)
    await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, xck_to_cat_amount)
    maker_funds -= xck_to_cat_amount
    ...
    success, trade_make, _, error = await trade_manager_maker.create_offer_for_ids(cat_for_chik, DEFAULT_TX_CONFIG)
    assert error is None
    assert success is True
    assert trade_make is not None
    ...
    success, trade_make, _, error = await trade_manager_maker.create_offer_for_ids(chik_for_cat, DEFAULT_TX_CONFIG)
    assert error is None
    assert success is True
    assert trade_make is not None
...
async def test_aggregated_trade_state(wallets_prefarm):
    ...
    success, trade_make_1, _, error = await trade_manager_maker.create_offer_for_ids(chik_for_cat, DEFAULT_TX_CONFIG)
    ...
    success, trade_make_2, _, error = await trade_manager_maker.create_offer_for_ids(cat_for_chik, DEFAULT_TX_CONFIG)
    ...
    await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, xck_to_cat_amount - 1)
    await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, xck_to_cat_amount - 1)
