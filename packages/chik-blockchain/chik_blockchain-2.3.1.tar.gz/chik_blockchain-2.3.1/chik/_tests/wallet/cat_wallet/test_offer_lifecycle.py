...
from chik.wallet.conditions import AssertPuzzleAnnouncement, ConditionValidTimes
...
from chik.wallet.trading.offer import OFFER_MOD, Offer
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XCK coins
...
async def test_complex_offer(cost_logger: CostLogger) -> None:
    async with sim_and_client() as (sim, sim_client):
        ...
        # Create an XCK Offer for RED
        chik_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            str_to_tail_hash("red"): [Payment(acs_ph, uint64(100), [b"memo"]), Payment(acs_ph, uint64(200), [b"memo"])]
        }
        chik_notarized_payments = Offer.notarize_payments(chik_requested_payments, chik_coins)
        chik_announcements = Offer.calculate_announcements(chik_notarized_payments, driver_dict)
        chik_secured_bundle = generate_secure_bundle(chik_coins, chik_announcements, uint64(1000))
        chik_offer = Offer(chik_notarized_payments, chik_secured_bundle, driver_dict)
        assert not chik_offer.is_valid()

        # Create a RED Offer for XCK
        ...
        new_offer = Offer.aggregate([chik_offer, red_offer, red_offer_2])
        ...
        # Create yet another offer of BLUE for XCK and RED
        ...
        assert new_offer.summary() == (
            {"xck": 1000, str_to_tail_hash("red").hex(): 350, str_to_tail_hash("blue").hex(): 2000},
