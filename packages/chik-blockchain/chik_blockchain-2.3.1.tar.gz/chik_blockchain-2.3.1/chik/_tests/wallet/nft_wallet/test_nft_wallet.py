...
async def test_nft_wallet_creation_and_transfer(wallet_environments: WalletTestFramework) -> None:
    ...
    await env_0.change_balances(
        {
            "xck": {
                "set_remainder": True,  # Not testing XCK reorg functionality in this test
            },
            "nft": {
...
async def test_nft_wallet_rpc_creation_and_list(
    ...
    tr1 = await api_0.nft_mint_nft(
        {
            ...
            "uris": ["https://www.chiknetwork.com/img/branding/chik-logo.svg"],
        }
    )
    ...
    tr2 = await api_0.nft_mint_nft(
        {
            ...
            "uris": ["https://chiklisp.com/img/logo.svg"],
            "meta_uris": [
    ...
    assert len(uris) == 2
    assert "https://chiklisp.com/img/logo.svg" in uris
...
async def test_nft_wallet_rpc_update_metadata(
    ...
    uris = coin["data_uris"]
    assert len(uris) == 1
    assert "https://www.chiknetwork.com/img/branding/chik-logo.svg" in uris
...
async def test_nft_with_did_wallet_creation(
    ...
    resp = await api_0.nft_mint_nft(
        {
            ...
            "hash": "0xD4584AD463139FA8C0D9F68F4B59F185",
            "uris": ["https://www.chiknetwork.com/img/branding/chik-logo.svg"],
            "target_address": encode_puzzle_hash(nft_ph, "txck"),
        }
    )
    assert resp.get("success")
    sb = resp["spend_bundle"]
    ...
    assert did_nft["data_uris"][0] == "https://www.chiknetwork.com/img/branding/chik-logo.svg"
...
async def test_nft_transfer_nft_with_did(
    ...
    resp = await api_0.nft_transfer_nft(
        dict(
            wallet_id=nft_wallet_0_id,
            target_address=encode_puzzle_hash(ph1, "xck"),
    ...
    resp = await api_0.nft_mint_nft(
        {
            ...
            "hash": "0xD4584AD463139FA8C0D9F68F4B59F185",
            "uris": ["https://www.chiknetwork.com/img/branding/chik-logo.svg"],
            "mu": ["https://www.chiknetwork.com/img/branding/chik-logo.svg"],
            "did": hex_did_id,
        }
    )
    assert resp.get("success")
...
async def test_nft_bulk_transfer(self_hostname: str, two_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
    ...
    ph = await wallet_0.get_new_puzzlehash()
    ph1 = await wallet_1.get_new_puzzlehash()
    address = encode_puzzle_hash(ph1, AddressType.XCK.hrp(wallet_node_1.config))
    if trusted:
        wallet_node_0.config["trusted_peers"] = {
            full_node_api.full_node.server.node_id.hex(): full_node_api.full_node.server.node_id.hex()
        }
