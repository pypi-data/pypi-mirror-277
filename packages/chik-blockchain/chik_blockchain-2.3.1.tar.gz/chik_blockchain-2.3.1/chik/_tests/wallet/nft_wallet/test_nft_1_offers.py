...
async def test_nft_offer_sell_nft(
    ...
    txs = await nft_wallet_maker.generate_new_nft(
        metadata,
        DEFAULT_TX_CONFIG,
        target_puzhash,
        royalty_puzhash,
    ...
    # maker create offer: NFT for xck
    ...
    offer_did_nft_for_xck = {nft_to_offer_asset_id: -1, wallet_maker.id(): xck_requested}

    success, trade_make, _, error = await trade_manager_maker.create_offer_for_ids(
        offer_did_nft_for_xck, DEFAULT_TX_CONFIG, {}, fee=maker_fee
    )
    ...
    expected_royalty = uint64(xck_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + xck_requested + expected_royalty
    expected_taker_balance = funds - taker_fee - xck_requested - expected_royalty
...
async def test_nft_offer_request_nft(
    ...
    txs = await nft_wallet_taker.generate_new_nft(
        metadata,
        DEFAULT_TX_CONFIG,
        target_puzhash,
        royalty_puzhash,
    ...
    offer_dict = {nft_to_request_asset_id: 1, wallet_maker.id(): -xck_offered}
    ...
    # assert payments and royalties
    expected_royalty = uint64(xck_offered * royalty_basis_pts / 10000)
    expected_maker_balance = funds - maker_fee - xck_offered - expected_royalty
    expected_taker_balance = funds - 2 - taker_fee + xck_offered + expected_royalty
    await time_out_assert(20, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_did_to_did(
    ...
    expected_royalty = uint64(xck_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + xck_requested + expected_royalty
    expected_taker_balance = funds - 1 - taker_fee - xck_requested - expected_royalty
    await time_out_assert(20, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_nft_for_cat(
    ...
    offer_did_nft_for_xck = {nft_to_offer_asset_id: -1, cat_wallet_maker.id(): cats_requested}
...
async def test_complex_nft_offer(
    ...
    funds_maker = int(funds_maker - FEE + XCK_REQUESTED + maker_xck_royalties_expected)
    funds_taker = int(funds_taker - FEE - XCK_REQUESTED - maker_xck_royalties_expected)
    ...
    # Now let's make sure the final wallet state is correct
    funds_maker = int(funds_maker + XCK_REQUESTED / 2)
    funds_taker = int(funds_taker - XCK_REQUESTED / 2)
