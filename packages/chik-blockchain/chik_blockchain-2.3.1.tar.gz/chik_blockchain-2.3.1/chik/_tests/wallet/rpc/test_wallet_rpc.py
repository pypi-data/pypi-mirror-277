...
from chik._tests.wallet.test_wallet_coin_store import (
...
from chik.wallet.conditions import ConditionValidTimes, CreateCoinAnnouncement, CreatePuzzleAnnouncement, Remark
...
from chik.wallet.util.query_filter import AmountFilter, HashFilter, TransactionTypeFilter
...
async def test_spend_clawback_coins(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = await wallet_1_rpc.send_transaction(
        wallet_id=1,
        amount=uint64(500),
        address=encode_puzzle_hash(wallet_2_puzhash, "txck"),
        tx_config=DEFAULT_TX_CONFIG,
        fee=uint64(0),
    ...
    tx = await wallet_2_rpc.send_transaction(
        wallet_id=1,
        amount=uint64(500),
        address=encode_puzzle_hash(wallet_1_puzhash, "txck"),
        tx_config=DEFAULT_TX_CONFIG,
        fee=uint64(0),
        puzzle_decorator_override=[{"decorator": "CLAWBACK", "clawback_timelock": 5}],
    )
...
async def test_get_transactions(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    await client.send_transaction(
        1, uint64(1), encode_puzzle_hash(puzhash, "txck"), DEFAULT_TX_CONFIG
    )  # Create a pending tx
    ...
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    await client.send_transaction(1, uint64(1), encode_puzzle_hash(ph_by_addr, "txck"), DEFAULT_TX_CONFIG)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    tx_for_address = await client.get_transactions(1, to_address=encode_puzzle_hash(ph_by_addr, "txck"))
...
async def _check_delete_key(
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["xck_target_address"] = encode_puzzle_hash(farmer_ph, "txck")
        test_config["pool"]["xck_target_address"] = encode_puzzle_hash(pool_ph, "txck")
...
async def test_key_and_address_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(ph, "txck")
    tx_amount = uint64(15600000)
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["xck_target_address"] = ""
        save_config(wallet_node.root_path, "config.yaml", test_config)
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["xck_target_address"] = ""
        test_config["pool"]["xck_target_address"] = ""
        save_config(wallet_node.root_path, "config.yaml", test_config)
...
# xck1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqt756q2
# $ chik wallet sign_message -m $(echo -n 'hello world' | xxd -p)
# -a xck1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqt756q2
#
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            # -a xck1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35s4qtrcu
            {
        ...
        (
            ...
            # -a xck1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35s4qtrcu
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "xck1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35s4qtrcu",
            },
            {"isValid": True},
        ),
        (
            {
                ...
                "address": "xck1hh9phcc8tt703dla70qthlhrxswy88va04zvc7vd8cx2v6a5ywysr6z8j5",
            },
            {"isValid": True},
        ),
        # Negative tests
        ...
        (
            # Valid signature but address doesn't match pubkey
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "xck1d0rekc2javy5gpruzmcnk4e4qq834jzlvxt5tcgl2ylt49t26gds6y239y",
            },
...
async def test_cat_spend_run_tail(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = await client.cat_spend(
        ...
        amount=uint64(0),
        tx_config=DEFAULT_TX_CONFIG,
        inner_address=encode_puzzle_hash(our_ph, "txck"),
