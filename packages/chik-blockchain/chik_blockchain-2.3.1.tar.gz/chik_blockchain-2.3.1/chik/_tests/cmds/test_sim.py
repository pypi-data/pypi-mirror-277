...
def test_every_simulator_command() -> None:
    ...
    assert "chik_full_node_simulator: started" in start_result.output
...
def stop_simulator(runner: CliRunner, simulator_name: str) -> None:
    ...
    assert "chik_full_node_simulator: Stopped\nDaemon stopped\n" == result.output
...
def _test_sim_status(runner: CliRunner, address: str, simulator_name: str) -> None:
    ...
    assert f"Current Farming address: {address}, with a balance of: 21000000.0 TXCK." in result.output  # default
