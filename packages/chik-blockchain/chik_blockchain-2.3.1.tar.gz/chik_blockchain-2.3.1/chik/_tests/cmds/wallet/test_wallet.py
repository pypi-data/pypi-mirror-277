...
from chik._tests.cmds.wallet.test_consts import (
    CAT_FINGERPRINT_ARG,
    FINGERPRINT,
    FINGERPRINT_ARG,
    STD_TX,
...
def test_get_transaction(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Status: In mempool",
        "Amount sent: 0.000012345678 XCK",
        "To address: xck1qyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqs8n36wt",
    ]
    v_assert_list = [
...
def test_get_transactions(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Amount received: 0.00001234568 XCK",
        "Amount received in clawback as sender: 0.000012345681 XCK",
        "To address: xck1qvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvps0h08dp",
        "To address: xck1qszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszq7mpefx",
    ]
    v_assert_list = [
        "'amount': 12345680",
        "'fee_amount': 1234569",
...
def test_show(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class ShowRpcClient(TestWalletRpcClient):
        async def get_wallets(self, wallet_type: Optional[WalletType] = None) -> List[Dict[str, Union[str, int]]]:
            ...
            wallet_list: List[Dict[str, Union[str, int]]] = [
                {"data": "", "id": 1, "name": "Chik Wallet", "type": WalletType.STANDARD_WALLET},
                {
    ...
    assert_list = [
        "Chik Wallet:\n   -Total Balance:         0.001 xck (1000000000 mojo)",
...
def test_send(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    # get output with all options but verbose
    addr = encode_puzzle_hash(get_bytes32(3), "xck")
    command_args = [
        "wallet",
        "send",
        "-a1",
    ...
    expected_calls: logType = {
        ...
        "send_transaction": [
            (
                1,
                1000000000000,
                "xck1qvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvps0h08dp",
                TXConfig(
...
def test_get_address(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class GetAddressWalletRpcClient(TestWalletRpcClient):
        async def get_next_address(self, wallet_id: int, new_address: bool) -> str:
            ...
            if new_address:
                return encode_puzzle_hash(get_bytes32(3), "xck")
            return encode_puzzle_hash(get_bytes32(4), "xck")

    inst_rpc_client = GetAddressWalletRpcClient()  # pylint: disable=no-value-for-parameter
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    # get output with all options but verbose
    addr1 = encode_puzzle_hash(get_bytes32(3), "xck")
    addr2 = encode_puzzle_hash(get_bytes32(4), "xck")
    command_args = [
        "wallet",
        "get_address",
        WALLET_ID_ARG,
...
def test_sign_message(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    inst_rpc_client = TestWalletRpcClient()  # pylint: disable=no-value-for-parameter
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    xck_addr = encode_puzzle_hash(get_bytes32(1), "xck")
    ...
    run_cli_command_and_assert(capsys, root_dir, command_args + [f"-a{xck_addr}"], assert_list)
    expected_calls: logType = {
        "sign_message_by_address": [(xck_addr, message.hex())],  # xck std
    }
    test_rpc_clients.wallet_rpc_client.check_log(expected_calls)
...
def test_make_offer_bad_filename(
    ...
    # we offer xck and a random cat via wallet id and request a random cat, nft via coin and tail
    command_args_dir = [
        "wallet",
        "make_offer",
        FINGERPRINT_ARG,
    ...
    expected_calls: logType = {
        ...
        "nft_calculate_royalties": [
            (
                ...
                {"XCK": 10000000000000, "test3": 100000},
            )
        ],
        "create_offer_for_ids": [
            (
...
def test_take_offer(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        "  OFFERED:\n"
        "    - XCK (Wallet ID: 1): 10.0 (10000000000000 mojos)\n"
