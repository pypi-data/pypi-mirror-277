...
from chik._tests.simulation.test_simulation import test_constants_modified
from chik._tests.util.misc import BenchmarkRunner, GcMode, RecordingWebServer, TestId, _AssertRuntime, measure_overhead
from chik._tests.util.setup_nodes import (
...
from chik.simulator.setup_services import (
    setup_crawler,
    setup_daemon,
    setup_full_node,
    setup_introducer,
...
def pytest_collection_modifyitems(session, config: pytest.Config, items: List[pytest.Function]):
    ...
    if len(all_error_lines) > 0:
        all_error_lines.insert(0, "custom chik collection rules failed")
...
async def wallet_nodes(blockchain_constants, consensus_mode):
    ...
    async with setup_simulators_and_wallets(
        2,
        1,
        dataclasses.replace(blockchain_constants, MEMPOOL_BLOCK_BUFFER=1, MAX_BLOCK_COST_KLVM=400000000),
    ) as new:
...
async def two_wallet_nodes_custom_spam_filtering(
    spam_filter_after_n_txs, xck_spam_amount, blockchain_constants: ConsensusConstants
):
    async with setup_simulators_and_wallets(
        1, 2, blockchain_constants, spam_filter_after_n_txs, xck_spam_amount
    ) as new:
        yield make_old_setup_simulators_and_wallets(new=new)
