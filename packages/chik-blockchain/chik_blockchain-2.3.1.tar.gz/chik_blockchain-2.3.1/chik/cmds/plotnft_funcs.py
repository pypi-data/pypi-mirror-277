...
from chik.cmds.wallet_funcs import print_balance, wallet_coin_unit
...
async def create(
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, _):
        fee_mojos = uint64(int(fee * units["chik"]))
        ...
        if user_input.lower() == "y" or user_input.lower() == "yes":
            try:
                ...
                print(
                    f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  chik start -r farmer"
                )
            return
        print("Aborting.")
