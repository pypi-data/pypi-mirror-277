...
from chik.cmds.cmds_util import CMDTXConfigLoader, get_wallet_client, transaction_status_msg, transaction_submitted_msg
from chik.cmds.units import units
from chik.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type
...
from chik.wallet.util.tx_config import DEFAULT_COIN_SELECTION_CONFIG
...
async def create_dao_wallet(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    proposal_minimum = uint64(int(Decimal(args["proposal_minimum_amount"]) * units["chik"]))
    ...
    final_fee: uint64 = uint64(int(fee * units["chik"]))

    fee_for_cat = Decimal(args["fee_for_cat"])
    final_fee_for_cat: uint64 = uint64(int(fee_for_cat * units["chik"]))
...
async def vote_on_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    fee = args["fee"]
    final_fee: uint64 = uint64(int(Decimal(fee) * units["chik"]))
    proposal_id = args["proposal_id"]
