...
from chik.cmds.peer_funcs import print_connections
from chik.cmds.units import units
from chik.rpc.wallet_request_types import GetNotifications
from chik.rpc.wallet_rpc_client import WalletRpcClient
from chik.types.blockchain_format.sized_bytes import bytes32
from chik.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
...
from chik.wallet.conditions import CreateCoinAnnouncement, CreatePuzzleAnnouncement
...
def print_transaction(
    ...
    if verbose:
        ...
        chik_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {chik_amount} {name}")
...
async def send(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        final_fee: uint64 = uint64(int(fee * units["chik"]))  # fees are always in XCK mojos
        ...
        print(f"To get status, use command: chik wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if offers == [] or requests == []:
            ...
            for item in [*offers, *requests]:
                            ...
                            if item in requests:
                                ...
                                if info.supports_did:
                                    ...
                                    royalty_asset_dict[name] = (
                                        encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.XCK.hrp(config)),
                ...
                if fee > 0:
                    print()
                    print(f"Including Fees: {Decimal(fee) / units['chik']} XCK, {fee} mojos")
                ...
                if royalty_asset_dict != {}:
                    ...
                    for nft_id, summaries in royalty_summary.items():
                        ...
                        for summary in summaries:
                            divisor = units["chik"] if summary["asset"] == "XCK" else units["cat"]
                    ...
                    for asset, requested_amount in total_amounts_requested.items():
                        divisor = units["chik"] if asset == "XCK" else units["cat"]
                ...
                with filepath.open(mode="w") as file:
                    ...
                    if offer is not None:
                        ...
                        print(
                            f"Use chik wallet get_offers --id "
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_xck: str = "XCK"
) -> None:
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["chik"]
        wid: str = "1" if asset_id == "xck" else ""
        mojo_amount: int = int(Decimal(amount))
        name: str = network_xck
        if asset_id != "xck":
            name = asset_id
            if asset_id == "unknown":
                name = "Unknown"
...
async def print_trade_record(record: TradeRecord, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['chik']} XCK, {fees} mojos")
    print("---------------")
...
async def take_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        offered, requested, _, _ = offer.summary()
        cat_name_resolver = wallet_client.cat_asset_id_to_name
        network_xck = AddressType.XCK.hrp(config).upper()
        print("Summary:")
        print("  OFFERED:")
        await print_offer_summary(cat_name_resolver, offered, network_xck=network_xck)
        print("  REQUESTED:")
        await print_offer_summary(cat_name_resolver, requested, network_xck=network_xck)
        ...
        for royalty_asset_id in nft_coin_ids_supporting_royalties_from_offer(offer):
            if royalty_asset_id.hex() in offered:
                ...
                royalty_asset_dict[encode_puzzle_hash(royalty_asset_id, AddressType.NFT.hrp(config))] = (
                    encode_puzzle_hash(address, AddressType.XCK.hrp(config)),
                    percentage,
                )
        ...
        if royalty_asset_dict != {}:
            ...
            for fungible_asset_id in fungible_assets_from_offer(offer):
                fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "xck"
                if fungible_asset_id_str in requested:
                    nft_royalty_currency: str = "Unknown CAT"
                    if fungible_asset_id is None:
                        nft_royalty_currency = network_xck
                    else:
            ...
            if fungible_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    print(f"  - For {nft_id}:")
                    for summary in summaries:
                        divisor = units["chik"] if summary["asset"] == network_xck else units["cat"]
                        converted_amount = Decimal(summary["amount"]) / divisor
                        total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                        total_amounts_requested[summary["asset"]] += summary["amount"]
                        print(
                ...
                for asset, amount in total_amounts_requested.items():
                    divisor = units["chik"] if asset == network_xck else units["cat"]
                    converted_amount = Decimal(amount) / divisor
                    print(f"  - {converted_amount} {asset} ({amount} mojos)")

        print(f"Included Fees: {Decimal(offer.fees()) / units['chik']} {network_xck}, {offer.fees()} mojos")
        ...
        if not examine_only:
            ...
            trade_record = await wallet_client.take_offer(
                offer,
                fee=fee,
                tx_config=CMDTXConfigLoader().to_tx_config(units["chik"], config, fingerprint),
            )
            print(f"Accepted offer with ID {trade_record.trade_id}")
            print(f"Use chik wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        await wallet_client.cancel_offer(
            offer_id, CMDTXConfigLoader().to_tx_config(units["chik"], config, fingerprint), secure=secure, fee=fee
        )
        print(f"Cancelled offer with ID {trade_record.trade_id}")
        if secure:
            print(f"Use chik wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:  # pragma: no cover
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["chik"]
    return "", units["mojo"]
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            response = await wallet_client.did_message_spend(
                did_wallet_id,
                CMDTXConfigLoader().to_tx_config(units["chik"], config, fingerprint),
                extra_conditions=(
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        royalty_address = (
            ...
            else ensure_valid_address(royalty_address, allowed_types={AddressType.XCK}, config=config)
        )
        target_address = (
            None
            if not target_address
            else ensure_valid_address(target_address, allowed_types={AddressType.XCK}, config=config)
        )
        fee: int = int(d_fee * units["chik"])
        try:
...
async def transfer_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            target_address = ensure_valid_address(target_address, allowed_types={AddressType.XCK}, config=config)
            fee: int = int(d_fee * units["chik"])
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        amount: uint64 = uint64(d_amount * units["chik"])
        message: bytes = bytes(message_hex, "utf8")
        fee: uint64 = uint64(d_fee * units["chik"])

        tx = await wallet_client.send_notification(address, message, amount, fee)

        print("Notification sent successfully.")
        print(f"To get status, use command: chik wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
...
async def sign_message(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if addr_type == AddressType.XCK:
            if address is None:
                print("Address is required for XCK address type.")
                return
    ...
    *, wallet_rpc_port: Optional[int], fp: Optional[int], fee: Decimal, tx_ids_str: str, force: bool = False
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        if fee < 0:
            ...
            return
        response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["chik"]), force)
        print(str(response))
        ...
        vc_record, txs = await wallet_client.vc_mint(
            ...
            (
                ...
                if target_address is None
                    ...
                    ensure_valid_address(target_address, allowed_types={AddressType.XCK}, config=config)
                )
            ),
            uint64(int(d_fee * units["chik"])),
        )
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        txs = await wallet_client.vc_spend(
            ...
            fee=uint64(int(d_fee * units["chik"])),
            tx_config=CMDTXConfigLoader(
                reuse_puzhash=reuse_puzhash,
            ).to_tx_config(units["chik"], config, fingerprint),
        )
