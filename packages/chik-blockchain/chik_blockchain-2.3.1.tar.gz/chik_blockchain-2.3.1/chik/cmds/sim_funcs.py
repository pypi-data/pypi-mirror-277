...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `chik keys show`.
    """
...
async def async_config_wizard(
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "chik")  # fix path for tests
...
async def print_status(
    ...
    """
    from chik.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically chik show -s)
