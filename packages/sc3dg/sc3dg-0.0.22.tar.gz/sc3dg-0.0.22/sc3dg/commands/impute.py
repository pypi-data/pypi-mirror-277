...
def impute(mcool, 
    ...
    """
    if cool_file_url is None:
        print("ERROR: Must provide a cool file URL or path.")
        return

    hic_data = cooler.Cooler(f"{cool_file_url}::/resolutions/{resolution}")
    rwr_results = {}
    ...
    merge_segement(rwr_results, save_path= output_file_path, resolution=resolution)
