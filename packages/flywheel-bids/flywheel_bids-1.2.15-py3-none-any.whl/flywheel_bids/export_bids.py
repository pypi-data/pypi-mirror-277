...
def warn_if_bids_invalid(f, namespace):
    ...
    if not metadata or metadata.get("valid") is None:
        ...
        logger.warning("File {} is not valid: {}".format(metadata.get("Filename"), metadata.get("error_message")))
...
def create_json_sidecar(metadata, namespace, outdir):
    ...
    if not metadata:
        logger.info(f"No metadata, besides possibly {namespace}, available to create sidecar")
        return
    ...
    new_path = Path(outdir, ns_data["Path"], Path(img_bids_name[: -len(ext)]).stem + ".json")
...
def export_bids(
    ...
    ctype, cid = determine_container(fw, project_label, container_type, container_id, group_id=group_id)
