    ...
    def apply_custom_initialization(self, rule_id, info, context):
        ...
        if rule_id in self.initializer_map:
            for init in self.initializer_map[rule_id]:
                if "where" in init:
                    if test_where_clause(init["where"], context):
                ...
                for key, valueSpec in propDef.items():
                    ...
                    if value is not None:
                        ...
                        if "$regex" in valueSpec:
                            ...
                            for regex in regex_list:
                                ...
                                if m is not None:
                                    ...
                                    break
                            if m is None:
                                logger.debug(f"Unable to match regex <{regex_list}> to <{value}>")
                        ...
                        if key != "$run_counter":  # run counters are handled later
                            logger.debug(f"Metadata key <{key}> does not exist on this object")
        else:
    ...
    for caseDef in switchDef["$cases"]:
        ...
        if compOperation:
            ...
            if its_a_match:
                ...
                logger.debug(f'no match for {compValue} for value {caseDef.get("$value")}')
    ...
    for propName, propDef in initializers.items():
        if isinstance(propDef, dict) and "$run_counter" in propDef:
            ...
            if current == "+" or current == "=":
                run_counter = counter_map[key]  # creates run_counter if it doesn't exist
                if current == "+":
    ...
    for field, match in conditions.items():
        if "$" in field:
            ...
            if isinstance(match, dict):
                outcome_vals.append(determine_nested(field, match, context, nested_match_values=[]))
            else:
                # multi-level condition
                for sub_match in match:
                    outcome_vals.append(determine_nested(field, sub_match, context, nested_match_values=[]))
        ...
        for m in match.items():
            ...
            if "$" in field and isinstance(deeper_match, list):
                for deep_element in deeper_match:
                    match_values.append(determine_nested(field, deep_element, context, nested_match_values=[]))
            else:
                # simple condition
                value = utils.dict_lookup(context, field)
        ...
        if keys_to_test and (any(isinstance(i, dict) for i in deeper_match.values())):
            for nested_key, match_vals in deeper_match.items():
                ...
                if isinstance(match_vals, dict) and len(match_vals) > 1:
                    ...
                    nested_match_values.append(determine_simple_condition(nested_key, match_vals, context))
