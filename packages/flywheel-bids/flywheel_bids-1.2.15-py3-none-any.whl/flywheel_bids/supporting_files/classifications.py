    ...
    for bids_folder, classificaton_dict in classifications.items():
        for k in classificaton_dict.keys():
            if k.lower() in parent_dir_name.lower():
                return bids_folder
    log.error(f"Could not match modality in classification table with {parent_dir_name}")
...
def search_classifications(bids_folder: str, search_name: str):
    ...
    if matches:
        ...
        log.error(f"Did not find info in the classification table under {bids_folder} for {search_name}")
