...
from flywheel_gear_toolkit.testing.hierarchy import fw_acquisition
...
class TestDownloadBidsDataForRunLevel:
    @patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.list_existing_dirs")
    def test_project_subject_session(self, mock_list_existing_dirs, mock_context):
        ...
        mock_context.download_project_bids.return_value = Path("/whatever/BIDS/dir/should/be/here/bids")
    ...
    @patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.download_bids_dir")
    def test_acquisition(self, mock_download_bids_dir, mock_context):
        ...
        result, err_code = download_bids_data_for_run_level(
            mock_context, "acquisition", mock_hierarchy, mock_folders, mock_src_data, False
        )
    ...
    def test_invalid_run_level(self, mock_context):
        ...
        result, err_code = download_bids_data_for_run_level(mock_context, "invalid", mock_hierarchy, [], None, False)
...
def test_list_existing_dirs_reports_missing_dirs(tmpdir):
    ...
    assert len(result_reqd) == 5  # Total number of suffixes available for curation in Flywheel
...
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.validate_bids", return_value=0)
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.copy_bidsignore_file")
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.download_bids_data_for_run_level")
def test_orchestrate_download_bids_succeeds(mock_download, mock_copybids, mock_validate, mock_tree, mock_context):
    ...
    err_code = orchestrate_download_bids(
        mock_context, hierarchy, tree=True, tree_title="Sycamore", skip_download=True, do_validate_bids=True
    )
    assert err_code == 0
...
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.copy_bidsignore_file")
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.download_bids_data_for_run_level")
def test_orchestrate_download_bids_no_validation(mock_download, mock_copybids, mock_validate, mock_tree, mock_context):
    # Fake a download location
    bids_dir = mock_context.work_dir / "bids"
    Path(bids_dir).mkdir(parents=True, exist_ok=True)
    mock_download.return_value = (bids_dir, 0)
    err_code = orchestrate_download_bids(
        mock_context, hierarchy, tree=True, tree_title="Sycamore", skip_download=True, do_validate_bids=False
    )
    assert err_code == 0
    mock_download.assert_called_once()
...
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.tree_bids")
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.validate_bids", side_effect=Exception)
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.copy_bidsignore_file")
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.download_bids_data_for_run_level")
...
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.copy_bidsignore_file")
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.download_bids_data_for_run_level")
def test_orchestrate_download_bids_no_data(mock_download, mock_copybids, mock_validate, mock_tree, mock_context):
    # Fake a download location
    bids_dir = mock_context.work_dir / "bids"
...
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.copy_bidsignore_file")
@patch("flywheel_bids.flywheel_bids_app_toolkit.utils.query_flywheel.download_bids_data_for_run_level")
def test_orchestrate_download_bids_api_exception(mock_download, mock_copybids, mock_validate, mock_tree, mock_context):
    # Fake a download location
    bids_dir = mock_context.work_dir / "bids"
    Path(bids_dir).mkdir(parents=True, exist_ok=True)
