...
@pytest.mark.parametrize("bids_app_command", [None, "arg1 arg2 --my_arg aa1 aa2", "--work-dir"])
...
def test_run_bids_algo(mock_exec_command, mock_log_info, mock_path_mkdir, mock_path_exists, extended_gear_context):
    ...
    mock_log_info.assert_called_once_with("Creating output directory %s", "/path/to/analysis_output_dir")
    mock_exec_command.assert_called_once_with([*command], dry_run=False, shell=True, cont_output=False)
