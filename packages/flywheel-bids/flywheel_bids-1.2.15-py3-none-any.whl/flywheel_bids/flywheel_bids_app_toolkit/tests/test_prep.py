...
from flywheel_gear_toolkit.testing.hierarchy import fw_project

from flywheel_bids.flywheel_bids_app_toolkit.prep import get_bids_data, set_participant_info_for_command
...
@patch("flywheel_bids.flywheel_bids_app_toolkit.prep.orchestrate_download_bids", return_value=0)
@patch("flywheel_bids.flywheel_bids_app_toolkit.prep.get_analysis_run_level_and_hierarchy")
def test_get_bids_data(mock_hierarchy, mock_orchestrate, mock_context, mock_client):
    ...
    result, errors = get_bids_data(mock_context, mock_hierarchy, bids_suffixes, tree_title)
