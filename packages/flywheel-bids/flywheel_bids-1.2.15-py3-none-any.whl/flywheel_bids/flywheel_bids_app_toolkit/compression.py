...
def change_to_relative_dir(results_dir: Path, results_html_filename: Path):
...
def unzip_archive_files(gear_context: GearToolkitContext, archive_key: str, unzip_dir: Path = None):
    ...
    Args:
        ...
        unzip_dir (Path): target destination path for the unzipped archive
    Returns:
...
def update_report_refs(
    ...
    tags (List): image or reportlet tags that need to be updated in the html source code
    ...
    for tag in tags:
        ...
        if svg_name and (bids_suffix and bids_suffix in svg_name) or (regex.match(str(results_html_filename.name))):
    ...
    log.debug(f"Updating tags: {[t[source] for t in tags if t not in orig_tags]}")
    return soup
    ...
    if Path(results_dir).exists():
        ...
        if unzipped_htmls:
            for html in unzipped_htmls:
                try:
                    zip_html_and_svg_files(results_dir, Path(html.name), destination_id, output_dir)
...
def search_for_html_report_files(folder_path: Path) -> List:
    ...
    html_files = []
    exclusion_substrings = ["citation"]
    for file in list(Path(folder_path).rglob("*.html")):
        if not any(sub in str(file.name).lower() for sub in exclusion_substrings):
...
def zip_html_and_svg_files(results_dir, results_html_filename: Path, destination_id, output_dir):
    """
    ...
    :param results_html_filename (Path): PosixPath; access just the base name describing the
            zipped result; e.g., sub-TOME3024_ses-Session2_acq-MPRHA_T1w.html with results_html_filename.name
    ...
    results_dir, results_html_filename = change_to_relative_dir(results_dir, results_html_filename)
    ...
    for links, v in search_dict.items():
        soup = update_report_refs(
            soup, results_html_filename, results_dir, zip_file, bids_suffix, link_type=links, source=v["source"]
        )
