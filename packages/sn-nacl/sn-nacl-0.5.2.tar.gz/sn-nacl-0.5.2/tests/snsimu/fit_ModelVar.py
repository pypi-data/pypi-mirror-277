...
class SModel(object):
    ...
    def theta_init(self, xdata, plot = False):
        ...
        if plot:
            ...
            pl.plot(data['Date'], jacobian*factor(J.T * y), 'k.')
            pl.show()
        return factor(J.T * y)
...
#data = generate_Lc(N = 100, sigma_err = 2, lab = lab)
def fit_sqp_lin(data, max_iter = 10, ll = 0.1, dchi2_min = 0.00001, reml = False, lab = 'cst', plot = True):
    """
    ...
    p = model.pars.free
    g = np.zeros_like(var_model.pars.free)#var_model.pars.free
    dp = np.zeros(p.shape)
    dg = np.zeros(g.shape)
...
def control(real = 10, N = 10, sigma_err = 2., variance_sn = 0.01, lab = 'cst', dbin = 0.01, plot = False):
    """
    ...
    for i in range(real):
        ...
        p0, g0, model, var_model = fit_sqp_lin(data, max_iter = 20, lab = lab, plot = plot)
        g.append(g0)
...
def outlier(nsig, nout = 10, sout = 10., dchi2 = 0.01,  N = 10, max_iter = 20, lab = 'dep'):
    n_old = 0
    n_new = 0
    data = generate_Lc(N = N, sigma_err = 2., variance_sn = 0.1, lab = lab)

    x = data['Date']
    y = data['Flux']
    sig = data['FluxErr']
    
    out = np.random.randint(15, N * 10, nout)
    print('\n',out,'\n')
    data['Flux'][out] += np.random.normal(0, sout, nout)
    ...
    for i in range(10):
        ...
        p, g, mod, var_mod = fit_sqp_lin(data[~bads], lab = 'dep', dchi2_min = dchi2, max_iter = max_iter, plot = False)
        ...
        for i in np.unique(data['id']):
            ...
            id_min = np.where(data['Flux'] == d['Flux'][0])[0]
            id_max = np.where(data['Flux'] == d['Flux'][len(d)-1])[0]
            indix_id = np.where(data['id'] == i)

            idx_mod = np.array([i for i in out if id_min <= i <= id_max])
            idx_out = np.array([i for i in np.where(bads)[0] if id_min <= i <= id_max])
            print(out)
            print('m=', idx_mod)
            print('o=', idx_out)
            
            #idx_mod = np.where(d['Date'].min() < out < d['Date'].max()) 
            #idx_out = np.where(d['Date'].min() < np.where(bads) < d['Date'].max())
            ...
            ax1.errorbar(d['Date'], d['Flux'], yerr = d['FluxErr'], marker='.', ls='', color=pl.cm.jet(i**2 + 10 *i))
            if len(idx_mod) != 0:
                ax1.errorbar(data['Date'][idx_mod], data['Flux'][idx_mod], marker='o', ls='', color=pl.cm.jet(i**2 + 10 *i))
            if len(idx_out) != 0:
                ax1.errorbar(data['Date'][idx_out], data['Flux'][idx_out], marker='*', ls='', color='r')#pl.cm.jet(i**3 + 10 *i))
            ax1.plot(d_sn['Date'], v, color=pl.cm.jet(i**2 + 10 *i), ls='-', label = str(i), linewidth =0.5)
        #ax1.errorbar(data['Date'][bads], data['Flux'][bads], marker='*', ls='', color='r')#pl.cm.jet(i**3 + 10 *i))
            #if lab == 'cst':
                #ax1.plot(d_sn['Date'], v + 3*(np.sqrt(g[i]**2 + err)), color=pl.cm.jet(i**2 + 10 *i), ls='--', linewidth = 0.5)
                #ax1.plot(d_sn['Date'], v - 3*(np.sqrt(g[i]**2 + err)), color=pl.cm.jet(i**2 + 10 *i), ls='--', linewidth = 0.5)
            #else :
            ...
            ax2.plot(d['Date'], d['Flux']-v,'k.')
        #ax1.plot(x[out], y[out], 'ro', label = 'modified (%i)' % (nout))
        #print(x[out], y[out])
        #ax1.plot(x[bads], y[bads], 'b*', label = 'outliers (%i)' % (len(bads[bads])))
        ax1.legend()
        
        res = val - y
        variance = np.diag(Var) + sig**2
        w = np.diag(1/variance)
        ndof = N - len(p) -len(g)

        #chi2 = (np.diag(w) * res**2 + len(res) * np.log(np.pi) + np.log(np.diag(w)))/ (len(data) - (p.shape[0] + g.shape[0]))
        ...
        supress = []
        for i in range(N) :
            idxx = np.where(data['id'] == i)
            if i == 0:
                for ij in idxx[0]:
                    supress.append(ij)
            else :
                old_bads_sn = bads[idxx]
                new_bads_sn = [np.abs(chi2[idxx]) > np.abs(nsig ** 2 * med_chi2)][0]  #####################"

                old_and_new = old_bads_sn & new_bads_sn
                
                for i in range(len(old_bads_sn)):
                    if ~old_bads_sn[i] & new_bads_sn[i]:
                        new_bads_sn[i] = True
                    else:
                        new_bads_sn[i] = False

                if np.where(np.abs(chi2[idxx]) == np.max(np.abs(chi2[idxx])))[0][0] in np.where(old_and_new)[0]:
                    try :
                        maxi = np.max(np.abs(chi2[idxx][new_bads_sn]))
                    except :
                        maxi = np.max(np.abs(chi2[idxx]))
                else :
                    maxi = np.max(np.abs(chi2[idxx]))

                print(maxi, chi2[idxx])
                nb_bads = len(new_bads_sn[new_bads_sn])
                if nb_bads == 0:
                    continue
                else:
                    if len(new_bads_sn[new_bads_sn]) == 1:
                        continue
                    elif len(new_bads_sn[new_bads_sn]) > 1:
                        idx_supress = np.where(np.abs(chi2) < maxi)

                    for ii in idxx[0]:
                        if ii in idx_supress[0]:
                            if ii not in np.where(old_bads_sn)[0]: #ne pas supprimer un ancien passer en dessous d'un nouveau
                                print(idxx, ii)
                                supress.append(ii)
                    #supress.append([ii for ii in idx_supress if ii in idxx[0]])

                
        #bads = chi2 > nsig ** 2 * med_chi2
        supress = np.array(supress)
        #print(supress)
        bads = np.abs(chi2) > np.abs(nsig ** 2 * med_chi2) ######################
        #print(np.where(bads))
        bads[supress] = False
        #print(np.where(bads))
        #34(2324)
        ...
        if len(old_bads_sn[old_bads_sn]) != 0 :
            if len(new_bads_sn[new_bads_sn]) == 0:
                stop = True
