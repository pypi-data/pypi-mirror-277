...
class WModel(object):
    ...
    def __call__(self, p):
        ...
        xdata = self.xdata
        if len(p) == 4:
            return np.diag(1/(p[3] + p[2] * xdata + p[1] * xdata**2 + p[0] * xdata**3))
        elif len(p) == 1:
            return np.diag(1/(p[0]*np.ones(xdata.shape)))
        elif len(p) == 2:
            return np.diag(1/(p[1] + p[0] * xdata))

# minimization function of the polynomial dependance

def fit_sqp_lin(y, x, variance, n_gamma = 2, max_iter = 100, ll = 0.1, dchi2_min = 0.0001, reml = False, plot = True):
    """
    ...
    chi2 = (np.log(2 * np.pi) * len(res)  + np.linalg.multi_dot([res.T,w,res]).sum() + np.log(np.diag(w)).sum())/ (len(y) - (p.shape[0] + g.shape[0]))
    ...
    while niter < max_iter:
        ...
        var = np.linalg.inv(H_l.toarray() + ll * np.ones(H_l.toarray().shape)).dot(resul) ###############np.linalg.inv(H_l.toarray()).dot(resul)
        dp = var[:p.shape[0]]
        dg = var[p.shape[0]:]
        ...
        new_chi2 = (np.log(2 * np.pi) * len(res)  + (res**2).sum() + np.log(np.diag(w)).sum())  / (len(y) - (p.shape[0] + g.shape[0])) #+ np.log(np.linalg.det(w))
    ...
    if plot :
        ...
        pl.errorbar(xx, p[0] * xx + p[1], yerr = np.sqrt(variance), ls = '--', color='r', linewidth = 2, label = 'real')
        if n_gamma == 1:
            pl.errorbar(xx, p[0] * xx + p[1], yerr = np.sqrt(g[0]), ls = '-.', color='b', label='simu')
            pl.text(6,11, ' n_iter : %i (dchi< %f) \n\n p0 : %f, p1: %f \n g0 : %f' % (niter,dchi2_min,p[0],p[1],g[0]), fontsize= 15)
        if n_gamma == 2:
            pl.errorbar(xx, p[0] * xx + p[1], yerr = np.sqrt(g[0] * xx + g[1]), ls = '-.', color='b', label='simu')
            pl.text(6,12, ' n_iter : %i (dchi< %f) \n\n p0 : %f, p1: %f \n g0 : %f, g1: %f' % (niter,dchi2_min,p[0],p[1],g[0],g[1]), fontsize= 15)
        if n_gamma == 4:
            pl.errorbar(xx, p[0] * xx + p[1], yerr = np.sqrt(g[0] * xx**3 + g[1] * xx**2 + g[2] * xx + g[3]), ls = '-.', color='b', label='simu')
...
class WdepModel(object):
    ...
    def para_init(self):
        ...
        fp = FitParameters(fp)
        fp['gamma0'] = 1.15 #0.015
        fp['gamma1'] = 0.45 #0.0045
        return fp
...
def fit_lin_var(y, x, variance, max_iter = 100, ll = 0.1, dchi2_min = 0.1, plot = True):
    """
    Minimization of the chi2.
...
def outlier_spq_lin(x, y, var, nsig = 5., n_gamma = 1, max_iter = 50, plot = True):
    n_old = 0
    n_new = 0

    bads = np.ones(x.shape) != np.ones(x.shape)

    for i in range(100):
        if n_old != n_new:
            n_old = n_new
        if type(n_gamma) == int :
            p, g, mod, mod_var = fit_sqp_lin(y[~bads], x[~bads], var[~bads], max_iter = max_iter, n_gamma = n_gamma, plot = False)
        else :
            p, g = fit_lin_var(y[~bads], x[~bads], var[~bads], max_iter = max_iter, ll = 0.1, dchi2_min = 0.00001, plot = False)
        ...
        chi2 = (np.log(2 * np.pi) * len(res)  + np.diag(w)*res**2 + np.log(np.diag(w)).sum())  / (len(y[~bads]) - (p.shape[0] + g.shape[0]))
        #chi2 = (np.log(2 * np.pi) * len(res)  + np.diag(w)*res**2)  / (len(y[~bads]) - (p.shape[0] + g.shape[0]))
        ...
        if plot: 
            ...
            if n_gamma == 1:
                pl.errorbar(x, p[0] * x + p[1], yerr = np.sqrt(g[0]), ls = '--', color='r', label = 'fit')#(a = %f, b = %f, v = %f)' % (p[0],p[1],g))
            elif n_gamma == 4:
                pl.errorbar(x, p[0] * x + p[1], yerr = np.sqrt(g[0] * x**3 + g[1] * x**2 + g[2] * x + g[3]), ls = '--', color='r', label = 'fit')
            else :
                if n_gamma == 2:
                    pl.errorbar(x, p[0] * x + p[1], yerr = np.sqrt(g[0] * x + g[1]), ls = '--', color='r', label = 'fit')
                else :
                    pl.errorbar(x, p[0] * x + p[1], yerr = np.sqrt(g[0] * (p[0] * x + p[1]) + g[1]), ls = '--', color='r', label = 'fit')
            #pl.errorbar(x, p[0]*x+p[1], yerr = g*np.ones(x.shape), color = 'r', fmt = '--', label = 'fit')#(a = %f, b = %f, v = %f)' % (p[0],p[1],g))
            pl.plot(x,1.235*x+9.123,'g--', label = 'real')#(a = %f, b = %f, v = %f)' % (1.235, 9.123, 1.35))
            pl.plot(x[bads], y[bads], 'b*', label = 'outlier')
            pl.legend()

        3442(234)  
        bads = chi2 > nsig ** 2 * med_chi2
        n_new = len(bads[bads])
        print('\n', n_new, n_old, '\n')
...
def control(N, n_gamma, npts = 100, max_iter = 100, outliers = False, dchi2_min = 0.00001, dbin = 0.001, plot = True, nsig = 1.001, sigchi2 = 5., out = 10.):
    """
    ...
    Input : - N  = int, number of realisation of the fit.
            ...
            - dbin = float, interval of histograms binning                               !!! Often needed to be change !!! 
    ...
    if type(n_gamma) == int : 
        ...
        for i in range(N): #start
            c, g, nout = outlier(nsig = sigchi2, nout = out, sout = 10., N = 1000, ng = n_gamma, max_iter = 20, var_fit = True, plot = False)
            coef[i] = c
            gammas[i] = g
            N_out[i] = nout
        ...
        gammas = np.zeros((N, 2))
        for i in range(N):
            c, g, nout = outlier(nsig = sigchi2, nout = out , sout = 10., N = 1000, ng = 'dep', max_iter = 20, var_fit = True, plot = False)
            coef[i] = c
            gammas[i] = g
            N_out[i] = nout
    """
    if type(n_gamma) == int :
        ...
        for i in range(N):
            ...
            if outliers :
                c, g = outlier_spq_lin(x, y, var, nsig = nsig, n_gamma = n_gamma, max_iter = max_iter, plot = False)
            else :
        ...
        for i in range(N):
            ...
            if outliers :
                ...
                c, g = fit_lin_var(y, x, var, max_iter = max_iter, ll = 0.1, dchi2_min = dchi2_min, plot = False)
                coef[i] = c
                gammas[i] = g
    ...
    if plot :
        ...
        rg = (yy.min(), yy.max())
        nbins = int((rg[1] - rg[0])/ (dbin*0.5))
        xh, yh, hh = pl.hist(coef.T[0], bins = nbins)#, range = (a-1,a+1))
        meda = np.median(coef.T[0])
        moya = coef.T[0].mean()
        pl.plot(a*np.ones((10)), np.linspace(0, xh.max(), 10), 'r--')
        pl.plot(meda * np.ones((10)), np.linspace(0, xh.max(), 10), 'b--')
        pl.plot(moya * np.ones((10)), np.linspace(0, xh.max(), 10), 'k--')
        pl.title('%i simu de %i npts: a = %f (=> simu moy = %f med = %f)' % (N, npts, a, moya, meda)) #coef.T[0].mean()))
        pl.savefig('pic0/%i_simu_%i_points_a_%i_out_%i_sigchi2_%i_ngam.png' % (N, npts, out, sigchi2, n_gamma)) 
        
        pl.figure()
        xx, yy = np.histogram(coef.T[1])
        rg = (yy.min(), yy.max())
        nbins = int((rg[1] - rg[0])/ (dbin*0.1))
        xh, yh, hh = pl.hist(coef.T[1])#, bins = nbins)#, range = (b-1,b+1))
        medb = np.median(coef.T[1])
        moyb = coef.T[1].mean()
        pl.plot(b*np.ones((10)), np.linspace(0, xh.max(), 10), 'r--')
        pl.plot(medb*np.ones((10)), np.linspace(0, xh.max(), 10), 'b--')
        pl.plot(moyb*np.ones((10)), np.linspace(0, xh.max(), 10), 'k--')
        pl.title('%i simu de %i npts : b = %f (=> simu moy = %f med = %f)' % (N, npts, b, moyb, medb))
        pl.savefig('pic0/%i_simu_%i_points_b_%i_out_%i_sigchi2_%i_ngam.png' % (N, npts, out, sigchi2, n_gamma))
        ...
        medg = np.zeros((n_gamma))
        moyg = np.zeros((n_gamma))
        for n in range(len(realg)):
            pl.figure()

            xx, yy = np.histogram(gammas.T[n])
            rg = (yy.min(), yy.max())
            nbins = int((rg[1] - rg[0])/ (dbin*10))
            ...
            pl.title('%i simu de %i npts : gamma_%i = %f (=> simu moy = %f med = %f)' % (N, npts, n, realg[n], moy, med))
            pl.savefig('pic0/%i_simu_%i_points_gamma%i_%i_out_%i_sigchi2_%i_ngam.png' % (N, npts, n, out, sigchi2, n_gamma))

            """
            pl.figure()
            pl.plot(N_out, np.abs(realg[n] - gammas.T[n]), 'b+')
            plottools.binplot(N_out.reshape(1, N)[0], np.abs(realg[n] - gammas.T[n]), nbins = len(np.unique(N_out.reshape(1, N)[0])+1), marker='p', color='Darkblue')
            pl.title('%i simu of  %i npts (with nsig : %f) : gamma_%i : %f ' %(N, npts, 3., n, realg[n]))
            pl.xlabel('number of outliers')
            pl.ylabel('residuals')
            pl.savefig('RES_%i_simu_%i_points_%i_out_%i_sigchi2_%i_ngam.png' % (N, npts, out, sigchi2, n_gamma))
            """
    return coef, gammas, moya, meda, moyb, medb, moyg, medg

def control_outlier(N = 200):
    NOUT = np.arange(0,5)*5
    for ng in [1, 2]:
        if ng == 1:
            realg = np.array([1.356])
        elif ng == 2:
            realg = np.array([1.325, 2.2222])
        Moyg = np.zeros((ng,len(NOUT)))
        Medg = np.zeros((ng,len(NOUT)))
        for npts in [100,250,500,1000]:
            for sc2 in [3,4,5,6]:
                comp = 0
                for out in NOUT:
                    coef, gammas, moya, meda, moyb, medb, moyg, medg = control(N = N , n_gamma= ng, npts = npts, outliers = True, sigchi2 = sc2, out = out)
                    for ii in range(ng):
                        Moyg[ii, comp] = moyg[ii]
                        Medg[ii, comp] = medg[ii]
                        
                    comp += 1

                for i in range(len(moyg)):
                    pl.figure()
                    pl.plot(NOUT, np.abs(realg[i] - Moyg[i]), 'b+')
                    pl.plot(NOUT, np.abs(realg[i] - Medg[i]), 'r+')
                    #plottools.binplot(N_out.reshape(1, N)[0], np.abs(realg[n] - gammas.T[n]), nbins = len(np.unique(N_out.reshape(1, N)[0])+1), marker='p', color='Darkblue')
                    pl.title('%i simu of  %i npts (with nsig : %f) : gamma_%i : %f ' %(N, npts, sc2, i, realg[i]))
                    pl.xlabel('number of outliers')
                    pl.ylabel('residuals')
                    pl.savefig('pic0/RES_ng%i_%i_simu_%i_points_%i_out_%i_sigchi2_%i_ngam.png' % (i, N, npts, out, sc2, ng))


def control_moy(n_gamma, N = 500, nb_pts  = [50, 100, 250, 500, 1000], max_iter = 10, dbin = 0.01, plot = True, outliers = False):
    """
    ...
    for nn in range(len(nb_pts)):
        c, g = control(N = N, max_iter = max_iter, n_gamma = n_gamma, npts = nb_pts[nn], dbin = dbin, plot = plot, outliers = outliers)
        A[nn] = c.T[0].mean()
        B[nn] = c.T[1].mean()
...
# fit without outlier, new fit function without variance fitting
# VARIANCE MODEL PARAMETERS MUST BE FIXED
def outlier(nsig, nout = 10, sout = 10., N = 1000, ng = 1, max_iter = 20, var_fit = True, plot = True):
    n_old = 0
    n_new = 0
    x, y, var = gen_var(N = N, ng = 1)

    out = np.random.randint(0, N, nout)
    y[out] += np.random.normal(0, sout, nout)
    bads = np.ones(x.shape) != np.ones(x.shape)
    ...
    for	i in range(10):
        ...
        if var_fit :
            if type(ng) == int :
                p, g, mod, mod_var = fit_sqp_lin(y[~bads], x[~bads], var[~bads], max_iter = max_iter, n_gamma = ng, plot = False)
            else :
                p, g = fit_lin_var(y[~bads], x[~bads], var[~bads], max_iter = max_iter, ll = 0.1, dchi2_min = 0.00001, plot = False)                
        else :
            p, g, mod, mod_var = fit_outlier(y[~bads], x[~bads], var[~bads], n_gamma = ng, plot = False)        
        ...
        if ng == 1:
            ...
            realg = np.array([1.2, 0.5])

        realsigma = np.zeros_like(x)
        nbrealg = len(realg)
        for i in range(nbrealg):
            if type(ng) == int :
                realsigma += realg[i]*x**(nbrealg-(i+1))
            else :
                realsigma += realg[i]*(p[0]*x+p[1])**(nbrealg-(i+1))
        if plot :
            pl.figure()
            pl.plot(x,y,'k.')
            #pl.errorbar(x,p[0]*x+p[1], color = 'r', fmt = '--', label = 'fit(a = %f, b = %f)' % (p[0],p[1]))
            #pl.errorbar(x,1.235*x+9.123, color = 'g', fmt = '--', label = 'fit(a = %f, b = %f)' % (1.235, 9.123))
            pl.errorbar(x,p[0]*x+p[1], yerr = np.sqrt(sigma), color = 'r', fmt = '--', label = 'fit(a = %f, b = %f)' % (p[0],p[1]))
            pl.errorbar(x,1.235*x+9.123, yerr = np.sqrt(realsigma), color = 'g', fmt = '--', linewidth = 0.5, label = 'fit(a = %f, b = %f)' % (1.235, 9.123))
            pl.plot(x[out], y[out], 'ro', label = 'modified (%i)' % (nout))
            pl.plot(x[bads], y[bads], 'b*', label = 'outliers (%i)' % (len(bads[bads])))
            pl.title('variance parameters = ' + str(realg) + ' => fit : '+ str(g))
            pl.legend()
        res = p[0]*x+p[1] - y 
        ndof = N - len(p)- nbg
        chi2 = res**2/ndof
...
def fit_outlier(y, x, variance, n_gamma = 2, max_iter = 100, ll = 0.1, dchi2_min = 0.0001, reml = False, plot = True):
    """
    Minimization of the chi2.
    ...
    chi2 = (np.log(2 * np.pi) * len(res)  + (res**2).sum() + np.log(np.diag(w)).sum() ) / (len(y) - (p.shape[0] + g.shape[0]))
    ...
    while niter < max_iter:
        ...
        H_l, resul = construction(H = H, J = J, n_gamma = n_gamma, g = g, reml = reml)
        var = np.linalg.inv(H_l.toarray() + 0.1 * np.ones(H_l.toarray().shape)).dot(resul) ###############np.linalg.inv(H_l.toarray()).dot(resul)
        dp = var[:p.shape[0]]
        dg = 0 #var[p.shape[0]:]
        ...
        w = np.diag(np.ones(len(y))) #########################
        res = w * (y- v)
        new_chi2 = (np.log(2 * np.pi) * len(res)  + (res**2).sum() + np.log(np.diag(w)).sum() )  / (len(y) - (p.shape[0] + g.shape[0])) 
        dchi2 = new_chi2 - chi2
        logging.info(' red chi2: %f -> %f [dchi2=%f]' % (chi2, new_chi2, dchi2))
        chi2 = new_chi2
        if niter >= 1 :
