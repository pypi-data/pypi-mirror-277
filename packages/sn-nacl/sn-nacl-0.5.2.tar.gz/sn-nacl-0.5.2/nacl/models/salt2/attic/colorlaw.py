...
class ColorLaw:
    r"""Implementation of the SALT2 color law (version 1)
        ...
        The SALT2 color law describes the color diversity of SNeIa. In SALT
        2.4, the color law is parametrized as follows:

        .. math::
            CL(\lambda;c) = 10^{0.4\ c \times P(\lambda_r)}

        where :math:`P(\lambda_r)` is a polynomial, such as :math:`P(\lambda_B)
        = 0` and :math:`P(\lambda_V) = 1`.
    ...
    def reduce(self, wl):
        """Linear remapping of the wavelength passed in argument.
