...
from nacl.models.salt2.variancemodels import LocalErrorSnake
from nacl import TrainingDataset, LogLikelihood
...
def test_deriv(vm, pars, block_name='M0'):
    """
    """
    pars.fix()
    pars.release(block_name)

    J, Jn = helpers.check_grad(vm, pars, dx=1.E-7)
...
if __name__ == '__main__':
    ...
    vm = LocalErrorSnake(model)
    ll = LogLikelihood(model, variance_model=vm)

    for block_name in ['X0', 'X1', 'col', 'tmax', 'M0', 'M1', 'CL', 'gamma']:
        logging.info(f'testing: {block_name}')
        test_deriv(vm, ll.pars, block_name)

    ll.pars.fix()
    ll.pars.release('M0')
    J, Jn = helpers.check_grad(vm, ll.pars, dx=1.E-7)
    ...
    ll.pars.fix()
    ll.pars.release('M1')
    J, Jn = helpers.check_grad(vm, ll.pars)
    fig.suptitle('M1')
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(16,5),
                             sharex=True, sharey=True)
    axes[0].imshow(J, aspect='auto', vmin=-0.1, vmax=0.1)
