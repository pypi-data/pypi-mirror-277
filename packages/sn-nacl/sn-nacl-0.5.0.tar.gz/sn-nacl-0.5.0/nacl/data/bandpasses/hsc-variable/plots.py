...
def plot_transmissions(band, axes=None, title='', legend=False):
    ...
    axes.set_xlabel('$\lambda [\AA]$') #  fontsize=18)
    axes.set_ylabel('$T(\lambda)$') # fontsize=18)
    fig = pl.gcf()
    fig.suptitle(title)
    if legend:
        pl.legend(loc='best')
...
def all_plots():
    ...
    plot_wave_eff_vs_rad('hsc::g',  axes=axes[0,0], ylabel='$\delta\lambda/\lambda$', title='g')
    ...
    plot_wave_eff_vs_rad('hsc::z',  axes=axes[1,0], xlabel='rad [cm]', ylabel='$\delta\lambda/\lambda$', title='hsc::z')
    plot_wave_eff_vs_rad('hsc::y',  axes=axes[1,1], xlabel='rad [cm]', title='hsc::y')
    pl.setp(axes[1,2], visible=False)
    fig.suptitle('HSC filter uniformity ($\lambda$)')


    # mean wavelength versus r
    fig, axes = pl.subplots(nrows=2, ncols=3, figsize=(12,8), sharex=True, sharey=True)
    plot_throughput_vs_rad('hsc::g',  axes=axes[0,0], ylabel='$\delta \int T/ \int T$', title='g')
    ...
    plot_throughput_vs_rad('hsc::z',  axes=axes[1,0], xlabel='rad [cm]', ylabel='$\delta\int T/\int T$', title='hsc::z')
    plot_throughput_vs_rad('hsc::y',  axes=axes[1,1], xlabel='rad [cm]', title='hsc::y')
    pl.setp(axes[1,2], visible=False)
    fig.suptitle('HSC filter uniformity ($\int T$)')
