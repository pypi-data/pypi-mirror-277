...
class ArticleAnalyst(ArticleSet):
    ...
    def getCit(self, saveAttr="cit", inArticles=False, simplified=True):
        """
...
if __name__ == '__main__':
    articleAnalyst = ArticleAnalyst(
        dataSource="file", filePath="./download/test/HCM_major.json"
    )
    # articleAnalyst = ArticleAnalyst(
    #     dataSource="query", query="HCM", save=True
    # )
    ...
    # ArticleAnalyst NER test
    # articleAnalyst.dropNa(inplace=True, subset=["TI", "AB", "MH"])  # drop article without MH, TI and AB
    # targetArticleAnalyst = articleAnalyst.search(Year=[(">", 2020)])
    # NERResults = articleAnalyst.returnNERResults(
    #     method="xiaoXiang", methodKwargs=dict(userName="1031033824521637888", pwd="FJSWD9Vp"),
    #     url="http://bern2.korea.ac.kr/plain"
    # )
    # # NERResultJsonFile = os.path.join(downloadPath, "../refData/XiZangCao_6.NERResult.json")
    # # with open(NERResultJsonFile, "r") as f:
    # #     NERResults = json.load(f)
    # targetArticleAnalyst.articles.apply(lambda article: article.setDocument())
    # G, allNodeList = articleAnalyst.returnNERGraph(
    ...
    #     minEdgeCount=3,
    #     minArticleCount=3,
    #     equalInfo=[["dl-3-n-butylphthalide", "NBP", "dl-NBP"]]
    # )
