...
from .nhxx_tools import get_nhx
...
def _try_recon(Tg, Ts, Ts_triples, root, gene_attr, species_attr, event_attr, fix_inconsistencies):
    ...
    #print('\n\n\n---------------------------')
    #print(get_nhx(Tg, 0, name_attr= 'accession', ignore_attrs= ['species']))
    #print('--------------------------\n\n\n')
...
def reconciliate_many(gTrees,
    ...
    recons= [(idx,) + _try_recon(Tg,
                                 ...
                                 event_attr,
                                 fix_inconsistencies)
             for Tg,idx in tqdm(zip(gTrees, gTrees.index),total= gTrees.shape[0])]
...
def recon_table_to_str(Ts, df_recs, orthogroup_column= 'OG'):
    ...
    df_r= pd.DataFrame(dict(tree= df_recs.reconciliated_tree.apply( lambda x: get_nhx(x, name_attr= 'accession') ),
                            ...
                            flipped_nodes= df_recs.apply(lambda row: flipped_to_str(row.flipped_nodes,
                                                                                    row.reconciliated_tree
                                                                                   ) , axis= 1)
                           ))
    df_r[orthogroup_column]= df_recs[orthogroup_column]

    return df_r[[orthogroup_column, 'tree', 'reconciliation_map', 'flipped_nodes']]
