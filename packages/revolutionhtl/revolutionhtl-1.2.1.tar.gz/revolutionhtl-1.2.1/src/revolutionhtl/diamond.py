...
def diamond_all_vs_all(fastas_dir,
    ...
    # Create file names
    species= {file:file.split('.')[0] for file in fasta_files}
    ...
    # Run pairwise
    n= len(fasta_files)
    for query ,target in tqdm(combinations(fasta_files, 2), total= int(n*(n-1)/2),
...
def diamond_makedb(diamond, fpath, f_reference, quiet= False):
    command= [diamond, 'makedb', '--in',
              fpath, '-d', f_reference]
    if quiet:
        command+= ['--quiet']
    return run(command)

def diamond_blastp(diamond, fpath, f_reference, f_out, quiet= False, evalue= 0.00001, identity= 25, cov= 50, max_target_seqs= 25):
