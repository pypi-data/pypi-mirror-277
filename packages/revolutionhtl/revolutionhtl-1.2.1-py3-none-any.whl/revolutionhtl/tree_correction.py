...
def correct_tree(Tg, Rs, root= 1,
    ...
    if algorithm=='prune_R':
        ret= prune_triples(Tg, R_I, force_phylogeny= force_phylogeny, update_lca= update_lca, inplace= inplace, root= root)
    elif algorithm=='prune_L':
        ret= prune_leaves(Tg, R_C, R_I, force_phylogeny= force_phylogeny, update_lca= update_lca, inplace= inplace, root= root)
    else:
