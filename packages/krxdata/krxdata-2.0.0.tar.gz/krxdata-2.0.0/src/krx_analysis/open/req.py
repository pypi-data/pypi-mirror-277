...
# ============================================================ Request.
def get_OTP(qs, **kw):
    url = f'{URI}/contents/COM/GenerateOTP.jspx'
    p_err = re.compile(r'function|html|<title>404\s*에러\s*페이지</title>')
    try:
        r = requests.get(url, params=qs, headers=_headers(), **kw)
    except Exception as e:
        dbg.exception(__file__, e)
        return None
    else:
        m = p_err.search(r.text)
        if m is None:
            if len(r.text) == 0:
                dbg.exception(__file__, "get_OTP", "len(r.text) == 0")
                return None
            else:
                return r.text.strip()
        else:
            dbg.exception(__file__, "get_OTP", f"m: {m}")
            return None


def post_data(data, **kw):
    url = f"{URI}/contents/OPN/99/OPN99000001.jspx"
    try:
