...
from .models import CameFeature, CameServerInfo, CameUser, CameLight, CameUpdates
...
class CameDomoticAPI:
    ...
    async def async_get_server_info(self) -> CameServerInfo:
        ...
        return CameServerInfo(
            ...
            features=[CameFeature(name) for name in json_response["list"]],
        )
    ...
    async def async_get_updates(self) -> CameUpdates:
        ...
        response = await self.auth.async_send_command(payload)
        json_response = await response.json(content_type=None)
        return CameUpdates(json_response)
