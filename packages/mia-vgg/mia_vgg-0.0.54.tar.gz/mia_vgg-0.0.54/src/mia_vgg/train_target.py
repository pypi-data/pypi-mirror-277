...
def train(task, fold, dtype):
    ...
    if dtype == "real":
        ...
        trainloader = DataLoader(celeba, batch_size=100, sampler=train_subsampler)

    if dtype == "synth":
        data = SyntheticCeleba(root=f"data/synthetic/{fold}", transform=transform)
        trainloader = DataLoader(celeba, batch_size=100)

    with open(Path(path, "test.pickle"), 'rb') as f:
        test_ids = pickle.load(f)
    test_subsampler = torch.utils.data.SubsetRandomSampler(test_ids)
    testloader = DataLoader(celeba, batch_size=100, sampler=test_subsampler)
    ...
    path = Path("result", "target", str(fold))
    ...
    #Save trained model
    torch.save(model.state_dict(), f"result/target/{fold}/target_weights")

    mia_size = min(200*nombre_batch, int(0.2*len(celeba)))
