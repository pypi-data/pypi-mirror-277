...
class ParentPara(Para):
    a = P(name='A', default=100)
    b = P(name='B', required=True, vmin=200, vmax=400)
...
class WithCompute(Para):
    a = P('A', default=100)
    b = P('B', default=lambda para: para.a * 2, vmin=200, vmax=400)
...
class MyTestCase(unittest.TestCase):
    ...
    def test_subclass(self):
        self.assertListEqual(list(ChildPara._items.keys()), ['a', 'b', 'c'])

    def test_mixin(self):
        self.assertListEqual(list(ChildParaByMixin._items.keys()), ['a', 'b', 'c'])
    ...
    def test_init_and_validation(self):
        ...
        with self.assertRaises(MissingRequiredParaItem):
            para3 = ParentPara()

        with self.assertRaises(ValueOutOfBound):
