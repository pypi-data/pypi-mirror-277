...
def encoder(ps: FSharpList[Process]) -> Json:
    ...
    def _arrow1542(oa: Source, ps: Any=ps) -> Json:
        return Source_ISAJson_encoder(oa)

    def _arrow1543(oa_1: Sample, ps: Any=ps) -> Json:
        return Sample_ISAJson_encoder(oa_1)

    def _arrow1544(oa_2: Material, ps: Any=ps) -> Json:
        return Material_ISAJson_encoder(oa_2)

    return Json(5, choose(chooser, of_array([try_include_list("sources", _arrow1542, source), try_include_list("samples", _arrow1543, samples), try_include_list("otherMaterials", _arrow1544, materials)])))
