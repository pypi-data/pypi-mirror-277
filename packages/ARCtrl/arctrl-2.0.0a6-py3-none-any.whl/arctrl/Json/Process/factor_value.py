...
def FactorValue_ISAJson_encoder(fv: FactorValue) -> Json:
    ...
    def _arrow1236(value: str, fv: Any=fv) -> Json:
        return Json(0, value)

    def _arrow1237(value_2: Factor, fv: Any=fv) -> Json:
        return Factor_ISAJson_encoder(value_2)

    def _arrow1238(value_3: Value, fv: Any=fv) -> Json:
        return Value_ISAJson_encoder(value_3)

    return Json(5, choose(chooser, of_array([try_include("@id", _arrow1236, fv.ID), try_include("category", _arrow1237, fv.Category), try_include("value", _arrow1238, fv.Value), try_include("unit", OntologyAnnotation_ISAJson_encoder, fv.Unit)])))


def _arrow1246(get: IGetters) -> FactorValue:
    def _arrow1240(__unit: None=None) -> str | None:
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("@id", Decode_uri)

    def _arrow1241(__unit: None=None) -> Factor | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("category", Factor_ISAJson_decoder)

    def _arrow1243(__unit: None=None) -> Value | None:
    ...
    return FactorValue(_arrow1240(), _arrow1241(), _arrow1243(), _arrow1245())


FactorValue_ISAJson_decoder: Decoder_1[FactorValue] = object(_arrow1246)
...
def ARCtrl_Process_FactorValue__FactorValue_toISAJsonString_Static_71136F3F(spaces: int | None=None) -> Callable[[FactorValue], str]:
    def _arrow1248(f: FactorValue, spaces: Any=spaces) -> str:
