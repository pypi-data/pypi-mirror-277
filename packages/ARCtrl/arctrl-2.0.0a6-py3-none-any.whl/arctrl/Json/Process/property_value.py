...
def encoder(pv: IPropertyValue_1[Any]) -> Json:
    ...
    if match_value_1 is None:
        ...
        if v.tag == 1:
            ...
            def _arrow1262(value_3: str, pv: Any=pv) -> Json:
                return Json(0, value_3)

            def _arrow1263(value_5: str, pv: Any=pv) -> Json:
                return Json(0, value_5)

            pattern_input_1 = (map(_arrow1262, oa_1.Name), map(_arrow1263, oa_1.TermAccessionNumber))
    ...
    def _arrow1265(value_11: str, pv: Any=pv) -> Json:
        return Json(0, value_11)

    def _arrow1267(value_13: str, pv: Any=pv) -> Json:
        return Json(0, value_13)

    def _arrow1268(x: Json, pv: Any=pv) -> Json:
        return x

    def _arrow1270(x_1: Json, pv: Any=pv) -> Json:
        return x_1

    def _arrow1272(value_15: str, pv: Any=pv) -> Json:
        return Json(0, value_15)

    def _arrow1274(value_17: str, pv: Any=pv) -> Json:
        return Json(0, value_17)

    return Json(5, choose(chooser, of_array([("@id", Json(0, gen_id(pv))), ("@type", Json(0, "PropertyValue")), ("additionalType", Json(0, pv.GetAdditionalType())), try_include("category", _arrow1265, pattern_input[0]), try_include("categoryCode", _arrow1267, pattern_input[1]), try_include("value", _arrow1268, pattern_input_1[0]), try_include("valueCode", _arrow1270, pattern_input_1[1]), try_include("unit", _arrow1272, pattern_input_2[0]), try_include("unitCode", _arrow1274, pattern_input_2[1]), ("@context", context_jsonvalue)])))


def decoder(create: Callable[[OntologyAnnotation | None, Value | None, OntologyAnnotation | None], _T]) -> Decoder_1[_T]:
    def _arrow1283(get: IGetters, create: Any=create) -> _T:
        ...
        return create(category, _arrow1282(), unit)

    return object(_arrow1283)
