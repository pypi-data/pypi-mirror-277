...
def compose_columns(columns: IEnumerable_1[FsColumn]) -> Array[CompositeColumn]:
    def _arrow953(columns_2: FSharpList[FsColumn], columns: Any=columns) -> CompositeColumn:
        return from_fs_columns(columns_2)

    return to_array(map(_arrow953, group_columns_by_header(to_list(columns))))
...
def to_fs_worksheet(table: DataMap) -> FsWorksheet:
    ...
    if len(DataMap__get_Table(table).Columns) == 0:
        ...
        def _arrow954(column_1: CompositeColumn, table: Any=table) -> FSharpList[FSharpList[FsCell]]:
            return to_fs_columns(column_1)

        def _arrow955(column: CompositeColumn, table: Any=table) -> enum_type("ARCtrl.Spreadsheet.ArcTable.ColumnOrder", int, [("InputClass", 1.0), ("ProtocolClass", 2.0), ("ParamsClass", 3.0), ("OutputClass", 4.0)]):
        ...
        columns: FSharpList[FSharpList[FsCell]] = collect(_arrow954, sort_by(_arrow955, of_array(DataMap__get_Table(table).Columns), ObjectExpr956()))
