...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Factor]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow802(i: int, matrix: Any=matrix) -> Factor:
        ...
        return initialize(matrix.ColumnCount, _arrow802)
...
def to_sparse_table(factors: FSharpList[Factor]) -> SparseTable:
    ...
    class ObjectExpr808:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow807(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr808())), matrix.ColumnCount)
