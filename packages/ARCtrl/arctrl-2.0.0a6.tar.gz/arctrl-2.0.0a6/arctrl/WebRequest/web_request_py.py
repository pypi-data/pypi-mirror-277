...
Response_reflection = _expr1940

def _expr1941() -> TypeInfo:
...
def Requests_createHeadersDict_9F3777D(headers: Any | None=None) -> Any | None:
    def mapping(values: Any, headers: Any=headers) -> Any:
        def _arrow1943(__unit: None=None, values: Any=values) -> IEnumerable_1[tuple[str, Any]]:
            def _arrow1942(pair: Any) -> tuple[str, Any]:
                return (pair[0], pair[1])

            return map_1(_arrow1942, values)

        return create_obj(to_list(delay(_arrow1943)))
...
def Requests_mapResponseType_Z730CA7D4(response: InteropResponseType) -> Response:
    ...
    def _arrow1945(__unit: None=None, response: Any=response) -> IEnumerable_1[tuple[str, str]]:
        def _arrow1944(header_name: str) -> IEnumerable_1[tuple[str, str]]:
            return singleton((header_name, response.headers[header_name]))

        return collect(_arrow1944, list(response.headers.keys()))

    class ObjectExpr1946:
        @property
        def Compare(self) -> Callable[[str, str], int]:
            return compare_primitives

    return Response(status_code, text, of_list(to_list(delay(_arrow1945)), ObjectExpr1946()), encoding)
