...
def get_templates(url: str | None=None) -> Async[Any]:
    ...
    def _arrow1948(__unit: None=None, url: Any=url) -> Async[Any]:
        def _arrow1947(_arg: str) -> Async[Any]:
            map_result: Any = Templates_fromJsonString(_arg)
            return singleton.Return(map_result)

        return singleton.Bind(download_file(url_1), _arrow1947)

    return singleton.Delay(_arrow1948)
