...
def send_signed_request(ws_config, method, url, path, headers={"Content-Type": "application/json"}, parameters=None, body=None):
    ...
    try:
        ...
        signature = get_hmac_signature(ws_config.secret_key, method, path, headers, parameters, body)
        
        # Include signature in headers
        headers['X-Signature'] = signature
        
        verify = True
        if ws_config.ignore_ssl:
            verify = False
        else:
            verify = True
