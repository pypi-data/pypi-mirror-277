...
def predict_tree():
    if exists(Path("./models/accuracy_tree.txt")):
        accuracy = loadtxt(Path("./models/accuracy_tree.txt"))
        visible=True
    else:
        visible=False
        accuracy=0
...
def predicted_tree():
    ...
    if request.method=="POST":
        ...
        filename = Path("./models/predicted_decisionDirection_DTreeC.csv")
        try:
...
def train_tree():
    if request.method == "POST":
        ...
        savetxt(Path("./models/accuracy_tree.txt"),[accuracy])
...
def remove_cache_filecsv():
    if exists(Path('.models/predicted_decisionDirection_DTreeC.csv')):
        remove(Path('.models/predicted_decisionDirection_DTreeC.csv'))
        return True
    return False
