...
from contrast.reporting import teamserver_messages, get_reporting_client
...
def _valid_proxy(config):
    ...
    """
    enabled = config.get_value("api.proxy.enable")
    if not enabled:
        # If proxy isn't enabled then it's "valid"
        _log("Proxy use disabled")
        return True

    url = config.get_value("api.proxy.url")
    if url:
...
def _valid_cert(config):
    ...
    """
    enabled = config.get_value("api.certificate.enable")
    if not enabled:
        # If certificate use isn't enabled then it's "valid"
        _log("Certificate use disabled")
        return True

    ca_file = config.get_value("api.certificate.ca_file")
    if not ca_file:
        _log("CA file path not provided")
        return False

    cert_file = config.get_value("api.certificate.cert_file")
    if not cert_file:
        _log("Cert file path not provided")
        return False

    key_file = config.get_value("api.certificate.key_file")
    if not key_file:
