...
from contrast.agent import agent_lib, thread_watcher, patch_controller, sys_monitoring
...
from contrast.reporting.reporting_client import MAX_ATTEMPTS, _ReportingClient
...
MIDDLEWARE_WITHOUT_RUNNER_WARNING = f"""
...
def initialize_settings():
    ...
    contrast_event.set_event_detail(module.settings.assess_event_detail)
    return True
...
def _check_middleware_warnings():
    ...
    if not is_runner_in_use():
        _log_and_warn(MIDDLEWARE_WITHOUT_RUNNER_WARNING)
...
def initialize():
    ...
    with module.init_lock:
        ...
        module.reporting_client = get_reporting_client()
