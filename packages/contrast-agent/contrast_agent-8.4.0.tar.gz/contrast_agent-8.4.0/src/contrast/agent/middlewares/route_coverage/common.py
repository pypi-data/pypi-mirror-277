...
def handle_route_discovery(
    framework: str, discovery_func: Callable[..., Dict[str, Route]], args
) -> None:
    """
    ...
    # threading.Thread(
    #     target=_do_route_discovery,
    #     args=(framework, discovery_func, args),
    #     name=ROUTE_DISCOVERY_THREAD_NAME,
    #     daemon=True,
    # ).start()
...
def _do_route_discovery(
    framework: str,
    discovery_func: Callable[..., Dict[str, Route]],
    args: tuple,
) -> None:
    """
    Top-level background thread function for route discovery.
    """
    from contrast.reporting import get_reporting_client, teamserver_messages
    from contrast.agent import agent_state

    discovered_routes = discovery_func(*args)
    msg = teamserver_messages.ApplicationInventory(discovered_routes)
    get_reporting_client().add_message(msg)
    agent_state.update_discovered_routes(discovered_routes)
    log_discovered_routes(framework, discovered_routes.values())
