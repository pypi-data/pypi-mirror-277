...
def do_starlette_route_observation(starlette_router_instance, *args, **kwargs):
    ...
    asgi_scope = args[0] if len(args) > 0 else kwargs.get("scope")
    ...
    view_func = asgi_scope.get("endpoint")
...
def register_patches():
    wrapt.register_post_import_hook(patch_starlette_requests, STARLETTE_REQUESTS)
    wrapt.register_post_import_hook(patch_starlette_routing, STARLETTE_ROUTING)
