...
def evaluate(
    ...
    *,
    ...
    try:
        ...
        print(f"Failed to post artifacts: {e}")
...
def log_model(
    ...
    try:
        ...
        response = ModelPlugin().save_model_details_to_db(registered_model_name)
        model_id = response["data"]["id"]

        if run_id and model_id:
            # Construct the model URI
            model_uri = f"runs:/{run_id}/{registered_model_name}"
            ModelPlugin().save_model_uri_to_db(model_id, model_uri)
    except Exception as e:
        print(f"Failed to log model details to DB: {e}")
...
def link_model_to_dataset(dataset_id, model_id):
    ...
    """
    return ModelPlugin().link_model_to_dataset(dataset_id=dataset_id, model_id=model_id)
...
def save_model_uri_to_db(model_id, model_uri):
    ...
    """
    return ModelPlugin().save_model_uri_to_db(model_id=model_id, model_uri=model_uri)
...
def save_model_details_to_db(registered_model_name):
    ...
    """
    return ModelPlugin().save_model_details_to_db(
        registered_model_name=registered_model_name
    )
...
def get_model_latest_version(registered_model_name):
    ...
    """
    return ModelPlugin().get_model_latest_version(
        registered_model_name=registered_model_name
    )
