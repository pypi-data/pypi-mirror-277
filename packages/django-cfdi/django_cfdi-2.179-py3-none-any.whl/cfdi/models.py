...
class DescargaCfdi(models.Model):
    ...
    def get_error_mensaje(self):
        if self.metodo_descarga == DescargaCfdi.SCRAPPING:
            respuesta_dict = json.loads(self.respuesta_solicitud)
            if respuesta_dict:
                if respuesta_dict.get("error") and "ímite de descargas" in respuesta_dict.get("error"):
                    return "Se llegó al límite de descargas diarias, solo es posible descargar 2,000 CFDI por día con la página del SAT"
                
                if respuesta_dict.get("resumen") and \
                    respuesta_dict.get("resumen").get("mensaje"):
                    
                    if "Captcha no válido" in respuesta_dict.get("resumen").get("mensaje"):
                        return "Captcha no válido, intente de nuevo"
                    
                    if "ciec incorrecta" in respuesta_dict.get("resumen").get("mensaje"):
