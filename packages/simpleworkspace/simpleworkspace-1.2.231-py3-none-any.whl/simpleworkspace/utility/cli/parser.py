...
class CLIParser(_Generic[_T]):
    def __init__(self, template:_Type[_T], description:str=None, add_help=True):
        ...
        self.internalParser = _argparse.ArgumentParser(description=description, add_help=add_help)
        self._Map_Parser(self.internalParser, template)
    ...
    def Parse(self, arguments:_Sequence[str]=None, ignoreUnkownArguments=True) -> _T:
        if(ignoreUnkownArguments):
            args, unkown = self.internalParser.parse_known_args(args=arguments)
        else:
            args = self.internalParser.parse_args(args=arguments)
        return args
