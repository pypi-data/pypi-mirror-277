...
export class GraphView extends DOMWidgetView {
  ...
  private leafletSupport?: LeafletSupport
  ...
  *edges(): Generator<Edge, void> {
  ...
  private mountComponent(
    ...
    this.leafletSupport = new LeafletSupport(
      graphComponent,
      widgetRootElement,
      this.hiddenNodes
    )
  ...
  /**
   * Widget is only visible when in current viewport (since JupyterLab 4.2)
   * @yjs:keep=isVisible
   */
  private isWidgetVisible() {
    // be extra safe due to backwards compatibility
    const widgetReference = this.pWidget ?? this.luminoWidget
    return widgetReference && typeof widgetReference.isVisible !== 'undefined'
      ? widgetReference.isVisible
      : true
  }

  private async initializeViewport() {
    // initialize the viewport only if the widget is actually visible (see registerIntersectionObserver), otherwise
    // the graphComponent or leaflet map has zero size
    if (!this.viewportInitialized && this.isWidgetVisible()) {
      this.viewportInitialized = true

      if (!LeafletSupport.isInMapMode(this.el)) {
        await this.graphComponent!.fitGraphBounds({ animated: false })
      }
      await this.morphSelectedLayout()

      // reset map viewport to account for starting directly in map mode with JupyterLab 4.2 and lazy cell rendering
      this.leafletSupport?.resetMapViewport()
    }
  }

  /**
   * since JupyterLab 4.2, the cell output has `display=none` when not in view
   * https://jupyterlab.readthedocs.io/en/stable/getting_started/changelog.html#full-notebook-windowing-mode-by-default
   *
   * Therefore, the GraphComponent (and Map) has no size initially, and needs to be explicitly resized once if the widget
   * is currently not in the view (e.g. "run all cells in notebook").
   *
   * Unfortunately, there is no JupyterLab trigger for the first initialization (or any other visibility changes), see comment in
   * https://github.com/jupyterlab/jupyterlab/blob/82f0327c9da2c096c663e6b22d5d8428b94d2447/packages/notebook/src/windowing.ts#L280C1-L287C8
   */
  private registerIntersectionObserver() {
    ...
    const observer = new IntersectionObserver(handleIntersect, {
      threshold: [0, 0.25, 0.5, 0.75, 1], // check multiple times, maybe the widget was not visible at certain thresholds
    })
  ...
  private registerResizeObserver() {
    const observer = new ResizeObserver((entries) => {
      ...
      if (newSize) {
        ...
        this.leafletSupport?.invalidateMapSize()
      }
