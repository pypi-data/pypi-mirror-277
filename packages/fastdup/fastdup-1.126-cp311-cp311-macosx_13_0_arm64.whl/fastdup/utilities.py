...
CYAN = f"\033[0;36m"
WHITE = f"\033[0;37m"
BRIGHT_BLACK = f"\033[0;90m"
BRIGHT_RED = f"\033[0;91m"
BRIGHT_GREEN = f"\033[0;92m"
BRIGHT_YELLOW = f"\033[0;93m"
BRIGHT_BLUE = f"\033[0;94m"
BRIGHT_MAGENTA = f"\033[0;95m"
BRIGHT_CYAN = f"\033[0;96m"
BRIGHT_WHITE = f"\033[0;97m"
...
BOLD_BRIGHT_BLACK = f"\033[1;90m"
BOLD_BRIGHT_RED = f"\033[1;91m"
BOLD_BRIGHT_GREEN = f"\033[1;92m"
BOLD_BRIGHT_YELLOW = f"\033[1;93m"
BOLD_BRIGHT_BLUE = f"\033[1;94m"
BOLD_BRIGHT_MAGENTA = f"\033[1;95m"
BOLD_BRIGHT_CYAN = f"\033[1;96m"
BOLD_BRIGHT_WHITE = f"\033[1;97m"

NO_COLOR = "\033[0m"

_GETTING_STARTED_LINK = 'https://docs.visual-layer.com/docs/getting-started-with-fastdup'

_DOC_MSG = f"""
Welcome to fastdup By Visual Layer!

fastdup allows you to rapidly explore and extract valuable insights from your visual data.

To get started:
1. Create a dataset object using fastdup.create() and provide it with the input folder containing your data
2. Analyze your dataset using the .run() function of the dataset object
3. Interactively explore your data on your local machine using the .explore() function of the dataset object

For more information, check our documentation {_GETTING_STARTED_LINK}.
"""

_CREATE_TEMPLATE_MSG = f"""
fastdup By Visual Layer, Inc. 2024. All rights reserved.

A fastdup dataset object was created!

{{}}
Work directory is set to {MAGENTA}"{{}}"{NO_COLOR}

The next steps are:
   1. Analyze your dataset with the {MAGENTA}.run(){NO_COLOR} function of the dataset object
   2. Interactively explore your data on your local machine with the {MAGENTA}.explore(){NO_COLOR} function of the dataset object

For more information, use {MAGENTA}help(fastdup){NO_COLOR} or check our documentation {_GETTING_STARTED_LINK}.
"""

_POST_RUN_MSG = f"""
Analysis complete. Use the {MAGENTA}.explore(){NO_COLOR} function to interactively explore your data on your local machine.

Alternatively, you can generate HTML-based galleries.
For more information, use {MAGENTA}help(fastdup){NO_COLOR} or check our documentation {_GETTING_STARTED_LINK}.
"""

def _create_template_msg(work_dir: Union[str, Path], input_dir: Union[str, Path, List[str], List[Path]]) -> str:
    if input_dir is None:
        input_dir_msg = (f'{YELLOW}Input directory is still not set. To proceed, you must provide an input directory to the\n'
                         f'.run() function or call .create() again.{NO_COLOR}')
    else:
        if isinstance(input_dir, (str, Path)):
            input_dir_type_msg = f'"{input_dir}"'
        elif isinstance(input_dir, list):
            input_dir_type_msg = f'{len(input_dir)} files'
        else:
            input_dir_type_msg = 'unknown type'
        
        input_dir_msg = f'Input directory is set to {MAGENTA}{input_dir_type_msg}{NO_COLOR}'

    return _CREATE_TEMPLATE_MSG.format(input_dir_msg, work_dir)
