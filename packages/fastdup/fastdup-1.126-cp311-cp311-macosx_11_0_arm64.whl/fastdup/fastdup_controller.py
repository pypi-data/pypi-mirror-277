...
class FastdupController:
    ...
    def run(self, input_dir: Union[str, Path] = None, annotations: Union[pd.DataFrame,list] = None, subset: list = None,
            ...
            print_summary: bool = False, print_vl_datasets_ref: bool = False, run_explore: bool = True, dataset_name: str = None,
            **fastdup_kwargs):
        """
        This function
        ...
        if run_explore:
            ...
            do_visual_layer(work_dir=self._work_dir, input_dir=vl_input,
                            dataset_name=dataset_name, overwrite=overwrite, run_server=False)
    ...
    def explore(self) -> None:
        from fastdup.fastdup_runner.run import do_visual_layer
        do_visual_layer(work_dir=self._work_dir, overwrite=False, run_server=True)
        ...
        if df_annot is not None:
            ...
            if data_type == FD.IMG:
                ...
                if len(df_annot) < len_df_annot:
                    self._logger.debug(f"Warning: removed {len_df_annot - len(df_annot)} duplicate bounding boxes")
            else:
