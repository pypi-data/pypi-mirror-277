...
def log_model(
    ...
    if auto_infer_signature and not signature:
        try:
...
def infer_jsl_signature(spark_model):
    ...
    if 'VISUAL_SECRET' in os.environ:
        os.environ['DB_ENDPOINT_ENV'] = 'True'
    else:
        if 'DB_ENDPOINT_ENV' in os.environ:
            del os.environ['DB_ENDPOINT_ENV']
    ...
    # TODO Fix bug nlu model.predict is adding origin_index to input data for some models
    # For example en.de_identify.clinical_pipeline
    output_data = spark_model.predict(input_data)
    signature = infer_signature(original_input_data, output_data)
...
def _unpack_and_save_model(spark_model, dst):
    ...
    try:
        if isinstance(spark_model, _PyFuncModelWrapper):
            print('Extracting from PyFuncModelWrapper')
            # spark model is nlu model
            spark_model: NLUPipeline = spark_model.spark_model
        if isinstance(spark_model, PipelineModel):
            print('Saving via Overwrite')
            ...
            try:
                ...
                pass
            try:
                print('Saving via vanilla pipe ')
                # Use NLUs save to inject metadata into pipe before saving it
                spark_model.save(dst, overwrite=True)
                # spark_model.vanilla_transformer_pipe.write().overwrite().save(dst)
            except Exception as err:
                print('Saving via nlu.save')
                print(err)
...
def _maybe_unpack_and_save_model(spark_model, dst):
    print('Maybe unpack and save')
    try:
        signature = _unpack_and_save_model(spark_model, dst)
        print('Maybe unpack and save succ')
        return True
    except Exception as err:
        print('Maybe unpack and save fail')
        return False
