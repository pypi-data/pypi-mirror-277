...
from highlighter.const import DATA_FILE_ATTRIBUTE_UUID, PIXEL_LOCATION_ATTRIBUTE_UUID
from highlighter.logging import LOG as logger
from highlighter.training_config import TrainingConfigType, get_training_config
...
def create_capability_for_artefact(
    ...
    # run mutations
    class MachineAgentVersion(BaseModel):
        id: str

    class CreateMachineAgentVersionPayload(BaseModel):
        errors: List[str]
    ...
    machine_agent_version_id = create_machine_agent_version_result.machineAgentVersion.id

    class CreatePipelineElementInputPayload(BaseModel):
        errors: List[str]
    ...
    class CreatePipelineElementOutputPayload(BaseModel):
        errors: List[str]
