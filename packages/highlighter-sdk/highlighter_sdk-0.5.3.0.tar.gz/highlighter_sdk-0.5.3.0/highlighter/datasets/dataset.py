...
from highlighter.base_models import DatasetSubmissionTypeConnection
...
from highlighter.io import create_data_files, multithread_graphql_file_download
from highlighter.logging import LOG
from highlighter.pagination import DEFAULT_PAGE_SIZE, paginate
...
class Dataset:
    ...
    def read_coco(cls, annotations_file: Path, bbox_only: bool = False):
        coco_reader = get_reader("coco")(annotations_file, bbox_only=bbox_only)
    ...
    def append(self, datasets: List["Dataset"]):
        ...
        new_data_files_dfs = [d.data_files_df for d in datasets]
        new_annotations_dfs = [d.annotations_df for d in datasets]
        self.data_files_df = pd.concat([self.data_files_df] + new_data_files_dfs, ignore_index=True)
        self.annotations_df = pd.concat([self.annotations_df] + new_annotations_dfs, ignore_index=True)
