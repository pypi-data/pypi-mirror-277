...
from highlighter.cropping import CropArgs, crop_rect_from_poly
from highlighter.datasets.interfaces import IWriter
from highlighter.io import _pil_open_image_path as read_image
...
def _create_full_data_file_folder_dataset(
    ...
    def make_class_folder(grp):
        ...
        for data_file_id in data_file_ids:
            ...
            ext = Path(data_file_filename).suffix
...
def _create_crop_folder_dataset(
    ...
    def make_crop(grp, orig_file_name, pil_data_file, root_dir):
        ...
        save_path = (
            root_dir / attr_record["value"] / f"{orig_file_name.stem}-{l}-{t}-{r}-{b}{orig_file_name.suffix}"
        )
    ...
    def make_crops(grp, data_file_dir=data_file_dir, root_dir=root_dir):
        ...
        try:
            data_file = read_image(data_file_path)
        except OSError as e:
            warn(f"{e}: '{data_file_path}', skipping.")
            return None

        grp.groupby("entity_id").apply(lambda row: make_crop(row, Path(grp.name), data_file, root_dir))
