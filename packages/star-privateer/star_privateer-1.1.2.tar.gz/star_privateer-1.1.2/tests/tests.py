...
class TestAnalysisMethods :
  ...
  def data (self) :
    ...
    with filename as f :
      hdul = fits.open (f)
      hdu = hdul[0]
      d = np.array (hdu.data).astype (float)
      hdul.close ()
    return d
  ...
  def testWavelet (self, s, t, dt, tmp_dir) :
    ...
    sp.plot_wps (t, p_in, wps, gwps, coi,
...
class TestRooster :
  ...
  def testLoadDefaultRoosterInstance (self) :
  ...
  def testTrainRoosterInstanceOnTheFly (self) :
    chicken = sp.load_rooster_instance (on_the_fly=True, verbose=True)
    chicken.getTrainingRotClassInfo ()
    chicken.getTestRotClassInfo ()
    chicken.getTrainingPeriodSelInfo ()
    chicken.getTestPeriodSelInfo ()
