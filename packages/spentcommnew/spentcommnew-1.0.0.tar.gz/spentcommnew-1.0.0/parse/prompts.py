summarize_system_message = """
    ...
    4- Notes for completion_year:
        4-1) The year should be in the format of YYYY. Example: "2021".
        4-2) First, try to find it from the Date field in the first page (with title "Qual Information"), if possible.
        4-3) If there are two dates mentioned, use the latest one. Example: "1/21/2020 - 3/12/2021", then it should be "2021".
        4-) There might be instances where some dates in the content are probably not referring to the year.
        ...
        4-5) If the year is not mentioned explicitly in the Date filed of the first page, try to infer it from the content or metadata (filename).

    5- Notes for title:
        5-1) If not found in the first slide, try to see if there is a title (peice of text) repeated in the begining of multiple pages and catch it.
             * Examples of repeated text patterns: 'client_name ... Case Study', or 'Case study | ... client_name ...', or '... | Case study', or '... - Case study' etc.
        5-2) If not found using the points mentioned above, try to infer it from the filename in metadata.
        5-3) Title should include clinet name and project name (to some extent).

    6- Notes for client name:
        6-1) First, try to find it in the first slide or in the metadata (file name).

    7- Notes for confidentiality:
        7-1) The first page title of "Qual Information \u2013 Internal Use Only" only refers to the information in that specific page, so do not use that for inferring confidentiality of the entire document.
        7-2) If "Qual Information - Internal Use Only" is repeated multiple times in the document's text, then the document should be considered 'Internal Use Only'.
...
summarize_user_message = """
    ...
    - title: The title of the case study. Consider the notes regarding title (especially repeated text pattern in the begining of multiple pages).
    ...
    - project_name: The project's name. Should not include any unnecessary like [], etc. at the beginning or end. Try to include client name in the project name.
    ...
    - agency_name: The agency name which did or delivered the project or engaged with the client in the project. Try hard to capture it from any page. Only one of the given agencies. Strictly adhere to the provided list.
    ...
    - confidentiality: Only one of "confidential", "internal use only", "publicly available", "not specified". Use all lowercase.
        Note: If "Internal Use Only" only is mentiond in one page only, it refers to the information in that specific page, so do not use that for inferring confidentiality of the entire document.
              If "Internal Use Only" or "Confidential" is repeated multiple times in the document's text, then the the value should be considered accordingly.
