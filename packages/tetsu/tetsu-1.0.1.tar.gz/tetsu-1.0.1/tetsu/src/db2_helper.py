...
class DB2Helper:
    ...
    def __get_conn_jdbc(self):
        ...
        try:
            conn = jaydebeapi.connect(
                ...
                url=f"jdbc:db2://{self.creds['hostname']}:{self.creds['port']}/{self.creds['database']}",
                driver_args={
                    "sslConnection": self.creds["ssl"],
                    "sslTrustStoreLocation": self.creds["truststore"],
                    "sslTrustStorePassword": self.creds["trustpass"],
    ...
    def check_db2_push(self, query: str, data: pd.DataFrame) -> bool:
        ...
        return (
            data.merge(stored_db).drop_duplicates().shape == data.drop_duplicates().shape
        )
