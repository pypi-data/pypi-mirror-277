...
instance = logger()
logger = instance.logger(name=__name__, handler="console")
...
def tm1_pull(
    ...
    if cloudant_doc is None:
        cloudant_doc = os.getenv("cloudant_document")
    if creds is None:
        creds = {
    ...
    tm1_object = TM1py.TM1Service(address=creds["hostname"], port=creds["port"])
    try:
        cellset_id = tm1_object.cubes.cells.create_cellset(mdx_query)
        extract_df = tm1_object.cubes.cells.extract_cellset_dataframe(
            cellset_id, skip_zeros=skip_zeros, include_attributes=True
        )
