...
def compile_ui(path: str) -> None:
    ...
    program_dir = pathlib.Path(path) / "jdMacroPlayer"
...
def build_wheel(wheel_directory, config_settings: Any = None, metadata_directory: Optional[str] = None) -> str:
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        try:
            shutil.rmtree(os.path.join(current_dir, "jdMacroPlayer", "gui", "ui"))
        except FileNotFoundError:
            pass
...
def get_requires_for_build_wheel(self, config_setting: Any = None) -> list[str]:
    if get_lrelease_command() is None:
