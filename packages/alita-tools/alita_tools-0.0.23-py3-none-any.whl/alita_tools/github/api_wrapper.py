...
For example, if you would like to add the contents of the file /test/test.txt where "existing contents" will be extended with "new contents", you would pass in the following string:
...
class AlitaGitHubAPIWrapper(GitHubAPIWrapper):
    ...
    def update_file(self, file_query: str) -> str:
        ...
        try:
            ...
            if not old_file_contents.strip():
                return "Old file contents must not be empty, nor contain only single new line. Add lines to be replaceed between OLD <<<< and >>>> OLD."
