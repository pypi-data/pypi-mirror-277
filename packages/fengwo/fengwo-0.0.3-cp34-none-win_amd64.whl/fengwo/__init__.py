...
def COST(HIGH,LOW,VOL,Turnrate,winpercent,radio:float=0.01,avg='hlavg')->np.ndarray:
    '''计算某盈利比例对应的股价
:HIGH-最高价[序列值]
:LOW-最低价[序列值]  
:VOL-成交量[序列值]，单位是手，万股等不重要,只要统一就行
...
def MACD(CLOSE,SHORT:int=12,LONG:int=26,MID:int=9,bit:int=2): #bit为保留小数的位数，取值不等于2或3时则输出全部小数
    '''MACD异同移动平均线
:CLOSE-收盘报价[序列值]
...
def TRIX(CLOSE,N:int=12,M:int=9)->np.ndarray:
...
def BARSLASTCOUNT(S)->np.ndarray:
    '''返回连续满足S条件的周期数。'''
    return pyML.BARSLASTCOUNT(S)

def FILTER(S,SN)->np.ndarray:
    '''过滤连续出现的信号：S满足条件后,将其后SN周期内的数据置为0,SN可以是变量。'''
    return pyML.FILTER(S,SN)

def BIAS(CLOSE,N1:int=6,N2:int=12,N3:int=24):
    '''BIAS乖离率指标
:CLOSE-收盘价[序列值]
:N1,N2,N3-周期[常量]
...
def BIAS_QL(CLOSE,N:int=6,M:int=6):
    '''BIAS乖离率(传统版)指标
:CLOSE-收盘价[序列值]
:N,M-周期[常量]
返回值:(BIAS,BIASMA)元组
...
def EXIST(S,N:int)->np.ndarray:
    '''返回在N个交易周期内是否存在S位True的值，N为常量'''
    return np.EXIST(S,N)

def BBI(CLOSE,M1:int=3,M2:int=6,M3:int=12,M4:int=24)->np.ndarray:
    '''BBI多空均线指标
:CLOSE-收盘价[序列值]
:M1,M2,M3,M4-周期[常量]
返回值:BBI数值'''
...
def DIFF(S)->np.ndarray:
    '''返回前一个值减后一个值的数值'''
    return pyML.DIFF(S)

def VALUEWHEN(COND,S)->np.ndarray:
    '''当COND条件成立时,取X的当前值,否则取VALUEWHEN的上个值'''
    return pyML.VALUEWHEN(COND,S)

def VR(CLOSE,VOL,N:int=26,M:int=6):
    '''VR成交量变异率指标
:CLOSE-收盘价[序列值]
:VOL-成交量[序列值]
:N,M-周期[常量]
...
def RSI(CLOSE,N1:int=6,N2:int=12,N3:int=24):
    '''RSI相对强弱指标
:CLOSE-收盘价[序列值]
:N1,N2,N3-3个周期[常量]
...
def LONGCROSS(S1,S2,N)->np.ndarray:
    '''两条线维持一定周期后交叉
:S1,S2-序列值,N-常量或序列值
...
def KTN(CLOSE,HIGH,LOW,N:int=20,M:int=10):                 #肯特纳交易通道, N选20日，ATR选10日
    '''KTN肯特纳交易通道指标
:CLOSE-收盘报价[序列值]
:HIGH-最高价[序列值]
:LOW-最低价[序列值]
...
def DMA(S,SN)->np.ndarray:
    '''求S的SN周期动态移动平均,SN可以为序列值'''
    return pyML.DMA(S,SN)

def TAQ(HIGH,LOW,N:int=20):
    '''唐安奇通道
:HIGH-最高价[序列值]
:LOW-最低价[序列值]
:N-周期[常量]
...
def BRAR(OPEN,CLOSE,HIGH,LOW,N:int=26):
    '''BRAR情绪指标
:OPEN-开盘价[序列值]
:CLOSE-收盘报价[序列值]
:HIGH-最高价[序列值]
...
def DPO(CLOSE,N:int=20,M:int=6):
    '''DPO 区间震荡线
:CLOSE-收盘价[序列值]
:N,M-周期[常量]
...
def DMA(CLOSE,N1:int=10,N2:int=50,M:int=10):
    '''DMA 平均差指标
:CLOSE-收盘价[序列值]
:N1,N2,M-周期[常量]
返回值:(DIF,DIFMA)元组,DIFMA为DIF的M日均值'''  
    return pyML.DMA(CLOSE,N1,N2,M)

DFMA=DMA

def DMI(CLOSE,HIGH,LOW,N:int=14,M:int=6):
    '''DMI 趋向指标
:CLOSE-收盘报价[序列值]
:HIGH-最高价[序列值]
:LOW-最低价[序列值]
...
def EXPMA(CLOSE,M1:int=12,M2:int=50):
    '''EXPMA 指数平均线指标
:CLOSE-收盘价[序列值]
:N1,M2-周期[常量]
...
def ASI(OPEN,CLOSE,HIGH,LOW,N:int=6):
    '''ASI 震动升降指标
:OPEN-开盘价[序列值]
:CLOSE-收盘报价[序列值]
:HIGH-最高价[序列值]
...
def getHPoint(S,N:int)->np.ndarray:
    '''指示当前位置的数据是否是一组数据的高点
:S-输入的数据[序列值]
:N-检测阈值[常量]
...
def getLPoint(S,N:int)->np.ndarray:
    '''指示当前位置的数据是否是一组数据的低点
:S-输入的数据[序列值]
:N-检测阈值[常量]
...
def getBestHLine(S,N1:int,N2:int):
...
def getBestLLine(S,N1:int,N2:int):
...
def RSRS(HIGH,LOW,N:int=18,M:int=600)->np.ndarray:
    '''RSRS 阻力相对支撑强度指标
...
def SAR(HIGH,LOW,start:int=4,stepstart:int=2,step:int=2,maxstep:int=20)->np.ndarray:
    '''SAR 抛物线指标
:HIGH-最高价[序列值]
:LOW-最低价[序列值]
:start:起始统计周期
...
def binddll(no:int,dllpath:str)->bool:
    '''绑定dll文件[Windows系统专用]
...
def TDXDLL1(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用1号DLL函数
:funcno-函数号
...
def TDXDLL2(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用2号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
...
def TDXDLL3(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用3号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
...
def TDXDLL4(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用4号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
...
def TDXDLL5(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用5号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
...
def TDXDLL6(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用6号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
...
def TDXDLL7(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用7号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
...
def TDXDLL8(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用9号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
...
def TDXDLL9(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用9号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
...
def TDXDLL10(funcno:int,ina,inb,inc)->np.ndarray:
    '''调用10号DLL函数
:funcno-函数号
:ina,inb,inc-输入的3个参数，其中ina必须为序列值
返回值为计算结果的序列'''
