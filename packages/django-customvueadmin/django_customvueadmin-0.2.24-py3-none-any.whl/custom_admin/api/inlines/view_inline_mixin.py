    ...
    def get_filters(self, filterset_class, request) -> typing.Dict[str, typing.Any]:
        ...
        for filter_key in filters.keys():
            ...
            if isinstance(_filter, dict):
                if 'from' in _filter:
                    d = datetime.strptime(filter.get('from'), DATETIME_FORMAT)
                    filter['from'] = datetime.combine(d, time.min)

                if 'to' in _filter:
                    d = datetime.strptime(filter.get('to'), DATETIME_FORMAT)
                    filter['to'] = datetime.combine(d, time.min)
